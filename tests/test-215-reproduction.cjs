 ...
 * manual drafts" -- it must NOT be papered over by weakening thresholds.
 *
 * DG-2 (215-05 navigator checkpoint, recorded 2026-07-10): the LIVE full-catalog
 * pass (--pairs full, ~2.2M cross-boundary pairs) was run and DID NOT surface
 * either acceptance pair after the field-contract fix (1e1c6657) rebalanced the
 * composite score distribution -- not a bug, a substrate-scale dilution: real
 * signal from 919 cited CONVERGES pairs gets outranked by sheer combinatorial
 * volume. The navigator's call: GRAPH is the canonical acceptance substrate
 * (both gems reproduce cleanly there -- arrhythmias rank 10, cerebral aneurysm
 * rank 16); full-catalog stays a supplementary tail-gem sweep, generated and
 * kept (evals/eureka/215-jhtv-portfolio-report.md/json) but NOT gating. This
 * test therefore reads the GRAPH-MODE JSON, not the full-mode one.
 *
 * It reads the LIVE graph-mode run's JSON sibling (Plan 05 Task 1 wrote it).
 * Absent -> exit 2 with a clear message so both the run-all-215 run_if guard and
 * a direct invocation stay honest.
 *
...
// DG-2: graph substrate is canonical for acceptance (see file header). The
// full-catalog sibling (215-jhtv-portfolio-report.json) is still generated by
// Task 1 and kept on disk as a supplementary sweep, but is not read here.
const JSON_PATH = path.join(REPO_ROOT, 'evals/eureka/215-jhtv-portfolio-report.graph-mode.json');
...
function main() {
  if (!fs.existsSync(JSON_PATH)) {
    process.stderr.write('reproduction: real-run graph-mode JSON missing - run Plan 05 Task 1 first (' + JSON_PATH + ')\n');
    process.exit(2);
  }
  let j;
  try {
  ...
  // Assertion 5: provenance -- LIVE run, GRAPH pairs mode (DG-2 canonical
  // acceptance substrate), AHP CR <= 0.1.
  ...
  ok(p.pairs_mode === 'graph', "provenance pairs_mode is 'graph' (got " + JSON.stringify(p.pairs_mode) + ")");
