...
ok('Behavior 13 (live-tree honesty check): check-render-coverage.cjs --check now correctly fails on the skills/*/SKILL.md keyspace with EXACTLY the known, tracked set of 5 pre-existing unwired declarations (surfaced for the FIRST TIME by this fix, not newly created); commands/*.md stays 0 unwired', function () {
  const registry = JSON.parse(fs.readFileSync(path.join(REPO, 'data', 'render-coverage-registry.json'), 'utf8'));
  ...
  assert.deepStrictEqual(
    unwiredSkills,
    KNOWN_UNWIRED_SKILLS,
    'if this list changed: either a skill was fixed (remove it here) or a NEW ' +
      'unwired declaration appeared (investigate before updating this list -- ' +
      'do not blindly add). These 5 are a SEPARATE, out-of-scope finding from ' +
      'the intern-w1-mode-gate-skip fix, tracked here so a real regression ' +
      '(a 6th surface) is still caught.'
  );

  // check-render-coverage.cjs --check now correctly exits non-zero because of
  // the 5 known skill gaps above -- this is the EXPECTED, DESIRED consequence
  // of closing RCA gap 1 (PRIMARY detection can finally see these surfaces).
  // Use spawnSync (not execFileSync) so the non-zero exit does not throw.
  const result = spawnSync('node', ['scripts/check-render-coverage.cjs', '--check'], { cwd: REPO, encoding: 'utf8' });
  assert.equal(result.status, 1, 'check-render-coverage.cjs --check is expected to exit 1 (5 known skill gaps)');
  for (const surface of KNOWN_UNWIRED_SKILLS) {
    assert.equal(result.stderr.indexOf(surface) !== -1, true, 'expected the RENDER GAP output to name ' + surface);
  }
});
