...
// The gated new member. SKIP-safe until 195-04 mints it in edges.cjs.
...
if (ALLOWED_EDGE_TYPES.has(NEW_EDGE)) {
  ok('ALLOWED_EDGE_TYPES has ' + NEW_EDGE + ' (minted in Wave 3)', true);
} else {
  console.log('  -- ' + NEW_EDGE + ' not yet minted: FLOOR SKIP-safe until 195-04 (Wave 3)');
}
