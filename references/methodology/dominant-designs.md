...
- **Research mode (deep dive, Claude Code)**: before the six phases, Larry proposes one search per evidence lane (variant census, convergence signals, S-curve limits, discontinuity signals) built from the domain you name. Nothing is searched until you approve or edit those queries. Every claim that comes back carries its source or is dropped, and each lane's evidence is filed to the room only when you say so. On Claude Desktop and Cowork, Larry runs the deep dive from what you know and tells you that the sourced research pass runs in Claude Code.
