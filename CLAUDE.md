...
| Handoff | Subject |
|---|---|
| `docs/2026-09-03-CONSTITUTION-v3.1.0-mos-reasoning-constitution.md` | **MOST RECENT.** This filename has no `-HANDOFF-` segment, so the `ls docs/*-HANDOFF-*.md` command above misses it entirely - read it anyway, it is the current position. v3.1.0, RATIFIED by navigator sign-off 2026-09-03, supersedes v3.0.0; the Ten Laws and most v3.0.0 doctrine carry forward by reference, deliberately not retyped here. SHIPPED on `main`: the `brain_query` silent-empty fix (`f264c843`, `8aca8af7`) and R19's `/mos:operator set` migration onto the real gate ledger (`e29a7480`, `a114a4ad`, `17a60439`) - cite the commits, never a count, since Part 0's own prose and its own table disagree on how many items shipped. RULED, design filed, NOT implemented: R16 (add `SOURCED_FROM` only; SUPPORTS/CONTRADICTS/REFINES/DERIVED_FROM already live), R17 (`epistemic_type` on every written node, sequenced behind a node-write consolidation pass), R18 (level cap on the 3-lane conversation-mode system), R20 (two-engine contract, Theo owns canon/provenance, Memgraph stays a bounded analytics service). Two traps for a downstream implementer: R18 carries a flagged `unresolved` concern that the 3-lane system may be a session-start one-time gate rather than the per-turn state machine the cap needs, read one more file before implementing; R16's corrected deprecation target is `RELATED_TO`, not `RELATES_TO` (14 files). R21 leaves the completion bar `unresolved` pending a navigator-supplied candidate. `/mos:operator reset` has the identical ungated disease as `set` and is a named deliberate follow-up. Companion, equally invisible to the glob: `docs/2026-09-03-DESIGN-t2-write-back-minimal.md` (minimal T2 write-back design, filed, not implemented). |
