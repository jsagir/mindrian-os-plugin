...
- **Part 9 - Memory Locality.** SQL (room.db) is the local mind; Brain reasons over typed packets, never raw memory; only a human confirms a truth-claim node, and only a human closes one as superseded (Appendix D entry 41). Deep dive: docs/MINDRIAN-CANON.md (Part 9).
