...
> **Routing note (PRECONDITIONS.md):** `bin/mindrian-tools.cjs` is currently broken by a better-sqlite3 / lazygraph-ops MODULE_NOT_FOUND. Route `/mos:vault import` directly to `node "${CLAUDE_PLUGIN_ROOT}/scripts/vault-import.cjs"`, never through `mindrian-tools.cjs`.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/vault-import.cjs" --path <vault> --room <room>
```
...
1. Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/vault-import.cjs" --path <vault> --room <room>` through Stage 02 exactly as
   today.
...
3. Present the summary in Body Shape E and ask the user to confirm. **The fan-out speeds up
   ...
   `node "${CLAUDE_PLUGIN_ROOT}/scripts/vault-import.cjs"` with the same flags for Stage 03: `scripts/vault-import.cjs`'s
