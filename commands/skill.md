...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/build-new-surface.cjs" --from-expert <surname|nodeId>
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/build-new-surface.cjs" --check --kind skill --name <expert>
```
