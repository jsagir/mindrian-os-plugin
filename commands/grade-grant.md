...
applicant's own draft text. See `${CLAUDE_PLUGIN_ROOT}/references/opportunities/tnufa-rubric.md` for the full
provenance and the matching-fund contradiction this rubric resolves.

## Setup

1. Read `${CLAUDE_PLUGIN_ROOT}/references/opportunities/tnufa-rubric.md` for the rubric's sources and known caveats.
...
3. **Load the rubric.** `node -e "console.log(JSON.stringify(require('${CLAUDE_PLUGIN_ROOT}/lib/core/eureka/grade-grant.cjs').loadRubric('tnufa')))"` (swap the id for the chosen program).
