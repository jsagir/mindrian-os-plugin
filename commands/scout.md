...
=======
- `/mos:scout hsi` -- HSI recomputation only (compute-hsi + detect-reverse-salients + hsi-to-lazygraph)
...
=======
# Step 3: Write HSI edges to LazyGraph SQLite (if available)
node "${PLUGIN_ROOT}/scripts/hsi-to-lazygraph.cjs" "$ROOM_DIR" 2>/dev/null || true
>>>>>>> worktree-agent-a7c8282c
```
