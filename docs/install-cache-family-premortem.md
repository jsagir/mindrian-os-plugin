...
> Date: 2026-05-14. Phase: 126 Plan 04 (Wave 3) -- the D4 deliverable.
> Status: Active. Linked from `docs/CANON-PHASE-MAP.md` Part 6.

Canonical family record for the install-cache failure surface. Six cases
...
## Section 1. Family history (6 cases)
...
| 6 | Windows dogfood + install-minisite stale | 2026-05 | Phase 126 (THIS phase) | Seven defenses ship in one phase. Renderer contract test (Plan 01); prerelease semver-pick fix (Plan 02); acceptance gate self-coverage (Plan 03); release-pipeline hardening -- Step 5.5 tag-push + Step 9.6 install-minisite HARD + Step 9.7 npx self-test (Plan 04); release-flight in acceptance (Plan 05); cache prune extension for stale-backup dirs (Plan 06); install-state.json schema v2 migration (Plan 07). |
...
self-coverage in Plan 03 + pipeline self-test in Plan 04). The defense AT
the surface always works; the failure mode shifts to the next adjacent surface.
