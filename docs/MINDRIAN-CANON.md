...
Three layers, each algorithmic:
  ...
  CROSS-DOMAIN MATCH  against e5 embeddings (multilingual-e5-large, 1024-dim, embedded LOCALLY with no network egress). Cross-domain analogies surface where embedding similarity crosses threshold but source domains differ. Every match is a candidate Opportunity Bank ADD with HSI score. Command-level wrappers: /mos:find-bottlenecks, /mos:find-connections, /mos:find-analogies, /mos:score-innovation.
...
- e5 (multilingual-e5-large, 1024-dim) embeddings, embedded LOCALLY with no network egress (Brain semantic search infrastructure). Corpus size: see docs/CORPUS-STATS.generated.md, which describes the retired incumbent's corpus and has not been regenerated against Theo.
...
Every typed edge in the room's graph is drawn from a closed, frozen allow-list (the constitutional counterpart of the closed verb vocabulary in Part 3). The vocabulary grows ADDITIVELY, never by per-phase invention, and a change that moves the frozen set is a canon amendment, not a command-level edit (Phase 108 frozen-taxonomy contract). The shipped vocabulary spans the decision-and-cascade edges (INFORMS, CONTRADICTS, CONVERGES, INVALIDATES, ENABLES, DEFERRED, REJECTED, REJECTED_BECAUSE, FOLLOWS_FROM, SUPERSEDES, DERIVED_FROM, FILED_AS_DECISION, OPERATOR_TRANSITION), the structural and lineage edges (AFFILIATED_WITH, FEEDS_INTO, VALIDATES, STATES, SUPPORTS, DESCRIBES, NESTED_WITHIN), the dial-decision edges (PIVOTED, SELECTED_REACH), the Knowledge-rung relationship edges (REFINES, ROOT_CAUSES, INSTANTIATES), and the domain-taxonomy relationship edges (DECOMPOSED_INTO, PART_OF, TAGGED_WITH, RELATED_TO). REFINES, ROOT_CAUSES, and INSTANTIATES were added by navigator-gated amendment (Appendix D entry 18): REFINES expresses a new claim that tightens or conditions a prior claim without invalidating it (the missing middle between INFORMS and CONTRADICTS); ROOT_CAUSES is the directional cause-to-effect edge; INSTANTIATES is the concrete-example-evidences-an-abstract-claim edge. DECOMPOSED_INTO, PART_OF, TAGGED_WITH, and RELATED_TO were added by navigator-gated amendment (Appendix D entry 21) to make domains, subdomains, and focus_areas first-class connected graph citizens: DECOMPOSED_INTO is the hierarchy edge (domain to subdomain, subdomain to focus_area); PART_OF is the structural-membership edge from any node to the domain or subdomain it belongs to; TAGGED_WITH is the lightweight categorization tag from any node to a domain or subdomain; RELATED_TO is the symmetric cross-domain relatedness edge between two taxonomy nodes. NESTED_WITHIN was added by navigator-gated amendment (Appendix D entry 23) as the room-lineage edge: a child room node NESTED_WITHIN its parent room node (source room:<child>, target room:<parent>), the graph-navigable representation of the nested-room fractal joint. It is a NEW dedicated lineage type, NOT a widening of PART_OF (which stays the domain-taxonomy structural edge whose targets are domain/subdomain/focus_area), so room-lineage walks never collide with the domain-taxonomy traversals. Fifteen further edge types shipped in `lib/core/navigation/edges.cjs` additively across multiple phases without a matching canon-text amendment landing at the time, and are ratified into this closed vocabulary by navigator-gated amendment (Appendix D entry 39): the memory-governance edges REMEMBERED_AS, ATTRIBUTED_TO, and NOT_REMEMBERED_BECAUSE (Phase 189-01) - REMEMBERED_AS records that a memory candidate got durably filed, ATTRIBUTED_TO records WHO decided and which memory layer the decision touched, and NOT_REMEMBERED_BECAUSE is the rejection-as-data edge for a candidate that was NOT filed; the cross-frame elevation edges SHARES_JOB and ELEVATES_TO (Phase 205-02) - SHARES_JOB is the horizontal-elevation signal that two open frames share the same underlying JTBD job, and ELEVATES_TO is the directed edge from a frame to the containing system the horizontal move named; the cross-room edge UMBILICAL_TO (Phase 195-04) - the peer-to-peer cord between two sibling rooms with no lineage between them; the reverse-salient discovery edges DISCOVERED and AUTHORED_BY (Phase 200-02) - DISCOVERED links a discovery to the reverse salient it surfaced, and AUTHORED_BY links a paper to its author in the expert graph; the entity-relationship edges COMPETES_WITH, USES_COMPONENT, and SUPPLIES_TO (Phase 218-01) - the rivalry edge, the built-on-this-component dependency edge, and the supplier-feeds-customer edge; the event-to-claim edge CONCERNS (quick task 260725-9ca) - the edge from a reified event to the claim it is about; the rubric-to-section edge MAPS_TO_SECTION (quick task 260806) - the edge from a grant-rubric criterion to the room section it maps to; and the provenance edges SOURCED_FROM and USES_FRAMEWORK (quick tasks 260903-gct and 260903-i2x) - SOURCED_FROM is the general provenance edge from any written reasoning node to the evidence it was actually sourced from, and USES_FRAMEWORK is the methodology-provenance edge from a written reasoning node to the framework active when it was written. `lib/core/navigation/edges.cjs` is the authoritative source of the closed `ALLOWED_EDGE_TYPES` set; this Canon's prose is the ratified list, but membership is enumerated from disk at run time, so a future additive type is a canon amendment and the total count is never a frozen literal a gate may hardcode.
...
Before building a new command, skill, agent, or hook, the builder must search the methodology surface first - commands/*.md, plus agents/*.md, pipelines/*/CHAIN.md, and skills/*/SKILL.md where the new work spans surface classes - enumerated from disk, never a frozen count (113 commands as of this amendment, illustrative only, per 340-LIVE-VERIFICATION.md). Most features are existing features repointed.

The justification bar for net-new capability is explicit: the builder must answer, in the plan, "which existing surface does this replace or extend, and why is repointing insufficient?" If the answer is weak, the plan does not ship. Surface area without integration is technical debt (see Moat Mandate).
...
**Narrowing for `superseded` (Phase 348, Appendix D entry 41).** For the `superseded` target specifically, on a truth-claim node, the "or system rules" clause above does not hold: only a human-attributed gate answer may *promote* a truth-claim node's status to `superseded`. Closing a believed fact is itself an assertion about the venture's world, and it carries the same human bar role 5 already places on promoting a node to `confirmed`. This narrows nothing else: `rejected` and `stale` remain agent-reachable, and the audit-node carve-out below is untouched - system-bookkeeping nodes stay exempt, as they already are. The chokepoint that enforces this narrowing: `lib/core/navigation/transitions.cjs::promoteNodeStatus` refuses an agent-attributed transition into `superseded` for a truth-claim node as of Phase 348. Full contract: `docs/SUPERSESSION-CONTRACT.md`.
...
- Memory locality passes through two constitutional chokepoints, not one: `lib/core/navigation.cjs` for navigate/read and `lib/core/node-insert.cjs` for write. See the subsection immediately below - navigating and writing are different constitutional properties, not one gate wearing two names.

### Two constitutional chokepoints: navigate and write

Part 9 names two constitutional chokepoints where earlier canon text named only one. `lib/core/navigation.cjs` is the NAVIGATE/READ chokepoint (Phase 109, cited in full in the Implementing-phase subsection below): SQL asks "what matters around this focus node?" and returns a ranked, explained neighborhood. `lib/core/node-insert.cjs` is the WRITE chokepoint (Phase 276's TOOLHON work): every production `INSERT INTO nodes` outside two named exclusions routes through it.

These are different constitutional properties, not the same gate under two names. Navigating asks what already matters around a focus node and returns an explanation; writing asserts a NEW fact into the local mind, and a new fact must declare what KIND of knowledge it is before it is allowed to land. The write chokepoint enforces this directly: `epistemic_type` is REQUIRED on every call and is validated against a closed ten-member enum BEFORE any other work in `insertNode` - before schema detection, before any database `prepare()` call. An absent, non-string, or out-of-enum `epistemic_type` throws immediately. This is fail-closed by construction: there is no code path in `insertNode` that reaches a database write without a validated `epistemic_type` already attached.

Two coverage exclusions exist, and they are named honestly here as exclusions BY DESIGN, not as an oversight the doctrine forgot: `lib/core/navigation/memory-events.cjs` (the append-only bookkeeping-dedupe writer) and `lib/core/rs-sqlite-mirror.cjs` (the bulk-write hot path, where a per-row chokepoint round trip would defeat the batch). Nodes written by these two files carry no chokepoint-validated `epistemic_type`. The write chokepoint's call sites are still actively accreting as more of `lib/` migrates onto it; any citation of that count is a live figure enumerated from disk at run time (`lib/core/`: 82 sites / 27 files; all of `lib/`: 112 sites / 35 files, per `340-LIVE-VERIFICATION.md` item 3), never a frozen constitutional number.
...
- **R16** Born-declared shape - every invocable surface - a command, an agent, a pipeline, OR a skill
  ...
  + pipelines + qualifying skills (249 declaring as of this amendment - Appendix D entry 40,
  2026-09-05 - per 340-LIVE-VERIFICATION.md: 113 commands + 10 agents + 4 pipelines + 122 skills,
  with the declaring/excluded splits no longer disjoint on every surface class and 53 advisory
  WARN-level declaration conflicts open, non-blocking - an illustrative snapshot, not a
  canon-frozen constant a future gate may hardcode). Frozen Part 3 scalars (MAX_K=3, DIAL_REACH_K=6, the 0.70/0.15 gate) are UNCHANGED by R16;
...
**Implementing mechanisms (Phase 275).** Layer 1 - Routing's `STATEMENT` field is substituted by `lib/core/room-skeleton-scaffold.cjs` at room-birth time, the one-sentence, always-visible purpose per section. Layer 2 - Contracts' per-section `CONTEXT.md` is written by the same file's `writeSectionContracts` function. Layer 3 - Reference's `references/` factory directory is written by `writeReferenceDocs` and registered as a structural (non-section) directory in `lib/core/section-registry.cjs`. Layers 0 and 4 carry no new citation here; the five Layer-to-Part mappings above stay byte-identical.
...
- Brain - the remote methodology repository, its current implementation Theo at theo-mcp.onrender.com (getBrainUrl() in lib/core/brain-client.cjs is the single origin resolver every runtime site reads). Strategic thinking tools only. Never a store for user data.
...
39. **Local graph and room substrate brought to currency: Part 9 two-chokepoint split, Part 4 edge-vocabulary reconciliation, Appendix B ICM citations (Phase 340 Wave B, 2026-09-05).** Phase 340 (canon-currency-audit-and-amendment) amended three Parts in one wave because they are one doctrinal act - the Canon's account of its own local substrate. Part 9 (Memory Locality and Interpretation) now names TWO constitutional chokepoints as two different constitutional properties rather than one citation folded into existing prose: `lib/core/navigation.cjs`, the NAVIGATE/READ chokepoint Phase 109 shipped, and `lib/core/node-insert.cjs`, the WRITE chokepoint Phase 276's TOOLHON work shipped with fail-closed `epistemic_type` validation, still actively accreting callers, and never before named in this Canon. Navigating asks "what matters around this focus node" and returns a ranked, explained neighborhood; writing asserts a new fact into the local mind and must declare what KIND of knowledge it is before it lands - two different constitutional properties, not one gate wearing two names. The write chokepoint's two documented coverage exclusions (`lib/core/navigation/memory-events.cjs` append-only bookkeeping dedupe, `lib/core/rs-sqlite-mirror.cjs` bulk-write hot path) are named honestly as exclusions BY DESIGN, not silently omitted. Part 4 (Every Choice Is Graph Data) reconciles its closed typed-edge vocabulary prose with what `lib/core/navigation/edges.cjs` actually ships: fifteen edge types - REMEMBERED_AS, ATTRIBUTED_TO, NOT_REMEMBERED_BECAUSE (Phase 189-01 memory governance), SHARES_JOB, ELEVATES_TO (Phase 205-02 cross-frame elevation), UMBILICAL_TO (Phase 195-04 cross-room), DISCOVERED, AUTHORED_BY (Phase 200-02 reverse-salient discovery), COMPETES_WITH, USES_COMPONENT, SUPPLIES_TO (Phase 218-01 entity relationships), CONCERNS (quick task 260725-9ca), MAPS_TO_SECTION (quick task 260806), SOURCED_FROM, USES_FRAMEWORK (quick tasks 260903-gct / 260903-i2x) - shipped additively across these phases without a matching canon-text amendment landing at the time, the exact Part-4-self-CONTRADICTS drift pattern entries 18, 21, and 23 each closed for an earlier cohort of types. Unlike entries 34 through 37, this entry does not claim to mint no edge type: it RECONCILES fifteen already-shipped edge types into the closed vocabulary the Canon's own prose should have carried since each shipped, named truthfully rather than implied as always ratified. Appendix B (Relationship to ICM Layers 0-4) gains real file citations to Phase 275's shipped L1/L2/L3 mechanisms - the `STATEMENT` field substitution, the per-section `CONTEXT.md` writer, and the `references/` factory directory, all in `lib/core/room-skeleton-scaffold.cjs` (with `lib/core/section-registry.cjs` registering `references/` as a structural directory) - closing a five-row table that carried zero file citations while the rest of this Canon cites concrete files everywhere else. The five existing Layer-to-Part mappings are byte-identical; this adds citations, it does not re-map layers. This was navigator-APPROVED at a blocking checkpoint on 2026-09-05 BEFORE any canon byte was written (the navigator confirmed the version target v1.25 -> v1.26). Entry 31's self-binding clause (no further Appendix D entry until entry 31 returns a live two-gauge reading) was RELEASED for this amendment by the Part-10 navigator-authority override (entry 20), recorded truthfully - no two-gauge reading was taken or fabricated, the release rests on navigator authority, the deferred reading stays a named debt, mirroring entries 32/33/34/35/36/37/38. Applied via the Part 6 dog-fooding canon-amendment-on-itself mechanism, mirroring entries 14/15/25/26/27/36/37/38. This amendment RATIFIES already-shipped code rather than minting new mechanisms, so canon and code are consistent the instant it lands, mirroring entry 35's framing: the write chokepoint, the fifteen edge types, and the three ICM mechanisms all already run in production; this entry brings the Canon's own text into agreement with them. The amendment mints NO new reach and NO new node type and opens NO Brain wire. The frozen Part 3 scalars are byte-identical: MAX_K=3, DIAL_REACH_K=6, the 0.70/0.15 gate are UNCHANGED. Landed as ONE atomic lockstep wave so CI never went RED: the Part 9 two-chokepoint subsection + the Part 4 fifteen-type reconciliation + the Appendix B citations + this Appendix D entry 39 + the header/footer Version 1.25 -> 1.26 + the CANON-PHASE-MAP v1.26 version-history row + the new canonical FLOOR test (`tests/test-canon-entry-39-graph-substrate-floor.cjs`, registered in `tests/run-all-340.sh` - a multi-Part placement-proof asserting each citation lands in its own Part slice, prior entries 1-38 preserved, frozen scalars byte-present, version 1.26, never a raw count of Appendix D entries or edge types) + the entry-31 FLOOR test version anchor moved 1.25 -> 1.26 (`tests/test-canon-entry-31-two-gauge-floor.cjs`, its byte-for-byte scalar assertions unweakened) + the entry-36 FLOOR test version anchor moved 1.25 -> 1.26 (`tests/test-canon-entry-36-shape-declaration-floor.cjs`, its prior-entry loop and frozen-scalar assertions unweakened) + the frozen-scalar FLOOR test (`tests/test-canon-frozen-scalars-floor.cjs`) kept GREEN, all moving together. Requirement ids: CANON-02, CANON-03, CANON-04. Implementing phases: 109 (navigate chokepoint), 276 (write chokepoint), 189-01 / 195-04 / 200-02 / 205-02 / 218-01 plus quick tasks 260725-9ca / 260806 / 260903-gct / 260903-i2x (the fifteen edge types), 275 (ICM L1-L3 mechanisms); this Phase 340 Plan 03 is the canon ratification. Header/footer Version 1.25 -> 1.26. Canon version bumped to 1.26.

40. **Corpus figures corrected and retired-backend references closed (Phase 340 Wave C, 2026-09-05).** Phase 340 (canon-currency-audit-and-amendment) corrected four stale figures the part-by-part audit found - each a fact about the stack that moved underneath the Canon's prose, not a doctrine change. Appendix C's Glossary Brain bullet named the retired Memgraph-era origin (was `pws-brain-mcp.onrender.com`; now names Theo's live origin `theo-mcp.onrender.com`, with `getBrainUrl()` in `lib/core/brain-client.cjs` cited as the single origin resolver every runtime site reads - "Brain" stays the constitutional ROLE name per Phase 339 D-09, Theo is named as its current implementation, not a rename). Part 2 Engine 1's CROSS-DOMAIN MATCH backend clause and code-reference bullet named a retired vector store (was Pinecone, 12,401 embeddings; now e5 / multilingual-e5-large, 1024-dim, embedded LOCALLY with no network egress, corpus size stated with the caveat that `docs/CORPUS-STATS.generated.md` describes the retired incumbent's corpus and has not been regenerated against Theo; the five command wrappers /mos:whitespace, /mos:find-bottlenecks, /mos:find-connections, /mos:find-analogies, /mos:score-innovation are unaffected). Part 7's reuse-before-build instruction bound a binding obligation to a stale count (was an instruction pinned to a fixed count of twenty-five commands, roughly a quarter the real surface; now the builder searches the methodology surface enumerated from disk - commands/*.md, plus agents/*.md, pipelines/*/CHAIN.md, skills/*/SKILL.md where the work spans surface classes - 113 commands as of this amendment, illustrative only, per 340-LIVE-VERIFICATION.md; the obligation itself is unweakened). Part 11's R16 illustrative surface-count snapshot was stale (the prior snapshot read one hundred twenty-six: 105 + 9 + 3 + 9, plus 5 skills exempt via connector.excluded; now 249 declaring as of this amendment per 340-LIVE-VERIFICATION.md: 113 commands + 10 agents + 4 pipelines + 122 skills, with the declaring/excluded splits no longer disjoint and 53 advisory WARN-level conflicts open, non-blocking). This is a factual and stack-currency correction only - no doctrine change lands in Part 7, Part 8, or Part 11: Part 7's reuse-before-build obligation remains exactly as binding, and Part 11's self-disclaiming doctrine sentences (NEVER a frozen scalar, ALWAYS enumerated from disk at run time, not a canon-frozen constant a future gate may hardcode) are byte-identical - only the illustrative parenthetical moved. Appendix D entries 11, 13, 16, and 36 are historical record of what was true when each was written and stay byte-identical; this entry supersedes them in place by recording the newer read, exactly as entry 16 superseded entry 13 without editing it. This was navigator-APPROVED at a blocking checkpoint on 2026-09-05 BEFORE any canon byte was written (the navigator confirmed the version target v1.26 -> v1.27) and separately ruled three named judgment calls: Ruling A = COMMANDS - the Core Value sentence's "25 methodology bots" figure is rewritten to "a growing methodology-command surface enumerated from disk" because that sentence describes what the live plugin delivers to a prospective user today, not the separate MindrianV2 codebase's historical 25-bot-prompt count, and leaving "25" there would misleadingly read as the live command surface; Ruling B = REMOVE ROW - the docu-optimizer row is deleted from CLAUDE.md's Project Skills table (the table now reads "No project skills currently registered") since `.claude/skills/docu-optimizer/SKILL.md` does not exist anywhere in the tree, confirmed live by `340-LIVE-VERIFICATION.md` item 14; Ruling C = state-with-caveat - Part 2's corpus-size clause states the figure's source with the "describes the retired incumbent, not regenerated against Theo" caveat rather than omitting it, per the Sourced Claims Doctrine Wave A of this same phase ratified (a source exists, scoped to the wrong backend, and is cited honestly with that scope named rather than hidden). Entry 31's self-binding clause (no further Appendix D entry until entry 31 returns a live two-gauge reading) was RELEASED for this amendment by the Part-10 navigator-authority override (entry 20), recorded truthfully - no two-gauge reading was taken or fabricated, the release rests on navigator authority, the deferred reading stays a named debt, mirroring entries 32/33/34/35/36/37/38/39. The frozen Part 3 scalars are byte-identical: MAX_K=3, DIAL_REACH_K=6, the 0.70/0.15 gate are UNCHANGED. The amendment mints NO reach/edge/node, opens NO Brain wire. `CLAUDE.md` carries the identical corrections in the SAME commit - its Canon Compliance Core Part 7 bullet, its Conventions reuse-before-build bullet, its Part 11 parenthetical, its Core Value sentence (per Ruling A), and its Project Skills docu-optimizer row (per Ruling B) - closing 340-RESEARCH.md Pitfall 3 (amending the Canon without the parallel CLAUDE.md fix in the same wave) structurally, not just this once. Requirement ids: CANON-05, CANON-06, CANON-07, CANON-08, CANON-09. Implementing phase: 339 (Theo cutover, cited here, not modified by this entry); this Phase 340 Plan 04 is the canon ratification. Landed as ONE atomic lockstep wave so CI never went RED: the Appendix C Brain bullet + the Part 2 backend clause and code-reference bullet + the Part 7 rewrite + the Part 11 parenthetical + this Appendix D entry 40 + the header/footer Version 1.26 -> 1.27 + the CANON-PHASE-MAP v1.27 version-history row + CLAUDE.md's five sibling edits (Canon Compliance Core Part 7 bullet, Conventions reuse-before-build bullet, Core Value sentence per Ruling A, Canon Compliance Core Part 11 parenthetical, Project Skills docu-optimizer row per Ruling B) + the new canonical FLOOR test (tests/test-canon-entry-40-corpus-figures-floor.cjs, registered in tests/run-all-340.sh - slice-scoped presence-and-absence assertions across Appendix C, Part 2, Part 7, Part 11, Appendix D, and CLAUDE.md, prior entries 1-39 preserved, frozen scalars byte-present, version 1.27, never a raw count of Appendix D entries or surfaces) + the entry-31 FLOOR test version anchor moved 1.26 -> 1.27 (tests/test-canon-entry-31-two-gauge-floor.cjs, its byte-for-byte scalar assertions unweakened) + the entry-36 FLOOR test version anchor moved 1.26 -> 1.27 (tests/test-canon-entry-36-shape-declaration-floor.cjs, its prior-entry loop and frozen-scalar assertions unweakened, including its CANON-PHASE-MAP /v1\.26/ token moved to /v1\.27/) + the frozen-scalar FLOOR test (tests/test-canon-frozen-scalars-floor.cjs) kept GREEN, all moving together. Header/footer Version 1.26 -> 1.27. Canon version bumped to 1.27.

41. **Part 9 narrowed: only a human-attributed gate answer may close a truth-claim node as `superseded` (Phase 348, 2026-09-16).** Phase 348 (the-supersession-node) narrows Part 9's Truth states (canonical) subsection for one target status. As written, that subsection permitted "user confirmation or system rules" to promote any status, including `superseded`; the roadmap card that opened Phase 348 forbids a system rule from closing a believed fact with no human, and this entry brings the Canon's own text into agreement with the code Phase 348-03 already shipped. The narrowing: for the `superseded` target specifically, on a truth-claim node, the "or system rules" clause no longer applies; only a human-attributed gate answer may promote a truth-claim node's status to `superseded`. The reason, in one clause: closing a believed fact is itself an assertion about the venture's world, carrying the same human bar role 5 already places on promoting a node to `confirmed`. What is NOT narrowed, stated so the narrowing cannot be over-read: `rejected` and `stale` remain agent-reachable, and the audit-node carve-out (system-bookkeeping nodes exempt) is untouched. This entry RATIFIES already-shipped code rather than minting a new mechanism: `lib/core/navigation/transitions.cjs::promoteNodeStatus`, the truth-state chokepoint, has refused an agent-attributed transition into `superseded` for a truth-claim node since Phase 348-03, so canon and code are consistent the instant this entry lands. This was navigator-APPROVED at a blocking checkpoint on 2026-09-16 BEFORE any canon byte was written (the navigator confirmed the version target v1.27 -> v1.28). Entry 31's self-binding clause (no further Appendix D entry until entry 31 returns a live two-gauge reading) was RELEASED for this amendment by the Part-10 navigator-authority override (entry 20), recorded truthfully - no two-gauge reading was taken or fabricated, the release rests on navigator authority, the deferred reading stays a named debt, mirroring entries 32/33/34/35/36/37/38/39/40. Applied via the Part 6 dog-fooding canon-amendment-on-itself mechanism, mirroring entries 14/15/25/26/27/36/37/38/39/40. The amendment mints NO new reach, NO new edge type, NO new node type, and opens NO Brain wire. The frozen Part 3 scalars are byte-identical: MAX_K=3, DIAL_REACH_K=6, the 0.70/0.15 gate are UNCHANGED. Requirement id: SUPER-15. Implementing phase: 348 (the 348-03 guard); this Phase 348 Plan 09 is the canon ratification. Landed as ONE atomic lockstep wave so CI never went RED: the Part 9 narrowing paragraph + this Appendix D entry 41 + the header/footer Version 1.27 -> 1.28 + the CANON-PHASE-MAP v1.28 version-history row + the new canonical FLOOR test (`tests/test-canon-entry-41-supersession-narrowing-floor.cjs`, registered in `tests/run-all-340.sh` - slice-scoped presence-and-absence assertions inside Part 9, entry 41 body isolation, prior entries 1-40 preserved, frozen scalars byte-present, version 1.28, never a raw count of Appendix D entries) + the parallel `CLAUDE.md` Part 9 Canon Compliance Core bullet carrying the identical narrowing in the SAME commit (closing 340-RESEARCH.md Pitfall 3) + six existing canon FLOOR tests' version anchors moved 1.27 -> 1.28 (`tests/test-canon-entry-31-two-gauge-floor.cjs`, `tests/test-canon-entry-36-shape-declaration-floor.cjs`, `tests/test-canon-entry-38-sourced-claims-floor.cjs`, `tests/test-canon-entry-39-graph-substrate-floor.cjs`, `tests/test-canon-entry-40-corpus-figures-floor.cjs`, `tests/test-canon-part-9-ratification.cjs`, each with its prior-entry loop and frozen-scalar assertions unweakened) + the frozen-scalar FLOOR test (`tests/test-canon-frozen-scalars-floor.cjs`) kept GREEN, all moving together. Header/footer Version 1.27 -> 1.28. Canon version bumped to 1.28.
