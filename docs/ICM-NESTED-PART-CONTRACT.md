...
Six gaps, carried from `docs/2026-09-14-LAYER-CONTRACT-AND-ICM-MAP.md` section 5, each with the evidence and the owner: a registered phase where one exists, or the word "unowned."

1. **No room-level `CONTEXT.md` (L1).** Phase 275 shipped L1 as a per-section STATEMENT field and L2 as the per-section `CONTEXT.md`; the room root itself holds identity, state, reasoning, and user files and has no router file answering "where do I go from here." Evidence: `ROOM.md`, `STATE.md`, `USER.md`, and `section-reasoning` all sit at the room root with no L1 entry file tying them together. Owner: unowned.
2. **No objects, processes, or change-impact cards.** Nothing in the repo answered "what else moves if I change X" before this document. Evidence: the room graph (`.mindrian/room.db`) could carry this answer as typed edges, but no artifact expressed it in a form a person could read directly. This document's own Change-impact tables, one per part in section 3 above, are this phase's direct answer to this gap. Owner: 344-07 (this document); stated plainly that a documented answer is not the same as a machine-enforced one, since nothing yet fails a build when a part's declared consumer set goes stale beyond the drift test in section 6 below.
3. **The L2 contracts have no consumer.** The best-realized ICM artifact in the codebase, the per-section `CONTEXT.md`, was inert. As of this phase, that is no longer fully true: `lib/core/doctor/icm-part-wiring-module.cjs` (344-05) reads the `## Commands that write here` and `## Inputs` headings under every registered room's per-section `CONTEXT.md` and counts them, under `node scripts/doctor.cjs --cascade-rooms`. Stated precisely, so the closure is not overstated: this is a READ that validates the contract's own shape is present, not a READ that injects the contract's content anywhere. It adds no assembly path into `getRoomContext` or any other context leg, which is exactly why the one-context-assembler question (WD-5) stays open and untouched by this phase. Owner: 344 (the counting half, closed 2026-09-14); the content-consuming half is unowned.
4. **Four incompatible statements of what ICM L0-L4 maps to.** `.claude/includes/architecture.md:7-11`, `docs/ARCHITECTURE-DEEP-DIVE.md:9-13` (which names a `ROUTING.md` that does not exist), `templates/icm/CLAUDE.md:13-17`, and `docs/MINDRIAN-CANON.md:759-768` each state the mapping differently, and `fleet-index` above (`INDEX.md` vs `.rooms/registry.json`) is a live symptom of the same disagreement. Owner: 344-08.
5. **Phase 275 left L4 (the factory-product split) report-only by design.** Invariant 5 (the factory/product boundary) is declared, not enforced; no gate fails a room that blurs the two. Owner: unowned; this is a stated, deliberate limitation of Phase 275's own scope, not a scheduled follow-up.
6. **Three registered, unbuilt phases carry the rest of the ICM validation instrument.** Phase 290 (the fractal memory contract), Phase 332 (the provenance sidecar), and Phase 334 (the room walk-test against the ICM reference form, ICM's own validation instrument) are all registered in the roadmap and none has run against a live MindrianOS room as a gate yet. Owner: 290, 332, and 334, respectively.

## 6. How this is measured

`lib/core/doctor/icm-part-wiring-module.cjs` is the organ. Its repo-side leg runs on every `node scripts/doctor.cjs` invocation and resolves this document's own declaration file, `data/icm-parts.json`, against the generated command registry and the filesystem; its live-room leg runs only under `node scripts/doctor.cjs --cascade-rooms` and adds the per-section `CONTEXT.md` heading census described in gap 3 above. The organ keeps to a counts-only discipline throughout: it reports raw counts with `status: 'ok'`, it carries no threshold of its own, it exports no `fix`, and it never states that a part is "missing" a consumer, only that a count is zero, which is a fact rather than a verdict. The threshold question, when a zero count should actually block something, belongs to SEED-074's own gate, and this phase deliberately does not open that gate here. No count measured by the organ is reproduced in this document, so this document cannot go stale the moment the organ's next run reports a different number.
