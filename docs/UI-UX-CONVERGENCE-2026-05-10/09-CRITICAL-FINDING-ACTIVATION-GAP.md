...
## 7b. Phase 91 already shipped -- the shell, not the wiring (correction, 2026-05-10)

The Navigation Engine (**Phase 91**) shipped in v1.11.0 (2026-05-01): `lib/core/navigation-engine.cjs` `decide()` core, the UserPromptSubmit integration that emits the `## NAVIGATION DECISION` block we see every turn, the 8-field trace contract, the tier modes. So the gap is NOT "Phase 91 needs to be built." It is: **the shipped engine is not wired to the graph or the Brain** -- `decide()` returns `fire_skill: null` / `offer_next_step: null` and `routing_source: legacy` because it reads file-presence, not `{local graph + BRAIN.md}`. The closing-the-loop work for v1.13.0 is therefore *wiring*, not *building*: make `decide()` consume `{graph state + BRAIN.md + the trigger map (00c)}` and populate `fire_skill`. Candidate vehicle: a new small phase ("91.6 navigation-engine-graph-wiring", ~2-3 days) OR fold into Phase 95 (already touches hook envelopes) + Phase 109 (the SQL spine). This is the single change that flips `routing_source: legacy` -> `engine`. See `00c-TRIGGER-MAP.md` Section 9.

## 8. Deepening the disruption -- six moves to ship the discontinuity (not just the blueprint)

The dominant-design analysis (`docs/UI-UX-CONVERGENCE-2026-05-10/` -- the "queryable assistant" is the prevailing design; the discontinuity is *initiative*) says MindrianOS has *designed* the post-discontinuity product but not *shipped* it. A blueprint is the moat -- but a blueprint is not a moat. Six moves, in order:

1. **Ship one sensor that visibly fires.** Not the engine -- one. The most visceral: first material -> auto-`explore-domains` + `whitespace` + `brain_framework_chain` -> the room is non-empty by turn 2 (Phase 117's cheap version). The day a session shows `routing_source: engine` instead of `legacy`, MindrianOS has *touched* the new design instead of just drawing it. Everything else is downstream of one trigger firing once.
2. **Make the receipt felt.** The room is invisible during a session (a folder you would have to go look at). Make it show -- the wiki/dashboard filling live as the conversation produces artifacts (v1.14.0); the room producing something useful in 30 seconds before any "investment" (Phase 118); the room as a *receipt* of the conversation, not a setup wizard (Phase 119). The new design is proven by the founder watching the room fill while she talks, not by an architecture diagram.
3. **Move the human from initiator to confirmer.** The Decision Gate (Shape F, Phase 88.2) only counts if it renders "here's what the system surfaced -- APPROVE / REJECT / DEFER", not "what do you want to do?" One is the queryable design (you decide what to do); the other is the new design (the system proposes, you ratify). That distinction *is* the disruption, expressed in one UI component.
4. **Instrument it so you can prove it.** The `routing_source: engine` trace, the 57x telemetry, the Brain-call count (64 -> climbing). The disruption you cannot measure is the disruption you cannot defend -- in a pitch, in a release gate, in a board meeting. Fix the dead telemetry hook (scout bug #4).
5. **Go deeper into the wedge, not wider.** The queryable design serves the general user fine and serves the *stuck founder who cannot name her decision* terribly (she does not know what to prompt). The new design serves her *because* it acts unasked. Double down on that user -- the owned-emotion first-touch (Phase 115), the dual-path opener, the empty-room-that-fills-itself. Out-proactive the queryable tools for the one user they fail; do not try to out-queryable them for everyone.
6. **The flywheel: the moat compounds with use, not features.** Every Decision Gate choice writes a typed edge (rejection is data, Canon Part 4). "The graph that knows WHEN" gets *calibrated* by real decisions -- so the disruption deepens *automatically* once the trigger list fires: the more it acts, the better it knows when to act. The flywheel does not spin while the loop is broken. Which orders everything: fix the floor (95.6) -> ship one sensor (move 1) -> make the receipt felt (moves 2-3, v1.14.0) -> wire the full engine. Every week spent polishing the queryable surface (better `/mos:` commands, prettier picker) instead of the proactive one is a week the disruption is not deepening -- and a week someone else might use to ship it first. The activation gap is the gap between *having designed the discontinuity* and *being it*.
