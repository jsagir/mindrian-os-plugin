...
| **"What's running on my machine?"** | the Wave-2 tester (cautious, reads system output) | "I think it's concerned the Mindrian will run for every Claude project I open, not just those associated with Mindrian?" | Scope transparency. He wants to *see* what surface MindrianOS occupies. |
...
- **"What's running on my machine?" -- consider a scope-visibility surface** (a one-line "MindrianOS active in: [scope]" indicator). Ties to the transparency value (Canon Part 8 graph boundary); the tester's persona note predicts it "will probably resonate when explained." Make it *visible*, not just explainable.
...
- **No tester evidence yet -- don't over-invest:** one-vs-two visual systems (1), De Stijl literalness (2), red-link colors (5), motion (9), dashboard redesign (10). The only thread is the tester's "recommend to the board" hypothesis -> design the **SnapshotHub export** for that audience; let the rest wait.
