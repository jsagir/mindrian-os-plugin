...
| `00c-TRIGGER-MAP.md` | The Brain-derived trigger map -- what auto-fires, on what signal, against what local-graph and memory-context state. The rigorous version of SEED-008's "The Trigger List": stage-axis sensors (Pre-Opportunity -> Ready to Build, from `TYPICAL_AT`), event-driven sensors (incl. the deep-research escalation sensor), scheduled sensors (incl. `/mos:scout`), meta/orchestrator commands, the local-graph contract, the memory-context contract, the closed-loop cycle, and what it means for v1.13.0. **This is the executable spec the shipped Phase 91 engine must be wired to.** | direct Brain consultation (Neo4j Aura live) |
...
- **Not ratified.** This is a working draft. The Minto pyramid in `08` rests on one untested assumption -- that Phase 121.5 can absorb the promoted token-core + Canon-v2 + linter scope without blowing its "last phase before the v1.13.0 final gate" timing. Run `/mos:challenge-assumptions` on that before committing the roadmap re-order. The `09` finding has a second untested assumption: sensors-first vs full Navigation Engine.
- **Planted as a seed.** The activation-gap finding (`09`) is captured as **`SEED-008`** (`.planning/seeds/SEED-008-intelligence-layer-activation-gap-close-the-loop.md`) -- a v1.13.0 FINAL RELEASE GATE concern, embedded in `.planning/milestones/v1.13.0-CLOSED-LOOP-ROADMAP.md` (`related_seeds`, Seed Disposition table, the "Loop-fires gate" blocker in the final-section). It carries the WHY / ACCEPTANCE (the 5-check "loop fires" test) / BREADCRUMBS forward so no Phase 91 / 95 / 109 / 117 / BRAIN.md-derivation work gets planned in isolation from the loop-closure picture, and so the gate doesn't ship a broken loop under the name "The Closed Loop."
- **Companion edits made this session:** `.planning/TODO.md` -- the v1.14.0 "The Visible Room" entry and the v1.10.8 smart-notebook entry were rewritten with explicit style-guide + UI/UX-acceptance-bar blocks (and the v1.14.0 entry's open-questions list gained the "extend ui-system to a browser surface?" + "sequence vs Phase 121.5" questions). `.planning/STATE.md` -- quick task `260510-or6` recorded. `.planning/milestones/v1.13.0-CLOSED-LOOP-ROADMAP.md` -- SEED-008 embedded (see above).
