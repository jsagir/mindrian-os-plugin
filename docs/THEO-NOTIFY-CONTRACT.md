...
| R4 | `VERSION-BUMP-CHECKLIST.md` and the release-process include name the step; `docs/OPEN-HANDOFFS.md` gains the Theo-side row | 349-05. WD-14 contradiction RULED at 349-03's checkpoint: WD-14 stands (`VERSION-BUMP-CHECKLIST.md` is NOT created); the roadmap's deliverable 4 is satisfied instead by amending `docs/RELEASE-CEREMONY-RULING-SYSTEM.md` RULE 5 place 8 plus `.claude/includes/release-process.md`, with the reconciliation recorded in NOTIFY-10 and the close-out. |
| R5 | A one-time bootstrap: before the phase closes, Theo has restamped against the current plugin version so the 343-07 gate reads green | **RULED at 349-03's checkpoint: disposition (ii), out of scope, registered as a card.** 349-06 registers a real, numbered `.planning/ROADMAP.md` card for the Theo-side consuming phase with a named owner and a concrete acceptance number (the same pattern Phase 348 used registering Phase 350 for its own deferred writer). NOTIFY-14 closes against the card's existence, not against a live restamp, and the phase close-out states in plain words that four of five roadmap deliverables shipped here and the fifth has an owner. See WD-349-9. |
...
## Navigator Ratification (349-03 checkpoint, 2026-09-16)

Recorded here, not re-derived, per this plan's own instruction that the library encode what was
ratified rather than a re-derivation of it. The orchestrator ran every verification command in
349-03's `how-to-verify` block live in the conversation (`git diff --name-only scripts/` empty,
`bash -n scripts/release.sh` exit 0, `bash tests/run-all-349.sh` PASS=9 FAIL=0 SKIP=3
EXPECTED-RED=2, `node scripts/doctor.cjs --acceptance` 20/20, `bash scripts/release.sh patch
--dry-run` showing the live `theo-stamp-gate: MISMATCH -- Theo's stamp is
command-registry@2.0.0-beta.12, expected command-registry@2.0.0-beta.40` line and 118 commits
ahead of origin, `git log --oneline -3`, `node lib/core/repo-version.cjs` = 2.0.0-beta.40), then
presented all five design forks plus the R5 disposition to the navigator via two real
AskUserQuestion gates. The navigator answered "approved as specified" on forks 1-4 and "out of
scope, register a card (Recommended)" on R5, with zero fork overrules.

**Forks 1-3, approved as specified (WD-349-2, WD-349-3, WD-349-4 above, flipped WORKING -> RULED):**
Step 5.6 as the step name; `--no-theo-notify` as a separate flag from `--no-theo-check`; the
plugin-computed live SHA-256 of the tagged commit's `data/command-registry.json` bytes as the
outbound `registryHash` meaning.

**Fork 4, the `VERSION-BUMP-CHECKLIST.md` contradiction, approved as specified: WD-14 stands.**
The two texts, quoted verbatim, side by side:

> Phase 343's WD-14 (RULED, STANDING, 2026-09-14): "`VERSION-BUMP-CHECKLIST.md` is NOT created in
> this repo. The seventh place is the `release.sh` step plus RULE 5's enumeration; the roadmap's
> reference to that filename is recorded as resolved."

> Phase 349's own roadmap card, deliverable 4 (2026-09-15): "VERSION-BUMP-CHECKLIST.md and the
> release-process include name the step."

Ruling: option (a). The roadmap's filename is shorthand for the resolution WD-14 already
recorded; deliverable 4 is satisfied by editing `docs/RELEASE-CEREMONY-RULING-SYSTEM.md` RULE 5
place 8 plus `.claude/includes/release-process.md` (349-05's scope), with the reconciliation
written down in NOTIFY-10 and in the close-out. WD-14 is NOT reversed. No new `WD-349-N` row is
minted for this fork; the R4 scope-table row above carries the ruling.

**Fork 5 / R5 disposition, ruled: (ii) out of scope, registered as a card (WD-349-9 above, flipped
WORKING -> RULED; the R5 scope-table row above carries the full ruling text).** Owner: 349-06.
NOTIFY-14 closes against the card's existence, not against a live restamp.
