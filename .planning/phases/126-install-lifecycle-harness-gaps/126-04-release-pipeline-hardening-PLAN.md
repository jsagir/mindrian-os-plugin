...
must_haves:
  truths:
    ...
    - "If MINISITE_DIR is unset AND --no-minisite is not passed, release.sh fails loud with an actionable error message + the exact `git remote add origin <url>` recovery command (Open Question 7 settled)"
    ...
    - "Dry-run mode shows all THREE new gates (Step 5.5, Step 9.6, Step 9.7) in --dry-run output"
  ...
  artifacts:
    - path: "scripts/release.sh"
      provides: "Step 5.5 (tag-push verification), HARD Step 9.6 (install-minisite lockstep), Step 9.7 (npx-publish self-test), --no-minisite flag, MINISITE_DIR + MINDRIAN_MINISITE_URL env vars"
      contains: "Step 5.5|Step 9.7|--no-minisite|MINDRIAN_MINISITE_URL"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 5 (install-minisite HARD -- MINISITE_DIR absent + no --no-minisite): unset MINISITE_DIR + MINDRIAN_INSTALL_SITE_DIR + ensure default $HOME/mindrianos-install-site does NOT exist (mock HOME to mktemp). Run release.sh WITHOUT --no-minisite. Assert release.sh exits non-zero with actionable error mentioning `--no-minisite` AND the recovery command `git remote add origin <url>`.
    - Test 6 (install-minisite HARD -- MINISITE_DIR set but origin remote missing): sandbox a minisite without an `origin` remote configured. Run release.sh. Assert: actionable error includes the exact `cd $MINISITE_DIR && git remote add origin <url>` recovery command (Open Question 7 settled).
    ...
    - Test 13 (--dry-run shows all 3 gates): run `release.sh --dry-run`. Assert output mentions "Step 5.5", "Step 9.6", "Step 9.7" by name.
  </behavior>
  <action>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - feedback_install_minisite_lockstep.md (the HARD contract spec from the memory rule -- this is the source of truth for Step 9.6)
  </read_first>
  <behavior>
  ...
  <action>
    ...
    - **Step 9.6 HARD (install-minisite lockstep)**: REPLACE the existing soft block (lines 433-517) with the HARD implementation. Behavior:
      ...
      if [ "$NO_MINISITE" = "1" ]; then
        ...
        if [ ! -d "$MINISITE_DIR" ]; then
          ...
          echo "      A) Set MINDRIAN_MINISITE_DIR to your minisite checkout path and re-run."
          echo "      B) Clone the minisite: gh repo clone <org>/mindrianos-install-site $MINISITE_DIR"
        ...
        if [ "$DRY_RUN" = "1" ]; then
          ...
          # Origin remote check (Open Question 7 settled: fail loud, do not auto-create)
          if ! git remote get-url origin >/dev/null 2>&1; then
            echo -e "${RED}  x $MINISITE_DIR has no 'origin' remote configured.${NC}"
            echo "    Recovery: cd $MINISITE_DIR && git remote add origin <git-url>"
            echo "              (e.g., git remote add origin git@github.com:<org>/mindrianos-install-site.git)"
    ...
    - **Step 9.7 (npx-publish self-test)**: INSERT between the HARD Step 9.6 above AND the existing full --acceptance Step (renamed to Step 9.8). Behavior:
      ...
      ```
    - **Step 9.8 (renamed existing Step 9.6 full --acceptance)**: keep existing block content unchanged; UPDATE the comment header + echo line to read `Step 9.8` (resolving the numbering collision).
    - **--dry-run output (lines 184-215)**: extend with new step names. Add lines mentioning "Step 5.5" (tag-push verify), "Step 9.6" (minisite HARD), "Step 9.7" (npx self-test), "Step 9.8" (full --acceptance).
  ...
  <verify>
    <automated>bash -n scripts/release.sh && node tests/test-release-bump-tag-and-publish-gates.cjs && bash scripts/release.sh --dry-run | grep -E "Step 5\\.5|Step 9\\.6|Step 9\\.7|--no-minisite"</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `bash scripts/release.sh --dry-run` output contains all three new step names + `--no-minisite` mention
    - `grep -c "Step 5.5\\|Step 9.7\\|--no-minisite\\|MINDRIAN_MINISITE_URL" scripts/release.sh` returns >= 4
    - Existing tests/run-all-123.sh continues to pass (the rename of existing 9.6 to 9.8 does not break tests that grep for "Step 9.6: doctor --acceptance" because the minisite block becomes the canonical Step 9.6 + the full --acceptance becomes Step 9.8; UPDATE any test asserting the old step name -- search for grep patterns in existing tests and update them)
  ...
  <done>
    Step 5.5 / 9.6 HARD / 9.7 implemented. --no-minisite flag wired. MINDRIAN_MINISITE_URL env var honored. 13-case test GREEN. --dry-run mentions the new gates. No regression in Phase 123 release-bump-algebra test (the bump arithmetic is unchanged).
  </done>
</task>
...
- `bash scripts/release.sh --dry-run` output contains Step 5.5 / 9.6 / 9.7 / --no-minisite
- `bash tests/run-all-123.sh` passes (regression guard: tests/test-release-bump-algebra.cjs continues to pass; the bump arithmetic is unchanged)
...
- Plan 04 acceptance criteria from CONTEXT.md "Acceptance Criteria (Nyquist UAT)" block all pass:
  ...
  - Dry-run mode shows all THREE new gates (Step 5.5, 9.6, 9.7)
  - If MINISITE_DIR is unset AND --no-minisite is not passed, release.sh fails with actionable error
...
- No regression in Phase 123 release-bump-algebra test
</success_criteria>
