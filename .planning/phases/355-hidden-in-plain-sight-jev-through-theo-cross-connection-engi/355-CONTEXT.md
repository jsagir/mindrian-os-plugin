...
### Rulings after research (2026-09-23, navigator; 355-RESEARCH.md corrections C1-C10 supersede any line numbers above where they differ)
- **D-47 (C1, C2):** The direction module is consumed by a FIFTH site, `lib/core/rs-differential-scorer.cjs:574` (`scoreMeasured`, live eureka path). Because `portfolio-dimensions.cjs:176-189` scores feasibility by label, the flip is paired with swapping those two feasibility branches in the same commit so eureka's ranking does not move; a test pins the pre-flip ranking on a fixture.
- **D-48 (C7, Q2):** A finding "already carries" a Framework name when, in this order and by exact match only: frontmatter `framework:`; then `methodology:` resolved through the command registry; then the finding's title. Anything else stamps `unverified` / `handle_unresolved` / `backend: not_called`. The resulting `not_called` rate is recorded, not hidden.
- **D-49 (C6, Q3, Q5):** Path kinds: a path whose relationship types are only structural anchors (`SOURCED_FROM`, `PART_OF`, `INSTANCE_OF`, `HAS_CHUNK`, `EXTRACTED_FROM`, `MENTIONS` and the like) never counts as verification; it renders as "shares a source record" and stamps `unverified` with reason `co_sourced_only`. `strong` / `indirect` require at least one lateral relationship type on the path (`EXTENDS`, `CONTRASTS_WITH`, `COMPLEMENTS`, `PRODUCES_INPUT_FOR`, `FEEDS_INTO`, `ADDRESSES`, `SUPPORTS`, `PRECEDES`, `REQUIRES_KNOWLEDGE_OF`, `GUARDED_BY`). Non-Framework nodes on a path render with their label in brackets. Theo paths carry no direction, so hops render with an undirected ` -- EDGE -- ` joiner, never an arrow (D-28's `-->` is withdrawn). Findings with no word-versus-meaning measurement (whitespace zones, find-connections bridges) carry `direction: none`.
- **D-50 (C9, Q4):** Desktop / Cowork cannot compute a stamp this phase (the guarded Brain shim exposes no `find_connections`; a new MCP surface is out of scope). Larry on those surfaces narrates only stamps a CLI run already stored in room.db and says "not yet checked; run the CLI to verify" otherwise. Recorded as a disclosed Tri-Polar limitation; a guarded `find_connections` passthrough on the shim is a deferred phase.
- **D-51 (C7, Q6):** `data/framework-names.json` IS regenerated this phase from a live Theo `list_frameworks` read (through `brain-client.cjs`, generic handles only), as a dated, hashed snapshot in the same shape as today's (`framework_names` + `curated_extras`, snapshot date and source hash recorded); the 3 stale entries are reviewed and dropped or kept with a note; the build step is offline-replayable (`--check` reads the committed snapshot, never the network).
- **D-52 (Q7):** The Python detector step is removed from all four callers: `lib/core/intelligence-cascade.cjs:471-483`, `scout-cadence-runner.cjs:391`, `commands/scout.md:273`, `skills/scout/SKILL.md:264`; scout's step-2 documentation names the CJS RS engine; `scripts/detect-reverse-salients.py` and `compute-hsi.py` stay as offline reference with a header note.
- **D-53 (C3, C4):** The eureka side-channel `SCHEMA_VERSION` bump 1 -> 2 moves every place the research lists together (the runner, `sensor-eureka.cjs:71`, the critic tags file check at `eureka-reach-runner.cjs:127`, and the fixtures), or the guard returns `guard_unavailable` on every scan. SENS-13 has no working producer in production (`auto-explore-fire.cjs:336-340` calls `runEurekaScan` with no pair and exits early), so the filing layer writes the v2 side channel itself; the fire-once test asserts against that writer.
- **D-54 (C5):** The stamp's Theo error mapping follows the research's corrected table: `tier_denied`, `rate_limited`, `invalid_key` and text-only replies are their own reasons under `backend: unavailable` (never reported as "endpoint unknown"); only a success with `refusals` (code `FRAMEWORK_NOT_FOUND`) is `endpoint_unresolved`.
- **D-55 (C8):** All Jev dev-time work adds profiles and guards to the shared `scripts/jev-devtime-client.cjs` (Phase 356's client; pending 354-17 also edits it), never the 353 builder's `jev()`; `pool` is copied, not required, into runtime modules.
- **D-56 (C10):** The stamp rides the existing opportunity-minting call in `bankStatements` (`scripts/eureka-portfolio-report.cjs`, transaction `BEGIN` at `:1599`), not a second writer; all Theo calls complete before the transaction opens.
- **D-57 (sequencing):** Execution starts only after Phase 354 closes; plans 354-11 through 354-18 are pending and touch `gate.cjs`, `tool-router.cjs`, `brain-client.cjs`, `part8-egress-guard.cjs`, `doctor.cjs` and `jev-devtime-client.cjs`, all of which this phase edits. 354-02 has landed (`b46aff3ab`); D-39 coordinates with 354-12 and 354-16 instead. Current line anchors: Part 8 recognizer `part8-egress-guard.cjs:455-465`, `BRAIN_URL` `brain-client.cjs:40`, gate check `gate.cjs:95`.
- **D-58 (tests):** 34 test files mention `structural_transfer`; the research names the 9 (incl. one fixture) that assert Convention B and must be amended with a phase citation; the 272 baseline fixtures come from Convention A and need no change (D-06 narrowed accordingly). The regex baseline's `anolog` typo in the integrative pattern is fixed as part of the HSI measurement, and its effect on the pre-fix accuracy is recorded.
- **Open for the planner:** whether a new doctor blocker needs a row in `data/doctor-modules.json` (the `icm-ruling-eval-fresh` precedent at `scripts/doctor.cjs:1948-2039` has none).
