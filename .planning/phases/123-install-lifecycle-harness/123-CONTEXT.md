...
- **D-35:** **Doc/auth fixes:** `docs/install/BRAIN-SETUP.md` and `.env.brain.template` state explicitly that the server is `Authorization: Bearer <key>` only (NOT `x-api-key` — a raw `x-api-key` request gets a 401 whose body points at `https://mindrianos.vercel.app/brain-access`); the no-key fallback message surfaces that `brain-access` request-access URL; the CHANGELOG prose about the client "calling `brain.mindrian.ai`" is softened/annotated ("currently `*.onrender.com`, moving to `brain.mindrian.ai`"; or add the `MINDRIAN_BRAIN_URL` override note); the `getApiKey()` docstring fix from D-32(a).
...
- `docs/install/BRAIN-SETUP.md`, `.env.brain.template`, `commands/setup.md`, `install.sh` — the doc/setup surfaces: make Bearer-only explicit (D-35), `chmod 600 ~/.mindrian.env` on write (D-34), surface the `https://mindrianos.vercel.app/brain-access` request-access URL.
