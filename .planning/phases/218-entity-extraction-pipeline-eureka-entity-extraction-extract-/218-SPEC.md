...
- **SQLite write safety (added 2026-07-12, research + empirically verified against this repo's actual Node v22.22.2):** `lib/core/room-db.cjs`'s `openRoomDb()` sets `journal_mode=WAL` and `foreign_keys=ON` today but passes no `timeout` to the `DatabaseSync` constructor and no code path wraps batch writes in an explicit transaction (`node:sqlite`'s `DatabaseSync` has no `.transaction()` helper, unlike `better-sqlite3`). Confirmed by live test on this machine: two `DatabaseSync` handles on the same WAL-mode file, one holding an open write transaction — the second writer fails in 0ms with the current default (no `timeout`), but waits the full configured duration (verified 2000ms -> ~2009ms elapsed) before failing when `{ timeout: 5000 }` is passed to the constructor. `openRoomDb()` MUST pass `{ timeout: 5000, ...existing opts }` to the `DatabaseSync` constructor (the constructor option, not a separate `PRAGMA busy_timeout` exec call — both work identically, but the constructor form can't be forgotten and applies from the first statement) and MUST additionally set `PRAGMA synchronous=NORMAL` (no constructor equivalent exists for this one, confirmed — stays a `db.exec()` call). The extraction pipeline's batch node+edge inserts MUST be wrapped in a single explicit `BEGIN` / `COMMIT` (with `ROLLBACK` on error), not per-row autocommit. Rationale: without a timeout, a concurrent write from the extraction worker while a live conversation holds a write lock throws immediate `SQLITE_BUSY` with zero retry window — reads are unaffected (WAL readers never block on writers), the risk is writer-vs-writer only, which is exactly the extraction-job-vs-live-write scenario this pipeline introduces.
