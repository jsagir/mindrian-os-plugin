...
### Known gap (reconstruction note added by Plan 109-12)

This SUMMARY is a post-hoc reconstruction restored from commit 5426e97 (`docs(109-00): complete Wave 0 substrate plan`) - the original was committed on an archived worktree branch and never landed on `main` when the worktrees were merged. On reconciliation (Plan 109-12, 2026-05-12) the `lib/memory/run-feynman-tests.cjs` TEST_FILES array on `main` carried only `tests/test-navigation-migration-views.cjs` from the Phase-109 family (added later by commit 7d87ed5's migration-view-collision fix); the 15 stub registrations described in this section's original text (commit 390549b `feat(109-00): register 15 Phase 109 test stubs in feynman runner`) were evidently dropped by a later refactor before `main` HEAD. The Phase-109 stubs and filled tests still ran via direct `node tests/test-*.cjs` invocation throughout the phase (and via `bash tests/run-all.sh`), so the gap never blocked execution - only the Feynman-suite registration was missing. Plan 109-12 reconciled the registry by re-adding the 15 Phase-109 suites (`test-navigation-acceptance`, `test-navigation-focus`, `test-navigation-neighborhood`, `test-navigation-perf-10k`, `test-navigation-memory-events`, `test-navigation-insights`, `test-navigation-chokepoint-hook`, `test-navigation-packet-builder`, `test-navigation-packet-part8-leak`, `test-brain-ingestion-part-9-invariant`, `test-room-home-vs-brain-derivation-regression`, `test-canon-part-9-ratification`, `test-navigation-migration-idempotent`, `test-navigation-migration-backfill`, `test-navigation-migration-coexistence`) alongside the already-present `test-navigation-migration-views`.
