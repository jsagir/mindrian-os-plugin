...
### spend_limit_exceeded - a self-validated failure class (navigator, 2026-07-13)
- **D-11:** During this exact phase's own planning session, four parallel background agents were killed simultaneously by an account-level Claude API monthly spend cap ("You've hit your monthly spend limit"). The navigator asked what this means "for any [Mindrian] user" - a real gap: the taxonomy covered per-request rate limits (Tier-1 retryable) but not a hard account-level cap. Added as a 13th FAILURE_CLASSES value, spend_limit_exceeded (221-01), ARCHITECTURALLY DISTINCT from every other class: (1) status is ALWAYS 'blocked', retryable is ALWAYS false - structurally enforced at envelope construction, not just validated; (2) it is a FIFTH shouldOfferTier3 never-case (221-02), because offering Tier-3 LLM recovery for "the LLM is out of budget" is a bootstrapping paradox - no amount of LLM reasoning fixes the LLM being unavailable; (3) it is a structural early-return BEFORE the tier loop, bypassing Tier-1/2/3 unconditionally regardless of stage, ctx.origin, or remaining budget (unlike every other failure_class, which is tier-loop-routed); (4) routes directly to human_required/manual_intervention_required naming the exact actionable step ("raise your limit at claude.ai/settings/usage, or wait for the monthly reset"). This is now the SECOND self-validated permanent fixture in this phase (the first being the vantage/authoritative_workspace_unavailable correction from the Phase 218 vantage error) - both born from something that actually happened during this wave's own execution, not a hypothetical.
