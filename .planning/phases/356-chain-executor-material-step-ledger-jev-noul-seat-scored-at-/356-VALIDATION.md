...
| R356-03 | fixture build: one entry per registry command, each with `command`, `p_irreversible`, `flag`, `flag_source`, `text_hash`; top-level `threshold`, `false_alarm_count`, `built_at`, `policy_hash`, `build_mode` | unit + shipped legs | `node tests/test-356-ledger-build.cjs` | ❌ W0 | ⬜ pending |
...
| R356-04 / D-14 | Jev judges all commands (superseded by final D-14); every `flag` = p >= T; chain-run false alarm fails the build (appeal gate), other false alarms are listed in `false_alarms[]` | unit | same file | ❌ W0 | ⬜ pending |
