...
- **D-14 (navigator): the navigator's label wins for labeled commands.** A ledger entry's `flag` = the answer-key label for every command in the key. Jev's `p_irreversible` and the tuned threshold decide `flag` only for commands that have no label (added after the key) or whose `text_hash` changed since labeling. Every entry records `flag_source: "label" | "jev"`. The Jev-vs-label disagreement count (false alarms and misses at T over labeled rows) is still computed, recorded in the ledger and reported: it is the trust measure for Jev's calls on new commands. Consequence: zero known false alarms ship, so Larry's chains (for example `/mos:find-analogies`, `/mos:find-connections`) are not halted by known-wrong flags. The research's Checkpoint E (false-alarm ruling) is resolved by this rule and removed.
