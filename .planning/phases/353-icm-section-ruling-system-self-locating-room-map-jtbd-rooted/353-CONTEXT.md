...
- OQ-353-1. Section JTBD canon: the mapping of every core and extended section to the closed 16-job vocabulary in `data/command-registry.json`; the planner drafts it from the registry and the section contracts, the navigator ratifies it once (it is a truth about the sections, not about any venture).
