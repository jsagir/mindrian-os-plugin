...
| 219-03-T1..T3 | 219-03 | 2 | REQ-2 harvest sensor | T-219-09 egress; T-219-10 ungated writes; T-219-11 spoofed side-channel | enum/handle-only evidence bag; zero-connection suppressed; read-only producer; freshness gate | unit | `node tests/test-219-harvest-sensor.cjs` | ❌ W0 (created by 219-03-T3) | ⬜ pending |
| 219-04-T1..T3 | 219-04 | 2 | REQ-3 qualification | T-219-13 auto-confirm; T-219-15 auto-fire | only human Qualify advances lifecycle; Skip writes REJECTED_BECAUSE; nothing files without Qualify | unit + gate | `node tests/test-219-qualify.cjs`; `node scripts/check-render-coverage.cjs` | ❌ W0 (created by 219-04-T1) | ⬜ pending |
| 219-05-T1..T4 | 219-05 | 3 | REQ-4 explore chain + D-16 corpus contract | T-219-17 egress; T-219-18 auto-fire cost; T-219-21 web outage | chain NEVER auto-fires on qualify; material steps halt; room-corpus degrade with honest provenance; post-filing extraction | unit | `node tests/test-219-explore-chain.cjs` | ❌ W0 (created by 219-05-T4) | ⬜ pending |
...
- [ ] `tests/test-219-harvest-sensor.cjs` - REQ-2 fixture bridge/contradiction + Four-Lens labels + zero-connection suppression - **219-03-T3**
- [ ] `tests/test-219-qualify.cjs` - REQ-3 rejection edge + lifecycle advance - **219-04-T1 (RED-first)**
- [ ] `tests/test-219-explore-chain.cjs` - REQ-4 chain composition + Minto shape + no-auto-fire + D-16 corpus contract - **219-05-T4**
...
|----------|-------------|------------|-------------------|
| ador-ip-test live end-to-end: extraction + metadata -> eureka banking -> harvest candidates -> navigator qualifies >=1 -> [Explore] -> Minto artifact in bank with cited sources + >=2 typed edges, lifecycle `explored`, explored artifact's entities visible via graph_query (D-16) | REQ-6 | Live-room checkpoint is the ONLY thing that catches the 218-class gap (fixture-green lied twice); requires human Qualify at the card | Plan 219-06 (Wave 4): Task 2 automated pipeline + evidence, Task 3 blocking navigator checkpoint; evidence in 219-VERIFICATION.md |
