...
- **Pre-flip reference reading (Session T, Theo commit `90219b4`, remote origin over HTTPS, keyless, SSE-framed, taken 2026-09-03 19:16Z; script `parity/20260903-coverage/preflip.py`, data `preflip-reference.json`, filed in `09-12-CLOSEOUT-STAGING.md` section E). FLIP-12 compares the installed-session reading against this line for line:
  ...
  - `brain_ask {op: "list_frameworks"}` (Theo commit `dfb44b2`, script `parity/20260903-coverage/preflip_op.py`, 1.189 s): payload keys EXACTLY `{coverage, diagnostics, op, rows}`; `op` = `list_frameworks`; `coverage {matched: 50, total: 420, status: "partial"}`; 50 alphabetical rows of `{alias_count, category, name}`, first five "10-10-10 Decision Method", "12 Leverage Points", "5 Whys Technique", "8-Step Model for Building a Breakthrough", "AGO Aims/Goals/Objectives". With no topK the op returns 50 and `total` is the live Framework count: after the flip read `total` 420 (or higher if canon moved) as correct and 419 as STALE (the 09-11 ledger row 13 `matched 5 / total 419` passed a five-row limit and predates the hat-cluster mint). For the enrichment-capture check (D-03 consumer 3, FLIP-03b) the discriminator is the PRESENCE of the `coverage` object with `status: "partial"` and integer `matched`/`total`, never the specific integers.
