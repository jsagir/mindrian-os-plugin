...
---
*Last updated: 2026-09-04 after Phase 275 (enlarge-room-schema-by-icm-layer-notion-gap-close-icm-archit, independent of the v2.1.0 milestone) completed. 8 plans across 5 waves: 275-01 grew `SECTION_NAMES` 8 to 11 (`opportunity-bank`, `funding`, `strategy`) and corrected 7 propagation-gap citations against the live command registry; 275-02 built the L1/L2/L3 scaffold mechanisms (per-section `statement` rendered in frontmatter and body, the `writeSectionContracts`/`writeReferenceDocs` writers, the new `references/` factory directory); 275-03 closed vocabulary drift in two runtime mirror modules (`room-birth.cjs`, `grade-grant.cjs`) that were already stale hand-copies; 275-04 turned all 7 documented-RED test assertions green with the count literal pinned at exactly one sanctioned site; 275-05/275-06 authored all 11 L2 `CONTEXT.md` contracts (8 original sections plus the pipeline pair `opportunity-bank`/`funding` and `strategy`); 275-07 authored the 2 L3 reference documents (`SECTION-SCHEMA.md`, `SUB-SCHEMAS.md`); 275-08 shipped the idempotent additive migration script (`scripts/migrate-room-sections-v275.cjs`), the phase's own 8-section assertion suite (`tests/test-275-section-schema.cjs`, negative-tested to prove it can fail), and the aggregator (`tests/run-all-275.sh`, `PASS=10 FAIL=0`). Post-execution code review (advisory gate, `275-REVIEW.md`) found and the orchestrator independently reproduced a real, live data-corruption bug: `SECTION_METADATA['strategy'].statement`/`.purpose` each carried an unescaped YAML mapping-value colon (`": "`), and the unquoted template substitution turned it into invalid YAML in every scaffolded room's `strategy/ROOM.md` under the default `venture` blueprint family, confirmed by feeding the actual generated file through `gray-matter` (the repo's own frontmatter parser, used by 13+ modules) and watching it throw. The fix (`275-REVIEW-FIX.md`, all 4 findings fixed) is root-cause, not cosmetic: a generic `escapeYamlDoubleQuoted()` helper now guards every free-text frontmatter substitution site in both the scaffold and the migration script's own backfill path, and it also caught an independently-reproduced twin defect in the `references/ROOM.md` identity-directory purpose text that the reviewer had not named. The test-suite gap that let CR-01 ship green (regex-matching raw bytes instead of actually YAML-parsing generated frontmatter) was closed in the same pass. Phase verification: 16/16 must-haves passed. Icm-architect (the standing consult for room/ICM/local-graph work per project convention) ran a pre-flight review of all 8 plans before execution (verdict: GO, no structural gap) and the walk test twice during execution (a freshly scaffolded room in 275-02, a migrated legacy room in 275-08, both passing). Backend-only, no release commit needed.*
