...
Not a recommendation to adopt the library (no JS binding exists; the SQL would need porting, and MindrianOS's schema needs `review_status`/provenance columns this minimal schema doesn't have). A reference implementation to plan against when 273 or SEED-074 gets to the schema-unification (M12) or bidirectional-traversal (U-2) fixes -- closer and more load-bearing than the two rejected crates were, because it is a pattern to port, not a dependency to reject.
**Requirements**: TBD
