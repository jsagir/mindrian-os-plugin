...
- [~] 229-09-PLAN.md -- Demo run + Amnon verdict checkpoint + HUJI calibration workshop [D6,D7] -- **DI-1/2/3/4/5 RESOLVED; demo now grades REAL content end to end but is STILL NOT gate-clean, blocked at DI-6/DI-7 (2026-07-16 second fix-and-verify):** DI-4 (broken Stage A->B handoff) fixed by a dual-write - `populateRoom` now renders the evidence into the section ROOM.md the grading spine reads (mirroring file-meeting), so Stage B (opus) grades REAL pitch content on both samples (no more empty-room refusal). DI-5 (disfluency-cleaning extraction) fixed by an explicit byte-verbatim quoting rule - verified: `vali- validating` / `surprising-- important` preserved verbatim. run-all-229 stays PASS=9. But this first TRULY end-to-end run exposed two NEW Stage-B-side blockers: **DI-6** (the grading spine packages non-verbatim quotes - safescan used elliptical `"engineer... developer"` spans that D1 correctly FAILED; study-app cleaned a `vali-` disfluency in its feedback) and **DI-7** (the D1 verifier's `extractQuotedSpans` only sees double/curly/blockquote quotes, so study-app's single-quoted feedback quotes bypassed the gate entirely -> a FALSE GREEN that hid a real non-verbatim quote). STOPPED per the "new bug -> STOP" mandate; nothing fabricated or force-passed. Real opus grades ($2.70 / $2.92, schema PASS) preserved verbatim under demo/blocked-run-2026-07-16/. NEXT: fix DI-7 (so the gate can't lie) then DI-6 (port the verbatim rule to rubric-huji/structure-argument), re-run both samples, then Tasks 2 (Amnon) + 3 (HUJI workshop). See demo/DEMO-VERDICT.md + deferred-items.md (DI-4..DI-7).
