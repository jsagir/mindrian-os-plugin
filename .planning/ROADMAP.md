...
**Goal**: Three related gaps found live this session while STATE.md was mid-write from two concurrent sessions, all informed by MotherDuck's Guides research (Bev Turnbaugh talk, SF meetup hosted by Uncork Capital; MotherDuck's 2026-07-29 blog post "Context belongs in the warehouse"): (1) STATE.md's compute-state regeneration has no schema-drift versioning or notification -- live-reproduced this session as a self-contradicting frontmatter (`stopped_at` referencing Phase 236 while `Current Position` said Phase 240 executing), a direct symptom of concurrent-session writes with no version stamp to detect the collision; (2) room.db's graph and BRAIN.md conflate the SEMANTIC layer (schema/joins/structure) and the CONTEXT layer (business-term definitions, institutional knowledge, exceptions) the same way the industry has historically done, per the MotherDuck talk's own framing -- worth an explicit documented distinction rather than an implicit one; (3) MotherDuck benchmarked their context layer on DABStep (+72 percentage points accuracy, -55% cost per run vs. an agent discovering context on its own); MindrianOS has no equivalent gate measuring whether room context actually improves Larry's answer accuracy before claiming the Data Room helps.
...
**Success Criteria** (what must be TRUE):
  ...
  1. STATE.md's compute-state artifact carries a schema version stamp; a regeneration that detects a version mismatch against the on-disk artifact surfaces a notification (not a silent overwrite) before writing, closing the exact self-contradicting-frontmatter class this session reproduced live. A mutation seeding a stale version number turns the drift-detection gate red.
  2. room.db's graph schema and BRAIN.md each carry an explicit, documented SEMANTIC-vs-CONTEXT layer distinction (schema/structure vs. business-term/institutional-knowledge), not an implicit conflation -- a doctrine-presence gate (the `143.2-doctrine-presence` idiom) asserts both terms are named and distinguished in the relevant docs.
  3. A benchmark gate exists measuring whether room context measurably improves Larry's answer accuracy on a fixed task set, mirroring MotherDuck's DABStep methodology at MindrianOS's own scale (not literally DABStep -- a comparable before/after-context accuracy delta on a small fixed local eval set). If no accuracy delta is measurable at this scale, that is itself the finding to record, not a reason to skip the gate.
...
**Goal**: Natural free-form language reliably fires the right reach candidate. Today's trigger-tier doctrine (Phase 172-07, `lib/core/sensors/sensor-types.cjs`, Canon Part 11 R3) ranks `signal` (explicit state event) and `context` (a LOCAL problem-state enum) above `keyword` (lexicon match, explicitly the FALLBACK tier) -- and has no content-relevance tier at all. An utterance that doesn't trip a structural state change and doesn't hit a hand-picked keyword list produces zero candidates, independent of how the dial ranks whatever else fired. This phase adds the missing tier and fixes the flat top-K cut that would otherwise let one trigger family dominate once a new one exists.

**Stack constraint (binding on the whole phase, not a suggestion):** this repo runs zero server infrastructure and the local room graph is SQLite (`room/.room-graph/`, per the standing `feedback_local_graph_sqlite` rule). `node:sqlite`'s FTS5 module with its native `bm25()` ranking function is CONFIRMED WORKING on this repo's exact runtime floor (v22.16.0+, verified live on v22.23.1 this session) with ZERO new dependencies. This is the vehicle for the new tier. Explicitly EXCLUDED, each already rejected elsewhere in this repo or off-limits by Canon boundary -- naming any of these in a future plan is a scope escape against this Goal, not an implementation detail:
  - **KuzuDB** -- retired; Phase 242's own release gate machine-checks that the dead KuzuDB warning never resurfaces (MOAT-02).
  - **Memgraph** -- not part of this stack anywhere; no local or remote surface uses it.
  - **Neo4j** (including Aura community detection / Leiden clustering) -- that graph is Brain's remote proprietary store; local trigger logic cannot depend on it, Canon Part 8/9.
  - **Any embedding model, vector index, or remote semantic-router service** -- would introduce server infrastructure this repo does not run, and Canon Part 8 blocks sending raw user turn text to any remote service (Brain included) for embedding.
  - **A brand-new npm dependency** -- FTS5 ships inside `node:sqlite` already; nothing to add.

**Depends on**: Nothing new. Soft-depends on the already-shipped Phase 141/142/143/143.1 sensor-and-dial machinery this phase extends in place.
**Requirements**: TRIG-01, TRIG-02, TRIG-03
**Success Criteria** (what must be TRUE):

  1. A new `content` trigger tier exists in `TRIGGER_TIERS` (`sensor-types.cjs`), scored via a `node:sqlite` FTS5 virtual table + `bm25()` over room.db content -- no embedding call, no network call, no new npm dependency. A turn whose text has no structural state-change signal and misses every sensor's keyword list, but has real lexical relevance to a room's stored content, still produces a fired candidate; a mutation removing the FTS5 query path turns this red.
  2. The dial (`lib/workflow/f-selector-ranker.cjs`) fuses candidate scores ACROSS trigger-tier families via Reciprocal Rank Fusion (rank position, not raw score comparison) before the `MAX_K=3` cut at line 733, since a `bm25()` score and today's `0..1` decay-weighted structural score are not on a comparable scale; a mutation reverting to the flat `scored.sort((a,b)=>b.score-a.score)` turns a same-family-domination regression test red.
  3. The top-3 cut applies a diversity term (MMR-shaped: `(1-lambda)*relevance - lambda*max_similarity_to_selected`) so three near-duplicate same-family candidates cannot crowd out a genuine cross-family hit, following the exact layered-adjustment-pass pattern `_applySens10Flip`/`_applyRoleLevelBias` already establish in this file -- not a new architectural concept, the third instance of one already in production.
