...
**Re-scoped and planned (2026-09-23).** The WAIT UPSTREAM condition above is satisfied: `@modelcontextprotocol/ext-apps` 2.0.0 shipped 2026-09-08 (peers on the v2 family), so the blocker is gone. Navigator-approved scope (`267-CONTEXT.md`, after the force-refreshed `267-RESEARCH.md`): a local-server-only, unified, internally staged SDK v1 -> v2 migration of `bin/mindrian-mcp-server.cjs` (stdio and both HTTP branches), `bin/mindrian-brain-mcp-client.cjs`, `bin/mindrian-mcp-shim.cjs`, `lib/mcp/adapter-client.cjs` and every `lib/mcp` registrar (51 variadic sites, hand-migrated, no codemod). Out of scope: `mcp-server-brain/` (dead), Theo (its own repo and its own v1-pin decision), `lib/core/brain-client.cjs`, the MRTR rework of gate rung (a), and any re-architecture of the flag-ON daemon's room-binding identity. Locked stage order: W0 zod 4 + sdk 1.30.1 on v1, W1 brain shim canary, W2 local server stdio branch, W3 HTTP branches, W4 remove v1 last. Seven NEW FAILURES are on record (three from research, four found and reproduced at plan time); six are fixed in the wave that owns their file, and one (flag-ON shim pre-seeded session id rejected, RCA 7) is filed and pinned for a navigator decision because its fix changes the identity model this phase is ruled not to touch.
...
- [ ] 267-04-PLAN.md -- W0: handshake-only stdio tee, live CLI probe, navigator Desktop/Cowork probe checkpoint (MCPV2-13). PARTIAL (2/3 tasks): tests/helpers/mcp-stdio-tee.cjs + self-test (8/8 green) + opt-in CLI probe shipped and committed; live CLI probe run once (claude 2.1.281, plain initialize at 2025-11-25, elicitation declared, no server/discover -- zero drift from 267-RESEARCH.md); 267-TRIPOLAR-PROBES.md opened with CLI filled, Desktop/Cowork snippets ready. BLOCKED on Task 3 (checkpoint:human-action, gate="blocking") -- awaiting the navigator to run the Desktop and Cowork probes; see 267-04-SUMMARY.md.
- [x] 267-05-PLAN.md -- W1 canary: v2 family installed and vetted, brain stdio shim on serveStdio, both eras, Part 8 intact (MCPV2-11, -10, -09). 267-05-SUMMARY.md: @modelcontextprotocol/{server,core,client}@2.1.0 installed in lockstep (npm view re-checked live, one deduped core instance, three dated VETTED allowlist entries), lib/core/mcp-dep-heal.cjs FALLBACK grew a third entry. tests/test-267-mcpv2-brain-shim.cjs (MCPV2-11) went RED (PASS=4 FAIL=2: the 2026 arm, the source arm) before migration, GREEN (PASS=6 FAIL=0) after. bin/mindrian-brain-mcp-client.cjs migrated to @modelcontextprotocol/server's McpServer + serveStdio, zero changes to the six registerTool calls/schemas, zero diff on brain-client.cjs/part8-egress-guard.cjs/mcp-server-brain against PLAN_BASE. Fixed two Rule-1 bugs in existing tests directly caused by the migration (test-267-mcpv2-sdk-era.cjs Arm B's premise did not survive contact with the real v2.1.0 package; test-257-strict-input-shapes.cjs Arm E needed narrowing for v2's registerTool arguments-normalization, verified not a bypass via new Arm E2). bash tests/run-all-267.sh PASS=23 FAIL=0 SKIP=10; every 267-BASELINE.md suite re-run with the same named pre-existing FAILs, nothing newly red beyond the two documented, fixed test bugs.
- [x] 267-06-PLAN.md -- W2: registration-API test (RED) then tool-router.cjs + contract-version.cjs to registerTool, CIRS gate per commit (MCPV2-02, -08). 267-06-SUMMARY.md: tests/test-267-mcpv2-registration-api.cjs (a recording fake McpServer, stack-frame attribution, `--file` per-commit mode plus a full-mode live wireSnapshot check) went RED (11 + 1 v1-variadic FAIL lines) before migration. tool-router.cjs's 11 sites and contract-version.cjs's 1 site rewritten to registerTool(name, {title, description, inputSchema: z.object(shape)}, cb) on the still-v1 McpServer, one commit per file, descriptions/shapes byte-identical (zod4-contract proof), titles mechanically derived. CIRS gate green at each commit. Fixed a Rule-1 regression this rewrite directly caused in 10 pre-existing test fixtures whose fake McpServer only understood the removed v1 `.tool()` form (added a `.registerTool()` capture sibling to each). bash tests/run-all-198.sh and run-all-267.sh both match 267-BASELINE.md exactly (13/3/0 and 23/1(expected)/9 respectively); full-mode registration-API check stays documented-RED until 267-09. One pre-existing, unrelated test-347 failure logged to deferred-items.md, not fixed.
- [x] 267-07-PLAN.md -- W2: gate.cjs stale elicitation premise fixed test-first (RCA 3) + gate/chain/claim registrars (MCPV2-07). 267-07-SUMMARY.md: tests/test-267-mcpv2-gate-premise.cjs went RED (stale "do not declare it" phrase, no 2.1.280 mention) before a comment-only fix (gate.cjs's two comment blocks now cite the live-verified Claude Code 2.1.280 elicitation declaration, Desktop/Cowork stated unprobed); RCA 3 resolved, moved to .planning/debug/resolved/, knowledge-base.md block added. gate.cjs (2 sites), chain.cjs (2), claim-verify.cjs (2), claim.cjs (1) migrated to registerTool(name, {title, description, inputSchema: z.object(shape)}, cb), one commit per file, CIRS/zod4/registration-API gates green after each -- 19 of 51 phase-wide sites now migrated. Fixed a Rule-1 regression across 8 pre-existing fake-server test fixtures the migration directly broke (registerTool capture siblings added, matching the 267-06 precedent). Three pre-existing, confirmed-unrelated findings logged to deferred-items.md (test-237-approve-executes.cjs's stale mutation-test needle; check-tool-honesty.cjs's scanAll() not recognizing registerTool( sites, a phase-wide gap already silently live since 267-06; test-353-filing-gate.cjs's EVENT_TYPES.size drift), none fixed (out of scope). bash tests/run-all-198.sh matches baseline exactly (13/3/0); bash tests/run-all-267.sh reads PASS=23 FAIL=2 SKIP=8 -- one documented-expected (full-mode registration-API check, red until 267-09) plus the one deferred check-tool-honesty regression named above, not a new functional defect.
- [ ] 267-08-PLAN.md -- W2: remaining ten tools/*.cjs registrars, one commit per file (MCPV2-02, -08)
- [ ] 267-09-PLAN.md -- W2: dead runtime-loop prompts fixed test-first (RCA 4) + prompts/resources registrars (MCPV2-16)
- [ ] 267-10-PLAN.md -- W2: ext-apps 2.x + app-views schema: -> inputSchema: fix with realpath-contained room_path (RCA 2) (MCPV2-04)
- [ ] 267-11-PLAN.md -- W2: the swap, v2 McpServer + serveStdio, dual-era wire test incl. elicitation arms, gated on the Desktop probe (MCPV2-02, -07)
- [ ] 267-12-PLAN.md -- W3: flag-OFF HTTP to createMcpHandler (RCA 1), tree watcher hoisted, localhost Host/Origin guards (MCPV2-05, -17)
- [ ] 267-13-PLAN.md -- W3: SIGTERM exit after teardown (RCA 5) and honest listen failure (RCA 6) (MCPV2-14)
- [ ] 267-14-PLAN.md -- W3: flag-ON daemon isLegacyRequest routing, sessionful legacy on v2 Node transport, modern leg legacy:'reject' (MCPV2-06, -17)
- [ ] 267-15-PLAN.md -- W3: shim + adapter-client to @modelcontextprotocol/client, default 2025 mode, RCA 7 pinned (MCPV2-06)
- [ ] 267-16-PLAN.md -- W4: Phase 354 tests and probes moved off v1, verdicts equal baseline (MCPV2-18)
- [ ] 267-17-PLAN.md -- W4: last four v1 importers, then uninstall @modelcontextprotocol/sdk in lockstep (MCPV2-18, -09, -12, -19)
- [ ] 267-18-PLAN.md -- close-out: final gates vs baseline, post-migration Tri-Polar checkpoint, requirements registration, four seeds, dual-home research trail (MCPV2-13 + registration)
...
- [x] 340-02-PLAN.md - Amendment Wave A, canon v1.24 to v1.25: the Sourced Claims Doctrine into Part 12 plus its required mirror into `agents/larry-extended.md`, as Appendix D entry 38, behind a blocking navigator APPROVE on the exact prose (CANON-01)
- [x] 340-03-PLAN.md - Amendment Wave B, canon v1.25 to v1.26: Part 9's two-chokepoint doctrinal split (D-01), Part 4's typed-edge-vocabulary reconciliation, and Appendix B's ICM code citations (D-02), as Appendix D entry 39 with a multi-Part placement-proof floor test, behind a blocking navigator APPROVE (CANON-02, CANON-03, CANON-04)
- [x] 340-04-PLAN.md - Amendment Wave C, canon v1.26 to v1.27: Appendix C's Theo origin, Part 2's e5-local backend, Part 7's de-frozen command surface and Part 11's refreshed snapshot, as Appendix D entry 40 in the entries-13/16 "corpus figures corrected" voice, with the parallel CLAUDE.md corrections in the SAME commit (Pitfall 3), behind a blocking navigator APPROVE plus three named rulings (CANON-05, CANON-06, CANON-07, CANON-08, CANON-09)
- [x] 340-05-PLAN.md - Phase close-out: post-amendment verification sweep (suite green, drift re-check, per-number Appendix D preservation proof, residual gaps named openly), `CANON-01 .. CANON-10` registered in `.planning/REQUIREMENTS.md`, and the Dev-Research Compositing trail filed to `rethinking-mindrianos/research/` mirrored to `mindrianOS/research/`. Autonomous (CANON-10)

Handoff (added by 344-09, 2026-09-14): `docs/2026-09-14-CANON-APPENDIX-B-PROPOSED-AMENDMENT.md` (Phase 344, LAYER-14/WD-1) is a named input for this phase's still-open sub-scope 4 (Extended ICM Schema) -- the literal proposed Appendix B Layer 0/Layer 4 wording, ready to lift when a Phase 340 continuation or navigator-designated successor next opens a Canon amendment window; zero Canon bytes have landed for it yet.

### Phase 341: Install and update overhaul: npm-source plugin artifact, heavy-dep cut, one install location, transactional update

**Goal:** Make `claude plugin install/update mos@mindrian-marketplace` complete on Windows, Mac and Linux by shipping a built artifact instead of a git checkout. Verified 2026-09-09 on two platforms (Windows 11 QA teardown + Linux dev box): the documented update path dies in `git clone` (`fatal: early EOF`) because release.sh Step 6.7 force-vendors `node_modules` into the release tag - 410 MB / 8,880 files at v2.0.0-beta.29, of which ~380 MB is `@huggingface/transformers` + onnxruntime-node/-web + sharp/libvips (23 native binaries for every platform), added 2026-07-05 by Phase 211-01 AFTER the 2026-05-21 "32M pure JS" audit that justified vendoring. The tag checks out at 755 MB / 16,597 files (`.planning` 57 MB and `tests` 44 MB ship too); product code is 30 MB / 2,079 files; the MCP-critical pure-JS dep set is 29 MB. Navigator-locked path (Decision Gate 2026-09-09): marketplace `source: npm` pinned to the version; `package.json` `files` whitelist = runtime tree; transformers out of `dependencies` (Eureka installs on demand, existing `encoder_unavailable` degrade + Doctor class S); pure-JS deps via `npm-shrinkwrap.json` + the loader's `npm ci --ignore-scripts` (ratified 2026-09-09: platform-correct per machine, sqlite-vec's five platform packages resolve locally; measured 9.3 s cold against the documented 60 s cutoff; `bundleDependencies` rejected by measurement, it packs only the publish host's platform package). Target tarball ~2,092 entries / 30.5 MiB, far under the platform's documented 256 MiB / 20,000-entry plugin ceiling; exclude `lib/wiki/editor-src` (its nested node_modules alone is 9,119 entries). Then: delete Step 6.7 + Commit A/B node_modules churn; retire `install.sh` as a primary path, `/mos:update` Steps 6-7 + restart touch-file, doctor classes A/H and the two-topology halves of I/J; collapse `/mos:update` to Check, Confirm, loader, Verify; add a Phase 298 harness policy (blocking rung) asserting published tarball size + entry count against the ceiling so a 211-01-class dependency can never silently 13x the payload again. Fallback: `archive` source (HTTPS zip + documented `sha256`, Claude Code v2.1.224+) via `git archive` + `export-ignore`. Findings prelude: `.planning/phases/341-*/341-FINDINGS.md`. Sibling scope: Phase 285 (SEED-015 selective install).
...
- [x] 343-01-PLAN.md - mint CENSUS-01..17, land the tracked twenty-one-row working-decisions ledger with its two not-measurable records and the Phase 273 non-goal map, and stand up the phase aggregator
- [x] 343-02-PLAN.md - the counts-only room-graph integrity organ: four measurable defect statements in one shared statement home, read-only door, null and never zero on a legacy schema, the writer gaps named in its own output
...
- [x] 343-07-PLAN.md - one home for the lockstep count, then the Theo command-layer stamp gate in the release preamble. One blocking navigator checkpoint before the release-train edit (autonomous: false)
...
- [x] 343-09-PLAN.md - phase close: full gate sweep, every CENSUS id closed with Measured proof, the validation map filled, the tracked close-out and the Phase 273 handoff

### Phase 344: The layer contract: name, describe, and pin every engineering layer of MindrianOS and every ICM nested part of a room, what each IS today and what it NEEDS TO BE (graph-engineering learning, 2026-09-14). Origin: the graph-engineering-vs-loop-engineering transcript draws a clean five-rung ladder, each rung engineering a wider scope than the one before, and MindrianOS has all five without ever naming them: PROMPT (one request: the Larry persona, persona variants, voice rules; core question, did I ask well), CONTEXT (what the model sees per turn: context_assemble, STATE.md, USER.md, ROOM.md, JTBD, the DirectiveEnvelope, SessionStart and UserPromptSubmit injection; core question, does it have the right information), HARNESS (tools, memory, scaffolding: the MCP tool surface, navigation.cjs chokepoint, room.db, memory_event, gate ledger, seeds, hooks; core question, can it act on the world and remember), LOOP (one agent's cycle with a stopping condition: a single /mos:<framework> run, body_shape, artifact filing, suggest_next, stop_gate_check, the Ask-Tell dial; core question, when does it check its work and stop), GRAPH (multi-agent coordination with shared state: chain_resolve, chain_run and the autonomous_safe versus material halt, the reach machinery and SENSOR_REGISTRY, /mos:pipeline, act-chain, act-swarm, the five-perspective fan-out, the Workflow harness-as-code, and the one reviewer node, the navigator at the gate; core question, who does what in what order and what state is shared). The langtalks corpus already carries the connective tissue with EXTRACTED confidence: Memory is part_of context engineering; the ICM note builds_on context engineering; harness reaches context through files and blast radius (Fragmented 307); loop reaches harness through verification loops; the orchestration layer reaches multi-agent systems through the agent harness. Two consequences the phase must write down as doctrine: the layers accumulate, they do not substitute (a good graph node is a well-designed loop, a good loop needs a solid harness), and the ICM nested parts are the CONTEXT layer in file form while the harness is what reads and writes them. Deliverables. (1) A layer contract document under docs/ (and mirrored into the help family map per Phase 343 item 4) with, per layer: definition, core question, the components that implement it with file paths, the layer above and below, what is thin or missing, and the single owner surface a change to that layer must go through. (2) An ICM nested-part contract, per room and per section: ROOM.md, STATE.md, USER.md, FEYNMAN.md, per-section REASONING.md (Feynman-MINTO), BRAIN.md, .room-graph/ SQLite, seeds/, the three memory layers, .snapshots/, opportunities and funding, personas, vault MANIFEST; for each, what it IS (file, producer command, consumers), what the icm-architect reference form says it NEEDS TO BE (objects, processes, change-impact, index), and the gap, with Phase 275's ICM audit as the baseline rather than a re-derivation. (3) A doctor check that every command declares its layer in frontmatter and every ICM part has a producer and at least one consumer, counts only. Input spec: docs/2026-09-14-LAYER-CONTRACT-AND-ICM-MAP.md (landed 2026-09-14 from the langtalks corpus pull plus the plugin walk); read it first, do not re-derive. Consults icm-architect (standing rule) and langtalks-graph-expert; graph engineering itself is not in the corpus until its extraction lands, so that rung cites the structured note directly and says so.
...
- [x] 344-09-PLAN.md - phase close: full gate sweep, requirements closed with measured proof, validation map filled, close-out record in docs/ and the rethinking-mindrianos room
...
- [x] 347-01-PLAN.md - the tracked shared-state contract (node shape, edge contract, five-kind mapping, SHARED-01..13, the five working decisions with reversal cost), the written-once test aggregator, and the layer: graph tripwire
...
- [x] 347-03-PLAN.md - lib/core/navigation/chain-state.cjs: the chain_state writer with its mandatory SOURCED_FROM anchor, the FEEDS_INTO-ordered reader, and three additive navigation.cjs re-exports
...
- [x] 347-12-PLAN.md - phase close: SHARED-01..13 registered with Measured proof, the validation map filled, the close-out record, the handoff entry and the rethinking-mindrianos filing

### Phase 348: The supersession node (graph-engineering learning 6, fact invalidation over time, 2026-09-14): a graph that only ever adds nodes cannot tell a stale belief from a current one, and MindrianOS's local room graph is exactly that today -- typed claims (fact, causal, heuristic, anomaly_cue, mental_model, assumption), typed edges (SUPPORTS, CONTRADICTS, SOURCED_FROM, USES_FRAMEWORK), a proposed-to-confirmed lifecycle gated by human approval, and optional per-claim valid_from/valid_until fields, but nothing that ACTS on a CONTRADICTS edge or an expired validity window once it exists. This gap was named directly against a comparable system in a 2026-09-14 dev-vetting session (grounded live via langtalks-graph-expert's corpus plus Tavily research): Zep/Graphiti, the field's current state of the art for agent memory, is bi-temporal -- every fact carries a validity window, and when a new fact contradicts an old one the old one is automatically marked superseded (never deleted, so history stays queryable) instead of standing uncorrected next to its own contradiction. Deliverables. (1) A supersession mechanism that fires as the CONSEQUENCE of a human-approved gate answer on a surfaced contradiction (never a silent background job -- the structural human-in-the-loop pattern this repo already runs everywhere else holds here too): when the navigator approves that claim A supersedes claim B, claim B's review_status is written as superseded, an additive status value, through navigation.cjs (the Part 9 sole write chokepoint), never deleting the node or its edges. (2) contradiction_check / findContradictions extended so a superseded claim is excluded from default reads but stays traceable with an explicit include-superseded query flag, the same invalidated-not-deleted contract Zep/Graphiti use. (3) Doctrine written into the larry-personality skill and cross-referenced in docs/MINDRIAN-CANON.md: superseded is not deleted, and this mechanism is the room graph's answer to bi-temporal fact invalidation, sourced explicitly to the 2026-09-14 comparative research trail filed in rethinking-mindrianos. Blocker to check first: the langtalks-graph-expert add_source call made during that same research session to file the comparison note failed with a live Gemini API 403 PERMISSION_DENIED (a credential/project problem on that tool's own backend, not this repo) -- confirm that is fixed, or route around it with a filed research doc instead, before this phase cites a live corpus entry for the Zep/Graphiti grounding. Consults icm-architect (standing rule for room/ICM/local-graph work) and langtalks-graph-expert (standing rule for memory/context-engineering work).

**Goal:** The shipped supersession mechanism is human-gated at the truth-state chokepoint (an
agent-attributed close of a truth-claim node into `superseded` is refused with
`agent_attribution_forbidden`), its two validity-window representations are reconciled onto the
authoritative `nodes.valid_from`/`valid_to` columns, `findContradictions` excludes superseded
claims by default and returns them on an explicit `include_superseded` flag while refusing to
read a reified `ContradictionEvent` node as a claim, the invalidated-not-deleted doctrine is
written into the Larry skill and narrowed into Canon Part 9 (Appendix D entry 41, v1.28), and the
whole loop is proven end to end on a fixture -- with the fleet census of 0 `CONTRADICTS` and 0
`SUPERSEDES` edges across 47 (measured again at close: 60) rooms explicitly UNCHANGED, because no
live writer exists yet.
**Requirements**: SUPER-01 .. SUPER-20 (minted 2026-09-16 in the Phase 348 plan set, ratifying
`348-RESEARCH.md`'s proposed `SUPER-` family as amended by `348-CONTEXT.md`'s D-01..D-09 locks,
registered in `.planning/REQUIREMENTS.md` at phase close by 348-10-PLAN.md per the ICML / TOOLHON
/ CANON / SHARED / STRAT / ARB precedent)
**Depends on:** Phase 347
**Plans:** 10/10 plans complete

Plans:

- [x] 348-01-PLAN.md - mint SUPER-01..20, ship the durable contract (docs/SUPERSESSION-CONTRACT.md: the invalidated-not-deleted contract, the four rulings, the D-01..D-09/WD-348-1..12 ledgers, the measured 47-room zero-census), and stand up the phase test aggregator
- [x] 348-02-PLAN.md - the two-schema-variant supersession fixture room, the SUPER-17 ten-step end-to-end loop proof (RED until 348-07), and the SUPER-01 comment-stripped source tripwire
- [x] 348-03-PLAN.md - the human-attribution guard on supersession (SUPER-02/03) and the bitemporal-close schema gate (SUPER-13), behind the phase's one blocking navigator checkpoint, ratified "Approved as specified"
- [x] 348-04-PLAN.md - the findContradictions reshape: the options bag, the review_status filter, the legacy-schema fall-through, and the D-04 reified-shape guard, with all five callers declared
- [x] 348-05-PLAN.md - the validity-window (SUPER-10/D-06) and TRANSITIONS-freeze (SUPER-09/D-05) rulings, each proven against measured reality rather than illustrative pseudocode
- [x] 348-06-PLAN.md - the MCP `include_superseded` flag on `contradiction_check` (SUPER-06), with SUPER-07's no-write tripwire pinned on the same declared-pure-read tool
- [x] 348-07-PLAN.md - the supersession gate module (`supersedeOnGateAnswer`) and the SUPER-17 load-bearing proof flipped from EXPECTED-RED to PASSED
- [x] 348-08-PLAN.md - the "Superseded is not deleted" doctrine subsection in larry-personality/SKILL.md, behind the phase's second blocking navigator checkpoint, which also ratified the canon-narrowing wording for 348-09
- [x] 348-09-PLAN.md - Canon Part 9 amendment: entry 41 narrows `superseded` to human-gated-only (v1.27 -> v1.28), full atomic lockstep across MINDRIAN-CANON.md, CANON-PHASE-MAP.md, CLAUDE.md and eight existing FLOOR tests' version anchors
- [x] 348-10-PLAN.md - phase close: full gate sweep, SUPER-01..20 closed with measured proof, the validation map filled, the close-out record, the langtalks-blocker handoff row, and the new Phase 350 card

### Phase 349: Release-to-Theo leading edge (graph-engineering learning, synchronous update between coupled systems, 2026-09-15): every plugin version update makes Theo aware immediately. Navigator ruling 2026-09-15: "every time MindrianOS gets a beta or any version update, Theo is immediately aware to make any needed changes in consultation with langtalks and the full stack." Today the coupling is one-directional and lagging only: Phase 343-07 shipped the Theo stamp gate (release.sh Step 0.6 fails closed when Theo's mappedBy lags the current plugin version) and Theo Phase 21 counts decay, but nothing on the plugin side TELLS Theo a version moved; the measured drift is 28 betas (mappedBy command-registry@2.0.0-beta.12 vs repo 2.0.0-beta.40) and the gate can only refuse, never repair. langtalks grounding (query_relationship 2026-09-15): Measurement decay is part_of the single-loop failure modes and critiques the code-review-agent loop; the Audit node is the lagging watcher; the corpus carries a Synchronous Update node; the leading edge is the missing half of the pair. Deliverables. (1) release.sh gains a post-tag step (beside Step 9 push and Step 5.5 tag verify) that emits a version-update signal to Theo: a GitHub repository_dispatch to jsagir/theo (event theo-resync, payload {version, commit, registryHash, command_registry_path}) plus a local handoff file under the plugin repo and a cross-session notify when a Theo session is live; the carrier is confirmed with the Theo side before the plan is written (jsagi-58 asked 2026-09-15). Canon Part 8: the payload is version, commit sha, registry hash and the generic registry path string, never room content. (2) The signal is audited in the release log like --no-theo-check (RULE 5 place 8 in docs/RELEASE-CEREMONY-RULING-SYSTEM.md gains its leading half), --dry-run reports without sending, and an unreachable GitHub API fails the step closed with the same audited opt-out shape. (3) A hermetic test proves send, dry-run-reports-only, unreachable-fails-closed and payload shape, never calling the live API. (4) VERSION-BUMP-CHECKLIST.md and the release-process include name the step; docs/OPEN-HANDOFFS.md gains the Theo-side row (Theo consumes the event in CI: re-emit the command layer, restamp, consult langtalks and run the full-stack pass; owned by the Theo repo's next phase). (5) A one-time bootstrap: before the phase closes, Theo has restamped against the current plugin version so the 343-07 gate reads green and the next cut needs no opt-out. Depends on Phase 343 (the lagging gate) and on the Theo repo's consuming phase for the end-to-end proof. Consults langtalks-graph-expert and icm-architect before any design assumption.

**Goal:** `scripts/release.sh` Step 5.6 fires a `repository_dispatch` at `jsagir/theo` on every real cut, immediately after the tag is verified at origin, carrying a closed four-key payload whose version is provably the released version and whose registry hash is taken from the tagged commit; the step is audited, fails closed, carries its own `--no-theo-notify` opt-out and provably sends nothing under `--dry-run`; RULE 5 place 8 now describes both halves of the Theo coupling as one place; and whether Theo ACTS on the event remains owned by the Theo repo, measured at `payloads_applied_since: 0`, with the consuming half registered as its own card (Phase 351).
**Requirements**: NOTIFY-01 .. NOTIFY-14 (minted in the Phase 349 plan set, 2026-09-16, ratifying and amending `349-RESEARCH.md`'s proposed `NOTIFY-` family; all fourteen closed with measured proof at phase close by `349-06-PLAN.md`, 2026-09-16)
**Depends on:** Phase 343 (the lagging Theo stamp gate); the end-to-end proof additionally waits on the Theo repo's consuming phase (jsagi-58, asked 2026-09-15; registered as Phase 351). Not sequential on Phase 348.
**Plans:** 6/6 plans complete

Plans:

- [x] 349-01-PLAN.md - NOTIFY-01..14 minted in `.planning/REQUIREMENTS.md`, the pre-phase emission census measured live (0 call sites vs Theo's own contradicting counters), `docs/THEO-NOTIFY-CONTRACT.md` authored, and `tests/run-all-349.sh` stood up
- [x] 349-02-PLAN.md - `tests/test-349-theo-notify-gate.cjs` (nine arms) and `tests/test-349-payload-boundary.cjs` (seven arms) authored RED, fixing the whole gate contract before any implementation exists
- [x] 349-03-PLAN.md - the navigator checkpoint ratifying five design forks plus the R5 disposition, then `scripts/release-lib/theo-notify-gate.sh` written GREEN against all sixteen RED arms
- [x] 349-04-PLAN.md - the live-path wiring: `scripts/release.sh` Step 5.6 and `scripts/doctor.cjs`'s `expectedSteps` entry, with a live-shelled proof that a dry-run sends nothing
- [x] 349-05-PLAN.md - RULE 5 place 8 amended to describe both halves as one place, `.claude/includes/release-process.md`'s "Telling Theo" subsection, and the dated Theo-side row in `docs/OPEN-HANDOFFS.md`
- [x] 349-06-PLAN.md - phase close: full gate sweep, NOTIFY-01..14 closed with measured proof, the validation map filled, the R5 disposition executed (Phase 351 card registered), and the close-out record

### Phase 350: The live CONTRADICTS writer (graph-engineering learning 6b, making the supersession loop fleet-observable, 2026-09-16): Phase 348 hardened, gated and proved the supersession mechanism end to end on a fixture, and its own close-out states plainly that the fleet census stayed at exactly zero (0 `CONTRADICTS` edges, 0 `SUPERSEDES` edges, 0 superseded nodes, across a fleet that grew from 47 to 60 rooms the same day) because nothing writes the input edge. This phase is what changes that number. It is registered here as a real, numbered card rather than a prose deferral, per Phase 348's D-02. Deliverables. (1) Un-neuter or replace the trigger at `lib/core/intel-pipeline.cjs:144`, where `void wirer;` explicitly no-ops the one `CONTRADICTS` writer reachable from a live pipeline (`lib/core/findings-wirer.cjs:204`, required at `intel-pipeline.cjs:119`), and rule whether the other four paper writers (`memory-cascade-f9-adapter.cjs:185`, `reconcile-f9-adapter.cjs:163`, `unknowns/verdict.cjs:64`, `reified-claim.cjs`'s `writeContradictionEvent`, which has no production caller) are kept, wired, or retired. (2) Rule the reified-versus-direct edge shape question Phase 348 scoped out (D-03), and either implement the `CONCERNS` hop in `findContradictions` or retire the reified writer. (3) Register the supersession gate consequence (`lib/core/temporal/supersession-gate.cjs::supersedeOnGateAnswer`) as a real invocable surface, born WIRED with its own declared `hitl_shape` and `layer` per Canon Part 11 R16, reusing the shipped gate module unmodified and riding the existing `gate-render`/`gate.cjs`/`gate-ledger` ladder rather than minting a second gate. (4) The acceptance number is a FLEET number, not a fixture number: after this phase, at least one live room carries a real `CONTRADICTS` edge and the loop has run end to end outside a test. (5) Thread a `resolveByUser`-resolved human `byUser` into `lib/core/close-loop-writer.cjs:478` (Phase 348 WD-348-6), so a BONO conclusion version cut stops being refused by the guard Phase 348 shipped. This card exists to prevent Phase 348's mechanism staying fixture-only forever because the gap lived in prose instead of in the roadmap.

**Goal:** [To be planned]
**Requirements**: TBD
**Depends on:** Phase 348 (closed 2026-09-16)
**Plans:** 0 plans

Plans:

- [ ] TBD (run /gsd-plan-phase 350 to break down)

### Phase 351: The Theo-side consuming CI for theo-resync (graph-engineering learning, the other half of the leading-edge coupling, 2026-09-16): Phase 349 shipped the plugin half of a two-way coupling. `release.sh` Step 5.6 now fires a `repository_dispatch` at `jsagir/theo` on every real cut, immediately after the tag is verified at origin, carrying a closed four-key payload (`version`, `commit`, `registryHash`, `command_registry_path`). Nothing on Theo's side consumes it yet: `payloads_applied_since: 0`, measured live 2026-09-15, so the drift the coupling exists to repair is still open. This card is registered as a real, numbered card rather than a prose deferral, per Phase 349's WD-349-9 and Phase 348's D-02 precedent (which registered Phase 350 the same way). Deliverables. (1) A `repository_dispatch: [theo-resync]` workflow in `jsagir/theo` that re-emits the command layer against the received `version` and restamps `mappedBy` to `command-registry@<version>`. (2) Hash verification on receipt: Theo computes its own fresh registry digest and compares it against the received `registryHash`, reporting a mismatch rather than silently proceeding. (3) The langtalks-graph-expert consult and the full-stack pass run on each re-emit. (4) Resolve the emission-count finding carried forward from Phase 349: name what produced Theo's own `payloads_emitted_since: 2` when this repo's source scan found zero call sites before this phase, and reconcile the two counters. (5) The acceptance number is a PLUGIN-SIDE number, deliberately: after Theo's CI runs once against a real release, `bash scripts/release.sh patch --dry-run 2>&1 | grep theo-stamp-gate` prints `PASS` in the MindrianOS-Plugin repo, closing the 343-07 lagging gate this coupling exists to keep green. This card exists to prevent Phase 349's dispatch firing into a void forever because the consuming half lived in a handoff row instead of in the roadmap. Owner: the Theo repo (`jsagir/theo`), via its own next planned phase -- this is a different repository's CI job, not a person on this side, and `docs/OPEN-HANDOFFS.md`'s Theo-side row names the same owner so the two agree.

**Goal:** [To be planned]
**Requirements**: TBD
**Depends on:** Phase 349 (closed 2026-09-16); owned by the Theo repo, not this one
**Plans:** 0 plans

Plans:

- [ ] TBD (run /gsd-plan-phase 351 to break down)

### Phase 352: The self-healing install (graph-engineering learning 7, the loop that must not lie to itself, 2026-09-16): every install and every update runs the full `doctor --all --fix` by itself, once, on the first run of a new version, and the navigator never types a repair command again. Navigator ruling 2026-09-16: "make the updated and installed version of Mindrian automate its own fix -- statusline and all the rest -- for any user after installing or updating, by default; make sure the UI works perfectly." Evidence, measured live 2026-09-16 on the installed marketplace cache at 2.0.0-beta.37 (repo at beta.40, beta.41 cut the same morning by a peer session), filed as `.planning/debug/doctor-auto-heal-qa-sweep-2026-09-16.md` (kind: qa-sweep) with the raw `--all --json` and `--ui-compliance --json` reports beside it in this phase's directory. Fourteen findings, each with its line. (F1) The doctor skill's own Step 2 command fails MODULE_NOT_FOUND: `CLAUDE_PLUGIN_ROOT` was empty inside the slash-command bash step, and the documented fallback `~/.claude/plugins/mindrian-os/scripts/doctor.cjs` does not exist under the marketplace-cache topology (there is no `~/.claude/plugins/mindrian-os/` directory at all); the skill's class-A prose still describes the legacy live-install path as the norm. claude-code-guide, 2026-09-16, against the current hooks and plugins references: `CLAUDE_PLUGIN_ROOT` carries no documented guarantee for slash-command bash steps. `lib/core/active-plugin-root.cjs::resolveActivePluginRoot` already resolves the root from `installed_plugins.json` (brain-smoke layer L1 proves it live: `source=installed_plugins.json, topology=marketplace-cache`), so the skill and every `/mos:*` command body must resolve through it, never through an env var that may be blank. (F2) `brain-smoke` under `--all` renders `⊘ undefined` while the targeted `--brain-smoke` run passes all 7 layers (origin theo-mcp.onrender.com, store 27,951 nodes): the class-M async carve-out at `scripts/doctor.cjs:3439-3480` owns its own output only in the standalone branch and never writes a `{status, detail}` row into `report.checks` when combined with `--all`, so the generic row loop at `:2234-2257` prints `undefined` and the JSON report carries `status: undefined, detail: ""` for it. A silent diagnostic, the exact failure Phase 217 existed to kill. (F3) Under `--fix --all`, four fix-log lines print `⊘ fix install-state: undefined` and `⊘ fix deployment-surfaces: undefined`: both `fix()` functions (`lib/core/doctor/install-state-module.cjs:482-489`, `deployment-surfaces-module.cjs:386-394`) return `{recoveries: [...], detail}` and the fix-log renderer at `doctor.cjs:2291` iterates the per-record entries reading `entry.detail || entry.note || entry.status || entry.action`, which the record shape only sometimes carries; the fix DID land (a targeted `--install-state` re-run was clean), so this is a return-contract gap: the D-03 parity test (`tests/test-doctor-module-contract-parity.cjs`, rule 9) validates `check()`'s return and never `fix()`'s. (F4) The same-invocation post-fix re-check lies: `install-state` reported 3 findings (`record-stale`, `vor-IP-vs-AV`, `vor-IP-vs-LV`), fixed, then re-reported 2 (`vor-IP-vs-LV`, `vor-AV-vs-LV`) inside the same run, then 0 on the next invocation; the re-check reads a version-of-record value resolved before the fix wrote it. An unattended harness cannot tolerate a re-check that reports stale state. (F5) `graph-derive-health` reports `35 of 55 room(s) need a derive re-enqueue` before AND after `--fix` re-enqueued all 35 (`graph-derive-health-module.cjs:496-498` vs `:577`), because enqueue is not derive; post-fix the row must read "queued, derive pending" and the acceptance must be measured after the sweep actually runs, or every auto-heal reads as failed forever. (F6) The F.1 Next Move gate offers "Run /mos:doctor --fix" on a run whose only drift class is `ui-compliance` (`fix_supported: false`): a fix that can fix nothing; the gate must offer `--fix` only when a flagged module declares `fix_supported: true`, and name the right verb otherwise. (F7) `-v` adds zero per-violation detail for `ui-compliance` in the human render; only `--json` carries the rows. (F8) `ui-compliance`'s 155 rows are 89 real and 66 scanner false positives: 41 `renderer-missing-zone1` + 44 `renderer-missing-zone4` across 44 `scripts/*.cjs` renderers (real UI debt); 69 `unauthorized-box-char` of which 57 sit inside `//` comments (`// ── Helpers ──` section dividers), 5 inside `/* */` blocks (`scripts/generate-lobby.cjs`), 4 are legitimate Shape-B tree glyphs `├─` / `└─` (`scripts/jtbd-command.cjs:132-133`, `scripts/memory-command.cjs:100-101`), and 3 are genuine (`scripts/help-renderer.cjs:195` `'─'.repeat(ruleLen)`, `scripts/test-doctor-recovery.cjs:222,224`); 1 `unauthorized-glyph` (`✗` at `scripts/test-doctor-recovery.cjs:43`). Root cause: `lib/core/doctor/ui-compliance-module.cjs:92-95` matches `FORBIDDEN_BOX_CHARS` against raw lines with no comment awareness and no `body_shape` awareness. (F9) The `--json` report has no `summary`, no `exit_code`, no fixable-ids list, and `accumulative-engine` carries an empty `detail`; a harness consuming JSON has nothing to key on. (A suspected dropped row, `cascade-rooms-active`, was verified NOT a render bug: it was `skip` pre-fix because the active room lacked its sentinel and became `warn` once `--fix` created it; the row renders. Its content stands as a room-state observation: `active=idem-room`, writes to `mindrianOS` would be silenced.) (F10) `scripts/sessionstart-post-update-preflight.cjs:185` reads `arr[0]` of the `installed_plugins.json` rows for `mos@mindrian-marketplace` and so compared this session's beta.37 (user scope) against a project-scope pin for `/mnt/c/Users/jsagi` at beta.5, printing a false "an update landed under this session (2.0.0-beta.37 -> 2.0.0-beta.5), restart" banner; claude-code-guide confirms the scope tie-break is undocumented upstream, so the detector must select the row whose `installPath` equals the root this session actually loaded from. (F11) The first-session banner asks the navigator to run `/mos:doctor --statusline-visibility` or `--fix` by hand to record a touch-file: the exact chore this phase deletes. (F12) Every `--all` run prints Node's `ExperimentalWarning: SQLite is an experimental feature and might change at any time` on stderr above the De Stijl header: UI noise, to be silenced at the doctor entry point (Context7 ruling on the Node 22 `--no-warnings=ExperimentalWarning` / `process.removeAllListeners('warning')` form, at plan time). (F13) `cascade-rooms --fix` created 7 sentinels and left 2 rooms whose registry entries point at directories that no longer exist (never auto-created, by design): registry ghosts need a declared rule (suggest behind a gate, never silent prune). (F14) `mcp-surface` warns 46 tools ~11,049 tokens against an 8,000 budget with no fix path and no re-base rule, and `deprecated-usage` counts 6 retired command names typed 756 times in 7 days (`/mos:diagnostics` 463x, `/mos:query` 113x, `/mos:hmi-status` 99x, `/mos:organize` 31x, `/mos:visualize` 28x, `/mos:heal` 22x): a hint that is not landing; ruled at plan time whether retired names forward silently. Hook-event grounding (claude-code-guide, 2026-09-16, hooks.md + plugins-reference.md): no hook fires on plugin install, update, enable or path swap; `SessionStart` (matchers `startup|resume|clear|compact|fork`) is the earliest post-update code point; hook stdout is never shown to the user (exit 0 JSON is captured, exit 1-254 stderr is shown, exit 2 blocks), so the reward line must ride `additionalContext`; `async: true` semantics and settings.json write safety at startup are undocumented, so the design below keeps the LOCAL heal synchronous inside the existing hook budget and detaches only the network probes. langtalks grounding (relationship_path, 2026-09-16): Measurement decay --mentioned_in--> note-graph-engineering-vs-loop-engineering <--mentioned_in-- Agent --builds_on--> Claude Code <--part_of-- Verification loops (4 hops, EXTRACTED); Self-healing is in the corpus via ep 65 "AI SRE" (Komodor) and the Lex Fridman SOTA-2026 transcript. The learning: an auto-heal is an SRE loop (detect, remediate, verify) and the verify step must re-measure from disk, never from the pre-fix reading, or it is measurement decay inside a single run (F4, F5). Deliverables. (1) The auto-heal policy: a `SessionStart` hook step, ordered after `sessionstart-npm-reconcile` and before Larry-load, that compares the running version (plugin.json at the resolved root) against the accumulative engine's existing watermark at `~/.mindrian/doctor-applied.json` and, on the first run of a new version (install OR update), runs `doctor --all --fix --json` once: LOCAL classes synchronously inside the hook budget, the async carve-outs (brain-smoke, eureka-smoke) detached with `unref`, writing a result record the next turn reads; then advances the watermark. Never blocks session start, never re-runs on the same version, degrades gracefully on every error (the Canon Part 8 class-flag invariant). Reuses the watermark, the accumulative engine, `scripts/post-update-activation.cjs` and the existing preflight; mints no second heal path. (2) A per-module `auto_heal: true|false` declaration in `data/doctor-modules.json`, enforced by a new D-03 rule, so the unattended run applies only idempotent LOCAL fixes (install-cache swap, sentinel create for existing dirs, install-state record, deployment-surface restamp, statusLine restamp, graph-derive re-enqueue) and every destructive or judgment fix (legacy-clone remove, registry-ghost prune, cache prune, active-room mismatch) is listed as "needs your call" behind an F.1 gate on the next turn, never run unattended. (3) The reward line: the next turn opens with one Shape-E line, glyph first, `doctor auto-heal on beta.N: 7 fixed, 2 need your call (ids)`, designed through the hooked-model lens as the first step of the post-update surface (trigger: a new version; action: none; reward: it healed itself and said so; investment: the watermark, so it never asks again); on Claude Desktop and Cowork, where no hook fires, the MCP server boot runs the same LOCAL auto-heal on first boot of a new version and the line is the state echo (Tri-Polar: a deliberate, stated call). (4) The doctor renders truthfully: F2, F3, F9 fixed (class M writes a real `{status, detail}` row under `--all`; every `fix()` record carries a non-empty `detail`, validated by D-03 rule 10; `--json` gains `summary {healthy, drift, fixable_ids, needs_gate_ids}` and `exit_code`); F4 and F5 fixed (the post-fix re-check re-resolves from disk, and a queued-not-yet-derived room reads "queued"); F6, F7 fixed (the gate offers `--fix` only when a fixable module is flagged; `-v` lists violations); F12 silenced; the doctor skill (F1) rewritten to resolve the root through `resolveActivePluginRoot` and to describe marketplace-cache as the default topology and legacy live-install as the exception. (5) F10 fixed (the stale-registry detector selects the `installed_plugins.json` row whose `installPath` is the running root) and F11 deleted (the first-session banner never asks for a command). (6) The UI contract met, not just measured: the `ui-compliance` scanner gains comment awareness (`//` and `/* */`), Shape-B tree-glyph awareness (a renderer whose declared `body_shape` is B may use `├─ └─ │`), and a declared exclusion class for `scripts/test-*.cjs`; then every one of the 44 renderers missing Zone 1 / Zone 4 is swept to the 4-zone anatomy (`-- MindrianOS -- <name> -- <state> --` header, `▶ /mos:` footer) through the shared renderer, and the 3 genuine box-char hits plus the `✗` glyph are replaced from the 12-glyph vocabulary; acceptance is `doctor --ui-compliance` at 0 violations on a scanner that no longer counts comments. (7) Registered as a harness policy in `data/harness-policies/` (`doctor-auto-heal`) so `doctor --acceptance`'s `harness-policies` blocker fails the release train if the policy stops running; a hermetic test proves: fresh-install fires once, update fires once, same-version does not fire, a hook error never blocks session start, no destructive fix runs unattended, the reward line renders with its glyph, and no `--all` output ever contains the literal string `undefined`. (8) F13 and F14 ruled at plan time (registry-ghost gate; `mcp-surface` budget re-base or demotion to info; retired-name silent forwarding). (9) The acceptance number is a FRESH-USER number: on a clean machine, `claude plugin install mos@mindrian-marketplace` followed by one session start, with no command typed, yields a `doctor --all` reporting 0 auto-healable drift and 0 ui-compliance violations; the same holds after `claude plugin update`; measured on this machine's update to the beta that carries this phase as the live proof, then cut as that beta. Canon: Part 7 (reuse: the watermark, the accumulative engine, `post-update-activation`, the harness runner, `resolveActivePluginRoot`), Part 8 (the auto-heal is LOCAL; brain-smoke stays diagnostic-only and opens no Brain write), Part 11 R16 (the policy and the needs-your-call gate are born WIRED with `hitl_shape: F.1`), Part 12 (the reward line wears the glyph), Part 6 (dog-fooded on this repo's own room first). Consults claude-code-guide (done, above), icm-architect (registry ghosts and the sentinel rule, standing consult), langtalks-graph-expert (done, above), Context7 (Node warning suppression, F12). Depends on Phase 217 (the module engine and the D-03 gate), Phase 127.2 (post-update activation and preflight), Phase 224-02 (graph-derive re-enqueue). This card exists because the navigator ran `/mos:doctor` five times by hand in one session to reach a healthy install, and a plugin that needs its owner to nurse it after every update will never survive a stranger's machine.

**Goal:** [To be planned]
**Requirements**: TBD
**Depends on:** Phase 217 (module engine + D-03 gate), Phase 127.2 (post-update activation + preflight), Phase 224-02 (graph-derive re-enqueue). Not on Phase 351.
**Plans:** 0 plans
...
- [x] 354-02-PLAN.md -- SYS-08: gate approval confirms the card's subject claim, not the decision node
- [x] 354-03-PLAN.md -- SYS-09: chain resume by positional step identity, predecessor output restore, journal agreement
- [x] 354-04-PLAN.md -- SYS-02: owner-token write lock, liveness-only recovery, owner-aware release, graph-ops handle
...
- [x] 354-08-PLAN.md -- SYS-06: POC lossless save, Host/Origin/token model, revision conflicts; legacy dashboard Host-on-read recheck

**Wave 4** *(tier 3, Theo contract-safety; depends on wave 3)*

- [x] 354-09-PLAN.md -- THEO-01: structural rung round trip, registry-only executable chains validated before act init, ranked-candidate provenance, disclosed degradation
- [x] 354-10-PLAN.md -- THEO-01: taxonomy_ladder casing matched to Theo's PROBLEM_TYPE_IDS (recommendChain untouched)
...
- [x] 354-13-PLAN.md -- SYS-07: acceptance-runner timing, bounded children, clean rerun and RCA classification
- [x] 354-14-PLAN.md -- SYS-04: registration report, stderr diagnostics, status_read tool_registration health
- [x] 354-15-PLAN.md -- SYS-05: extract_shallow honest pure-parse contract (D-354-SYS05)
...
- [x] 355-07-PLAN.md -- HIPS-08, HIPS-09: Jev dev-time profiles on scripts/jev-devtime-client.cjs with per-question egress ceilings, model-version pinning and the response parser (completed 2026-09-23; three additive EGRESS_PROFILES entries + jev() timeoutMs/honorRetryAfter, scripts/jev-question-ceilings.cjs and scripts/jev-response-schema.cjs, zero network measured, tests/test-356-jev-client.cjs stays green)
- [x] 355-08-PLAN.md -- HIPS-04: data/framework-names.json regenerated from a live Theo read as a dated, hashed snapshot with offline --check (completed 2026-09-24; askOp('list_frameworks',{limit:1000}) deliberately trips Theo's own row cap -- confirmed between 200 and 220 -- so the flow falls to the bucketed pullTheoFrameworks read every time, landing the FULL 410-name live canonical set rather than a partial <=200 one; snapshot_date/source_sha256 recorded; stale_review carries 19 dropped (none referenced by any commands/*.md, independently re-verified) rather than RESEARCH.md's earlier 3, because the canonical bucketed pull also excludes alias nodes RESEARCH.md's direct-existence probe did not filter -- see 355-08-SUMMARY.md Deviations)

**Wave 3** *(depends on 355-02, 355-03, 355-05, 355-06, 355-07, 355-08)*

- [x] 355-09-PLAN.md -- HIPS-01: direction flips in hsi-lsa, hsi-engine, rs-innovation-classifier (none fallback) and the fourth producer, all delegating to the module (completed 2026-09-24; rs-math, hsi-lsa, hsi-engine delegate to direction-convention.cjs classify/classifyDiff; hsi-engine's computeHsiMatrix classifies on the raw pre-coercion matrix cells so the none sentinel survives; rs-innovation-classifier's single-axis branches both come from classify(lsa, bert), below-floor pairs emit 'none'; rs-discovery-engine routes 'none' pairs past thesis generation with skipped_reason 'no_direction', kept not dropped; legs B, C, D, F of tests/test-355-direction-agreement.cjs turn green -- E/G/H remain for 355-10/355-11; four pre-existing unrelated lib/memory/test-rs-*.cjs failures logged to deferred-items.md, not fixed)
- [x] 355-10-PLAN.md -- HIPS-01: D-47 atomic - rs-differential-scorer scoreMeasured flip plus the portfolio-dimensions feasibility-branch swap, with the ranking pinned first (completed 2026-09-24; tests/fixtures/355/eureka-ranking-pin.json pins 14 pairs' feasibility/composite-score/rank on unflipped code, then scoreMeasured delegates to direction-convention.cjs classify(lexical, semantic) and portfolio-dimensions.cjs's feasibilityFromRs swaps its two label branches in the same commit -- the pin stays 45/45 green, byte-unchanged, after the flip; test-211 Tests 5-6 and test-215 Tests 3-4 amended for the label swap, feasibility numbers unchanged; leg E of tests/test-355-direction-agreement.cjs turns green -- G/H remain for 355-11; no other scoreMeasured consumer branches on the label value, so no second swap needed; one pre-existing unrelated test-218-cohort-stratification.cjs failure logged to deferred-items.md, not fixed)
- [x] 355-11-PLAN.md -- HIPS-01: the Python detector removed from all four callers; scripts kept as offline reference (completed 2026-09-24; lib/core/intelligence-cascade.cjs and scripts/scout-cadence-runner.cjs no longer invoke detect-reverse-salients.py -- the cadence runner's new rs-engine-cjs step, lib/core/rs-engine.cjs's runModeInternal, carries SCHED-02's reverse-salient sensor instead, unconditionally, not gated on scikit-learn; commands/scout.md and skills/scout/SKILL.md step 2 document the same CJS engine; both Python scripts marked offline-reference-only, comment-only diffs; tests/test-scout-cadence-fires.cjs amended for the rs-engine-cjs step name; leg G of tests/test-355-direction-agreement.cjs turns green -- leg H's last two unresolved hits, lib/core/rs-chain-feeder.cjs and lib/memory/test-rs-discovery-engine.cjs, are outside this plan's files_modified scope, carried forward for a later plan; five unfiltered REVERSE_SALIENT readers found and logged for 355-12)
- [x] 355-12-PLAN.md -- HIPS-01: stored-label readers re-derive direction through the module; the duplicate SURPRISE_TYPES enums import DIRECTIONS; eureka_critic zod enum pinned (completed 2026-09-24; eureka-critic.cjs SURPRISE_TYPES, eureka-offer.cjs DIRECTION_ENUM, grill-engine.cjs EUREKA_SURPRISE_TYPES and tool-router.cjs's eureka_critic zod schema all alias direction-convention.cjs's DIRECTIONS; hsi-to-graph.cjs writes HSI_CONNECTION surprise_type as classify(lsa_sim, semantic_sim), never trusting the stored string; nl-graph-queries.cjs re-derives surprise_type on every hsi_connections read and suppresses REVERSE_SALIENT innovation_type unless the edge's source is rs-engine; fabric-chat.cjs and generate-chat-embed.cjs schema descriptions carry the same honesty note; legs A-G of tests/test-355-direction-agreement.cjs are green -- leg H's last two unresolved hits (lib/core/rs-chain-feeder.cjs, lib/memory/test-rs-discovery-engine.cjs) and the hsi-to-graph.cjs REVERSE_SALIENT DELETE's missing source filter are outside this plan's files_modified scope, carried forward)
- [x] 355-13-PLAN.md -- HIPS-07: three synthetic cross-domain fixture rooms under tests/fixtures/355-rooms/ (ill-defined, extend-the-opportunity, control with planted bridges and false friends) with the path guard (completed 2026-09-24; room-ill-defined, room-extend and room-control authored, each 4 sections of 3 or 4 artifacts, 150-300 words, D-48 framework/methodology frontmatter mix; room-control plants 4 meaning bridges and 4 false friends in planted-cases.json, joined only after judging; tests/test-355-fixture-rooms.cjs PASS 34 FAIL 0; guardian green on all three rooms; note: the path guard itself (guardRoomPath) is scripts/measure-355-hit-rate.cjs's job in 355-24, not this plan's -- this plan's own scope is the three rooms, the README and planted-cases.json)
- [x] 355-14-PLAN.md -- HIPS-04, HIPS-09: Theo response capture fixtures, templated citation pairs, and the navigator's blind citation-gold sitting (checkpoint) (completed 2026-09-24; citation gold machine-labeled by claude-opus-5.5 under the navigator's ruling, 0/43 by the navigator) [checkpoint]
- [x] 355-15-PLAN.md -- HIPS-08: HSI thinking-mode measurement (regex vs Jev, full set with none, per-mode confusion, 3 repeat runs) and the signed adoption decision (checkpoint) (completed 2026-09-24; not_adopted, bar failed on all 3 repeats at n = 45) [checkpoint]

**Wave 4** *(depends on 355-04, 355-06, 355-08, 355-09, 355-10, 355-11, 355-12)*

- [x] 355-16-PLAN.md -- HIPS-02, HIPS-04, HIPS-05: whitespace and HSI stamped at the output layer (opt-in --stamp), decimals replaced by band words, compute untouched (completed 2026-09-24; scripts/whitespace-command.cjs's map/analyze/score subcommands are async, awaiting stampFindings before printing renderScanLines/renderAnalyzeLines/renderNoveltyLines -- every rendered decimal replaced by a sparsity rank or a novel/moderate/covered band word; scripts/whitespace-to-graph.cjs and scripts/hsi-to-graph.cjs both gain an opt-in --stamp flag, awaiting Theo BEFORE any BEGIN and merging toNodeProps(stamp) into the SAME existing WhitespaceZone/HSI_CONNECTION write (lib/core/lazygraph-ops.cjs's addWhitespaceZone gained a small optional stamp-prop merge for this, D-16); without --stamp both scripts are byte-identical so scout-cadence-runner.cjs's SCHED-02 background call stays Theo-free; commands/whitespace.md, skills/whitespace/SKILL.md, commands/scout.md and skills/scout/SKILL.md render rules updated to reproduce the stamp block and disclosure line verbatim, never a number, with the D-50 Desktop/Cowork fallback sentence; tests/test-355-producer-whitespace-hsi.cjs: 58/58 assertions, Theo up and down, a planted-decimal negative control, node-prop re-parse proofs via fromNodeProps for both producers)
- [x] 355-17-PLAN.md -- HIPS-04, HIPS-05: find-bottlenecks and find-connections stamps (thin CJS entry for the prompt-driven path) (completed 2026-09-24; lib/core/rs-engine.cjs's runModeInternal gains an optional opts.stampFn, awaited before writeReverseSalientEdges opens the graph, merging a caller-supplied Map of already-flattened stamp props into the SAME REVERSE_SALIENT edge write, never requiring the stamp module itself; lib/agents/reverse-salient-agent.cjs builds that stampFn on lib/core/verification-stamp.cjs (rsEndpoints, D-48 local resolution), reuses the same computed stamps for the surfaced F.0 finding (python backend stamps individually, no stored props), and the F.0 body now carries the finding's verification stamp block via the new renderBottleneckFinding pure seam, dropping the old bracketed signed_diff/abs_diff number; scripts/stamp-connections.cjs ships the thin D-18 CLI entry for find-connections (--pair, --pairs-json, --json, --help; exact resolveEndpoint only, direction always 'none' per D-49, never an MCP tool); commands/find-bottlenecks.md, skills/find-bottlenecks/SKILL.md, commands/find-connections.md and skills/find-connections/SKILL.md updated to reproduce the stamp block verbatim, drop 'confidence scores', and carry the D-50 Desktop/Cowork fallback sentence; tests/test-355-producer-rs-connections.cjs: 43/43 assertions, Theo up and down, node-prop re-parse proofs, exact-name refusal, --json re-parse through Stamp)
- [x] 355-18-PLAN.md -- HIPS-04, HIPS-05: eureka renders carry the stamp line and evidence block; Score and lsa columns dropped (completed 2026-09-24; scripts/eureka-portfolio-report.cjs's eurekaEndpoints (D-48 local resolution) + stampRankedPairs stamp every ranked pair via verification-stamp.cjs, called from main() behind an opt-in --stamp flag (default off, zero-network by default) strictly before bankStatements opens its transaction (D-56); the embedded Markdown Ranked table drops the Score column, prints rank/A/B/weak dims/tail glyph/mode plus each row's own stamp block (or an honest 'not verified this run' line), ending in one disclosureLine('eureka'); lib/core/eureka/report-html.cjs's rankedTable replaces Score/Banked with a Verification column (tier word + formatPathText, or the honest unverified reason), sweeping the whole row's plain text through assertNoScalar; both reasoning-mode renders (md + html + statement cards) drop the lsa_similarity column/tag; lib/core/eureka/qualify-opportunity.cjs's componentValue renders a finite number as 'measured' (Canon Part 12), and formatComponentLines/renderQualificationCard prepend a candidate's stored stamp (fromNodeProps) as the F.1 card's first WHY lines; commands/eureka.md and skills/eureka/SKILL.md updated to reproduce the stamp block verbatim and carry the D-50 Desktop/Cowork fallback sentence; tests/test-355-producer-eureka.cjs: 54/54 assertions across stamp/report/html/qualify sections, including a planted-decimal negative control)
- [x] 355-19-PLAN.md -- HIPS-06, HIPS-01: D-53 atomic - eureka side-channel SCHEMA_VERSION 1 to 2 across runner, sensor, critic-tags check and fixtures; SENS-13 evidence bag gains the stamp enums (completed 2026-09-24; eureka-reach-runner.cjs's SIDE_CHANNEL_SCHEMA_VERSION and sensor-eureka.cjs's SCHEMA_VERSION both bumped to 2 in one commit, decoupled from the new CRITIC_TAGS_SCHEMA_VERSION (1, data/eureka-critic-tags.json's own version) so probeGuard stays available; SURPRISE_TYPES/FIRING_SURPRISE_TYPES now alias direction-convention.cjs's DIRECTIONS (closing the last two 355-19 carve-outs in tests/test-355-direction-readers.cjs's static sweep); buildSideChannelPayload/validateClosedSchema carry the v2 key set (stamp + opportunity_handle, both null on the hook-path scan producer); writeStampedSideChannel (atomic, validated) and markEurekaReachSurfaced (the idempotent fire-once ledger writer at <room>/.mindrian/eureka-reach-ledger.json) exported for 355-20/355-22 to call; the sensor's evidence bag gains opportunity_handle and six stamp_* fields (stamp_path via formatPathText), reads the ledger read-only and fails closed on a corrupt one, and stays zod-free with zero writes; tests/test-355-side-channel-v2.cjs: 63/63 assertions; full tests/run-all-213.sh: PASS=8 FAIL=0 SKIP=0; one pre-existing tests/test-237-session-scope.cjs Leg 4 gap (its own SENSOR_REQUIRE_FILES list missing four post-237 sensors) logged to deferred-items.md, not fixed)

**Wave 5** *(depends on 355-06, 355-07, 355-15, 355-16, 355-17, 355-18, 355-19)*

- [x] 355-20-PLAN.md -- HIPS-06: filing on the existing opportunity node via writeOpportunityNode extraProps inside bankStatements, SOURCED_FROM edges via writeEdge, born proposed (completed 2026-09-24; bankStatements accepts opts.stampsByKey/roomDir/runMode/stampElapsedMs -- a stamped entry extends extraProps with the toNodeProps fields + pws_stage (read once from ROOM.md before BEGIN) + engine_mode, reason/formula_version become 'eureka stamped finding'/'stamp-v1'; SOURCED_FROM edges (relation sourced_from, origin eureka-355) land on each end source artifact node beside the DERIVED_FROM edges; after COMMIT, writeStampedSideChannel writes the v2 side channel for the highest-ranked banked transferable-critic firing-band finding (opportunity_handle = the minted node id), and one cross_connection_stamped memory_event lands (new EVENT_TYPES member, honors MINDRIAN_DISABLE_MEMORY_EVENT); tests/test-355-filing.cjs 43/43, test-355-producer-eureka.cjs 54/54, check-substrate --diff clean)
- [x] 355-21-PLAN.md -- HIPS-04, HIPS-08, HIPS-09, HIPS-10: Part 8 egress sweep leg, the CIRS wiring test, and the doctor blocker cross-connection-honesty
- [x] 355-28-PLAN.md -- HIPS-08: CONDITIONAL - distill Jev's thinking-mode labels into a data/ rule table from a separate set and re-clear the adoption bar; no-ops with a recorded SKIPPED status if adoption was rejected in 355-15 (completed 2026-09-24; SKIPPED, navigator decision not_adopted in 355-15)

**Wave 6** *(depends on 355-20)*

- [x] 355-22-PLAN.md -- HIPS-05, HIPS-06: gate_answer _promoteCardSubject widened to opportunity with its regression test, SENS-13 fire-once dedup ledger, the stamped reach-card variant (completed 2026-09-24; gate.cjs's row.type check widens to accept 'opportunity' alongside 'claim' (already a TRUTH_CLAIM_TYPES member, Phase 348 guard unchanged), _maybeLogGateOutcome writes one local cross_connection_gate_outcome memory_event on an approve/reject outcome for a stamped opportunity; navigation-engine.cjs's decide() calls markEurekaReachSurfaced immediately after the Phase 222 ranking for a winning eureka_bridge reach, soft-fail; dial-label-composer.cjs's deep_research family gains the signal-aware stamped-finding card variant, byte-identical to formatStampLines(stamp, 'card'); tests/test-355-gate-opportunity-promotion.cjs (33 assertions) and tests/test-355-sens13-fire-once.cjs (21 assertions) both green; test-354-gate-subject-promotion.cjs, test-213-sensor-eureka.cjs, test-213-touchpoints.cjs, test-213-no-force.cjs, test-219-harvest-sensor.cjs, test-dial-label-bank-drift.cjs, build-connector-registry --check and check-render-coverage --check all green; test-345-gate-ratify.cjs's pre-existing anchor_confirmed failure and test-237-session-scope.cjs Leg 4 both confirmed pre-existing and unrelated)

**Wave 7** *(depends on 355-04, 355-14, 355-16, 355-17, 355-18, 355-22)*

- [x] 355-23-PLAN.md -- HIPS-02, HIPS-04, HIPS-05: the Larry Desktop stamp rule (stored stamps only), the no-decimal grep with Theo up and down, Tri-Polar and stamp-coverage tests (completed 2026-09-24; skills/larry-personality/SKILL.md gains "## Cross-connection stamps (Phase 355)" plus four recorded prose fixtures under tests/fixtures/355/prose/; tests/test-355-no-decimal.cjs sweeps all five producers, F.1 zones.body, the reach card and the four prose fixtures Theo up/down for a bare decimal/percent or a banned over-claim, 8 negative controls, 73/73; tests/test-355-tri-polar.cjs proves the same stamp reads the same tier/path/direction/judge on CLI + Desktop prose, and the same path plus a matching verified/unverified classification on the reach card's compact one-liner, 22/22; tests/test-355-stamp-coverage.cjs proves produced == rendered == Stamp-parseable by integer equality across all five producers, judge:'none' + disclosure-line uniformity, the AC7 no-citation-judgment sweep, and a byte-true replay of the real 355-14 Theo capture (148 pairs, 42 verified/106 unverified), 31/31; tests/test-355-floor-sweep.cjs gains leg 4b (dependent-output render leg), 109/109; bash tests/run-all-355.sh PASS=50 FAIL=3 SKIP=3, the same three pre-existing failures every 355 plan since 355-01 has carried, zero new regressions; a D-30 decimal-regex edge case (a decimal immediately before a sentence-ending period is not caught) found and logged to deferred-items.md, out of this plan's declared file scope)
...
- [x] 361-01-PLAN.md -- DDR361 family minted (13 rows); pre-phase baseline fixture (base sha, registry hash, quick-pass text, find_connections slice) plus its test; `tests/run-all-361.sh` written once
...
- [x] 361-04-PLAN.md -- `agents/dominant-design-researcher.md`, read-only by a host-enforced `tools:` list mirrored in `allowed-tools:` (D-17), connector-excluded; born-wired ledgers regenerated
- [x] 361-05-PLAN.md -- `theo-structure.cjs` (generic handle only; Theo when it has steps, reference otherwise, reason named, D-09/D-16) plus the Research mode note in the reference (Tri-Polar)
...
- [x] 298-02-PLAN.md - the committed converged-room fixture, scaffold-born and scrubbed (D-03)
- [x] 298-03-PLAN.md - lift the transcript reader into lib/hmi/turn-text.cjs, repoint check-card-fire
- [x] 298-04-PLAN.md - readable F.8 governance basket rows and the Part 8 comment correction (D-01, D-01a)
- [x] 298-05-PLAN.md - the D3/D4/D5 operating policy in the skill and the agent; Desktop wire untouched
...
- [x] 298-08-PLAN.md - evaluatePromotion, the never-warn doctor module, the D-04a proposal filed not built
- [x] 298-09-PLAN.md - data/harness-policies/: the closed _schema.json and the eleven slice-1 policies
...
- [x] 310-01-PLAN.md -- verify-tag-push library with injected git probes, Step 5.5 rewired to warn-on-confirmed-push, and a byte-level scope tripwire proving no other release gate moved
