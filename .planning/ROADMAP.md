...
- [~] 229-09-PLAN.md -- Demo run + Amnon verdict checkpoint + HUJI calibration workshop [D6,D7] -- **DI-1/2/3 RESOLVED + judge CALIBRATED live; demo STILL BLOCKED by DI-4 (2026-07-16 fix-and-verify):** the 3 CLI/auth blockers are fixed + committed (DI-1 inline `--json-schema`; DI-2 strip draft-2020-12 `$schema`; DI-3 one keychain auth mechanism, both stages + judge). run-all-229 stays PASS=9. The live judge calibration CLEARED: **Spearman 0.901** (min 0.7), Dental post>pre, DnATA<Lucid -> JUDGE CALIBRATED. Stage A verified working. But the FIRST end-to-end run exposed **DI-4 (architectural):** the Stage A->Stage B evidence handoff is broken - `populateRoom` writes the room GRAPH (`room.db`, 11 claim nodes) but the grading session can't read it (no `sqlite3` in its allowed tools) and the section markdown is empty, so opus graded an EMPTY ROOM and correctly refused to fabricate (`scores: {}`). Plus **DI-5:** Stage A cleaned disfluencies, breaking the D1 verbatim gate. STOPPED per the "4th bug -> STOP" mandate (DI-4 is a navigator decision on the grading input contract). No artifact fabricated. See demo/DEMO-VERDICT.md + deferred-items.md (DI-4/DI-5). Tasks 2 (Amnon verdict) + 3 (HUJI workshop) still pending, now behind DI-4/DI-5.
