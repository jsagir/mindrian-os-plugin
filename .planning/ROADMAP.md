...
**Plans:** 11 plans in 6 waves (Wave 1: 01-03 parallel RED-first Wave 0 scaffolding; Wave 2: 04-05 parallel infra + numeric primitives; Wave 3: 06-07 parallel Convention A / Convention B ports; Wave 4: 08-09 parallel orchestration; Wave 5: 10 caller wiring + D-09 rule-6 amendment; Wave 6: 11 final gate + requirements registration). Scope note: D-10 (navigator ruling, 2026-08-31) descoped Mode B/C external Pinecone corpus to a follow-up phase; this plan set also explicitly defers `lib/core/rs-differential-scorer.cjs`'s embedded Python bridges, `lib/core/rs-pinecone-bridge.cjs`, and `scripts/detect-reverse-salients.py` -- named explicitly in `272-10-PLAN.md`'s Objective and `272-11-PLAN.md`'s `DEFERRED-SCOPE.md`, not silently dropped.

Plans:

- [ ] 272-01-PLAN.md -- Wave 0 test infra: `tests/run-all-272.sh` aggregator, the sklearn `svd_flip` sign-convention spike (pinned live against installed sklearn 1.8.0), and 5 fixed-input unit RED tests (tfidf parity, abs-diff top-k, direction convention, spectral, hsi-lsa algorithm)
- [ ] 272-02-PLAN.md -- Wave 0 fixture room (90+ artifacts) + 20-pair probe corpus + ARPACK-regenerated Python baseline + measured noise-floor gate threshold + 2 RED integration tests (rs-engine contract, rank agreement)
- [ ] 272-03-PLAN.md -- Wave 0 dispatch/infra RED tests: dispatch-chokepoint + rule6-amended acceptance greps, cache-probe/cache-location unit tests, Pinecone /embed unit test (mocked fetch)
- [ ] 272-04-PLAN.md -- `rs-backend-dispatch.cjs` (D-04 flag chokepoint), `pinecone-inference.cjs` (D-01 /embed module), `embedding-spine.cjs` D-06/D-07 cache fixes
- [ ] 272-05-PLAN.md -- `lib/core/numeric/svd.cjs` (deterministic truncated SVD + sklearn's verified svd_flip) and `lib/core/numeric/tfidf.cjs` (sklearn-compatible TF-IDF)
- [ ] 272-06-PLAN.md -- `lib/core/rs-math.cjs`: full port of `rs_math.py` (Convention A, topic-keyword-membership signal)
