...
**Requirements:** GDH-01 one resolver (`.room-root`, not registry active), GDH-02 derivation-in-loop, GDH-03 sub-room rollup (per-sub-room db + parent read-side rollup), GDH-04 non-.md reach (non-destructive .docx/.html extractor; source file UNTOUCHED), GDH-05 typed-edge derivation (runChain-driven, fable-mode-critiqued, lands `review_status: proposed`), GDH-06 backfill (`/mos:graph --derive`; acceptance fixture b2-journey 0 -> N), GDH-07 idempotent (re-run is a no-op).
...
**Status:** Scoped 2026-06-19 (SPEC + CONTEXT committed, decisions D-169-01..05 locked). Next: research -> /gsd-plan-phase 169 -> check -> revise -> execute.
