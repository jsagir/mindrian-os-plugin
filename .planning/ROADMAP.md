...
- [ ] 270-04-PLAN.md -- MEMOP-07/08: RED pins for the Phase 8 cross-room fence (read-only parameterized ATTACH, both edges tables byte-identical, apostrophe-bearing room name still contributes) and pre-room identity-write reachability under an isolated HOME. **PARTIAL (2026-08-27): Task 1 (tests/test-270-cross-room-fence.cjs) committed `d4976fc5`. Task 2 (tests/test-270-identity-write.cjs) is genuinely blocked, not skipped by choice -- its own action text requires reading `270-DECISIONS.md`'s `## OQ-2 ANSWER` to pick between a 5-leg vs 3-leg test shape, and that file does not exist until plan 270-01's navigator gate is answered. Do not write a 270-04-SUMMARY.md until Task 2 completes; a resuming session should treat this as the safe_resume_gate "production commit exists, SUMMARY.md missing" case, close out via Task 2 once OQ-2 is known, and only then mark this plan complete.**
