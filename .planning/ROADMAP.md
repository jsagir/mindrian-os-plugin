...
**Status:** IN PROGRESS 2026-06-18. Plan 01 SHIPPED (HARN-01: the 3-MAP DIGEST manifest + build-harness-manifest.cjs generator with STALE/UNRESOLVED/MALFORMED --check + recipe-maps loadManifest() + Part 8 boundary scan; run-all-167.sh 6/6). Plan 02 SHIPPED (D-167-03 BOTH: the manifest --check wired into the live .git/hooks/pre-commit + the tracked scripts/install-pre-commit.sh template + run-all-167.sh; path-scoped on the manifest + generator + the three source maps; exit-2/exit-1 on drift with a regenerate recovery line; guard-fires proven; run-all-167.sh 7/7). Remaining: HARN-02 fable-mode step discipline + HARN-03 /mos:new-surface generator. Sequence: BEFORE 164/165 (navigator-LOCKED 2026-06-18; revised order 163 -> 166 -> 167 -> 164 -> 165) so 164/165 inherit generated wiring + fable-mode from birth.
