...
- **Gate 0 -- RESOLVED BY NAVIGATOR RULING 2026-08-10 (release-first):** the v2.0.0-beta.1 cut proceeds NOW because every beta.13 install carries a dead Brain path (246-01-LIVE-RESULT); the 234-08 foreign-host verify MOVES to post-release verification, not waived - recorded here. Original gate text preserved below.
- **Gate 0 original text -- official v1.15.0 close-out FIRST:** finalize the v1.15.3-beta.x train to the stable v1.15.x release (`release.sh --finalize` flow: npm @latest, tag, marketplace pin, full lockstep per the release hard rules), closing Phase 234 as the last v1.15.0 phase. Per the standing rule (`feedback_dev_repo_fix_not_live_until_released`), v1.15.0 is not "shipped" until this release actually cuts and is picked up.
- **Gate 1 - v1.16.0 train disposition: RESOLVED (navigator ruling 2026-08-10): FOLD INTO the v2.0.0 train.** v1.16.0-beta.13 stays the last v1.16.0 beta; no v1.16.0 stable will be cut. The next release cut opens v2.0.0-beta.1 via release.sh --start-prerelease. Gate 0 (the v1.15.0 close-out, whose remaining substance is Phase 234-08 Task 2's foreign-host human-verify) still precedes that first cut.
