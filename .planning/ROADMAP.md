...
Plans:
- [ ] 123-01-PLAN.md -- scripts/release.sh overhaul: semver devDep + --prerelease/--finalize/--start-prerelease bump algebra + two-commit form + dirty-repo guard + Step 9.5 @mindrian_os/install rename (wave 1)
- [ ] 123-02-PLAN.md -- install-state record (~/.mindrian/install-state.json, single writer in session-start) + data/deployment-surfaces.json manifest + lib/core/active-plugin-root.cjs topology extension (wave 2)
- [ ] 123-03-PLAN.md -- doctor classes I + J + --install-state flag + aggressive --fix (legacy migration, installed_plugins.json repair) + Bug 7 fix + INSTALL_DIR repoint + per-class test fixtures (wave 3)
- [ ] 123-04-PLAN.md -- mindrian-os doctor --acceptance (5-point + --pre-tag sub-mode, checklist runner, CALLS verify-release, mktemp npx sandbox) + release.sh Step 6.6 + Step 9.6 hard-abort gates + retire scripts/release-beta-smoke.sh (wave 4)
- [ ] 123-05-PLAN.md -- lib/core/cache-prune.cjs (keep active + N=2, never the active, skip if installed_plugins.json unreadable) wired into session-start (on version change) + doctor --fix (unconditional) + the @mindrian_os/cli sweep + commands/setup.md:145 URL fix (wave 5)
