# Milestone v2.0.0 "Build the Loop" Requirements (ARCHIVED at close, 2026-08-13)

**Close-out status: 20/23 closed on evidence; 3 carried open by explicit navigator ruling
(2026-08-13): SWEEP-02 (floor-gated - now enrichment work, not surgery), CACHE-03 (needs a
live interactive session's hit-rate measurement), AVAIL-03 (operator legs: suspension click +
restore rehearsal). Carried-open items are recorded in the v2.0.0-ROADMAP.md archive summary
and STATE.md Deferred Items - not silently dropped, not fabricated as closed.**
...
- [x] **AVAIL-01**: A scheduled synthetic probe exercises the live contract surface
      ...
      DONE 2026-08-11 (enactment-night handoff, state of record): hourly GitHub Action on
      the brain repo (:17) runs real tool calls; failure email out-of-band; first run green.
      Upstream Claude Code bug filed: anthropics/claude-code#85631.
