...
| **learning-loops** | SEED-002, SEED-009 | Calibrated learning from accumulated outcome data: SEED-002 is lab-side APO over command prompts; SEED-009 is per-room ranker weight adaptation. Both are gated on corpus-size thresholds that only fire post-launch. Both honor Canon Part 8 (data stays where it's owned). | TBD v1.14.0+ when corpus thresholds clear | When `/gsd:new-milestone` opens v1.14.0+ AND (Phase 121 trajectory telemetry shows >=100 entries OR tester cohort >=30), evaluate both. SEED-009 is also gated on F-selector outcome edges >=1000. |
