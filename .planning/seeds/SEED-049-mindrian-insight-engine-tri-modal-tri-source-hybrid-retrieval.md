...
**What "done" looks like:** `nodeText()`/`indexedText()` need a resolution fallback, keyed by node type: for `Artifact`/`memory_artifact` nodes, follow `path` (relative to the room dir) and read the actual file, extracting real body content (not just frontmatter title) via the same markdown-parsing convention `lib/vault/room-scanner.cjs` already uses elsewhere (Part 7 reuse). For `claim` nodes, `source_segment` needs a resolver back to whatever artifact/section it references (its exact addressing scheme needs one more read of `navigation.cjs`'s claim-writing path to confirm precisely - not yet done this session). For `WhitespaceZone`/other analysis-output node types, add their real content field (`hypothesis`, etc.) to the checked key list rather than assuming a fixed four-key list covers every node type MindrianOS produces.
