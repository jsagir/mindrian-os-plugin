...
- [ ] **TRIG-01**: A `content` trigger tier exists, scored via `node:sqlite` FTS5 + `bm25()` over room.db content -- zero embedding calls, zero new dependencies, zero network. Fires on lexical/content relevance to a room's stored material, independent of structural state change or a hand-picked keyword hit.
- [ ] **TRIG-02**: The dial (`f-selector-ranker.cjs`) fuses candidate scores across trigger-tier families via Reciprocal Rank Fusion (rank position, not raw score) before the `MAX_K=3` cut, since a `bm25()` score and the existing `0..1` decay-weighted score are not comparable.
- [ ] **TRIG-03**: The top-K cut applies an MMR-shaped diversity term so same-family candidates cannot crowd out a genuine cross-family hit, following the `_applySens10Flip`/`_applyRoleLevelBias` layered-adjustment-pass pattern already in production.
