...
**Phases:** 104 revival (per-command-ui-wrapping → renamed: per-room-wiki-completeness) with 5 sub-plans (104-01 room resolution wiring; 104-02 graph-as-homepage + auto-create; 104-03 Wikipedia zones + section-to-section hyperlink wiring + cross-room link resolution + RED-LINK rendering; 104-04 click-red-wikilink-to-research + Phase 32-02 chat tool-call wiring; 104-05 9-tier freshness frontmatter + content gap dashboard + multi-layout Cytoscape). Plus deferred from Closed Loop: Phase 100 (JTBD inference engine), Phase 101 (selector library JTBD-aware), Phase 112 (GraphRAG retrieval + Room Budding).
...
| v1.14.0 | **The Visible Room** | Make the invisible visible -- inside and outside the room. One renderer, two surfaces (Wiki + SnapshotHub), same DNA | 104 revival (5 sub-plans) + 124 (was 100) + 125 (was 101) + 112 (GraphRAG + Room Budding) + **123 (SnapshotHub MVP, fused 2026-05-07)** + SEED-001/003-A5/004/006 | Lawrence Aronhime tester preview HARD release gate (Canon Part 6) |
