...
beta.11 is the v1.13.0 CAPSTONE -- its headline content is the **Workflow Layer** (framework <-> command registry + reliable invocation; spec at `.planning/WORKFLOW-LAYER-SPEC.md`; doc at `docs/WORKFLOWS.md`). Phase 122 (Workflow Layer) is now built; the items below cover it plus the npm-installer / npm-rename work that also landed on `main` toward the capstone. The capstone was re-numbered from beta.10 to beta.11 on 2026-05-12: `1.13.0-beta.10` had already been published to npm that day as a token-validation build (dist-tag `@next`, no git tag, not on the marketplace), so the tagged + marketplace'd capstone takes the next number. The maintainer-gated release steps (cut the tag, pin the marketplace `source.ref`, `npm publish` with the `@next` dist-tag) are listed in the Maintainer Notes at the end of this block -- they are NOT performed by Phase 122.
