## [Unreleased] -- v2.0.0-beta.40 (in progress)
...
- **The arbitration node (Phase 346).** One ranked posture decision per turn, replacing "whichever rule fired last": a nine-rule enforcement ladder decides enforce-vs-judge, composed with the existing delivery and autonomy axes into a single result attached directly to `decide()`. Ships `detectEscapeHatch`, the missing producer for the "just tell me" / "bottom line" escape hatch on Claude Code (it existed as doctrine but had no code behind it until now). Replayed against 8 real recorded WATCH-log misfires from the 2026-07-02 "Larry feels less like Larry since hard-fail checks arrived" regression report: all 8 suppressed, both positive controls still fire correctly. Byte-identical no-op when the resolver has nothing to say, so a fault degrades to exactly today's behavior.
- **The supersession node (Phase 348).** Closed a real security hole: `transitions.cjs`'s human-attribution guard only covered confirming a claim, not superseding one, so any agent holding a database handle could silently close a human-confirmed truth claim. Now gated the same way confirmation always was. Ships the actual missing wire (`lib/core/temporal/supersession-gate.cjs`) that turns a human-approved contradiction resolution into a real, traceable supersession (never a deletion), a reshaped `findContradictions` (options bag, `review_status` filter, a guard against misreading a reified event as a claim), and a validity-window reconciliation. Canon Part 9 narrowed from 1.27 to 1.28 (Appendix D entry 41) so the constitution matches what the code has always been supposed to do, and the "superseded is not deleted" doctrine is now written into Larry's own voice.
- **Release-to-Theo leading edge (Phase 349).** `release.sh` gains Step 5.6: the moment a real release lands, it fires a signal to Theo (the teaching graph) telling it a version moved, instead of waiting for Theo to notice its own drift on the next cut. Pairs with the existing Step 0.6 lagging gate (RULE 5 place 8 now documents both halves in one place). Audited `--no-theo-notify` opt-out; a dry run reports and sends nothing.
- **The room-graph audit node and counter-metric rule (Phase 343).** A per-room integrity census (dangling edges, unanchored claims, stale unconfirmed guesses) registered as an insight sensor, plus a standing rule that any sensor optimizing a number must declare a paired counter-metric so success can't be gamed silently.
- **The layer contract (Phase 344).** Every command, agent, pipeline, skill and MCP tool now declares which of the five engineering rungs it belongs to (prompt, context, harness, loop, graph), with a doctor gate checking the declaration is present.
- **The strategy node (Phase 345).** A slow, periodic reach that re-climbs the room's own goal ladder and proposes a rewritten JTBD at a Decision Gate when a room stalls, so a room can't hit a stale goal cleanly for weeks without anyone asking whether the goal is still right.
- **The shared-state contract for chains (Phase 347).** Multi-step chains (`chain_run`, `/mos:pipeline`, act-chain, the five-perspective meeting fan-out) now hand off a real, typed, graph-anchored record between steps instead of raw conversation prose, with declared routing (`on_pass`/`on_fail`/`fan_out`/`fan_in`/`reviewer`) and a real reviewer-node identity guard.
- 

## [2.0.0-beta.39] - 2026-09-14
