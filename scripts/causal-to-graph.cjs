#!/usr/bin/env node
/**
 * causal-to-graph.cjs -- SQLite Writer for Causal Extraction Results
 * ==================================================================
 ...
 *
 * Usage: node scripts/causal-to-graph.cjs /path/to/room
 *
 * Migrated from causal-to-kuzu.cjs (KuzuDB) to SQLite via lazygraph-ops.cjs.
...
async function main() {
  ...
  if (!roomDir) {
    process.stderr.write('Usage: node scripts/causal-to-graph.cjs /path/to/room\n');
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(`Wrote ${written} causal claims from ${sourceArtifact} to SQLite graph\n`);
    ...
    if (Array.isArray(data.cascades) && data.cascades.length > 0) {
      ...
      process.stderr.write(`Wrote ${cascadeWritten} CASCADES_TO edges to SQLite graph\n`);
    }

  } catch (e) {
    process.stderr.write(`Causal-to-graph error: ${e.message}\n`);
    process.exit(1);
  } finally {
    if (db) {
      try {
