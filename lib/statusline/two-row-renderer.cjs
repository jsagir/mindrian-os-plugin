 ...
 * @param {string} [state.posture] - COMPOSED posture (push_forward|hold|
 *                                   pull_back), a frozen enum handle. When set,
 *                                   a posture badge segment is appended to row 2
 *                                   alongside the operator slot (Phase 177
 *                                   BCH-12). Additive: the no-posture path is
 *                                   byte-identical to the prior renderer.
...
function renderRow2(state, columns) {
  ...
  // Posture badge segment (Phase 177 BCH-12). The COMPOSED posture
  // (push_forward / hold / pull_back) is a frozen enum handle, surfaced as a
  // plain upper-case tag alongside the operator slot. It is the statusline
  // second-surface read-out of the same composed posture render-v2 paints into
  // the Zone 1 badge. The badge is a plain enum-handle label -- there is no
  // praise/grade tag, the same invisibility-by-absence rule as render-v2
  // (the badge reflects posture, never a compliment). The segment uses the
  // same cols-threshold drop discipline as the neighboring operator slot
  // (drops below 80) and the re-measure/truncate policy below still applies.
  // Statusline text stays plain (no raw ANSI SGR bytes) per this surface's
  // existing byte-clean policy. The no-posture path is byte-identical to today.
  if (cols >= 80 && s.posture) {
    segments.push('⟐ ' + String(s.posture).toUpperCase());
  }
