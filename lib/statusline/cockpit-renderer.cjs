...
// footer-affordance baseline is 192-03): when a stance override is active
// (~/.mindrian/stance-state.json via lib/core/stance-state.cjs, folded into the
// signal set by lib/statusline/cockpit-signals.cjs), _render adds (a) a passive
// [stance] chip as its OWN new segment after the identity segment, and (b) forces
// the locked voice-glyph color (redteam=red / tell-act=blue) in the SAME render
// pass -- the pole IS the voice glyph, not a second signal. When no override is
// active (stance null/absent, the default) the output is byte-identical to
 ...
 *   stance_forced_color string -- 'red' (redteam) | 'blue' (tell-act) | null; overrides the Tier-1 voice glyph
...
function _render(s) {
  ...
  // Tier-1 voice glyph. SEED-042 (192-04): when a stance forces a locked voice
  // color (redteam=red / tell-act=blue, resolved in cockpit-signals from
  // forcedVoiceColorForStance), it OVERRIDES natural voice-mark detection in this
  // same render pass -- the pole IS the voice glyph. research / ask force no color
  // (s.stance_forced_color null), so natural detection still governs. Byte-stable
  // by default: with no forced color the glyph is exactly resolveVoiceGlyph(s).
  let voiceGlyph = isLarry ? resolveVoiceGlyph(s) : null;
  if (isLarry && voiceMark && typeof s.stance_forced_color === 'string' && s.stance_forced_color) {
    const forcedGlyph = voiceMark.glyphForColor(s.stance_forced_color);
    if (forcedGlyph) voiceGlyph = forcedGlyph;
  }
