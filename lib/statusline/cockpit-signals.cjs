 ...
 * governing thought is the secondary fallback; the honest "--" placeholder is the
 * safe default (Ruling 3c, 2026-07-02): it reads as "no routed step yet", NEVER the
 * lying word "continue" (which pretended a live cue existed when it did not). The
 * routed cache is now ALSO fed by the per-turn decide() cue (fire_skill / fired
 * reach), not just the offer-resolver, so the routed leg wins far more often --
 * closing the "dead Next: continue" root cause.
 * @param {Object} opts
 * @returns {string}
 */
...
function collectSignals(opts) {
  ...
  return {
    ...
    // Stance override (SEED-042 posture dial, 192-04; softened by Phase 210 item B):
    // when a stance is active the renderer shows a passive [stance] chip and
    // applies the stance's DEFAULT voice color (redteam=red / tell-act=blue) as a
    // preference -- natural voice detection wins when it yields a color. Absent /
    // null override -> both fields are null and the render is byte-identical to
    // pre-plan (R4). Degrades silently on any read failure, matching the
    // surrounding convention. Field names kept stable across the seam.
