...
/**
 * JTBD -> CLI semantic color map (Phase 102 D-06).
 *
 * Maps each of the 13 canonical Phase 100 JTBD handles to one of the 5
 * SKILL.md §4 CLI colors (red / yellow / cyan / green / gray). NO new
 * colors invented; the 5-color contract is the boundary.
 *
 * Color anchors:
 *   - red    : commit / blocker / contradiction (high-stakes)
 *   - yellow : questioning / challenge / warning
 *   - cyan   : informational / cross-reference / capture
 *   - green  : forward action / commitment / success
 *   - gray   : meta / no specific action (the canvas)
 *
 * @type {Readonly<Record<string,string>>}
 */
const JTBD_CLI_COLOR = Object.freeze({
  'decide-pursue': 'red',
  'find-problem': 'yellow',
...
/**
 * Posture -> CLI semantic color map (Phase 177 BCH-12).
 *
 * Maps each of the 3 FROZEN posture ids (sensor-types.cjs POSTURE_IDS:
 * push_forward / hold / pull_back) into the SAME 5-color CLI contract used by
 * JTBD_CLI_COLOR (red / yellow / cyan / green / gray). NO 6th CLI color is
 * invented (blue/black/white are HTML/Mondrian only). The 3 postures map:
 *   - pull_back     -> red    : the engine held the navigator back (high-stakes)
 *   - hold          -> yellow : the engine paused / questioned (caution)
 *   - push_forward  -> green  : the engine drove forward (commitment / go)
 *
 * INVISIBILITY BY ABSENCE (the BCH-12 headline guarantee): this map has NO
 * 'praise' key and NO 'grade' key. There is no engine state for a compliment,
 * so there is no color to paint one. The badge structurally CANNOT flatter --
 * you cannot color what the map does not name. The badge is driven from the
 * COMPOSED posture (the engine's composed decision, never a model emit), so it
 * is painted at composition and cannot lie about a later state.
 *
 * @type {Readonly<Record<string,string>>}
 */
const POSTURE_CLI_COLOR = Object.freeze({
  pull_back: 'red',
  hold: 'yellow',
  push_forward: 'green',
});
...
/**
 * ANSI SGR escape sequences for the 5-color contract (SKILL.md §4).
 * Used by Zone 1 left-rail accent (D-07). TTY-gated at render time so
 * non-TTY captures (CI logs, JSON-mode emission) stay byte-clean.
 *
 * @type {Readonly<Record<string,string>>}
 */
 ...
 * @param {string|null} [args.posture]  - COMPOSED posture (pull_back|hold|
 *                                        push_forward) or null. When present it
 *                                        DRIVES the Zone 1 badge color (BCH-12);
 *                                        jtbd is the fallback when posture is
 *                                        absent. Painted at composition so it
 *                                        cannot lie. (default null)
...
function render(args) {
  ...
  const posture = args.posture || null;
  ...
  // 3. Zone 1 left-rail accent (Phase 102-05; D-06 + D-07).
  //    Subtle JTBD-aware accent: a single colored ■ glyph prepended to
  //    the existing Zone 1 header. ALL gates must hold:
  //      (a) jtbd is non-null and present in JTBD_CLI_COLOR (D-06);
  //      (b) compact mode is OFF (D-08; uses the unified `compact` flag
  //          from slot 1 above, so the local env+budget gate from the
  //          102-05 worktree's standalone build collapses out -- single
  //          source of truth for compaction);
  //      (c) process.stdout.isTTY is true -- non-TTY captures (CI logs,
  //          piped output, JSON-mode emission) MUST stay byte-clean per
  //          the RENDER-102-05 strip-ANSI invariant.
  //    The ■ glyph itself is shape, not color -- emit it in BOTH TTY and
  //    non-TTY modes so the strip-ANSI(TTY) === non-TTY byte-identical
  //    invariant holds. Only the ANSI SGR bytes are gated on isTTY.
  //    Mutate a shallow copy so we never alter the caller's zones object.
  //
  //    Phase 177 BCH-12: the color SOURCE is now posture-first. The COMPOSED
  //    posture (pull_back/hold/push_forward) DRIVES the badge color when
  //    present; jtbd is the fallback. The badge is painted from the engine's
  //    composed posture so it cannot lie about a later state, and because
  //    POSTURE_CLI_COLOR has NO praise/grade key the channel structurally
  //    cannot flatter (invisibility by absence). Resolution order:
  //      posture (if in POSTURE_CLI_COLOR) -> jtbd (if in JTBD_CLI_COLOR).
  const postureColorName = posture && POSTURE_CLI_COLOR[posture];
  const jtbdColorName = jtbd && JTBD_CLI_COLOR[jtbd];
  const accentColorName = postureColorName || jtbdColorName;
  if (!compact && accentColorName) {
    const useColor = process.stdout.isTTY === true;
    const color = useColor ? ANSI[accentColorName] : '';
    const reset = useColor ? ANSI.reset : '';
    zones = Object.assign({}, zones);
    zones.header = color + '■' + reset + ' ' + (zones.header || '');
  }
...
module.exports = {
  ...
  isCompact,
  applyCompaction,
  // Phase 102-05 exports for test fences + downstream introspection.
  JTBD_CLI_COLOR,
  ANSI,
  // Phase 177 BCH-12 export: the posture-driven badge color map.
  POSTURE_CLI_COLOR,
};
