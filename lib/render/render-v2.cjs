...
function render(args) {
  ...
  // 3. Zone 1 left-rail accent (D-06 + D-07; Plan 102-05).
  ...
  //      (b) compact mode is OFF (D-08; sibling 102-02 owns the unified
  //          compaction signal — until then, this plan honors D-08 via
  //          a local env+budget gate so the "compact drops the accent"
  //          invariant holds in isolation);
  ...
  if (jtbd && JTBD_CLI_COLOR[jtbd]) {
    let compactLocal = false;
    if (process.env.CLAUDE_CONTEXT_USED_PCT) {
      const pct = parseFloat(process.env.CLAUDE_CONTEXT_USED_PCT) / 100;
      if (!isNaN(pct) && pct > 0.80) compactLocal = true;
    }
    if (!compactLocal && tokenBudget && tokenBudget.total) {
      if ((tokenBudget.used / tokenBudget.total) > 0.80) compactLocal = true;
    }
    if (!compactLocal) {
      // The ■ glyph itself is shape, not color — emit it in BOTH TTY and
      // non-TTY modes so the strip-ANSI(TTY) === non-TTY byte-identical
      // invariant (RENDER-102-05) holds. Only the ANSI SGR bytes are
      // gated on process.stdout.isTTY. Result: non-TTY captures (CI
      // logs, JSON-mode emission, piped output) stay byte-clean of
      // escape codes while still carrying the structural accent.
      const useColor = process.stdout.isTTY === true;
      const color = useColor ? ANSI[JTBD_CLI_COLOR[jtbd]] : '';
  ...
  // 102-02..04 wire them up. They are already pulled out above so callers
  // can rely on the destructuring contract. Plan 102-05 has consumed
  // jtbd + tokenBudget; siblings 102-02 / 102-03 / 102-04 will consume
  // mode, tier, roomDir, provenance.
  void mode; void tier; void roomDir; void provenance;
