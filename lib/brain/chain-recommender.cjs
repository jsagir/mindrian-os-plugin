 ...
 *
 * Canon Part 8 (Graph Boundary): the only Brain touch in recommendFrameworkChain
 * itself is lib/core/brain-client.cjs isAvailable() (a sync boolean). Any
 * FEEDS_INTO Cypher carries ONLY framework names + problem-type / phase enums,
 * bound via brain-client.sanitizeCypherInput and forwarded as $-params -- never
 * a command string, never user content (no artifact bodies, no meeting text, no
 * proprietary numbers). This file contains zero command literals (the
 * pre-commit / test grep guard from 122-02 enforces that).
 *
 ...
 * externally blocked.
 *
 * Quick 260819-c8j (WS-C2/C3) -- the live async chain path SHIPPED.
 * ==============================================================================
 * The paragraph above used to describe the live Brain chain path as unbuilt
 * "122-04 async wiring". That is now false in one direction and true in
 * another: the live path SHIPPED as chainOfferForReach() / adaptChainToRunInput()
 * below, consuming lib/core/brain-client.cjs's recommendChain() wrapper (the
 * quick 260819-c8j sibling of feedsIntoChains). recommendFrameworkChain() and
 * recommendCanonicalTargets() above are UNCHANGED and stay synchronous -- 14+
 * callers key on recommendFrameworkChain's plain string[] return, so this
 * quick task does not reshape it. This file, not a new one, hosts the
 * consumer: tests/test-252-guard-census.cjs census.1 fails the build for any
 * UNCLASSIFIED file containing an executable is/ensureAvailable( call, and
 * this file is already ROUTE-classified -- a new module would be an
 * unclassified guard site the moment it called brainClient.isAvailable().
 *
 * License: BSL 1.1.
 */
...
const brainClient = require('../core/brain-client.cjs');              // the ONLY Brain path: isAvailable() + sanitizeCypherInput + recommendChain/normalizeFrameworkName
...
// Quick 260819-c8j (WS-C3): the ONE governed framework-to-command door (Part
// 7, R4: mint no second resolver). commandsForFramework / composeWorkflow
// NEVER touch the Brain (grep-guarded in lib/workflow/command-resolver.cjs's
// own header).
const commandResolver = require('../workflow/command-resolver.cjs');
...
// Quick 260819-c8j (WS-C2): the companion HANDLE the three producer sensors
// (lib/core/insight-sensors.cjs:617, sensor-methodology-decision.cjs:108-110,
// sensor-diffusion-adoption.cjs:218) prefix onto a fired reach's `companions`
// array. Frozen so parseChainCompanions below has a single source of truth.
// Parsing SPLITS on ':' rather than writing a command prefix as a literal --
// this constant is a data handle, never a command string (fence 4).
const CHAIN_COMPANION_HANDLE = 'brain_framework_chain';
...
// ---------- Companion consumer + chain adapter (Quick 260819-c8j, WS-C2/C3) ----------

/**
 * parseChainCompanions(reach) -> string[]
 * Reads a fired reach's frozen `companions` string array (lib/core/sensors/
 * sensor-types.cjs makeReach) and returns the ordered, deduped list of
 * problem_type handles named by CHAIN_COMPANION_HANDLE-prefixed companions.
 *
 * - reach null / not an object / no `companions` array -> []
 * - a companion is split on ':'; the first segment must equal
 *   CHAIN_COMPANION_HANDLE and the second segment must be non-empty after
 *   trim, else the companion is skipped (defensive: reach.companions strings
 *   are attacker-influenceable in the sense that tuple.problem_type
 *   originates from the diagnose-command classification over room text).
 * - segments past the second (e.g. sensor-methodology-decision.cjs:110's
 *   third `:<framework>` segment) are IGNORED here on purpose -- consuming
 *   that segment is a named follow-up (see 260819-c8j-SUMMARY.md "named
 *   follow-ups deliberately deferred"), not a silent drop: it is read, seen,
 *   and explicitly not acted on yet.
...
function parseChainCompanions(reach) {
  ...
  for (const c of reach.companions) {
    ...
    const parts = c.split(':');
    if (parts.length < 2) continue; // bare handle, no second segment -- skip
    if (parts[0] !== CHAIN_COMPANION_HANDLE) continue;
    const handle = parts[1].trim();
    if (handle.length === 0) continue;
    if (seen.has(handle)) continue;
    seen.add(handle);
    out.push(handle);
    // parts[2+] (e.g. the sensor-methodology-decision.cjs third segment) is
    // deliberately ignored -- see docblock above.
  }
  return out;
}

// Note tokens the offer's disclosure shape carries. Closed vocabulary so a
// consumer can switch on a known set rather than pattern-matching prose
// (T-c8j-08 mitigation: a disclosed degrade is never silent).
function _noteForCallToolResult(result) {
  if (result === null) return 'brain_unreachable';
  if (result && typeof result === 'object' && typeof result.error === 'string') {
    return result.error; // 'tier_denied' | 'invalid_key' | 'invalid_problem_type'
  }
  if (!result || result.grounded === false || !Array.isArray(result.chain) || result.chain.length === 0) {
    return 'unknown_problem_type';
  }
  return null;
}

function _disclosureOffer(problemType, note) {
  return {
...
/**
 * chainOfferForReach(reach, opts) -> Promise<object|null>
 * The async offer layer, above decide(), never inside a sensor (fence 1).
 * Consumes the FIRST brain_framework_chain companion on `reach` (one Brain
 * call per pull bounds the cost of a path that runs on every suggest_next --
 * consuming every companion would multiply wire calls by reach breadth for no
 * additional first-offer value).
 *
 * - no companion -> null (nothing to offer; the field is omitted upstream).
 *   This is deliberately NOT the same value as "Brain is down" -- conflating
 *   the two is exactly the silent degrade the honesty rail forbids
 *   (T-c8j-08).
 * - companion present, Brain unavailable / unreachable / denied -> a
 *   `grounded:false` disclosure object with a closed note token, never a
 *   throw, never silence.
 * - companion present, Brain answers -> a `grounded:true` offer carrying the
 *   Brain's `chain` verbatim plus `chain_input` (ready for chain_run) and
 *   `unmapped_count` from the adapter.
 *
 ...
 */
async function chainOfferForReach(reach, opts) {
  const o = (opts && typeof opts === 'object') ? opts : {};
  let problemType;
  try {
    const companions = parseChainCompanions(reach);
    if (companions.length === 0) return null; // nothing to offer, no Brain touch
    problemType = companions[0];
  } catch (_e) {
    return null; // parse failure is treated as "nothing to offer", never a throw upstream
  }

  try {
    if (!brainClient || typeof brainClient.isAvailable !== 'function' || !brainClient.isAvailable()) {
      // Do NOT return null here: null means "no companion", and conflating
      // "Brain is down" with "nothing to offer" is the silent degrade the
      // honesty rail forbids (T-c8j-08). This file's existing
      // refusal-messaging.cjs require (recommendFrameworkChain's disclosure
      // branch above) stays untouched -- census.2 seam liveness is satisfied
      // by that pre-existing executable require, not a new one here.
      return _disclosureOffer(problemType, 'brain_unavailable');
    }

    const result = await brainClient.recommendChain(problemType);
    const note = _noteForCallToolResult(result);
    if (note !== null) {
      return _disclosureOffer(problemType, note);
    }

    // success: result.grounded === true and result.chain is a non-empty array
    // (guaranteed by the frozen v1 contract -- see brain-client.cjs's
    // recommendChain docblock -- so no try/catch scaffolding is built here
    // for a throw the contract says cannot happen).
    const adapted = await adaptChainToRunInput(result.chain, o);
    return {
      problem_type: problemType,
      chain: result.chain,
      grounded: true,
      unmapped_count: adapted.unmapped.length,
      chain_input: adapted.chain_input,
      note: (typeof result.note === 'string') ? result.note : null,
    };
  } catch (_e) {
    return _disclosureOffer(problemType, 'offer_error');
  }
}

/**
 * adaptChainToRunInput(brainChain, opts) -> Promise<{chain_input: string[], unmapped: string[], divergences: object[]}>
 * Maps a Brain `chain[]` (from recommend_chain) to a framework-NAME array
 * ready for chain_run's start contract (lib/mcp/tools/chain.cjs:479 --
 * z.array(z.string().min(1)).min(1)). NEVER computes posture: posture
 * authority stays LOCAL in recipe-maps.cjs via chain_run; the Brain
 * recommends, never triggers (fence 6).
 *
 * For each step (order preserved):
 *   - a non-string / missing step.framework is skipped and counted unmapped.
 *   - a framework the local framework_index knows (commandResolver.
 *     commandsForFramework returns non-empty) is passed through UNTOUCHED,
 *     zero normalize calls.
 *   - a framework the local index misses triggers EXACTLY ONE
 *     brainClient.normalizeFrameworkName call. The wrapper has had zero
 *     runtime callers until now, so its payload shape is not pinned by any
 *     shipped consumer -- the normalized string is read defensively across
 *     the plausible shapes (.canonical / .name / .normalized / a bare
 *     string). A hit against the local index after normalization uses the
 *     NORMALIZED name (the whole point of the retry); a miss keeps the
 *     ORIGINAL name and pushes it onto `unmapped` -- never invented, never
 *     dropped. composeWorkflow degrades an unknown framework to
 *     {command:null, optional:true} honestly and chain_run surfaces it as a
 *     manual step.
 *   - a divergence is recorded (never reconciled toward the Brain) when the
 *     local commands are non-empty AND step.commands is a non-empty array
 *     AND the two sets differ (compared sorted, de-duplicated).
 *
 * With opts.db, each divergence writes ONE brain_cmdmap_divergence
 * memory_event (best-effort, lazily required to avoid a load cycle, mirrors
 * brain-client.cjs's _logEventBestEffort reasoning). Without opts.db, zero
 * writes -- the divergences still return as data.
 *
 * @param {Array<{framework?: string, commands?: string[]}>} brainChain
 * @param {{db?: object}} [opts]
 * @returns {Promise<{chain_input: string[], unmapped: string[], divergences: Array}>}
 */
async function adaptChainToRunInput(brainChain, opts) {
  const o = (opts && typeof opts === 'object') ? opts : {};
  const chain = Array.isArray(brainChain) ? brainChain : [];
  const chainInput = [];
  const unmapped = [];
  const divergences = [];

  for (const step of chain) {
    const rawName = (step && typeof step.framework === 'string') ? step.framework : null;
    if (rawName === null) {
      continue; // not a step we can name -- skip, do not invent a placeholder
    }

    let localCommands = commandResolver.commandsForFramework(rawName);
    let outputName = rawName;

    if (localCommands.length === 0) {
      let normalizedName = null;
      try {
        const normResult = await brainClient.normalizeFrameworkName(rawName);
        // Defensive shape read: this wrapper has zero pinned runtime callers
        // before this quick task, so its exact payload key is unproven.
        if (typeof normResult === 'string') {
          normalizedName = normResult;
        } else if (normResult && typeof normResult === 'object') {
          if (typeof normResult.canonical === 'string') normalizedName = normResult.canonical;
          else if (typeof normResult.name === 'string') normalizedName = normResult.name;
          else if (typeof normResult.normalized === 'string') normalizedName = normResult.normalized;
        }
      } catch (_e) {
        normalizedName = null;
      }

      if (typeof normalizedName === 'string' && normalizedName.length > 0 && normalizedName !== rawName) {
        const retryCommands = commandResolver.commandsForFramework(normalizedName);
        if (retryCommands.length > 0) {
          outputName = normalizedName;
          localCommands = retryCommands;
        }
      }

      if (localCommands.length === 0) {
        // still unmapped after the one retry -- keep the ORIGINAL name,
        // never invent, never drop.
        unmapped.push(rawName);
      }
    }

    chainInput.push(outputName);

    // Divergence check: only meaningful when BOTH sides declare commands.
    const brainCommands = Array.isArray(step && step.commands)
      ? step.commands.filter((c) => typeof c === 'string')
      : [];
    if (localCommands.length > 0 && brainCommands.length > 0) {
      const localSorted = Array.from(new Set(localCommands)).sort();
      const brainSorted = Array.from(new Set(brainCommands)).sort();
      const differs = localSorted.length !== brainSorted.length
        || localSorted.some((c, i) => c !== brainSorted[i]);
      if (differs) {
        divergences.push({
          framework: outputName,
          brain_commands: brainSorted,
          local_commands: localSorted,
        });
      }
    }
  }

  if (o.db && divergences.length > 0) {
    for (const d of divergences) {
      try {
        // Lazy-require to avoid a load cycle (mirrors brain-client.cjs's
        // _logEventBestEffort reasoning at its own module boundary).
        require('../core/navigation.cjs').logMemoryEvent(o.db, 'brain_cmdmap_divergence', {
          framework: d.framework,
          brain_command_count: d.brain_commands.length,
          local_command_count: d.local_commands.length,
          brain_commands: d.brain_commands.join(','),
          local_commands: d.local_commands.join(','),
          source_path: 'system:brain-cmdmap',
          created_by: 'system',
          dedupe_key: 'cmdmap:' + d.framework,
        });
      } catch (_e) {
        // best-effort: a telemetry write must never break the adapter's
        // return value.
      }
    }
  }

  return { chain_input: chainInput, unmapped: unmapped, divergences: divergences };
}

module.exports = {
  recommendFrameworkChain: recommendFrameworkChain,
  recommendCanonicalTargets: recommendCanonicalTargets,
  // Constants exposed for invariant tests + the async chain path.
  DEFAULT_SEED: DEFAULT_SEED,
  MAX_CHAIN_LENGTH: MAX_CHAIN_LENGTH,
  FEEDS_INTO_CYPHER: FEEDS_INTO_CYPHER,
  DEFAULT_PRIMARY_LABEL: DEFAULT_PRIMARY_LABEL,
  // Quick 260819-c8j (WS-C2/C3): the live companion consumer + chain adapter.
  chainOfferForReach: chainOfferForReach,
  adaptChainToRunInput: adaptChainToRunInput,
  parseChainCompanions: parseChainCompanions,
  CHAIN_COMPANION_HANDLE: CHAIN_COMPANION_HANDLE,
};
