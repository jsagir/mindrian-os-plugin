...
function loadNeo4jDriver() {
  ...
  //
  // Phase 341-04 (D-02): mcp-server-brain/ is not in package.json's `files`
  // allowlist and never was, so this path was already unreachable on any
  // npm-installed copy of the plugin -- it only ever worked under the old
  // git-clone artifact, where the whole repo shipped. Confirmed operator-only
  // (Wave 1 never calls this): the live path is reached only through
  // makeBatch(...).execute()/rollback() from scripts/curation-132-03-dedup-
  // held-rename.cjs, scripts/curation-132-05-pseudonymize.cjs and
  // scripts/verify-phase-132.cjs -- all one-time migration/verification
  // scripts run by an operator from a dev checkout, never from a user-facing
  // command. A bare require() here would surface as a raw MODULE_NOT_FOUND,
  // the worst possible message for a path this file's own comments already
  // mark "Wave 1 NEVER calls this" -- so fail honestly instead.
  const driverPath = path.join(PLUGIN_ROOT, 'mcp-server-brain', 'node_modules', 'neo4j-driver');
  if (!fs.existsSync(driverPath)) {
    throw new Error(
      'curation-batch: neo4j-driver not found at ' + driverPath + '. ' +
      'This is an operator-only path (Neo4j curation-batch writes) that needs ' +
      'mcp-server-brain/node_modules/neo4j-driver, which is not part of the ' +
      'published npm plugin artifact. Run this from a dev checkout with ' +
      'mcp-server-brain/ dependencies installed, not from an installed plugin.'
    );
  }
