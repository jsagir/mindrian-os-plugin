...
function tightenSchemaWithCheckConstraints(db) {
  // Step 2: re-create-table-with-NOT-NULL plus CHECK constraints.
  // Canonical SQLite 12-step recipe. foreign_keys is flipped OFF by the caller
  // (runMigration) OUTSIDE the BEGIN/COMMIT and restored after -- it MUST be off
  // for this rebuild: DROP TABLE nodes below is the FK PARENT, and the edges->nodes
  // FK trips "FOREIGN KEY constraint failed" on any room whose edges table has rows
  // when foreign_keys is ON. (This comment earlier wrongly claimed FK behavior was
  // unchanged; that only held for fresh rooms with no edges.)
...
function runMigration(db) {
  ...
  // Canonical SQLite 12-step recipe: foreign_keys MUST be OFF for the nodes-table
  // rebuild (tightenSchemaWithCheckConstraints) because DROP TABLE nodes is the FK
  // PARENT and trips the edges->nodes FK when edges has rows + foreign_keys is ON.
  // The PRAGMA is a no-op INSIDE a transaction, so flip it OUTSIDE the BEGIN/COMMIT
  // and restore after. Fresh rooms (no edges) passed before; legacy rooms WITH edges
  // failed "FOREIGN KEY constraint failed" (the 2026-06-29 motj-ecosystem heal).
  ...
  try {
    ...
    // Safety belt (production-data heal): the rebuild preserves node ids, so the
    // edges->nodes references stay valid; verify ZERO FK violation before committing.
    const fkViolations = db.prepare('PRAGMA foreign_key_check').all();
    if (fkViolations.length > 0) {
      throw new Error('phase-109 migration introduced ' + fkViolations.length + ' FK violation(s); rolling back');
    }
    insertSentinel(db);
    db.exec('COMMIT');
  } catch (err) {
    try { db.exec('ROLLBACK'); } catch (_rbErr) { /* ignore */ }
    try { db.exec('PRAGMA foreign_keys=ON'); } catch (_e) { /* restore best-effort */ }
    throw err;
  }
