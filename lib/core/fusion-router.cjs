 ...
 * are live AND no horizontal move fired during the session, SUGGEST exactly ONE
 * OFFERED cross-frame hypothesis (hedged, offer-as-question) as a default, never
 * a requirement (Phase 210-D: the pick is suggested:true, forced:false -- the
 * taxonomy stays a lens, the quorum stops being a mechanical must-follow; nothing
 * downstream may treat the suggestion as mandatory or blocking). The suggested
 * hypothesis is OFFERED ONLY -- it is NEVER auto-committed to the graph as a
 * decision (T-205-07-E: no edge, no decision node is written here). When a
 * horizontal move already fired during the session, the quorum suggests zero
 * (the connection was made).
 *
 * ctx adds: horizontalFiredThisSession (boolean) -- did runFusion fire a horizontal
 *   move during the session. Absent / false -> the quorum may suggest one.
 *
 * @param {object} ctx
 * @returns {object}
 */
function sessionEndQuorum(ctx) {
  ...
  // D-Q1 boundary-pass: only suggest when 2+ frames are live AND nothing connected.
  if (frames.length < 2 || alreadyFired) {
    return {
      forced: false,
      suggested: false, // 210-D: no suggestion pressure in the zero case either.
  ...
  // Suggest EXACTLY ONE offered hypothesis over the strongest candidate pair. Prefer
  // a job-test relation if one exists; else offer the first two open frames as a
  ...
  return {
    // Phase 210-D downgrade: the quorum SUGGESTS its cross-frame hypothesis as a
    // default; it never forces the pick. `forced` stays present as an explicit
    // false (never deleted) so any consumer reading the key degrades safely.
    forced: false,
    suggested: true,
    reason: 'cross-frame-suggestion', // names the downgrade (210-D).
    count: 1, // EXACTLY one offered hypothesis (D-Q1, unchanged).
    hypothesis,
    frames_live: frames.length,
