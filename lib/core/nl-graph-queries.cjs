...
// Phase 355 D-07: a stored surprise_type/innovation_type may carry the
// retired convention (or, for REVERSE_SALIENT, come from a producer other
// than rs-engine). Neither is trusted as read; see the two call sites below.
...
const QUERY_TEMPLATES = {
  ...
  hsi_connections: {
    ...
    params: ['min_score'],
    build: (p) => ({
      // Phase 355 D-07: lsa_sim/semantic_sim are selected alongside the
      // stored surprise_type so executeNLQuery can re-derive the label fresh
      // (below) instead of trusting the stored string.
      sql: "SELECT e.source, e.target, json_extract(e.properties, '$.hsi_score') AS hsi_score, json_extract(e.properties, '$.surprise_type') AS surprise_type, json_extract(e.properties, '$.lsa_sim') AS lsa_sim, json_extract(e.properties, '$.semantic_sim') AS semantic_sim, json_extract(n1.properties, '$.title') AS source_title, json_extract(n2.properties, '$.title') AS target_title FROM edges e JOIN nodes n1 ON e.source = n1.id JOIN nodes n2 ON e.target = n2.id WHERE e.type = 'HSI_CONNECTION' AND CAST(json_extract(e.properties, '$.hsi_score') AS REAL) >= ? ORDER BY CAST(json_extract(e.properties, '$.hsi_score') AS REAL) DESC",
  ...
  reverse_salients: {
    ...
    params: [],
    build: () => ({
      // Phase 355 D-07: innovation_type is suppressed (NULL) unless the edge
      // carries properties.source === 'rs-engine' -- a legacy Section-level,
      // Python/hsi-to-graph.cjs-sourced edge never had an honest single-pair
      // measurement behind that label. No wire-id literal appears in this
      // SQL; the label itself is never re-minted here, only gated.
      sql: "SELECT e.source, e.target, json_extract(e.properties, '$.differential_score') AS diff_score, CASE WHEN json_extract(e.properties, '$.source') = 'rs-engine' THEN json_extract(e.properties, '$.innovation_type') ELSE NULL END AS innovation_type, json_extract(e.properties, '$.innovation_thesis') AS thesis FROM edges e WHERE e.type = 'REVERSE_SALIENT' ORDER BY CAST(json_extract(e.properties, '$.differential_score') AS REAL) DESC",
      bindings: []
    })
  },
...
function executeNLQuery(conn, intent, params = {}) {
  ...
  try {
    ...
    const rows = translated.bindings.length > 0
      ...
      : stmt.all();
    if (intent === 'hsi_connections') {
      // Phase 355 D-07: a stored surprise_type may carry the retired
      // convention (compute-hsi.py / a pre-355 hsi-to-graph.cjs write).
      // Never trust it as read -- re-derive fresh from the stored
      // (lsa_sim, semantic_sim) pair through the one module every time this
      // query runs.
      for (const row of rows) {
        row.surprise_type = directionConvention.classify(row.lsa_sim, row.semantic_sim);
      }
    }
