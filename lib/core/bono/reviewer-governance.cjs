...
const REVIEWER_GOVERNANCE = Object.freeze({
  ...
  ip: Object.freeze({
    discipline: 'freedom-to-operate skeptic: look for a reason to reject before crediting an IP claim',
    hard_gate: false,
    rules: Object.freeze([
      'at least one disconfirming evidence item is present',
      'the first cited evidence item is disconfirming',
    ...
    evidence_policy: 'disconfirming_first',
    discipline_source: 'ACH disconfirming-first (mirrors Black); Tnufa ip_protection_use_of_funds',
  }),
});
...
function enforceReviewerGovernance(category, argument) {
  ...
  } else if (key === 'process' || key === 'legal' || key === 'reporting') {
    if (evidence.length === 0) {
