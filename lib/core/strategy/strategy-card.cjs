...
/**
 * _buildOptions(goal, reach, ladderText) -> the frozen four-entry option
 * array, in STRATEGY_OPTION_IDS order, each with a non-empty label.
 */
function _buildOptions(goal, reach, ladderText) {
  const evidence = (reach && typeof reach === 'object' && reach.evidence && typeof reach.evidence === 'object')
    ? reach.evidence : {};
  const ratifiedJob = (typeof goal.jtbd === 'string' && goal.jtbd) ? goal.jtbd : 'unset';
...
async function buildStrategyCard(opts) {
  ...
  return {
    ...
    options: _buildOptions(goal, reach, ladderText),
