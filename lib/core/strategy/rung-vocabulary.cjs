 ...
 *
 * 2. THE WIRE VOCABULARY IS THEO'S OWN taxonomy_ladder ENUM (Phase 354-10,
 *    THEO-01 taxonomy casing, CTX-TAXO, P2), and it is reached ONLY through
 *    `toLadderRung`. Theo's real input schema is
 *    `z.enum([...PROBLEM_TYPE_IDS])` with `PROBLEM_TYPE_IDS = ['UnDefined',
 *    'IllDefined', 'WellDefined', 'Wicked']`
 *    (/home/jsagi/Theo/src/mcp/content/vocabulary.ts:53-58,
 *    taxonomy-ladder.ts:285-286, inspected read-only at commit 881e147).
 *    `lib/core/part8-egress-guard.cjs` declares `TAXONOMY_RUNGS` as that
 *    SAME Theo-cased closed set (`part8-egress-guard.cjs:340`), the exact
 *    set its own `taxonomy_ladder` known-shape proof accepts (`:452-458`).
 *    Before 354-10 this module and the guard both used a lowercase/hyphenated
 *    vocabulary that agreed with EACH OTHER but not with Theo's actual
 *    schema -- a caller that sent the guard the (then-current) lowercase
 *    string reached Theo, and Theo's own z.enum silently rejected it. That
 *    was the CTX-TAXO defect: two locally-consistent layers disagreeing with
 *    the one layer that actually validates the value. `toLadderRung` is the
 *    one place that conversion happens, so it happens correctly or not at
 *    all; after 354-10 the conversion is the identity function, because the
 *    PERSISTED and WIRE vocabularies are now the same Theo-cased strings.
 *
...
// bidirectionally (no leftovers in either direction). Phase 354-10
// (THEO-01 taxonomy casing): the WIRE vocabulary is now Theo's own
// PROBLEM_TYPE_IDS casing, so this mapping is the identity function --
// still named and still exported (rather than inlined as a pass-through at
// the call site) because it remains the ONE place a future rung, or a
// future provider whose casing differs again, gets projected.
// ---------------------------------------------------------------------------
 ...
 *
 * @param {*} theoRung
 * @returns {('Wicked'|'UnDefined'|'WellDefined'|'IllDefined'|null)}
 */
