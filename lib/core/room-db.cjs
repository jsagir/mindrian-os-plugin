...
// right now. Phase 276-10 (D-276-5, M8): a caller that reaches this error has
// already waited out the roughly five second busy window openRoomDb requests
// via the `timeout` option below (SQLite's own busy handler, not a JS-level
// retry) -- retrying immediately will not help. Surface it, or tell the user
// to close the other session. It is deliberately distinct from
// RoomDbBrokenError (nothing is damaged; waiting fixes it) and deliberately
// distinct from "no room db" / cold start (the room HAS history, it is just
// unreachable this instant). Treating a busy room as a cold start is exactly
// the data-loss path GRAPHDB-02 exists to close.
//
// Measured at Phase 276-10 execution time (re-run, not inherited): grepping
// RoomDbBusyError across lib/, scripts/, bin/, hooks/ and filtering for
// retry|backoff|sleep|attempt returns zero matches. The only two consumers
// (graph-refine-loop.cjs, graph-derivation.cjs) re-throw; neither retries.
// A general JS-level retry/backoff wrapper (bounded attempts, jitter, whether
// it applies to reads) was considered and NOT built here -- real design work
// that would compete with this phase's own scope for a comment correction.
// This is the same move as fixing a claim rather than the code when the code
// is already right: `timeout: 5000` on the DatabaseSync construction below
// already IS the retry, so the comment now says what the code actually
// offers instead of promising a JS-level retry that has never existed.
