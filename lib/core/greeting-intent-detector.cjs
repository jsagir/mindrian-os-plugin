...
function classify(text) {
  ...
  // IN-01 fix (267.2-REVIEW.md): word_count is computed from `truncated`, matching every
  // other feature below. Only char_count is the file's one deliberate untruncated
  // exception (documented in the header comment above) -- word_count was previously
  // computed from the untruncated `trimmed`, which was undocumented and inconsistent with
  // that stated contract (not a security issue in itself: split(/\s+/) has no backtracking
  // risk, but the inconsistency was real).
  const words = truncated.trim().split(/\s+/).filter(Boolean);
