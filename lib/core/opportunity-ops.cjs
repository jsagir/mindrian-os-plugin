...
function parseFrontmatter(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Top-level key: value
    // Phase 220-02 (additive broadening): keys may carry digits after the
    // first character (the D-03 sidecar contract pins `content_sha256`; the
    // old [a-z_]+ silently dropped any digit-bearing key). Strictly widens
    // what parses; every previously-parsed key parses identically.
    const topMatch = line.match(/^([a-z_][a-z0-9_]*):\s*(.*)$/);
    if (topMatch) {
    ...
    if (listMatch && currentKey) {
      ...
      const nestedMatch = itemVal.match(/^([a-z_][a-z0-9_]*):\s*(.+)$/);
      if (nestedMatch) {
    ...
    const nestedFieldMatch = line.match(/^\s{4,}([a-z_][a-z0-9_]*):\s*(.+)$/);
...
function bankOpportunity(roomDir, opportunity) {
  ...
  // Additive: pass an optional provenance scalar through to the frontmatter so a
  // banked opportunity can name the source graph edge it traces to (Phase 156
  // FW-09: HSI_CONNECTION / REVERSE_SALIENT / ROOT_CAUSES). Emitted only when the
  // caller supplies it, so every existing caller's output is byte-identical.
  if (opportunity.provenance) {
    fmLines.push(`provenance: "${String(opportunity.provenance).replace(/"/g, '\\"')}"`);
  }

  // Additive (Phase 219-04, D-20): an optional engine_mode scalar rides the
  // frontmatter so a bank entry honestly names how it was scored ('engine' vs
  // 'llm_manual_baseline' -- the OFFERED manual fallback). llm_manual_baseline
  // entries are EXCLUDED from calibration sets by this marker; it never flips
  // calibration/baseline status. Emitted only when the caller supplies it, so
  // every existing caller's output stays byte-identical (the provenance idiom).
  if (opportunity.engine_mode) {
    fmLines.push(`engine_mode: "${String(opportunity.engine_mode).replace(/"/g, '\\"')}"`);
  }

  // Additive (Phase 223-02, D-01 close-the-loop dual write): the six reader
  // fields that listOpportunities / computeOpportunityBankState surface
  // (funder / program / deadline / relevance_score -- status + created already
  // emit above) PLUS the shared artifact_id that cross-references this bank .md
  // to its room.db opportunity node. Emitted ONLY when the caller supplies each
  // field, so every existing bankOpportunity caller's output stays byte-identical
  // (the provenance / engine_mode additive idiom). The reader and the six
  // required fields are UNCHANGED; this only ADDS optional frontmatter lines.
  if (opportunity.funder) {
    fmLines.push(`funder: "${String(opportunity.funder).replace(/"/g, '\\"')}"`);
  }
  if (opportunity.program) {
    fmLines.push(`program: "${String(opportunity.program).replace(/"/g, '\\"')}"`);
  }
  if (opportunity.deadline) {
    fmLines.push(`deadline: "${String(opportunity.deadline).replace(/"/g, '\\"')}"`);
  }
  if (opportunity.relevance_score != null) {
    fmLines.push(`relevance_score: ${opportunity.relevance_score}`);
  }
  if (opportunity.artifact_id) {
    fmLines.push(`artifact_id: "${String(opportunity.artifact_id).replace(/"/g, '\\"')}"`);
  }
...
module.exports = {
  listOpportunities,
  listFunding,
  // Additive export (Phase 163-03): the shipped generic frontmatter parser the
  // Tier-0 cold-start fallback in get-domains-for-trends.cjs reuses, so that
  // reader does NOT hand-roll a parser (Canon Part 7 reuse-before-build). The
  // function is a pure self-contained string parser with zero closure deps.
  parseFrontmatter,
