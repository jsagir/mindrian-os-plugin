...
// ---------------------------------------------------------------------------
// purgeLegacySelfReferentialEntities(db) -> { ok, purgedNodes, purgedEdges }
//
...
// 65 real artifacts on the reporting room). The minting fix is additive, so those
// rows persist on every room scanned before it. This primitive removes them.
//
// The signature is EXACT and machine-authored, and it is the only thing deleted:
//   id LIKE 'entity:%' AND source_path LIKE 'entity:%'
//   AND type IN ENTITY_NODE_TYPES
//   AND review_status is not 'confirmed'
// A row with a real room-relative source_path is never touched. A row a human
// confirmed is never touched (Canon Part 9: only a human closes a confirmed
// node). Edges touching a purged row go with it (they are its provenance and
// relations; orphaned they would be dangling). Per-id prepared statements inside
// one transaction: fixed SQL text, no assembled IN lists, all-or-nothing. This
// module owns the primitive because it minted the rows; callers reach it only
...
function purgeLegacySelfReferentialEntities(db) {
  if (!db || typeof db.prepare !== 'function') return { ok: false, reason: 'no_db', purgedNodes: 0, purgedEdges: 0 };
  let ids;
  try {
    ids = db.prepare(LEGACY_SELECT_SQL).all().map(function (r) { return r.id; });
  } catch (e) {
    return { ok: false, reason: 'select_failed', detail: String(e && e.message).slice(0, 80), purgedNodes: 0, purgedEdges: 0 };
  }
  if (ids.length === 0) return { ok: true, purgedNodes: 0, purgedEdges: 0 };
  ...
  try {
    ...
    return { ok: false, reason: 'purge_failed', detail: String(e && e.message).slice(0, 80), purgedNodes: 0, purgedEdges: 0 };
  }
  return { ok: true, purgedNodes: purgedNodes, purgedEdges: purgedEdges };
}
