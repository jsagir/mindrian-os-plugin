...
const EVENT_TYPES = Object.freeze(new Set([
  ...
  //                            'set'|'override'|'clear') AND, per the
  //                            graph-edge-pending-undrained-dead-letter-queue
  //                            RCA (2026-07-28), the across-session lifecycle
  //                            values 'promote'|'park'|'complete' emitted by
  //                            lib/hmi/across-session-memory.cjs. kind is not
  //                            enum-enforced in code; this comment is the
  //                            source of truth for the full vocabulary.
  ...
  //   created_by=system review_status=confirmed is canon-legal.
  'derivation_skipped',
  // Quick 260917-o1e (R1): the SUCCESS-side sibling of derivation_skipped,
  //   emitted by the SAME two writers (scripts/gsd-graph-derive-drain.cjs and
  //   lib/core/graph-backfill.cjs) when a derive pass finishes with the encoder
  //   available. Closes the bug this quick task exists to fix: the one-sided
  //   memory that let a stale derivation_skipped(encoder_unavailable) event lock
  //   a room's doctor verdict forever, because the only counter-evidence
  //   graph-derive-health-module.cjs accepted was a heal-attempt marker file
  //   that fix() refused to write for exactly those rooms. Additive; mirrors the
  //   Phase 224-02 derivation_skipped 1-string idiom. logEvent already rejects
  //   event_type values outside EVENT_TYPES, so this is accepted ONLY because it
  //   is now IN the Set. The floor-not-size contract (tests assert FLOOR + named
  //   membership, never an exact .size) makes the addition safe.
  // Canon Part 8: payload carries SCALAR fields only -- pairs_scored (int),
  //   edges_written (int), trigger basename, dedupe_key handle; NEVER artifact
  //   body text, never crosses to Brain.
  // Canon Part 9: written ONLY via navigation.logMemoryEvent. System-bookkeeping
  //   node per the Part 9 v1.5 audit-node carve-out (records what the system DID
  //   -- a derivation pass completed with the encoder available -- not a venture
  //   truth-claim) so created_by=system review_status=confirmed is canon-legal.
  'derivation_completed',
  // Phase 249-01 (ENRICH-01): a live reach that hits a 0-2/4 framework (or an
  //   ungrounded discover_structure) was captured into the enrichment queue
  //   (lib/core/enrichment-queue.cjs). Emitted from the brain-client.cjs
  //   capture seams (orchestrationReadiness/discoverStructure wrappers,
  //   fetchFrameworkChainSlice piggyback probes) via _logEventBestEffort, the
  //   same best-effort mechanism as the brain_packet_rejected precedent above.
  //   Additive extension only; mirrors the Phase 224-02 derivation_skipped
  //   1-string idiom. logEvent already rejects event_type values outside
  //   EVENT_TYPES, so this is accepted ONLY because it is now IN the Set. The
  //   floor-not-size contract (tests assert FLOOR + named membership, never an
  //   exact .size) makes the addition safe.
  // Canon Part 8: payload carries SCALAR fields only -- framework handle,
  //   integer readiness_score (or null), source_path; NEVER artifact body
  //   text, NEVER crosses to Brain (this event is LOCAL room.db bookkeeping,
  //   the enrichment queue file itself is the thing that later crosses via
  //   ENRICH-02's admin-gated ingest, not this event).
  // Canon Part 9: written ONLY via navigation.logMemoryEvent (_logEventBestEffort
  //   in brain-client.cjs is the sole emission site). System-bookkeeping node
  //   per the Part 9 v1.5 audit-node carve-out (records what the system DID --
  //   an enrichment miss was queued -- not a venture truth-claim) so
  //   created_by=system review_status=confirmed is canon-legal.
  'enrichment_queue_captured',
  // Quick 260819-c8j (WS-C3): the local Brain/local command-map disagreement
  //   record. Emitted by lib/brain/chain-recommender.cjs's adaptChainToRunInput
  //   when a Brain-supplied chain[].commands array disagrees (compared sorted,
  //   de-duplicated) with commandResolver.commandsForFramework's LOCAL
  //   resolution for the same framework name. Additive extension only; mirrors
  //   the Phase 249-01 enrichment_queue_captured 1-string idiom verbatim.
  //   logEvent already rejects event_type values outside EVENT_TYPES, so this
  //   is accepted ONLY because it is now IN the Set. The floor-not-size
  //   contract (tests assert FLOOR + named membership, never an exact .size)
  //   makes the addition safe.
  //   brain_cmdmap_divergence -> emitted ONCE per divergent framework, only
  //     when the caller supplies opts.db (suggest_next deliberately does not,
  //     fence 5: "no write of any kind"). A dedupe_key of 'cmdmap:' + framework
  //     rides the shipped 60s dedupe TTL so a polled surface cannot spam the
  //     graph. The disagreement is recorded as DATA -- never reconciled in the
  //     Brain's favor; posture and command authority stay LOCAL (fence 6).
  // Canon Part 8: payload carries SCALAR fields only -- framework handle,
  //   brain_command_count / local_command_count integers, brain_commands /
  //   local_commands as comma-joined command-slug strings; NEVER prose, NEVER
  //   crosses to Brain (this event is LOCAL room.db bookkeeping).
  // Canon Part 9: written ONLY via navigation.logMemoryEvent, from
  //   adaptChainToRunInput's write leg (guarded on opts.db, best-effort,
  //   never throws). System-bookkeeping node per the Part 9 v1.5 audit-node
  //   carve-out (records what the system OBSERVED -- a map disagreement --
  //   not a venture truth-claim) so created_by=system review_status=confirmed
  //   is canon-legal.
  'brain_cmdmap_divergence',
  // Phase 345-03 extension (STRAT-04/STRAT-05; the strategy node's cadence +
  // cool-down telemetry mirror). EXACTLY 2 net-new types; mirrors the Phase
  // 249-01 / 260819-c8j 1-string additive idiom verbatim. logEvent already
  // rejects event_type values outside EVENT_TYPES, so these are accepted ONLY
  // because they are now IN the Set. Tests assert a FLOOR + named membership +
  // a delta of EXACTLY 2, never a brittle absolute size, so a future phase
  // adding a third type cannot regress this baseline.
  //   strategy_proposed  -> one row per strategy gate card actually rendered.
  //     Payload is enum and scalar only: {goal_version, rung, jtbd,
  //     reaches_since, claims_since, anchor_node_id, source_path, created_by,
  //     dedupe_key}. anchor_node_id is a LOCAL room-graph id and never crosses
  //     the wire; it rides here so the CLI and MCP surfaces can be correlated
  //     later (345-ICM-CONSULT AP-G6).
  //   strategy_throttled -> one row when the cool-down suppresses a proposal.
  //     Payload mirrors canary.cjs::emitThrottleEvent: {reason, rate,
  //     sample_size, threshold, throttled_at, source_path, created_by} where
  //     reason is one of the three closed enums min_interval, dismissal_rate,
  //     reject_suppression.
  // Canon Part 8: both payloads carry scalar enums, counts, and a LOCAL node-id
  //   handle only; no prose, no venture content, never crosses to Brain.
  // Canon Part 9: written ONLY via navigation.logMemoryEvent
  //   (lib/core/strategy/goal-cadence.cjs readLastProposalEpochMs reads
  //   strategy_proposed back; lib/core/strategy/strategy-throttle.cjs
  //   emitThrottleEvent is the sole writer of strategy_throttled).
  'strategy_proposed',
  'strategy_throttled',
  // Phase 346-05 extension (ARB-07/ARB-08; the arbitration node's own audit row).
  // EXACTLY ONE net-new type; mirrors the Phase 345-03 2-string / Phase 249-01
  // 1-string additive idiom verbatim. logEvent already rejects event_type values
  // outside EVENT_TYPES, so this is accepted ONLY because it is now IN the Set.
  //   arbitration_decided -> one row per turn's composed arbitration result
  //     (lib/core/arbitration.cjs::resolveArbitration, 346-04). Payload carries
  //     the three axis values with their rationale tokens, the ranked order, the
  //     six-input census (read/missing), the floors applied, and whether the
  //     turn's posture flipped from the prior turn in the same session (flip,
  //     flipped_axes, flip_reason). Emitted by
  //     lib/core/navigation/arbitration-log.cjs::logArbitrationDecision, riding
  //     a dedupe_key of 'arb:<session_id>:<turn_id>' on the shipped 60s
  //     DEDUPE_TTL_MS so a hook that fires twice in one turn writes one row, not
  //     two. This is a memory_event, not a JSONL file (CLAUDE.md:153, the single
  //     SQL navigation chokepoint is the local mind, Canon Part 9), so a future
  //     promotion review can query it the same way every other typed signal in
  //     this Set is queried.
  // Canon Part 8: the payload carries closed-enum tokens and integers only,
  //   never prose, never user text, and never crosses to Brain. This is LOCAL
  //   room.db bookkeeping.
  // Canon Part 9: written ONLY through the navigation chokepoint, from
  //   logArbitrationDecision's write leg (guarded on a db handle, best-effort,
  //   never throws). System-bookkeeping node per the Part 9 v1.5 audit-node
  //   carve-out (it records what the system DECIDED, not a venture truth-claim),
  //   same carve-out class as brain_cmdmap_divergence above, so
  //   created_by=system review_status=confirmed is canon-legal.
  'arbitration_decided',
  // Phase 355-20 extension (HIPS-06, D-53 C4, AI-SPEC Section 7). EXACTLY ONE
  // net-new type; mirrors the additive idiom verbatim -- logEvent already
  // rejects event_type values outside EVENT_TYPES, so this is accepted ONLY
  // because it is now IN the Set.
  //   cross_connection_stamped -> one row per scripts/eureka-portfolio-
  //     report.cjs bankStatements() run that stamped at least one candidate
  //     (--stamp opt-in). Payload carries { producer, finding_count,
  //     tier_counts, reason_counts, backend, judge, theo_ms_bucket } --
  //     closed enum tokens, counts and one bucketed latency string, never
  //     prose or a room.db handle; honors MINDRIAN_DISABLE_MEMORY_EVENT.
  // Canon Part 8: enum/scalar counts only, never crosses to Brain.
  // Canon Part 9: written ONLY through this chokepoint (navigation.
  //   logMemoryEvent); system-bookkeeping node (records that a banking run
  //   stamped findings, not a venture truth-claim), the same carve-out class
  //   as arbitration_decided above.
  'cross_connection_stamped',
  // Phase 355-22 extension (HIPS-06, D-39, AI-SPEC Section 7). EXACTLY ONE
  // net-new type; mirrors the additive idiom verbatim -- logEvent already
  // rejects event_type values outside EVENT_TYPES, so this is accepted ONLY
  // because it is now IN the Set.
  //   cross_connection_gate_outcome -> one row per gate_answer approve/reject
  //     outcome on a stamped opportunity subject (lib/mcp/tools/gate.cjs
  //     _maybeLogGateOutcome). Payload carries { finding_id, tier, direction,
  //     response } -- an opaque local node id, three closed enum tokens,
  //     never prose or a room.db handle; honors MINDRIAN_DISABLE_MEMORY_EVENT.
  // Canon Part 8: opaque id + enum tokens only, never crosses to Brain.
  // Canon Part 9: written ONLY through this chokepoint; system-bookkeeping
  //   node (records the gate's own acceptance-rate telemetry, not a venture
  //   truth-claim), the same carve-out class as cross_connection_stamped above.
  'cross_connection_gate_outcome',
]));
