...
// Phase 348 (SUPER-08 / D-04). The ContradictionEvent label is read LIVE from
// the reified writer's own table, never re-typed as a literal, so a rename
// in reified-claim.cjs cannot silently reopen the guard's hole.
const { REIFIED_EVENT_TYPES } = require('./reified-claim.cjs');
...
// Phase 348 (SUPER-04). findContradictions is reshaped to the deliverable-2
// contract: an options bag (the additive-floor idiom, mirrored from
// edges.cjs:812-816 and this file's own findUnsupportedClaims /
// findStaleClaims siblings) and a review_status filter.
//
// includeSuperseded defaults to false, which is a DEFAULT-BEHAVIOR change
// across the five callers enumerated in docs/SUPERSESSION-CONTRACT.md (348-04
// Task 3 declares each one). The measured fleet state at the time of this
// change was zero CONTRADICTS edges across 47 rooms (348-RESEARCH.md Risk
// (b)), so no live read changed on the day it shipped.
//
// The legacy fall-through is vacuous satisfaction, not a silent skip: a
// column that does not exist cannot hold 'superseded', so excluding
// superseded rows is trivially true on a schema with no review_status column
// at all, and the pre-348 query is the correct behavior there. Mirrors the
// PRAGMA table_info(nodes) probe idiom (node-insert.cjs::isMigratedSchema).

// Phase 348 (SUPER-08 / D-04). Superseding an event node is meaningless: the
// reified ContradictionEvent --CONTRADICTS--> rivalClaim shape
// (reified-claim.cjs) is explicitly OUT of scope for this phase (D-03), so a
// reified endpoint is REFUSED rather than projected as "claim A". The label
// is read live above (REIFIED_EVENT_TYPES), never re-typed. The skip reason
// is the single closed token defined below. Honest-refusal precedent:
// lib/core/temporal/supersession.cjs:128-133 (surface the failure with a
// named reason rather than swallowing it).
...
function reviewStatusColumnExists(db) {
  try {
...
// getNeighborhood's own recursive CTE (neighborhood.cjs) selects
// last_seen_at / confidence / source_section / review_status / created_by --
// the Phase 109/160 provenance columns a bare legacy 3-column nodes table
// (id, type, properties) does not carry. That is a PRE-EXISTING limitation
// of the shared neighborhood walk, not something this phase's reshape
// introduces, but SUPER-04's own legacy-fixture requirement ("neither
// throws") means findContradictions cannot let it propagate. Fall back to a
// schema-agnostic BFS over the edges table alone (source/target/type are the
// one shape every schema variant shares) rather than widening the shared
// neighborhood module's column list, which would be a change well outside
// this task's own file scope.
function legacyReachableIds(db, focusNodeId, maxDepth) {
  const reachable = new Set([focusNodeId]);
  let frontier = [focusNodeId];
  for (let depth = 0; depth < maxDepth && frontier.length > 0; depth++) {
    const placeholders = frontier.map(() => '?').join(',');
    const rows = db.prepare(
      "SELECT source AS s, target AS t FROM edges WHERE source IN (" + placeholders + ") OR target IN (" + placeholders + ")"
    ).all(...frontier, ...frontier);
    const next = [];
    for (const row of rows) {
      if (!reachable.has(row.s)) { reachable.add(row.s); next.push(row.s); }
      if (!reachable.has(row.t)) { reachable.add(row.t); next.push(row.t); }
    }
    frontier = next;
  }
  return reachable;
...
function findContradictions(db, focusNodeId, opts) {
  const options = (opts && typeof opts === 'object' && !Array.isArray(opts)) ? opts : {};
  const includeSuperseded = options.includeSuperseded === true;
  // WD-348-4: an OPTIONAL caller-supplied sink for the D-04 skip reason.
  // Absent bag, absent sink, byte-identical return -- the additive floor
  // forbids changing the return shape, and an honest refusal needs its
  // reason retrievable rather than implied. A non-array value is ignored.
  const skipSink = Array.isArray(options.skipped) ? options.skipped : null;

  const exists = db.prepare('SELECT 1 AS x FROM nodes WHERE id = ?').get(focusNodeId);
  if (!exists) return [];
  // Walk the neighborhood; collect node ids reachable from focus. Falls back
  // to legacyReachableIds when the shared neighborhood walk cannot run
  // against this schema (see legacyReachableIds's own comment).
  let reachable;
  try {
    const neighbors = getNeighborhood(db, focusNodeId, { maxDepth: 2, topK: 200 });
    reachable = new Set([focusNodeId]);
    for (const n of neighbors) reachable.add(n.id);
  } catch (_e) {
    reachable = legacyReachableIds(db, focusNodeId, 2);
  }
  // Find edges of type CONTRADICTS where both endpoints are in the reachable set.
  const ids = Array.from(reachable);
  const placeholders = ids.map(() => '?').join(',');

  const hasReviewStatus = reviewStatusColumnExists(db);
  const statusSelect = hasReviewStatus
    ? ", n_a.review_status AS s_status, n_b.review_status AS t_status "
    : " ";
  // SQLite's null-safe IS NOT: a NULL status (legal on a partially migrated
  // row) is KEPT, not silently dropped. A plain <> would drop every
  // NULL-status row, a second, unannounced behavior change. The literal
  // 'superseded' is a closed-enum constant, not caller input (T-348-18: no
  // caller value reaches the SQL as anything but a controlled branch).
  const supersededFilter = (hasReviewStatus && !includeSuperseded)
    ? " AND n_a.review_status IS NOT 'superseded' AND n_b.review_status IS NOT 'superseded'"
    : "";
  ...
  const result = [];
  for (const row of rows) {
    // D-04 guard: fires on the endpoint's node TYPE, not an id prefix, so a
    // differently-shaped id of the real event type is still caught and a
    // claim-typed node whose id merely resembles an event id is never
    // wrongly skipped. Do NOT walk back to the real claim participant via
    // the event's other role edge here: D-03 puts the reified shape out of
    // scope for this phase, and resolving it that way would implement that
    // out-of-scope shape under a different name. The Phase 350 card carries
    // the reified-shape follow-on.
    if (row.s_type === CONTRADICTION_EVENT_LABEL || row.t_type === CONTRADICTION_EVENT_LABEL) {
      if (skipSink) {
        skipSink.push({ source: row.s, target: row.t, reason: REIFIED_SHAPE_SKIP_REASON });
      }
      continue;
    }
    result.push({
