 ...
 *                                  -> { ok, ... } | { ok:false, reason }
 *   MEMORY_KINDS                   -> frozen array of the 6 memory kinds
...
// The 6 user-memory kinds (ROOM / STATE / MINTO / BRAIN / FEYNMAN / USER).
...
// writeMemoryArtifactNode(db, {section, kind, path, hash}) -- UPSERT a typed
// memory_artifact node (the user-memory twin of writePlanningArtifactNode).
// Validates kind against MEMORY_KIND_SET; JSON.stringify the properties
// {section, kind, path, hash}; runs the INSERT ... ON CONFLICT(id) DO UPDATE
// upsert. created_by='system', confidence 1.0, review_status='confirmed' (the
// Canon Part 9 v1.5 audit-node carve-out -- system-bookkeeping, NOT a truth
// claim). source_path is a LOCAL handle 'memory:'+section+':'+kind, never user
// prose. The `hash` field is a sha256 content-hash handle, never prose.
...
function writeMemoryArtifactNode(db, params) {
  ...
  const { section, kind, path: artifactPath, hash } = params;
  ...
  try {
    db.prepare(
      ...
      "VALUES (?, 'memory_artifact', ?, ?, 'system', 1.0, 'confirmed', ?, ?) " +
...
// writeGoverningThoughtNode(db, {section, hash}) -- UPSERT a typed
...
function writeGoverningThoughtNode(db, params) {
  ...
  try {
    db.prepare(
      'INSERT INTO nodes ' +
      '(id, type, properties, source_path, created_by, confidence, review_status, created_at, last_seen_at) ' +
      "VALUES (?, 'governing_thought', ?, ?, 'system', 1.0, 'confirmed', ?, ?) " +
      'ON CONFLICT(id) DO UPDATE SET ' +
      'properties = excluded.properties, last_seen_at = excluded.last_seen_at'
    ).run(nodeId, propsJson, sourcePath, nowMs, nowMs);
  } catch (e) {
...
function writeNavigatorPersonaNode(db, params) {
  ...
  try {
    db.prepare(
      'INSERT INTO nodes ' +
      '(id, type, properties, source_path, created_by, confidence, review_status, created_at, last_seen_at) ' +
      "VALUES (?, 'navigator_persona', ?, ?, 'system', 1.0, 'confirmed', ?, ?) " +
      'ON CONFLICT(id) DO UPDATE SET ' +
      'properties = excluded.properties, last_seen_at = excluded.last_seen_at'
    ).run(nodeId, propsJson, sourcePath, nowMs, nowMs);
  } catch (e) {
...
// properties carry only {section, summary_hash, source} -- a section enum, a
// sha256 summary handle, and a ledger-origin enum, never decision prose.
// Defensive; never throws on caller input.
function writeDecisionNode(db, params) {
  if (!isPlainObject(params)) {
    return { ok: false, reason: 'invalid_params' };
  }
  const { decisionId, section, summaryHash, source } = params;
