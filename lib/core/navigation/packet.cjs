...
async function buildBrainPacket(db, job, focusNodeId, opts) {
  ...
  const nearestAssumptions = neighborhood
    ...
    .slice(0, 5)
    .map((n) => safeNodeProjection(db, n, privacyMode));
  // Phase 348 (SUPER-05): DEFAULT (superseded excluded). A closed claim is
  // exactly what should not be projected into the Canon Part 8 Brain
  // Context Packet; excluding it is strictly safer than the status quo.
