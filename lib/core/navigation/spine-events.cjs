 ...
 *   Phase 276-10 (F-selector path, ROADMAP Phase 273 C5): a busy room.db does
 *   NOT fall through to the cache -- both getters retry the SELECT through the
 *   read-only door on a busy write-door open (WAL readers never block writers),
 *   so a room with real history is never misreported as a cold start merely
 *   because a foreign connection holds the write lock. Only a genuinely broken
 *   room or a genuinely absent room falls through to the cache.
 *
...
// Phase 276-10 (TOOLHON-10/TOOLHON-11, F-selector path / RESEARCH A11): both
// getters open room.db to run a pure SELECT (findRecentChanges), then fall
// through to the JSON cache on ANY open failure -- pre-276-10 this collapsed
// a busy room's REAL history to the same value a genuine cold start would
// show (ROADMAP Phase 273 C5: "a momentarily-busy room reads as an empty
// cold-start room to the F-selector"). A11 measured (276-02-SUMMARY.md) that
// these two do NOT share _emit's { ok:false, reason:'no_room_db' } shape;
// their bare catch is a DIFFERENT, worse swallow that discards the
// distinction entirely. This shared helper narrows that swallow using the
// SAME _classifyOpenError Task 1 extracted, retrying ONLY the busy case
// through the existing read-only door (openRoomDbReadOnlyForCaller below):
// WAL readers never block writers (room-db.cjs:249-251's own reasoning,
// already this file's precedent for that door), so a write-door open that
// fails only because a migration needs the write lock does not mean the
// read-only SELECT these getters run needs it too. A broken room's bytes
// are unreadable through either door, so room_db_broken (and any
// unclassifiable open failure) still falls through to the cache fallback,
// unchanged from the pre-276-10 behavior for that case -- this is additive,
// not a rewrite of every failure path.
function _openForRead(roomDir) {
  try {
    return roomDbMod.openRoomDb(roomDir);
  } catch (err) {
    const classified = _classifyOpenError(err);
    if (classified.reason === 'room_db_busy') {
      return openRoomDbReadOnlyForCaller(roomDir);
    }
    return null;
  }
}
