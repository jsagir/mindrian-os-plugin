...
function countClaimCounterMetric(db, _opts) {
  ...
  if (columnAware) {
    ...
    // The schema exists but cannot answer a created_at window question:
    // strict null, never 0, because 0 would read as "measured and clean."
    claimsFiledPastCitationLag = null;
    claimsNoIncomingEdgePastCitationLag = null;
  } else {
    // 'unreadable': there is structurally nothing to count (no nodes table
    // at all, or an unparseable one), the same condition as an empty graph.
    // Zero is a direct, honest answer here, not an assertion about a
    // column that exists but was never read.
