...
function fileEvidenceWithReadback(db, params) {
  ...
  try {
    ...
    // FILEVAL-03 REMIND half (Phase 142): the round-trip values the read-back
    // already validated are the recall the navigator would be shown ("here is
    // what just landed"). The shipped success return discarded them; surface
    // them so the REMIND half is actively surfaceable, not computed-then-thrown.
    // This is LOCAL recall (Part 8): the values came back OUT of the local row,
    // they never egress. No new query surface, no Brain-adjacent path.
    const readback = buildReadback(row, landed);
    return { ok: true, node_id: nodeId, readback: readback };
  } catch (e) {
...
// buildReadback(row, landed) -- assemble the human-readable round-trip recall
// from the row that was just read back. Only values that came OUT of the local
// row are included (Part 8: local recall, never egress). Returns a flat object
// of non-empty, human-readable fields the navigator can be reminded with.
function buildReadback(row, landed) {
  const rb = {
    node_id: row && row.id ? String(row.id) : '',
    type: row && row.type ? String(row.type) : '',
    review_status: row && row.review_status ? String(row.review_status) : '',
  };
  // The 4 LOCKED provenance fields, as they round-tripped through the row.
  for (const field of LOCKED_PROVENANCE_FIELDS) {
    if (landed && typeof landed[field] === 'string' && landed[field].length > 0) {
      rb[field] = landed[field];
    }
  }
  // artifact_path (D-10) when present.
  if (landed && typeof landed.artifact_path === 'string' && landed.artifact_path.length > 0) {
    rb.artifact_path = landed.artifact_path;
  }
  // topic / summary are the human-facing description of what was filed.
  if (landed && typeof landed.topic === 'string' && landed.topic.length > 0) {
    rb.topic = landed.topic;
  }
  if (landed && typeof landed.summary === 'string' && landed.summary.length > 0) {
    rb.summary = landed.summary;
  }
  return rb;
}

// surfaceFileEvidenceResult(result) -- the FILEVAL-03 surfacing layer. Larry
// must SHOW the read-back result at the conversation surface, never swallow it.
// This is the honesty rule made surfaceable: it turns BOTH a failed filing
// (ok:false) and a landed filing (ok:true) into a Larry-facing payload with a
// human-readable message. Pure function, no I/O, never throws, never egresses.
//
// Returns: { surfaced:true, ok, reason?, node_id?, readback?, message }
function surfaceFileEvidenceResult(result) {
  const r = (result && typeof result === 'object') ? result : {};
  const isOk = r.ok === true;
  const nodeId = typeof r.node_id === 'string' ? r.node_id : '';

  if (!isOk) {
    // Honesty signal: a filing that did not land is SURFACED, not swallowed.
    const reason = typeof r.reason === 'string' && r.reason.length > 0 ? r.reason : 'unknown';
    let message;
    if (reason === 'filing_did_not_land') {
      message = 'Heads up: I tried to file that evidence, but the read-back did not confirm it landed. ' +
        'Nothing was silently saved. We should retry before relying on it.';
    } else {
      message = 'Heads up: the filing did not complete (reason: ' + reason + '). ' +
  ...
  // REMIND path: a landed filing -- recall what was just saved for the navigator.
  const rb = (r.readback && typeof r.readback === 'object') ? r.readback : {};
  const what = rb.topic || rb.summary || rb.type || 'the evidence';
  const tier = rb.evidence_tier ? (' (' + rb.evidence_tier + '-tier)') : '';
  const src = rb.source ? (' from ' + rb.source) : '';
  const message = 'Filed and confirmed: ' + what + tier + src + '. ' +
    'It landed as ' + (rb.review_status || 'proposed') + ' and is in the room graph now.';
  return {
    surfaced: true,
    ok: true,
    node_id: nodeId || undefined,
...
module.exports = { fileEvidenceWithReadback, surfaceFileEvidenceResult };
