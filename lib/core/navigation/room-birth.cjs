...
// requested with opts.bornWired === true. It layers five guarantees on top of
...
//   (4) SEED-001's SIX side-effects wire atomically or the whole birth unwinds
//       via a compensating reverse-order rollback (clone of room-discard-cascade)
//       -- no half-born orphan (Part 11 R1/R2 fail-closed). Side effect six
//       (Phase 353 Plan 01, R-353-E, helper _rebuildRoomMaps) rebuilds the
//       child's room map then the parent's, LAST, inside the same FINALIZE
//       try; its rollback clause is _bornWiredRollback's own sixth,
//       best-effort step: after fs.rmSync removes the child, the PARENT map
//       is rebuilt again (a rebuild-from-disk is idempotent, so "rebuild it
//       again" is the correct compensating action, not a restored backup).
//   (5) Phase 353 Plan 01 (D-353-5): one F.8 job-declaration card, fired
//       beside the approval card (runJobGate, the runBirthGate twin).
//       hitl_shape: F.8, hitl_why: the card offers the closed job vocabulary
//       (section-registry.cjs::JOB_VOCABULARY) as a paged option set with
//       'custom' as the declared escape hatch. The declaration lives at
//       THIS module seam, not on a command's own frontmatter, because no
//       in-repo surface supplies options.jobGate at HEAD (measured:
//       options.birthGate has zero suppliers outside this file, and
//       lib/mcp/tools/room.cjs registers room_list/room_state_bound/
//       room_search with no birth tool) -- declaring F.8 on a command that
//       does not yet exist would declare a fork Canon Part 11 forbids. The
//       first command or MCP tool that supplies jobGate declares hitl_shape:
//       F.8 in its own frontmatter at that moment, not before.
//
...
// runJobGate(options, ctx) -- Phase 353 Plan 01 Task 6 (D-353-5): resolve the
// ONE F.8 job-declaration card a sub-room birth fires. options.jobGate
// travels the SAME contract shape runBirthGate's options.birthGate already
// does: a function (ctx) -> answer, or a plain decision object (the Tri-Polar
// form: CLI renders the card and hands the function; Desktop/Cowork pass a
// plain object since there is no MCP sub-room birth tool at HEAD to hook).
//
// Deliberate ASYMMETRY with runBirthGate, stated here because it is the one
// place the two twins differ: an unapproved BIRTH is fail-CLOSED (a folder
// that should not exist must never be created), while an UNDECLARED JOB is
// fail-OPEN ({declared:false, job_id:'', option_key:'custom'}) because
// "undeclared" is a legal, named state with a defined resolution (D-353-5's
// parent fallback plus the doctor flag, both Plan 02) -- never a reason to
// block the birth itself.
//
// Validation happens HERE, before any byte is written (T-353-26): an absent
// gate, a throwing gate, a non-object answer, the literal 'custom', and any
// answer whose job_id fails isDeclaredJob (wrong case, out-of-vocabulary,
// non-string, oversized, '__proto__', a forged/replayed value) ALL resolve
// to the same undeclared shape. No gate-supplied string ever reaches
// frontmatter or the map unchecked.
function runJobGate(options, ctx) {
  const undeclared = { declared: false, job_id: '', option_key: 'custom' };
  const g = options.jobGate;
  let answer = null;
  if (typeof g === 'function') {
    try { answer = g(ctx); } catch (_e) { return undeclared; }
  } else if (g && typeof g === 'object') {
    answer = g;
  }
  if (!answer || typeof answer !== 'object') return undeclared;
  const jobId = answer.job_id;
  if (jobId === 'custom') return undeclared;
  if (!sectionRegistry.isDeclaredJob(jobId)) return undeclared;
  return { declared: true, job_id: jobId, option_key: jobId };
}
...
//
// Phase 353 Plan 01 (R-353-E): a sixth, best-effort step re-runs the PARENT
// map rebuild after the child directory is gone. Reasoning stated once here:
// a rebuild-from-disk is idempotent (D-353-2), so the compensating action for
// "the parent map was rebuilt with a child that no longer exists" is
// "rebuild it again", never "restore a backup" -- there is no backup to
// restore, and there does not need to be one. `parentRoomDir` is optional
// (the pre-353 call sites and the non-bornWired path never had a parent map
// to fix, so passing it is opt-in and its absence is a byte-identical no-op).
function _bornWiredRollback(roomDir, slug, roomsHome, dbHandle, parentSlug, parentRoomDir) {
  ...
  if (parentRoomDir) {
    try {
      ...
      const parentMap = roomMap.buildRoomMap(parentRoomDir);
      const writeResult = roomMap.writeRoomMap(parentRoomDir, parentMap);
      if (writeResult && writeResult.ok) {
        roomMap.writeSelfBlocks(parentRoomDir, parentMap);
      }
    } catch (_e) {
      // Best-effort: the rollback itself must never throw.
    }
  }
}
...
// Phase 353 Plan 01 Task 6 (D-353-5): patch the declared job_id (plus
// job_source:'declared') onto the CHILD's OWN root ROOM.md, cloning
// _patchChildStateParent's atomic tmp+rename idiom above. NOT a side effect
// (see the STEP 1 call site's comment for why); a private, child-local
// scaffold patch that runs once, at birth, on the file scaffoldRoomSkeleton
// just wrote. Reads with gray-matter (never a thirteenth hand-rolled
// parseFrontmatter) and never overwrites a job_id a human -- or an earlier
// call -- already put there. Returns a boolean, never throws.
function _patchChildRoomJob(roomDir, jobId) {
  ...
  try {
    ...
    if (typeof data.job_id === 'string' && data.job_id.length > 0) {
      return true; // already authored; never overwrite (Canon Part 9)
    }
    data.job_id = jobId;
    data.job_source = 'declared';
    const newRaw = matter.stringify(parsed.content, data);
    const tmp = roomMdPath + '.tmp.' + process.pid + '.' + Math.random().toString(36).slice(2, 8);
...
// SEED-001 side-effect 6 (Phase 353 Plan 01, R-353-E): rebuild the CHILD room
// map first, then the PARENT room map, as the LAST action inside the
// FINALIZE try. lib/core/room-map.cjs is required LAZILY, inside the
// function body, so a map failure can never break module load for an
// unrelated birth path (top-level rooms never call this function at all).
// Never throws; returns a boolean the FINALIZE block folds into `allWired`
// exactly like the other five side effects.
function _rebuildRoomMaps(childRoomDir, parentRoomDir) {
  try {
    // eslint-disable-next-line global-require
    const roomMap = require('../room-map.cjs');

    const childMap = roomMap.buildRoomMap(childRoomDir);
    const childWrite = roomMap.writeRoomMap(childRoomDir, childMap);
    if (!childWrite.ok) return false;
    roomMap.writeSelfBlocks(childRoomDir, childMap);

    const parentMap = roomMap.buildRoomMap(parentRoomDir);
    const parentWrite = roomMap.writeRoomMap(parentRoomDir, parentMap);
    if (!parentWrite.ok) return false;
    roomMap.writeSelfBlocks(parentRoomDir, parentMap);
...
// Quick task 260917-ild (Codex F4): the BRAIN.md stub body, extracted to a
// module-scope constant using the SAME {{TOKEN}} convention
// templates/room-skeleton/*.tmpl use (lib/core/room-skeleton-scaffold.cjs's
// renderTemplate). BRAIN.md has no on-disk template file (unlike ROOM/STATE/
// MINTO/USER), so lib/core/eureka/scaffold-template-index.cjs sources its
// BRAIN kind FROM this exported constant instead -- additive export, no
// behaviour change: _writeBrainStub below now renders from it rather than
// hand-assembling the same string inline.
const BRAIN_STUB_TEMPLATE = '---\nkind: BRAIN\nroom: {{SLUG}}\nderived: false\n---\n\n' +
  '# BRAIN.md\n\n' +
...
function _writeBrainStub(roomDir, slug) {
  ...
  const content = BRAIN_STUB_TEMPLATE.replace(/\{\{SLUG\}\}/g, String(slug));
  try {
...
function birthRoom(opts) {
  ...
  // Phase 353 Plan 01 Task 6: default undeclared. STEP 1 reads this after
  // scaffoldRoomSkeleton returns to decide whether to patch job_id onto the
  // child ROOM.md; a top-level (non-born-wired) birth never overwrites this
  // default, so no job card fires and no job_id key ever appears for it.
  let jobDeclaration = { declared: false, job_id: '', option_key: 'custom' };
  if (bornWired) {
    ...
    // Phase 353 Plan 01 Task 6 (D-353-5): the ONE F.8 job-declaration card,
    // fired right beside the approval card so a sub-room birth costs the
    // navigator exactly one card moment. runJobGate validates the answer
    // against the closed vocabulary BEFORE any byte is written (T-353-26).
    // The declared job or 'custom' rides the SAME gateAnswers array STEP 2's
    // existing drainBirthGateAnswers already drains -- no second gate path,
    // no new gate-ledger row, no fourth card-fire recording site.
    jobDeclaration = runJobGate(options, { slug: slug, parent: parent, parentRoomDir: parentRoomDir });
    gateAnswers.push({
      gate_id: 'SUBROOM_JOB',
      canonical_verb: 'Approve',
      option_key: jobDeclaration.option_key,
      alias_label: (jobDeclaration.declared ? ('Job: ' + jobDeclaration.job_id) : 'Job: undeclared (custom)') + ' for sub-room ' + slug,
      ts: Date.now(),
    });
  }
  ...
  try {
    ...
    // Phase 353 Plan 01 Task 6 (D-353-5): patch the declared job_id onto the
    // CHILD's own root ROOM.md right here, immediately after scaffolding
    // writes it, and BEFORE side effect six (FINALIZE) ever builds the
    // child's map -- so the child's FIRST map build already carries job_id,
    // with no second pass. Deliberately NOT a seventh side effect: the six
    // side effects are the PARENT-facing wiring guarantees `allWired` gates,
    // while a declared job is a child-local scaffold fact, and
    // _bornWiredRollback's fs.rmSync already removes the entire child tree
    // on any unwind, so a declared job can never leave a partial child
    // behind. Only fires for a born-wired birth whose card was answered.
    if (bornWired && jobDeclaration.declared) {
      _patchChildRoomJob(roomDir, jobDeclaration.job_id);
    }
    ...
    if (bornWired) { _bornWiredRollback(roomDir, slug, roomsHomeForBirth, null, parent, parentRoomDir); }
  ...
  // block and is verified. ANY of the six missing -> compensating rollback so
  // NO half-born orphan remains (Part 11 R1/R2 fail-closed).
  //
  // Phase 353 Plan 01 (R-353-E, D-353-5): side effect six -- the child's and
  // the parent's room maps are rebuilt LAST, after every other side effect,
  // because the map's `children` field and the parent's own self-block must
  // reflect the fully-wired child (registry lineage, STATE.md links) rather
  // than a half-finished one. Rebuilds the CHILD map first, then the PARENT
  // map second (a stub with no children yet gains nothing from being rebuilt
  // before the child it is about to list exists).
  // --------------------------------------------------------------------------
  if (bornWired) {
    const se = { s1: false, s2: false, s3: false, s4: false, s5: false, s6: false };
    ...
    try {
      ...
      se.s6 = _rebuildRoomMaps(roomDir, parentRoomDir);
      ...
      if (typeof options._faultInject === 'string' && /^s[1-6]$/.test(options._faultInject)) {
    ...
    const allWired = se.s1 && se.s2 && se.s3 && se.s4 && se.s5 && se.s6;
    if (!allWired) {
      _bornWiredRollback(roomDir, slug, roomsHomeForBirth, null, parent, parentRoomDir);
...
module.exports = {
  birthRoom, drainBirthGateAnswers, writeSectionNodes, SECTION_NAMES,
  // Quick task 260917-ild (Codex F4): additive export, no behaviour change.
  BRAIN_STUB_TEMPLATE,
};
