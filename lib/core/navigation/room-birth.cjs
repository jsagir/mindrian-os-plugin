...
// SECTION_NAMES mirrors lib/core/room-skeleton-scaffold.cjs (single source of
// truth there; duplicated here for seedSection iteration without requiring the
// scaffold module's internal array, since scaffold does not export it).
...
function writeSectionNodes(db, sectionSlugs) {
  ...
  for (const slug of slugs) {
    ...
    try {
      insertNode(db, slug, 'Section', props, { source_path: 'system:section-anchor', created_by: 'system' });
...
function birthRoom(opts) {
  ...
  const gateAnswers = Array.isArray(options.gateAnswers) ? options.gateAnswers : [];
