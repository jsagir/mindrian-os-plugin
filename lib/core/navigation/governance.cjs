...
//
// Returns [{ candidate_id, kind, confidence, source_path, claim_text,
// knowledge_type }, ...]. candidate_id/kind/confidence/source_path are the exact
// fields governance-candidate.validateCandidate expects minus the
// target_section / affected_sections / layer the raiser fills in (Phase 189-02
// Task 2). claim_text / knowledge_type (Phase 298-04, D-01) are read out of the
// node's properties JSON blob (the `text` / `knowledge_type` keys
// typed-claim.cjs's writeClaimNode writes) so the F.8 basket can render a
// readable row instead of an opaque structural handle; both degrade to null on
// a malformed blob rather than dropping the row. roomId is preserved for API
// symmetry (unused in v1; the room.db handle is already scoped to one room).
// Defensive: NEVER throws; returns [] on any query failure or missing handle.
function findGovernanceCandidates(db, roomId, opts) {
  ...
  try {
    const rows = db.prepare(
      "SELECT n.id, n.type, n.confidence, n.source_path, n.properties "
      + "FROM nodes n "
    ...
    return rows.map((r) => {
      // D-01: carry the claim text and DIKW knowledge_type out of the properties
      // JSON blob so the F.8 basket can render a readable row (raiser.cjs, Task
      // 2). A single malformed blob degrades to null/null for THIS row only --
      // never drops the row and never breaks the outer never-throw contract.
      let claimText = null;
      let knowledgeType = null;
      try {
        const props = JSON.parse(r.properties);
        if (props && typeof props === 'object') {
          claimText = (typeof props.text === 'string') ? props.text : null;
          knowledgeType = (typeof props.knowledge_type === 'string') ? props.knowledge_type : null;
        }
      } catch (_parseErr) {
        // malformed properties blob: claimText/knowledgeType stay null.
      }
      return {
        candidate_id: r.id,
        kind: r.type,
