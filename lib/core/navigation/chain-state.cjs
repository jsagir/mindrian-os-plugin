...
function findChainStateNodeByStep(db, runId, stepIndex) {
  ...
  let rows;
  try {
    rows = db.prepare('SELECT id, properties FROM nodes WHERE type = ?').all(CHAIN_STATE_NODE_TYPE);
  } catch (_e) {
    return null;
  }
...
function readChainState(db, runId) {
  ...
  while (true) {
    let edgeRow;
    try {
      edgeRow = db.prepare('SELECT target FROM edges WHERE type = ? AND source = ?').get('FEEDS_INTO', currentId);
    } catch (_e) {
      break;
    }
