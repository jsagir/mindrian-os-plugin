 ...
 *   evidence-claim.cjs:90-100): knowledge_type, conditions, counter_conditions,
 *   valid_from, valid_until, source_speaker, source_segment, and the optional
 *   AMB-01 disambiguation marker are ALL additive keys inside the properties TEXT
 *   blob -- NEVER DDL columns. review_status is the ONLY CHECK-constrained column
...
function writeClaimNode(db, params) {
  ...
  try {
    ...
    // The DO UPDATE SET clause EXCLUDES review_status, so a human-confirmed claim
    // re-projected by the system stays confirmed (no downgrade).
    db.prepare(
      'INSERT INTO nodes ' +
      '(id, type, properties, source_path, created_by, confidence, review_status, created_at, last_seen_at) ' +
      "VALUES (?, 'claim', ?, ?, 'system', 1.0, 'proposed', ?, ?) " +
...
module.exports = { writeClaimNode, KNOWLEDGE_TYPES, CLAIM_NODE_ID };
