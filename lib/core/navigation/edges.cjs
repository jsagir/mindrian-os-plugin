...
const ALLOWED_EDGE_TYPES = Object.freeze(new Set([
  ...
  //   never crosses to Brain. Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  //   mid-call -- see 273-RESEARCH.md D-04).
  //
  ...
  //   FILED_AS_DECISION never crosses to Brain. Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  ...
  //   cross to Brain. Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller
  //   passed in, so it is physically incapable of opening or writing a
  //   second room's db (Phase 8 cross-room fence; no runtime assertion
  //   exists because no code path has been found or hypothesized where a
  //   handle is swapped mid-call -- see 273-RESEARCH.md D-04).
  //
  ...
  //   Cross-room aggregation is prevented structurally: writeEdge(db, params)
  //   only ever holds the one db handle its caller passed in, so it is
  //   physically incapable of opening or writing a second room's db (Phase 8
  //   cross-room fence; no runtime assertion exists because no code path has
  //   been found or hypothesized where a handle is swapped mid-call -- see
  //   273-RESEARCH.md D-04).
  //
  ...
  //   Brain. Cross-room aggregation is prevented structurally: writeEdge(db,
  //   params) only ever holds the one db handle its caller passed in, so it
  //   is physically incapable of opening or writing a second room's db
  //   (Phase 8 cross-room fence; no runtime assertion exists because no code
  //   path has been found or hypothesized where a handle is swapped
  //   mid-call -- see 273-RESEARCH.md D-04).
  'DECOMPOSED_INTO',
  'PART_OF',
  'TAGGED_WITH',
  ...
  //   INVALIDATES / ENABLES never cross to Brain. Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  ...
  //   DISCOVERED / AUTHORED_BY never cross to Brain. Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  ...
  //   Brain (Canon Part 8 LOCAL -> BRAIN: NO). Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  //   mid-call -- see 273-RESEARCH.md D-04).
  'CONCERNS',
  // Quick task 260806-grant-grader-room-graph extension (Grant-Rubric Graph
  // Substrate; the ONE net-new edge type the grant-rubric map needs). Added
  // ADDITIVELY to this closed allow-list, never invented per-phase -- mirrors
  // the quick 260725-9ca CONCERNS additive idiom verbatim (the LIGHTER
  // additive-comment-only idiom; this is a routine additive vocabulary
  // extension, not a navigator-gated blocking-checkpoint ceremony). The floor
  // (DEFERRED / REJECTED + every prior type) is byte-identical except this one
  // addition; the floor test asserts named MEMBERSHIP, never an exact `.size` /
  // count, so this cannot regress baseline.
  //
  //   MAPS_TO_SECTION -- the rubric-criterion-to-room-section edge. Source = a
  //     grant_criterion anchor node (grant_criterion:<program>:<criterion_id>,
  //     minted by lib/core/navigation/grant-rubric.cjs writeGrantRubricGraph),
  //     target = the standard Section node it maps to (the 8 canonical slugs
  //     room-birth.cjs writeSectionNodes mints at birth). It is the graph-native
  //     home of the grant rubric's room_section map: one edge per criterion with
  //     a non-null room_section, so "which sections does this program's rubric
  //     touch, and which criteria live where" is a graph walk, not a JSON read.
  //     A criterion with room_section null (a pure post-award process/reporting
  //     item) gets NO edge -- the null is honest, never forced.
  //
  // Edge vocabulary decision (quick 260806 PLAN.md D2, LOCKED): PART_OF /
  //   TAGGED_WITH are NOT reused -- their frozen legal targets are domain /
  //   subdomain / focus_area ONLY (Phase 163-01), and Phase 169-00 explicitly
  //   REJECTED widening PART_OF endpoints as the pattern for a new joint (the
  //   NESTED_WITHIN precedent: mint a dedicated type, keep taxonomy traversals
  //   unpolluted). The verdict->section gap-profile edge reuses the pre-existing
  //   INFORMS member with scalar count properties, so this block adds exactly
  //   ONE new member.
  //
  // Canon Part 4: every choice is graph data; MAPS_TO_SECTION IS the graph-
  //   native artifact of the rubric's room_section mapping (Canon Part 9: files
  //   preserve meaning, SQL remembers and navigates).
  // Canon Part 8: properties are ENUM/scalar ONLY (a relation enum + the
  //   program slug + the category enum); NEVER prose, NEVER a criterion's
  //   details/common_mistake BODY (those stay in the LOCAL fixture JSON). This
  //   is a LOCAL room.db edge; writeEdge takes (db, params) over a LOCAL
  //   room.db handle; MAPS_TO_SECTION NEVER crosses to Brain (the grant rubric
  //   is real IIA domain data, empirically blocked by the Part 8 egress guard
  //   -- see grade-grant.cjs header). Cross-room aggregation is prevented structurally:
  //   writeEdge(db, params) only ever holds the one db handle its caller passed
  //   in, so it is physically incapable of opening or writing a second room's
  //   db (Phase 8 cross-room fence; no runtime assertion exists because no
  //   code path has been found or hypothesized where a handle is swapped
  //   mid-call -- see 273-RESEARCH.md D-04).
  'MAPS_TO_SECTION',
  // Quick task 260903-gct extension, implementing R16 of the ratified MOS
  // Reasoning Constitution v3.1.0 section 2.2
  // (docs/2026-09-03-CONSTITUTION-v3.1.0-mos-reasoning-constitution.md,
  // navigator sign-off 2026-09-03). v3.0.0's original R2 listed five edges
  // to add -- SUPPORTS, CONTRADICTS, REFINES, DERIVED_FROM, SOURCED_FROM --
  // and this session confirmed by direct code read that four of those five
  // (SUPPORTS line 296, CONTRADICTS line 163, REFINES line 339, DERIVED_FROM
  // line 57) were ALREADY live. R16 struck those four; this block mints
  // exactly ONE member.
  //
  // The additive-floor contract: the floor (DEFERRED / REJECTED + every
  // prior type) is byte-identical except this one addition; tests assert
  // named MEMBERSHIP, never an exact `.size`, so this cannot regress the
  // baseline.
  //
  //   SOURCED_FROM -- the PROVENANCE edge. Source = a written reasoning node
  //     (a claim, conclusion, decision, or artifact node); target = a node id
  //     that write was actually sourced FROM (its evidence or subject node).
  //     Per the filed T2 write-back design
  //     (docs/2026-09-03-DESIGN-t2-write-back-minimal.md line 17):
  //     "SOURCED_FROM from the new node to every node id the gate decision
  //     was actually about."
  //
  //   AXIS CONTRAST with DERIVED_FROM (encode it, do not conflate, mirroring
  //   the UMBILICAL_TO vs NESTED_WITHIN contrast at line 542): DERIVED_FROM
  //   is the Phase 120-00 STRUCTURAL ENFORCEMENT edge -- narrow and typed,
  //   source = a Breakthrough node, target = an Artifact node, and a
  //   Breakthrough CANNOT surface without at least one of them. SOURCED_FROM
  //   is the GENERAL provenance edge for any written reasoning node back to
  //   its evidence; it enforces no surfacing precondition and its endpoints
  //   are not restricted to Breakthrough/Artifact.
  //
  //   NO CONSUMER SHIPS IN THIS TASK: the T2 write-back path that will emit
  //   SOURCED_FROM is DESIGNED and FILED but NOT implemented, and that design
  //   names R16 as its blocking dependency. This is the land-the-type-before-
  //   the-consumer idiom NESTED_WITHIN (line 471) and DISCOVERED /
  //   AUTHORED_BY (line 588) were grown by.
  //
  // Canon Part 4: every choice is graph data; SOURCED_FROM IS the graph-native
  //   artifact of provenance (Canon Part 9: files preserve meaning, SQL
  //   remembers and navigates).
  // Canon Part 8: properties are ENUM / scalar ONLY (a relation enum plus
  //   node-id handles); NEVER prose, NEVER a claim or artifact BODY. This is
  //   a LOCAL room.db edge; writeEdge takes (db, params) over a LOCAL room.db
  //   handle; SOURCED_FROM NEVER crosses to Brain. Cross-room aggregation is
  //   prevented structurally: writeEdge(db, params) only ever holds the one
  //   db handle its caller passed in, so it is physically incapable of
  //   opening or writing a second room's db (Phase 8 cross-room fence; no
  //   runtime assertion exists because no code path has been found or
  //   hypothesized where a handle is swapped mid-call -- see
  //   273-RESEARCH.md D-04).
  'SOURCED_FROM',
  // Quick task 260903-i2x extension, implementing T2's node-writing half of
  // the ratified MOS Reasoning Constitution v3.1.0
  // (docs/2026-09-03-CONSTITUTION-v3.1.0-mos-reasoning-constitution.md).
  // The filed T2 write-back design (docs/2026-09-03-DESIGN-t2-write-back-
  // minimal.md line 19 and line 39) names this edge as T2's own scope, and
  // CLAUDE.md's open-handoff row independently names "T2's node-writing
  // half (claim-node creation + SOURCED_FROM/USES_FRAMEWORK edges ...)" as
  // the remaining work this task closes.
  //
  //   USES_FRAMEWORK -- the METHODOLOGY-PROVENANCE edge. Source = a written
  //     reasoning node (the decision node a gate approval mints); target =
  //     a `framework:<slug>` handle naming the methodology that was active
  //     when the decision was made.
  //
  //   AXIS CONTRAST with SOURCED_FROM (encode it, do not conflate): SOURCED_
  //     FROM points at the EVIDENCE a write was sourced from; USES_FRAMEWORK
  //     points at the METHOD used to reach it. Different questions,
  //     different targets, no overlap.
  //
  //   BRAIN-VS-LOCAL COLLISION, named honestly: a USES_FRAMEWORK relationship
  //     already exists in the Brain graph (references/brain/room-hierarchy-
  //     schema.md:72, Room -> Framework, 86 edges per docs/BRAIN-GRAPH-
  //     CENSUS.generated.md). This LOCAL member is a same-named but
  //     INDEPENDENT local edge; per Canon Part 8 it NEVER crosses to Brain,
  //     and the two are never synced or compared. The framework NAME is a
  //     generic methodology handle (the only class of token Part 8 permits
  //     near a boundary), never room content.
  //
  //   NOT the land-the-type-before-the-consumer idiom (unlike NESTED_WITHIN
  //     / DISCOVERED): this type's consumer (lib/core/navigation/reasoning-
  //     write.cjs) ships in the SAME task set that mints this type.
  //
  // Canon Part 4: every choice is graph data; USES_FRAMEWORK IS the
  //   graph-native artifact of methodology provenance (Canon Part 9: files
  //   preserve meaning, SQL remembers and navigates).
  // Canon Part 8: properties are ENUM / scalar ONLY (a relation enum plus a
  //   framework slug); NEVER prose, NEVER a claim or artifact BODY. This is
  //   a LOCAL room.db edge; writeEdge takes (db, params) over a LOCAL room.db
  //   handle; USES_FRAMEWORK NEVER crosses to Brain. Cross-room aggregation
  //   is prevented structurally: writeEdge(db, params) only ever holds the
  //   one db handle its caller passed in, so it is physically incapable of
  //   opening or writing a second room's db (Phase 8 cross-room fence; no
  //   runtime assertion exists because no code path has been found or
  //   hypothesized where a handle is swapped mid-call -- see
  //   273-RESEARCH.md D-04).
...
//
// Returns { ok: true, edge_id, type, source, target, written } on success, or
...
//
// Phase 273 (D-01/D-01a/D-01b/D-06) -- closes Critical C1 (silent-discard) and
// Critical C2 (schema-mismatch throw) named by the sqlite-graph-layer code
// review. C1: the WHERE-guarded UPDATE above can genuinely suppress a write
// (a confirmed edge's properties are intentionally NOT overwritten) but the
// pre-273 code returned ok:true either way, with zero signal that the write
// was a no-op. The additive `written` field closes that: true when
// run().changes > 0, false when the confirmed-guard suppressed the row, with
// `reason: 'suppressed_by_confirmed'` naming why. `ok` NEVER flips on
// suppression -- 77 call sites across 43 files read `.ok`, ~30 branch on it,
// and room-birth.cjs:948 rolls back an entire room birth on `!ok`; changing
// what `ok` means would be a 43-file regression, not a fix (D-01a).
//
// C2: a handle opened via lazygraph-ops.cjs::openGraph (the base schema) has
// no review_status column and no identity table for the Phase 224 migration
// to run against (D-01b -- a structural conclusion, not a preference: see
// edgesHasReviewStatus below). The pre-273 code always referenced
// review_status in its INSERT, so it threw "table edges has no column named
// review_status" against that handle instead of degrading. writeEdge now
// probes the schema (edgesHasReviewStatus) and branches: the wide 5-column
// insert with the confirmed-guard WHERE clause when the column exists, a
// narrow 4-column insert with NO WHERE guard when it does not (nothing can be
// "suppressed" referencing a column that is not there, so every write on that
// branch reports written:true). When a caller supplies review_status against
// the narrow schema, the gap is never silently dropped: D-06 requires an
// explicit `review_status_persisted: false` flag on the return so the caller
// can see the value did not land, mirroring the same "never mask a
// data-shape gap" lesson C1 itself teaches.
const VALID_REVIEW_STATUS = Object.freeze(new Set(['proposed', 'confirmed']));

// DEPRECATED_EDGE_TYPES -- Quick task 260903-gct, R16 of the ratified MOS
// Reasoning Constitution v3.1.0 section 2.2 (navigator sign-off 2026-09-03).
// This is a SOFT deprecation, not removal. RELATED_TO is live in production
// across 14 files including the production writers
// lib/core/navigation/typed-domain.cjs (its DOMAIN_EDGE_SUBSET, consumed by
// linkDomainToRelated) and scripts/huji-intake.cjs:410, plus test-suite
// writers (tests/helpers/fixture-room-219.cjs:207,
// tests/test-201-graph-refine-loop.cjs:81,
// tests/test-edges-domain-taxonomy-floor.cjs:120, which explicitly asserts
// rr.ok === true). Deleting RELATED_TO from ALLOWED_EDGE_TYPES would return
// invalid_edge_type to every one of those writers, and would additionally
// turn the tests/run-all-163.sh frozen-edge-set gate RED: that gate greps
// this file for the quoted literal 'RELATED_TO' (its DOMAIN_EDGES array,
// tests/run-all-163.sh lines 172-181).
//
// Shaped as a frozen Map from edge_type to its deprecation message, so a
// future second deprecation is a one-line addition and each type carries its
// own guidance.
//
// The RELATED_TO message is honest on three points: (1) it is deprecated ON
// WRITE per constitution v3.1.0 R16 and REMAINS allowlisted; (2) there is NO
// removal version -- the ruling says "deprecated on write", not "removed",
// so no target version is invented here (unlike room-ops.cjs's legitimate
// v1.14.0 citation, naming one here would be a fabricated commitment nobody
// ratified); (3) guidance names a SPECIFIC typed predicate to pick instead,
// since there is no single mechanical replacement: DECOMPOSED_INTO / PART_OF
// / TAGGED_WITH for domain-taxonomy structure, or SUPPORTS / REFINES /
// CONTRADICTS / SOURCED_FROM / DERIVED_FROM for a reasoning link.
const DEPRECATED_EDGE_TYPES = Object.freeze(new Map([
  ['RELATED_TO',
    "edge_type 'RELATED_TO' is deprecated on write per MOS Reasoning "
    + 'Constitution v3.1.0 R16. It REMAINS allowlisted -- no removal version '
    + 'is set. Prefer a specific typed predicate instead: DECOMPOSED_INTO / '
    + 'PART_OF / TAGGED_WITH for domain-taxonomy structure, or SUPPORTS / '
    + 'REFINES / CONTRADICTS / SOURCED_FROM / DERIVED_FROM for a reasoning '
    + 'link.'],
]));

// _deprecationWarned / _warnDeprecatedEdgeOnce -- the explicit per-type
// once-guard. Required, not optional (constitution v3.1.0 R16 discovered
// facts F-08): writeEdge runs per-write, and room-ops.cjs's claim that
// process.emitWarning "dedups by (name, code) per Node process
// automatically" is UNVERIFIED and moot there anyway (that module's warning
// fires once at module-load time, not per-call). Mirrors
// brain-client.cjs::_warnLegacyOnce's module-level guard idiom (itself
// modeled on checkFilePermissions._warned) and room-ops.cjs's stable
// MOS_DEP_* code convention, so test environments can filter via
// NODE_NO_WARNINGS or a process.on('warning') handler.
const _deprecationWarned = new Set();
function _warnDeprecatedEdgeOnce(edge_type) {
  if (_deprecationWarned.has(edge_type)) return;
  _deprecationWarned.add(edge_type);
  process.emitWarning(
    DEPRECATED_EDGE_TYPES.get(edge_type),
    { type: 'DeprecationWarning', code: 'MOS_DEP_EDGE_' + edge_type }
  );
}

// edgesHasReviewStatus(db) -- Phase 273 (D-01b) schema probe. Mirrors
// lib/core/node-insert.cjs::isMigratedSchema's SHAPE (a PRAGMA table_info
// probe with a defensive false-on-failure default) without duplicating its
// MEANING -- that helper probes the `nodes` table for a different marker
// column; this one probes `edges` for `review_status`. Kept as a narrowly-
// scoped sibling function, not folded into writeEdge's body, per the same
// one-folder-one-job separation node-insert.cjs already established between
// isMigratedSchema and insertNode. Recomputed fresh on every call (never
// cached) -- the schema shape of a given handle does not change mid-process,
// but a cached boolean would be one more piece of hand-maintained state this
// chokepoint does not need.
function edgesHasReviewStatus(db) {
  try {
    return db.prepare('PRAGMA table_info(edges)').all().some((c) => c && c.name === 'review_status');
  } catch (_e) {
    return false; // defensive default, matching node-insert.cjs::isMigratedSchema
  }
}
...
function writeEdge(db, params) {
  ...
  // R16 soft-deprecation probe (constitution v3.1.0 section 2.2). Placement
  // is load-bearing: AFTER the ALLOWED_EDGE_TYPES gate means a REJECTED type
  // (e.g. the misspelled RELATES_TO) never warns -- it stays a silent
  // fail-closed, which is the constitution's own correction. BEFORE the
  // insert means the warning describes the CALL and still fires even if the
  // write later throws.
  const deprecated = DEPRECATED_EDGE_TYPES.has(edge_type);
  if (deprecated) _warnDeprecatedEdgeOnce(edge_type);
  ...
  const hasReviewStatus = edgesHasReviewStatus(db);
  let runResult;
  try {
    if (hasReviewStatus) {
      ...
      // C2 (Phase 273): base openGraph schema has no review_status column and
      // no identity table to run the Phase 224 migration against (D-01b).
      // Degrade gracefully: 4-column insert, no confirmed-guard WHERE clause
      // (the column it would reference does not exist here, so nothing can
      // be "suppressed" -- every write on this branch lands).
      runResult = db.prepare(
  ...
  const written = Number(runResult.changes) > 0;
  const result = {
    ok: true, edge_id: edgeId, type: edge_type, source: source_id, target: target_id,
    written,
  };
  if (!written) result.reason = 'suppressed_by_confirmed';
  if (reviewStatusValue !== null) {
    // D-06: when the caller supplied a review_status, always name whether it
    // actually landed in a real column -- never silently mask the base-schema
    // gap (false) and never leave the migrated-schema success (true) implicit.
    result.review_status_persisted = hasReviewStatus;
  }
  // R16 additive signal, same idiom as `written` (D-01) and
  // `review_status_persisted` (D-06): a machine-readable field a caller can
  // branch on without `ok` changing meaning. Do NOT touch `ok`, do NOT add a
  // failure branch, and do NOT add this field to any of the early `ok: false`
  // returns above (constitution v3.1.0 R16 discovered facts F-04: 77 call
  // sites across 43 files read `.ok`, ~30 branch on it, and
  // room-birth.cjs:948 rolls back an entire room birth on `!ok`; changing
  // what `ok` means would be a 43-file regression, not a fix). The return
  // field is the RELIABLE signal; the warning is the human one --
  // process.emitWarning with type DeprecationWarning is suppressed under
  // NODE_NO_WARNINGS or process.noDeprecation, and result.deprecated still
  // lands in every one of those cases.
  if (deprecated) result.deprecated = true;
  return result;
}

module.exports = { ALLOWED_EDGE_TYPES, DEPRECATED_EDGE_TYPES, writeEdge };
