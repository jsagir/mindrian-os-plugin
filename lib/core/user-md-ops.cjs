...
function readUserMd(userMdPath) {
  ...
  // body below is the else-branch: with NO override active this block falls
  // through and readUserMd stays byte-identical to its prior behavior. The
  // override taxonomy validation already happened in setOverride; here we
  // only map fields onto the emptyUser shell. NEVER break readUserMd: any
  // failure in this block falls through to the real file read.
  try {
