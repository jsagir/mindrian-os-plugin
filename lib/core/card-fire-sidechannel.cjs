 ...
 * is left intact even though scopedRecords unions its FRESH records into this
 * session's read. Consuming it here would let ANY concurrently-running session's
 * Stop hook delete a sessionless mint belonging to a DIFFERENT session before
 * that session ever adjudicated it -- silently losing a genuine force-fire. The
 * residual sessionless bleed stays bounded to TURN_FRESH_MS by fix (A) in
 * scopedRecords above, and none of the three live instances of this defect
 * involved a NO_SESSION_KEY record (all three were session-scoped). A caller
 * with no sessionId of its own reads AND consumes the NO_SESSION_KEY bucket as
 * its own primary bucket, exactly mirroring readReachedGates's precedence.
 */
