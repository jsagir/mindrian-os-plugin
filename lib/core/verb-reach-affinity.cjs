...
// ---------------------------------------------------------------------------
// AFFINITY_FLOOR -- the ABSOLUTE cosine below which a verb is treated as having
// no reach affinity at all (null), as opposed to AFFINITY_MARGIN above, which
// is a RELATIVE top1-minus-top2 spread.
//
// WHY THIS IS A SEPARATE CONSTANT (245-04 Task 2 deviation, Rule 1)
//
// 245-04-PLAN specified ONE constant serving both roles: "top1 < AFFINITY_MARGIN
// in absolute terms -> null". That conflation is a defect. The two roles need
// numbers on completely different scales, because a sentence encoder's cosine
// is not a zero-based scale: MongoDB/mdbr-leaf-ir (the shipped default) returns
// roughly 0.40 to 0.55 for two ARBITRARY UNRELATED English phrases. A 0.05
// absolute floor is therefore structurally unreachable, the null branch is dead
// code, and every verb gets force-fitted onto some reach. The first live
// derivation run proved exactly that: it produced a 5-way 0.2 split for
// 'Synthesize' and a 3-way split for 'Bank Opportunity', erasing the F-8
// coverage finding this plan exists to record honestly.
//
// TUNABLE-LATER. Calibration, stated honestly rather than presented as derived:
// 0.70 sits inside a 0.069-wide EMPTY band in the first live cosine matrix,
// which separated every forward-map-confirmed verb (top1 in 0.7615 .. 0.8626)
// from every verb with no reach preimage (top1 in 0.5229 .. 0.6929). It also
// coincides with this repo's already-frozen Canon Part 3 RECOMMEND_FLOOR of
// 0.70, the value the codebase already treats as "confident enough to
// recommend". It is NOT a value calibrated against an outcome corpus, because
// none exists yet. Revisit alongside AFFINITY_MARGIN.
//
// Env override MINDRIAN_AFFINITY_FLOOR, same defensive parse: anything
// non-numeric, non-finite, non-positive, or >= 1 silently falls back.
//
// Like AFFINITY_MARGIN, this affects the BUILD-TIME derivation only. It has no
// runtime effect on its own; the runtime reads the committed table.
// ---------------------------------------------------------------------------
const DEFAULT_AFFINITY_FLOOR = 0.70;

function resolveAffinityFloor() {
  const raw = process.env.MINDRIAN_AFFINITY_FLOOR;
  if (raw === undefined || raw === null || String(raw).trim() === '') {
    return DEFAULT_AFFINITY_FLOOR;
  }
  const n = Number(String(raw).trim());
  if (Number.isFinite(n) && n > 0 && n < 1) return n;
  return DEFAULT_AFFINITY_FLOOR;
...
const AFFINITY_FLOOR = resolveAffinityFloor();
...
module.exports = {
  ...
  DEFAULT_AFFINITY_FLOOR: DEFAULT_AFFINITY_FLOOR,
  resolveAffinityFloor: resolveAffinityFloor,
};
