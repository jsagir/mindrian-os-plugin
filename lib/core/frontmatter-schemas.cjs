...
const SCHEMAS = Object.freeze({
  ...
  'ROOM.md': Object.freeze({
    ...
    optional: Object.freeze([
      ...
      'founder',
      'shared_with',
      // Phase 353 Plan 01 (D-353-2): the derived, fingerprinted self-location
      // block. Emitted by lib/core/room-map.cjs::writeSelfBlocks on every
      // root/section/structural/sub-room ROOM.md (R-353-B: never on an
      // artifact folder). Rebuildable from disk; a disagreement is a doctor
      // finding, not a preference.
      'icm_self',
      // Phase 353 Plan 01 (D-353-5): the declared job a sub-room answered at
      // birth through the F.8 job card, validated against JOB_VOCABULARY and
      // written by room-birth.cjs's _patchChildRoomJob. Absent when
      // undeclared or answered 'custom' -- absence is a legal state, not a
      // gap the validator should flag.
      'job_id',
      // Phase 353 Plan 01 (D-353-5): rides beside job_id, currently always
      // 'declared' when job_id is present. Names the provenance so a future
      // resolution mechanism (Plan 02's parent fallback) can be told apart
      // from a birth-time declaration without re-deriving it.
      'job_source',
    ]),
  }),
  ...
  // Phase 353 Plan 02 Task 4 (D-353-7): the ruling document generator
  // (lib/core/room-skeleton-scaffold.cjs::writeSectionContracts) stamps
  // exactly these four keys on every section CONTEXT.md at generation
  // time. required stays [] because a CONTEXT.md predating this phase (or
  // a section with an undeclared job) carries none of them and must not
  // retroactively "break" on that account. Without this arm, CONTEXT.md
  // falls through to artifact-default and every regeneration lands four
  // spurious 'unknown' violations through the PostToolUse hook.
  'CONTEXT.md': Object.freeze({
    required: Object.freeze([]),
    optional: Object.freeze(['icm_layer', 'job_id', 'ruling_fingerprint', 'generated_at']),
  }),
...
function selectSchemaKey(filePath) {
  ...
  if (base === 'CONTEXT.md') return 'CONTEXT.md';
  return 'artifact-default';
}
...
function validate(filePath, frontmatter) {
  ...
  if (schemaKey === 'MINTO.md') {
    ...
    try {
      ...
      // Quick 260914-ntk (Finding 4.3): addViolation() in
      // feynman-minto-invariants.cjs builds {category, severity, message,
      // field} and NEVER a `type` -- that shape is that module's pinned
      // public contract, and summarizeViolations (the missing-vs-unexpected
      // bucketing) is internal to scripts/frontmatter-schema-validator.cjs,
      // which has no module.exports at all. This delegation arm is the one
      // seam that is both the real boundary between the two vocabularies AND
      // directly testable through the exported validate(). Two of the three
      // missing-field messages read "Missing or empty frontmatter field:
      // <name>", but last_generated_at's reads "Missing last_generated_at
      // (backfill case; rerun regen to populate)" -- a different prefix.
      // /^Missing\b/ catches all three and correctly leaves the two
      // freshness violations ("last_artifact_write_seen_at is in the
      // future...", "Feynman-MINTO is stale...") in the non-missing bucket.
      const violations = ((r && Array.isArray(r.violations)) ? r.violations : []).map((v) => Object.assign({}, v, {
        type: (v && v.field && /^Missing\b/.test(String(v.message || ''))) ? 'missing' : ((v && v.type) || 'invalid'),
      }));
      return {
