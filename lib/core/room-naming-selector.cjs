...
// Phase 128.1: the registry directory-lock. session-binding.cjs does not
// export withRegistryLock, so this writer mirrors the same os.mkdir
// directory-lock idiom (atomic on POSIX + Windows Git-Bash; 2s TTL stale
// break; 200ms retry budget) so the rename write does not race the
// session-scoped resolver writes (RESEARCH.md Pitfall 2).
const REGISTRY_LOCK_TTL_MS = 2000;
const REGISTRY_LOCK_RETRY_BUDGET_MS = 200;

function withRegistryLock(registryPath, fn) {
  const lockDir = registryPath + '.lock';
  const startedAt = Date.now();
  let acquired = false;
  while (Date.now() - startedAt < REGISTRY_LOCK_RETRY_BUDGET_MS) {
    try {
      ...
      if (err && err.code !== 'EEXIST') {
        // Non-contention error -- run unguarded rather than hard-fail.
        break;
      }
      try {
        const st = fs.statSync(lockDir);
        if (Date.now() - st.mtimeMs > REGISTRY_LOCK_TTL_MS) {
          try { fs.rmdirSync(lockDir); } catch (_e) { /* race; continue */ }
          continue;
        }
      } catch (_e) { /* stat race; continue */ }
      const spinUntil = Date.now() + 5;
      while (Date.now() < spinUntil) { /* short spin under contention */ }
    }
  }
  try {
    return fn();
    ...
    if (acquired) {
      try { fs.rmdirSync(lockDir); } catch (_e) { /* best-effort release */ }
    }
  }
}
...
async function _resolveRename(roomsHome, previousSlug, candidate, decisionPath, decidedBy, envelope, dispatcher, userInputChannel, surface) {
  ...
  // Direct registry mutation: rename the key. Phase 128.1: the read-modify-write
  // runs under the registry directory lock (it does not race the session-scoped
  // resolver writes), and it re-points every sessions[*].room and last_active
  // that still names the old slug onto the new slug so no session is left bound
  // to a renamed-away slug (D-02/D-03).
  try {
    const registryPath = path.join(roomsHome, '.rooms', 'registry.json');
    if (fs.existsSync(registryPath)) {
      withRegistryLock(registryPath, function () {
        ...
        if (reg && reg.rooms && reg.rooms[previousSlug]) {
          ...
          if (reg.last_active === previousSlug) reg.last_active = newSlug;
          // Re-point every session binding that named the old slug.
          if (reg.sessions && typeof reg.sessions === 'object') {
            for (const sid of Object.keys(reg.sessions)) {
              const entry = reg.sessions[sid];
              if (entry && entry.room === previousSlug) entry.room = newSlug;
            }
          }
