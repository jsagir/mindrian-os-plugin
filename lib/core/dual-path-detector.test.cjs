...
run('F-02 medium prose 200 words with stuck-language anchors -> type', () => {
  ...
  // NOTE: must avoid words containing "lp" (helps the case-insensitive LP\b match)
  // and "ind" / "ind\b" sequences (matches IND\b case-insensitively). Spec-locked
  // regex from DISCRETION-03 has these quirks; fixtures route around them.
  const text = (
    "I keep coming back to this same fork in the road and do not know which side is the right one to walk down. " +
    "I am trying to figure out if the conviction I had three months ago was real conviction or just enthusiasm dressed up as belief. " +
    ...
    "Other days I wake up and cannot shake the feeling that we are building something nobody actually wants, and the small handful of paying customers we have are exceptions and not signal. " +
    ...
    "I have been turning that conversation over for weeks now. I do not know what to do with it. " +
    "Walk me through this slowly because the inside of my own head is not where the answer is going to come from."
  );
  const r = classify(text);
  // Expect stuck_language true (via "I keep coming back" / "I am trying to" / variants),
  // low newline density, no section_header, no domain markers
  assert.equal(r.features.stuck_language, true);
  assert.equal(r.features.section_header, false, 'F-02 must not match SECTION_HEADER');
  assert.equal(r.features.domain_marker, false, 'F-02 must not match DOMAIN_MARKER');
...
run('F-11 long conversational stuck-language 350 words no headers no markers -> ambiguous (Pitfall 3)', () => {
  ...
  // Avoid "kind" / "wind" / any *ind word-boundary token (matches IND\b case-insensitively under DOMAIN_MARKER spec).
  const sentences = [];
  for (let i = 0; i < 15; i++) {
    sentences.push("I keep coming back to whether what we are building is actually the thing the market is asking for or just a thing we want to make because we are good at making it.");
  }
  for (let i = 0; i < 10; i++) {
    sentences.push("I am trying to be honest with myself about it but the kid of honesty I need is the sort that does not happen inside your own head.");
  }
  ...
  assert.equal(r.features.stuck_language, true);
  assert.equal(r.features.section_header, false);
  assert.equal(r.features.domain_marker, false, 'F-11 must not match DOMAIN_MARKER (Pitfall 3 isolation)');
