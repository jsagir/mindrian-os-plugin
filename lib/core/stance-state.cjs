...
// (commands/stance.md) and the relevance-conditioned footer-offer doctrine
// (skills/larry-personality/SKILL.md, softened from every-turn by Phase 210 item B).
//
// ===========================================================================================
...
// User-facing PROSE may still call it "the posture dial" per the roadmap's canonical language; only the
...
// DEFAULT_VOICE_COLORS -- the DEFAULT/RECOMMENDED two-of-four voice-glyph mapping (Phase 210
// item B softened this from a hard override to a preference): redteam recommends the RED
// challenge square; tell-act recommends the BLUE building square. research and ask make NO
// color claim (Claude's discretion; natural voice-mark detection continues). The consumer
// (lib/statusline/cockpit-renderer.cjs) treats these as the default when natural detection is
// silent; a confident natural detection wins. These map INTO the existing frozen 5-color
// De Stijl palette (lib/hmi/voice-color-mark.cjs); this mints NO sixth color.
const DEFAULT_VOICE_COLORS = Object.freeze({
  redteam: 'red',
  'tell-act': 'blue',
});
...
/**
 * The DEFAULT/RECOMMENDED voice-glyph mapping: returns 'red' for 'redteam', 'blue' for
 * 'tell-act', and null for 'research', 'ask', null, or any invalid input. The name is
 * historical (Phase 192 shipped it as a forced override); Phase 210 item B softened the
 * semantics to a preference the renderer applies when natural voice detection is silent.
 * The exported name is KEPT so consumers (lib/statusline/cockpit-signals.cjs readStanceState)
 * and their degrade floors stay intact. Maps INTO the existing frozen 5-color De Stijl
 * palette; mints no sixth color. Pure, never throws.
 * @param {*} stance
 * @returns {('red'|'blue'|null)}
 */
function forcedVoiceColorForStance(stance) {
  return Object.prototype.hasOwnProperty.call(DEFAULT_VOICE_COLORS, stance)
    ? DEFAULT_VOICE_COLORS[stance]
    : null;
}
