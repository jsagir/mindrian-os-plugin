...
// Quick 260917-o1e (R1c): the SUCCESS-side mirror of _discloseSkip above,
// mirroring it exactly. Opens the room db, logs ONE derivation_completed
// memory_event with SCALAR-only fields (Part 8), closes. Soft-fail: advisory,
// never blocking.
function _discloseCompletion(roomDir, pairsScored, edgesWritten) {
  let navigation;
  try {
    navigation = require('./navigation.cjs');
  } catch (_e) {
...
function _prepareBackfill(roomDir, approvedBy, result, approveFolders) {
  // Per-folder approval gate (WR-09): graph.md specifies the Part 3 Decision
  // Gate PER detected folder, but a bare approvedBy healed EVERY detected
  // folder in one shot -- approving one while rejecting another in the same
  // run was inexpressible. approveFolders (optional) is an allow-list of
  // folder slugs or paths; when present, only listed folders receive the
  // approvedBy handle (the rest surface as ok:false / no_approval, i.e.
  // REJECT/DEFER captured). When ABSENT, behavior is unchanged (approvedBy
  // gates all detected folders) -- back-compatible.
  const allow = Array.isArray(approveFolders) ? approveFolders.map((f) => String(f)) : null;
  function folderApproved(folder, slug) {
    if (!allow) return true;
    return allow.some((f) => {
      if (f === slug) return true;
      try { return path.resolve(f) === path.resolve(folder); } catch (_e) { return false; }
    });
  }
  ...
  for (const found of detected) {
    ...
    const gateApprovedBy = folderApproved(folder, slug) ? approvedBy : '';
    const heal = healRoom({ folder: folder, parentRoomDir: roomDir, slug: slug, approvedBy: gateApprovedBy });
    if (heal && heal.ok) {
...
function _runBackfillSync(classifier, targets, deriveFn, selfCritiqueFn, result, skipRebuild) {
  for (const t of targets) {
    // KNOWN, ACCEPTED RACE (documented): this runner is synchronous for
    // byte-compat with pre-224 callers, so the async _rebuildRoom is
    // fire-and-forget here -- derivation may run against the pre-rebuild index
    // and the after-count may read mid-rebuild. A caller that needs STEP 2
    // sequenced before STEP 3 must use the async path (_runBackfillAsync
    // awaits the rebuild). _rebuildRoom never rejects (internal try/catch).
    //
    // skipRebuild (Phase 233-03): an opt-in for a caller that ALREADY ran the
    // structural index itself. See the runDeriveBackfill jsdoc for why this is a
    // correctness fix and not just an optimization.
    if (!skipRebuild) { try { _rebuildRoom(t); } catch (_e) { /* tolerate */ } }
    let runRes;
    try {
...
async function _runBackfillAsync(classifier, parentRoomDir, targets, deriveFn, selfCritiqueFn, injected, probeOpts, result, skipRebuild) {
  ...
  // Quick 260917-o1e (R1c): whether the probe RAN and reported the encoder
  // available. Gated on this, NOT on `!injected` -- when a deriveFn is
  // injected the probe below is skipped entirely (the test-seam branch), so
  // the backfill never learned whether the encoder was available and must not
  // claim it was.
  ...
  // D-04 TOCTOU closure (mirrors the drain): a passing one-time probe does not
  // prove the encoder survives the whole pass. On the DEFAULT path the producer
  // receives the SAME seam the probe used (scoreOpts = probeOpts, so probe and
  // scoring can never diverge onto different encoders) plus an onOutcome counter;
  // any pair that came back encoder_unavailable AFTER the probe still gets a
  // derivation_skipped disclosure with the affected pair count instead of a
  // silent success report.
  let encoderFailedPairs = 0;
  const producerOpts = injected ? undefined : {
    scoreOpts: probeOpts,
    onOutcome: (o) => {
  ...
  for (const { t, pairs } of pairsByTarget) {
    // STEP 2 is SEQUENCED here: the rebuild is awaited so derivation never runs
    // against a not-yet-rebuilt index, never contends with the rebuild's open
    // db handle, and the after-count never reads mid-rebuild.
    //
    // skipRebuild (Phase 233-03): an opt-in for a caller that ALREADY ran the
    // structural index itself. See the runDeriveBackfill jsdoc.
    if (!skipRebuild) { try { await _rebuildRoom(t); } catch (_e) { /* tolerate */ } }
    let runRes;
    try {
      ...
      for (let i = 0; i < pairs.length; i += BACKFILL_PAIR_CHUNK) {
        ...
        for (const pair of chunk) {
          ...
          const cands = await deriveFn({ roomDir: t, artifactPair: pair }, producerOpts);
  ...
  // Post-run disclosure for a MID-RUN encoder degrade (the probe passed, some
  // pairs still returned encoder-silence). Advisory + additive: the result also
  // carries the count so a command surface can render the degrade.
  if (encoderFailedPairs > 0) {
    result.encoderFailedPairs = encoderFailedPairs;
    _discloseSkip(parentRoomDir, encoderFailedPairs);
  } else if (probePassed) {
    // Quick 260917-o1e (R1c): the pass finished with the encoder available and
    // no pair degraded mid-run -- the success-side mirror of the skip
    // disclosure above. `parentRoomDir` is the correct target because it is
    // the same room _discloseSkip writes to, so the skip and the completion
    // land in one db and their timestamps are comparable.
    const edgesWritten = result.rooms.reduce((n, r) => n + (r.edges || 0), 0);
    _discloseCompletion(parentRoomDir, totalPairs, edgesWritten);
  }
...
// Does an injected deriveFn belong on the async runner? True for a declared
// AsyncFunction, and ALSO for a plain function that returns a thenable (probed
// with a degenerate step that a producer answers cheaply with []). See the
// useAsync comment in runDeriveBackfill.
function _isPromiseReturning(fn) {
  if (fn && fn.constructor && fn.constructor.name === 'AsyncFunction') return true;
  try {
    const probe = fn({ roomDir: '', artifactPair: null });
    if (probe && typeof probe.then === 'function') {
      // Detach the probe promise (it resolves to [] by contract; a rejection is
      // irrelevant to routing and must never surface as unhandled).
      probe.then(function () { /* noop */ }, function () { /* noop */ });
      return true;
    }
  } catch (_e) {
    /* a throwing probe keeps the legacy sync path */
  }
  return false;
}

/**
 * runDeriveBackfill({roomDir, approvedBy, approveFolders, deriveFn, selfCritiqueFn, probeOpts, skipRebuild})
 ...
 *
 * skipRebuild (optional, default FALSE so every existing caller is byte-unchanged):
 * suppress this function's own internal structural rebuild. Phase 233-03 added it
 * as a CORRECTNESS fix, not an optimization. The internal rebuild is a
 * DELETE FROM edges; DELETE FROM nodes; then reindex. A caller that runs the
 * RCA-mandated heal pipeline (structural index -> scoped HSI -> hsi-to-graph ->
 * this) has already written HSI_CONNECTION / REVERSE_SALIENT edges by the time it
 * gets here, and the internal rebuild silently DELETES them: the pipeline reports
 * "wrote 20 connection edges" and the room ends up with zero. Proven live on the
 * RCA's own evidence room. A caller that already indexed the room passes
 * skipRebuild:true. A caller that heals folders (approvedBy) must NOT, because a
 * freshly-minted child room still needs its first index pass.
 *
 * approveFolders (optional array of folder slugs or paths): the PER-FOLDER
 * Decision Gate allow-list. When present, only listed detected folders are
 * healed under approvedBy; unlisted folders surface unhealed (no_approval).
 * When absent, approvedBy gates every detected folder (pre-224-review shape).
 *
...
function runDeriveBackfill(args) {
  ...
  // The default score-based producer is async; an injected async producer is too.
  // A synchronous injected deriveFn (the legacy heuristic) takes the sync path.
  // Detection is by RETURN VALUE, not constructor-name sniffing: an arrow/plain
  // function that RETURNS a Promise (a wrapper around the async score producer),
  // a .bind()ed async fn, or a transpiled async fn all report 'Function' but
  // must still take the async runner -- routed to the sync runner, runDerivation
  // rejects their Promise return and the backfill silently derives nothing.
  // The probe call uses a degenerate step ({roomDir:'', artifactPair:null}),
  // which every well-behaved producer answers with [] (or a Promise of []) and
  // no side effect; a throwing probe stays on the legacy sync path.
  const useAsync = !injected || _isPromiseReturning(deriveFn);
  ...
  const targets = _prepareBackfill(roomDir, approvedBy, result, opts.approveFolders);

  // Default FALSE: absent an explicit opt-in, every pre-233 caller keeps the
  // internal rebuild it has always had.
  const skipRebuild = (opts.skipRebuild === true);

  if (useAsync) {
    return _runBackfillAsync(
      classifier, path.resolve(roomDir), targets, deriveFn, selfCritiqueFn, injected, opts.probeOpts, result, skipRebuild
    );
  }
  return _runBackfillSync(classifier, targets, deriveFn, selfCritiqueFn, result, skipRebuild);
}
