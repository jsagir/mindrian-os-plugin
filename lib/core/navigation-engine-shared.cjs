 ...
 *   SECTION_WEIGHTS        frozen map -- Section 3.2 (required + optional)
...
/**
 * Section 3.2 weights. Required sections sum to exactly 1.0:
 *   0.35 + 0.20 + 0.15 + 0.10 + 0.10 + 0.05 + 0.05 = 1.00
 *
 * Optional sections (flagged_contradictions_xroom, hsi_signals) carry
 * weight 0 and are surfaced as decision_trace caveats only.
 */
const SECTION_WEIGHTS = Object.freeze({
  // Required (sum to 1.0):
  pattern_matches: 0.35,
  framework_chain_predictions: 0.20,
  cross_domain_analogies: 0.15,
  wicked_indicators: 0.10,
  unfilled_opportunity_matches: 0.10,
  assessment_thinking_chain_position: 0.05,
  problemtype_classification: 0.05,
  // Optional (caveat only):
  flagged_contradictions_xroom: 0,
  hsi_signals: 0,
});
