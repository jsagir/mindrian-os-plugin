...
function emptyDecisionTrace() {
  return {
    ...
    // Phase 346 (346-07, the decide() attachment): the arbiter's ranked
    // result. Default null so the trace shape is stable across every path
    // (tier_0 / legacy / mode_a / mode_b / fault) and a consumer never reads
    // undefined where null belongs. decide() replaces this with the
    // lib/core/arbitration.cjs resolveArbitration() result, which is
    // closed-enum tokens and numbers only (Canon Part 8). A null value is
    // the honest state for a turn where the resolver faulted. Same
    // null-default precedent as its four siblings above: navigated_neighborhood,
    // projection_offer, brain_pattern_verb, brain_pattern_verb_confidence.
    arbitration: null,
  };
}
