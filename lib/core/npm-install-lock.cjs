 ...
 *
 * A caller on the MCP connect path (Phase 266 MCPFIX-03) passes
 * `opts.timeoutMs` here as the REMAINING SLICE of a process-wide connect
 * budget (Phase 266 Plan 05, MCPFIX-03 gap closure), not a fixed short
 * constant -- the HOST is already counting down its own ~30-second connect
 * timeout while this process waits, and that slice shrinks across the
 * sequence of connect-path heal calls a single MCP entry point makes before
 * answering `initialize`. Sitting for the full default WAIT_TIMEOUT_MS (200s)
 * would guarantee the host gives up first. Returning `false` early in that
 * case is the CORRECT outcome: the caller reports a bounded failure instead
 * of being killed as unresponsive. A caller with no host clock (the
 * SessionStart reconcile hook, via runGuardedInstall with no opts) keeps the
 * original 200-second default.
 *
 * @param {string} dir
 * @param {object} [opts]
 * @param {number} [opts.timeoutMs] - per-call override, the caller's
 *   REMAINING connect-path budget slice on the MCP connect path (Phase 266
 *   Plan 05). Used only when it is a finite number greater than zero;
 *   otherwise falls back to WAIT_TIMEOUT_MS. Does NOT change WAIT_TIMEOUT_MS,
 *   STALE_THRESHOLD_MS or POLL_INTERVAL_MS -- the bug_001 invariant chain
 *   (install timeout < STALE < WAIT) depends on those defaults staying fixed.
