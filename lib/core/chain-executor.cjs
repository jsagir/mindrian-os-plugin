...
function _ralphSafeRetry(params) {
  ...
  const maxRetries = Math.max(0, Math.min(params.cap, params.budgetRemaining));
  let attempts = 0;
  ...
  if (_critiqueFailed(verdict)) quality = LOW_QUALITY;
  return { result: result, quality: quality, attempts: attempts };
}
