...
function runChain(steps, opts) {
  ...
  const chainRunId = _mintChainRunId();
...
async function _runChainResilient(list, o) {
  ...
  for (let i = 0; i < list.length;) {
    ...
    // SKIPPED. The first un-journaled step is the resume re-entry point. This
    // linear resume-skip is unrelated to resolveSuccessor's routing (it walks
    // list order, not a declared route), so it advances i explicitly now that
    // the for-statement no longer auto-increments.
