...
// Fork a bind-worker and resolve when it exits 0.
function runBindWorker(worker, sessionId, roomSlug, ctx) {
  return new Promise((resolve, reject) => {
    const child = require('node:child_process').fork(worker, [sessionId, roomSlug], {
    ...
    child.on('exit', (code) => {
      if (code === 0) resolve();
      else reject(new Error('bind-worker ' + sessionId + ' exited with code ' + code));
    });
  });
}

test('bindSessionRoom: two concurrent forked writers both retain their sessions entry (3x)', async () => {
  ...
  for (let run = 0; run < 3; run++) {
    ...
    try {
      ...
      // Launch both writers concurrently -- they race the same registry.json.
      await Promise.all([
        runBindWorker(worker, idA, 'room-a', ctx),
        runBindWorker(worker, idB, 'room-b', ctx),
      ]);
      const reg = readRegistry(ctx.home);
