...
// ===========================================================================
// LOCKED primary_label priority order (contract of record, 2026-06-01)
// ===========================================================================
// A teaching-graph node may carry several of the 10 methodology labels. The
// primary_label fed to the hash (and emitted in this index) is the
// HIGHEST-priority label the node carries, by this FIXED order. The order is
// LOCKED: the live teaching graph's 721 backfilled correlation_ids were
// computed against this exact priority resolution, so any reorder would fork
// from the live Brain. Lower array index = higher priority.
//
// This is the single source of truth for the order. scripts/backfill-correlation-id.cjs
// REUSES resolvePrimaryLabel from here (Canon Part 7 reuse-before-build) so the
// backfill's primary_label resolution and this index's labels cannot drift.
...
/**
 * resolvePrimaryLabel(allLabels) -> string | null
 *
 * Given the full label set a node carries, return the highest-priority label by
 * the LOCKED LABEL_PRIORITY order. A node carrying no priority-list label falls
 * back to the first non-empty label as-stored (so the node still gets an id; the
 * fork-vs-live risk only exists for nodes whose label set overlaps the 10).
 * The :Archived exclusion is the CALLER's job (scope filter), not this function's.
 *
 * PURE: no fs, no Brain, no network. Tolerant: non-array / empty -> null.
 *
 * @param {Array<string>} allLabels  - every label the node carries
 * @returns {string|null} the resolved primary_label, or null if no usable label
 */
...
module.exports = {
  ...
  resolvePrimaryLabel: resolvePrimaryLabel,
};
