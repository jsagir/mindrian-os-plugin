 ...
 *
 * Phase 347-09 (SHARED-09): additive routing render. A step carries none,
 * some, or all of five OPTIONAL fields beyond the original name/framework/
 * status triple: `on_pass` / `on_fail` (a target step name/id, drawn as a
 * labelled arrow sourced from this step instead of the default unlabelled
 * i -> i+1 successor arrow), `fan_out` (an array of target labels, drawn as
 * a Mermaid subgraph with one arrow from this step into it), `halted` +
 * `halt_reason` (a halt node attached to the step that halted, styled with
 * the existing DS_HEX.red), and `reviewer` (`{ kind }`, a distinctly styled
 * node naming the reviewer kind). A step declaring none of these renders
 * exactly the linear diagram this function has always drawn (the floor
 * every plan since has to keep byte-identical) -- every new line is
 * appended only when its own trigger field is present, never unconditionally.
 * No new color: every fill below is a DS_HEX property already declared
 * above, never a fresh hex literal.
 *
 * @param {Array<{name: string, framework?: string, status?: string,
 *   on_pass?: string|number, on_fail?: string|number,
 *   fan_out?: Array<string|number>, halted?: boolean, halt_reason?: string,
 *   reviewer?: {kind: string}}>} steps
 * @returns {string} Valid Mermaid syntax
 */
function generateMermaidChain(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // Chain arrows: the default i-1 -> i floor, SUPPRESSED only when the
    // predecessor declares its own routing (on_pass/on_fail) -- that
    // predecessor draws its own labelled arrow(s) below instead. A step
    // declaring neither key never suppresses this, so an undeclared chain
    // renders byte-identical to before this plan (the floor Test 1 pins).
    if (i > 0) {
      const prev = steps[i - 1];
      const prevHasRouting = !!(prev && (prev.on_pass !== undefined && prev.on_pass !== null
        || prev.on_fail !== undefined && prev.on_fail !== null));
      if (!prevHasRouting) {
        const prevId = mermaidId(prev.name || `step_${i - 1}`);
        lines.push(`  ${prevId} --> ${id}`);
      }
    }

    // Labelled conditional arrows: a step declaring on_pass and/or on_fail
    // draws one labelled arrow per declared key, sourced from this step.
    if (step.on_pass !== undefined && step.on_pass !== null) {
      lines.push(`  ${id} -->|pass| ${mermaidId(String(step.on_pass))}`);
    }
    if (step.on_fail !== undefined && step.on_fail !== null) {
      lines.push(`  ${id} -->|fail| ${mermaidId(String(step.on_fail))}`);
    }

    // Fan-out subgraph: the declared targets share one input record, drawn
    // as a Mermaid subgraph with one arrow from the coordinator step into it.
    if (Array.isArray(step.fan_out) && step.fan_out.length > 0) {
      const subgraphId = `${id}_fanout`;
      lines.push(`  subgraph ${subgraphId} [Fan-out]`);
      for (const target of step.fan_out) {
        const targetId = mermaidId(String(target));
        lines.push(`    ${targetId}["${target}"]`);
        styles.push(`  style ${targetId} fill:${DS_HEX.muted},color:${DS_HEX.cream},stroke:${DS_HEX.cream}`);
      }
      lines.push('  end');
      lines.push(`  ${id} --> ${subgraphId}`);
    }

    // Halt node: the run's recorded halt, attached to the step it halted on.
    if (step.halted) {
      const haltId = `${id}_halt`;
      const reason = step.halt_reason || 'halted';
      lines.push(`  ${haltId}["Halted: ${reason}"]`);
      styles.push(`  style ${haltId} fill:${DS_HEX.red},color:${DS_HEX.cream},stroke:${DS_HEX.cream}`);
      lines.push(`  ${id} --> ${haltId}`);
    }

    // Reviewer node: distinctly styled (DS_HEX.amethyst, used by no other
    // status), its label names the reviewer kind (navigator or subagent).
    if (step.reviewer && step.reviewer.kind) {
      const reviewerId = `${id}_reviewer`;
      lines.push(`  ${reviewerId}["Reviewer: ${step.reviewer.kind}"]`);
      styles.push(`  style ${reviewerId} fill:${DS_HEX.amethyst},color:${DS_HEX.cream},stroke:${DS_HEX.cream}`);
      lines.push(`  ${id} --> ${reviewerId}`);
    }
  }
