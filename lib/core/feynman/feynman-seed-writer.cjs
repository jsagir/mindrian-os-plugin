...
// Quick task 260917-ild (Codex F4): the default seed sentence, extracted to a
// module-scope constant. FEYNMAN.md has no on-disk template file (unlike
// ROOM/STATE/MINTO/USER), so lib/core/eureka/scaffold-template-index.cjs
// sources its FEYNMAN kind FROM this exported constant instead -- additive
// export, no behaviour change: seedSection below now reads from it rather
// than hand-typing the same sentence inline.
const FEYNMAN_DEFAULT_SEED = 'Plain-language essence pending. Replace this seed with the one-line "explain it to a smart twelve-year-old" message.';
