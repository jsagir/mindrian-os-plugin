...
// Task 1 ships the full resolver disabled at the call site; Task 2 wires the
// no_op / SUPERSEDES branches onto it.
...
async function ingestUrl(roomDir, url, opts) {
  ...
  try {
    ...
    // Ground truth is room.db + the filed frontmatter; the ledger is only a
    // fast path and is rebuilt on the way through. Task 1 ships the filed-only
    // path (prior deliberately unresolved); Task 2 wires the no_op /
    // SUPERSEDES branches onto resolvePriorArtifact against its RED suite.
    const prior = null;
