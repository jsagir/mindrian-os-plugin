...
// A Stage-A-failing candidate returns its gate route/tag IMMEDIATELY with the
// 'xxxxxx' sentinel (the judge is never reached; Stage-A gates are code-certain).
// On pass, the two-pass rubric runs and the verdict is computed by code.
// (Plan 212-02 Task 2 extends the pass path with the calibration-confidence guard.)
