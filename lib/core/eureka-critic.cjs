...
// SURPRISE_TYPES below is an alias of DIRECTIONS, not a second copy.
...
// wire-contract enum, not a reasoning tag. Phase 355 D-07: sourced from the
// one module, not a local copy of the two literal ids.
...
async function classifyCandidate(candidate, opts) {
  ...
  try {
    ...
    if (calibrated && embedderMatches) {
      // Phase 355 D-07: prefer an explicit candidate.surprise_type, then a
      // fresh re-derivation from the candidate's own (lsa, semantic) pair
      // through the one module. SURPRISE_TYPES (sourced from DIRECTIONS
      // above) is two-valued by design (D-04: the critic only ever sees
      // pairs that already passed the generator's floors), so a classify()
      // 'none' (no pair measured at all) falls back to the historical
      // default rather than smuggling a third value onto a closed two-member
      // enum -- a disclosed limitation, not a bug: a candidate with neither
      // an explicit surprise_type nor a measurable similarity pair has
      // nothing honest to report, and this payload has no "unknown" slot.
      const derivedDirection = candidate &&
        directionConvention.classify(candidate.lsa_similarity, candidate.semantic_similarity);
      const payload = assembleCriticPayload({
        ...
        surprise_type: (candidate && candidate.surprise_type) ||
          (derivedDirection && derivedDirection !== directionConvention.NONE ? derivedDirection : 'structural_transfer'),
