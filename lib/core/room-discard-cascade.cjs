...
// break; 200ms retry budget) so the discard write does not race the
...
function discardPlaceholderRoom(roomsHome, slug, opts) {
  ...
  try {
    ...
    // Phase 128.1: the read-modify-write runs under the registry directory lock
    // (it does not race the session-scoped resolver writes), and it also clears
    // every sessions[*] entry still bound to the discarded slug so no session is
    // left pointing at a removed room (D-02/D-03).
    const registryPath = path.join(roomsHome, '.rooms', 'registry.json');
    if (fs.existsSync(registryPath)) {
      const registryWritten = withRegistryLock(registryPath, function () {
        const raw = fs.readFileSync(registryPath, 'utf8');
        const reg = JSON.parse(raw);
        if (reg && typeof reg.rooms === 'object' && reg.rooms[slug]) {
          delete reg.rooms[slug];
          if (reg.active === slug) reg.active = '';
          if (reg.last_active === slug) reg.last_active = '';
          // Drop every session binding that pointed at the discarded room.
          if (reg.sessions && typeof reg.sessions === 'object') {
            for (const sid of Object.keys(reg.sessions)) {
              const entry = reg.sessions[sid];
              if (entry && entry.room === slug) delete reg.sessions[sid];
            }
          }
        ...
        if (reg && typeof reg.rooms === 'object') {
