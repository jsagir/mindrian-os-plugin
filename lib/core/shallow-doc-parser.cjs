...
//
// Decision D-354-SYS05 (354-CONTEXT.md): the MCP extract_shallow tool
// (lib/mcp/tools/dual-path.cjs) calls extractShallow(text, sessionId) with
// no third opts argument BY DESIGN, not by omission -- both its documented
// callers run before a room exists or is bound, so setFocus (which only
// fires when opts.db is supplied, see the guard below) and this
// memory_event call are structural no-ops on that path, not a bug.
// Persistence is claim_write's job (writeClaimNode, review_status
// proposed) once a room is bound and the navigator confirms a claim.
...
// if no db handle is available (pre-room, MCP extract_shallow path -- the
// pure-parse contract of decision D-354-SYS05), call extractDomains to get
// handles and surface them as text ('Your domains: X, Y, Z -- sweep will
// run at birth') without filing claims.
