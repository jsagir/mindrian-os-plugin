...
// The free-form tool-input keys a brain_ask / brain_query call carries a raw
// string on (not a typed packet). brain_ask -> { question }, brain_query ->
// { cypher } (and the Plurai fixture uses { brain_query_payload }).
 ...
 *
 * Pull the raw outbound string a free-form brain_ask / brain_query call carries
 * (question / cypher / brain_query_payload). Top-level only: an opaque
 * tool_input blob with no free-form key yields null (-> ambiguous, not allow).
 */
 ...
 *   2. _isFreeFormTool is NOT broadened. brain_search stays out of it
 *      deliberately (see .planning/debug/245-part8-contentless-block.md, the
 *      D-28 FLAGGED disposition): a search string IS real user content, so its
 *      block may well be correct and widening it is a real egress-surface
 *      change that needs its own navigator decision.
 ...
 *
 * brain_ask / brain_query send a raw string, not a typed packet.
 */
 ...
 *   3. A free-form brain_ask / brain_query string that clears default-deny AND
 *      carries generic methodology vocabulary -> allow; else ambiguous.
