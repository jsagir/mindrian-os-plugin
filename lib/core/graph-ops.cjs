...
// Promise-chain write queue: serializes all KuzuDB writes.
// Read operations bypass this queue (KuzuDB allows concurrent reads).
...
/**
 * Build graph.json from KuzuDB as primary source.
 * Falls back to file-scanning buildGraph if KuzuDB fails.
 * @param {string} roomDir - Path to room directory
...
function buildGraphFromKuzu(roomDir, outputPath) {
  ...
  const scriptPath = path.join(SCRIPTS_DIR, 'build-graph-from-kuzu.cjs');
  ...
  try {
    ...
    if (fs.existsSync(resolvedOutput)) {
      return { success: true, outputPath: resolvedOutput, source: 'kuzudb' };
    }
  } catch (_e) {
...
/**
 * Persist a decision edge (INVALIDATES/CONFIRMS/DEFERRED) to KuzuDB.
...
module.exports = { buildGraph, buildGraphFromKuzu, indexArtifact, rebuildGraph, queryGraph, graphStats, enqueueWrite, persistDecisionEdge };
