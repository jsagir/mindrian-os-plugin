 ...
 *     NODE, never on the edge -- Part 9 Pitfall 1) plus a typed EDGE via
 *     navigation.writeEdge (cascade edge_type from the frozen set, enum/scalar
 *     properties only). An unjustified candidate (fable-mode {passed:false}) is
...
function runDerivation(args) {
  ...
  // artifactPair). The producer may return a Promise (the default async
  // transport) or an array (the deterministic stub); both are accepted.
  ...
  try {
    ...
    for (let s = 0; s < stepInputs.length; s += 1) {
      ...
      for (const candidate of candidates) {
        ...
        let nodeId = 'claim:' + sessionId + ':' + finding.node_seed;
        ...
        //    no raw edge SQL). The edge carries enum/scalar properties
        //    ONLY -- NEVER review_status (Pitfall 1).
        let edgeOk = true;
        if (db) {
