...
/*
 * OWNERSHIP OF reg.active (RCA registry-active-session-unbound-inheritance).
 *
 * reg.active is a single machine-wide slug. An UNBOUND session inheriting it is
 * correct for the tier-0 single-session user (Decision #8) and WRONG when a
 * different, concurrently-live session set it seconds ago. A bare "does
 * active_session equal my session id" test cannot separate those two: a fresh
 * unbound session never wrote `active`, so the match is always false and the
 * check silently degrades into "never inherit" -- which kills the card for every
 * user who never binds.
 *
 * The separating signal is LIVENESS OF THE SETTER, and it needs no new
 * infrastructure. The only writer of `reg.active` (scripts/room-registry
 * set-active / create) runs inside the owning session's own process tree, so it
 * already has both halves in its environment: CLAUDE_CODE_SESSION_ID (the same
 * UUID the hook reader resolves) and CLAUDE_PID (the `claude` CLI process, which
 * lives exactly as long as the session). Recording them beside `active` makes
 * registry.json its own liveness record, and the reader gets a THREE-way answer:
 *
 *   unowned      -- no active_session field (a pre-upgrade registry, or a
 *                   set-active run outside a session). Today's behavior.
 *   self         -- I set it. Inherit.
 *   stale        -- someone else set it, and that process is gone. Inherit.
 *                   This is the tier-0 restart case: quit Claude Code, reopen,
 *                   new unbound session, old pid dead -> inherit exactly as before.
 *   foreign-live -- someone else set it AND their process is still running.
 *                   Do NOT inherit. This is the bleed.
 *
 * Every uncertain path resolves to inheritable:true, so the tier-0 promise holds
 * by construction: no session id, no ownership fields, no pid, an unprobeable
 * pid, an unparseable timestamp, or any thrown error all reproduce today's
 * behavior exactly. Only a positively-proven live foreign owner declines.
 */

// Fail-open ceiling on the pid probe, NOT a liveness window. A pid can be
// recycled by the OS; beyond this age the probe is no longer trustworthy
// evidence of the ORIGINAL process, so ownership degrades to `stale` and the
// reader inherits (today's behavior). Deliberately generous: real sessions run
// for many hours, and a long-running owner must keep being recognised as live.
// This is NOT a frozen-family scalar (MAX_K / DIAL_REACH_K / 0.70 / 0.15 are
// untouched).
const OWNERSHIP_MAX_AGE_MS = 86400000; // 24h

/**
 * ownerProcessAlive(pid) -> boolean. Composes session-presence.cjs's shipped
 * `isAlive` (process.kill(pid, 0), itself cloned from write-lock.cjs) rather
 * than minting a fifth copy of the probe (Canon Part 7, the SEED-034
 * four-guessers lesson). Only that ONE primitive is reused; the module's
 * per-room presence LEDGER stays out of this path (it is per-room, so it could
 * never answer "is the session that set reg.active still live").
 *
 * A probe that cannot run returns FALSE, which routes to `stale` ->
 * inheritable -> today's behavior. Never throws.
 *
...
function ownerProcessAlive(pid) {
  ...
  try {
    ...
    return presence.isAlive(pid) === true;
  } catch (_e) {
    // Cannot probe -> fail OPEN (not live -> inheritable).
    return false;
  }
}

/**
 * resolveActiveOwnership -- who set reg.active, and are they still here?
 *
 * Returns { state, owner, pid, at, inheritable }. `state` is one of
 * 'unowned' | 'override' | 'anonymous-reader' | 'self' | 'stale' | 'foreign-live'.
 * `inheritable` is false ONLY for 'foreign-live'. NEVER throws.
 *
 * Canon Part 8/9: reads registry.json and probes a local pid. Zero network,
 * zero graph surface.
 *
 * @param {{sessionId?: string, home?: string, reg?: object, now?: number}} [opts]
 * @returns {{state: string, owner: string|null, pid: number|null, at: string|null, inheritable: boolean}}
 */
function resolveActiveOwnership(opts) {
  const unowned = { state: 'unowned', owner: null, pid: null, at: null, inheritable: true };
  try {
    const o = opts || {};
    ...
    // An explicit CLAUDE_ACTIVE_ROOM override outranks the registry in
    // resolveActiveRoom, so it must outrank ownership too: an operator or a
    // hermetic test that pins the room is never second-guessed.
    if (process.env.CLAUDE_ACTIVE_ROOM) {
      const v = String(process.env.CLAUDE_ACTIVE_ROOM).trim();
      if (v.length > 0 && fs.existsSync(v)) {
        return { state: 'override', owner: null, pid: null, at: null, inheritable: true };
      }
    }

    const reg = (o.reg && typeof o.reg === 'object') ? o.reg : readRegistry(home);
    if (!reg) return unowned;

    const owner = (typeof reg.active_session === 'string' && reg.active_session.length > 0)
      ? reg.active_session
      : null;
    // No ownership recorded: a pre-upgrade registry, or a set-active run outside
    // any session. Byte-identical to pre-fix behavior (the backward-compat floor).
    if (!owner) return unowned;

    const rawPid = reg.active_session_pid;
    let pid = null;
    if (Number.isInteger(rawPid) && rawPid > 0) {
      pid = rawPid;
    } else if (typeof rawPid === 'string' && /^[0-9]+$/.test(rawPid)) {
      const parsed = parseInt(rawPid, 10);
      if (Number.isInteger(parsed) && parsed > 0) pid = parsed;
    }
    const at = (typeof reg.active_session_at === 'string' && reg.active_session_at.length > 0)
      ? reg.active_session_at
      : null;

    const sessionId = (typeof o.sessionId === 'string' && o.sessionId.length > 0)
      ? o.sessionId
      : null;
    // A reader with no identity cannot claim the value is foreign. Fail open.
    if (!sessionId) {
      return { state: 'anonymous-reader', owner: owner, pid: pid, at: at, inheritable: true };
    }
    if (sessionId === owner) {
      return { state: 'self', owner: owner, pid: pid, at: at, inheritable: true };
    }

    // Pid-recycle ceiling (see OWNERSHIP_MAX_AGE_MS).
    const nowMs = Number.isFinite(o.now) ? o.now : Date.now();
    const atMs = at ? Date.parse(at) : NaN;
    if (Number.isFinite(atMs) && (nowMs - atMs) > OWNERSHIP_MAX_AGE_MS) {
      return { state: 'stale', owner: owner, pid: pid, at: at, inheritable: true };
    }

    if (!ownerProcessAlive(pid)) {
      return { state: 'stale', owner: owner, pid: pid, at: at, inheritable: true };
    }
    return { state: 'foreign-live', owner: owner, pid: pid, at: at, inheritable: false };
  } catch (_e) {
    // Defense in depth: an ownership fault must never cost a session its room.
    return unowned;
  }
}
 ...
 *
 * OPT-IN ownership gate (RCA registry-active-session-unbound-inheritance): pass
 * `requireOwnership: true` and leg B additionally refuses to inherit a reg.active
 * that a DIFFERENT, still-live session set (see resolveActiveOwnership). It
 * defaults FALSE on purpose. resolveWriteRoom and the 9 MCP tool modules keep
 * calling this with the flag OFF, so their behavior is byte-identical and the
 * write path carries zero blast radius; only the F.1 reach-card consumer -- the
 * site the RCA identifies as THE bleed (card header_room + decision-trace write
 * location) -- opts in. A caller that opts in MUST treat a null return as a
 * deliberate DECLINE and must not fall back to the machine-wide value, or the
 * gate is a no-op.
 *
 ...
 *
 * @param {{sessionId?: string, home?: string, requireOwnership?: boolean}} [opts]
 * @returns {{slug: string, abs_path: string, source: string}|null}
 */
function resolveSessionRoom(opts) {
  ...
  try {
    // Opt-in ownership gate. Reached ONLY for a session with no usable
    // session.primary, i.e. exactly the unbound case this gate exists for. A
    // bound session never gets here, so binding always outranks ownership.
    if (o.requireOwnership === true) {
      const ownership = resolveActiveOwnership({ home: home, sessionId: sessionId });
      if (ownership && ownership.inheritable === false) return null;
    }
