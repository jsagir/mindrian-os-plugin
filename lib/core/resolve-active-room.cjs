...
function resolveWriteRoom(opts) {
  ...
  // Leg 2: session.primary, but only when the room exists on disk. A primary
  ...
  try {
    ...
    const primary = (binding && typeof binding.primary === 'string' && binding.primary.length > 0)
      ...
      : null;
    if (primary) {
      const absPath = path.join(home, primary);
      if (fs.existsSync(absPath)) {
        return { slug: primary, abs_path: absPath, source: 'session.primary' };
      }
    }
  } catch (_e) { /* leg 2 failed -- fall through to the demoted reg.active */ }

  // Leg 3: reg.active DEMOTED to the fresh-session seed-default (PSB-15).
  try {
    const active = resolveActiveRoom({ home: home });
    if (active) {
      return { slug: active.slug, abs_path: active.abs_path, source: 'reg.active' };
    }
  } catch (_e) { /* leg 3 failed -- total miss */ }
...
module.exports = { resolveActiveRoom, resolveActiveRoomSlug, resolveActiveRoomDir, resolveWriteRoom, resolveSessionScope };
