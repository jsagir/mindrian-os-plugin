...
function absDiffTopk(lsaMatrix, semanticMatrix, opts) {
  ...
  const absDiff = [];
  for (let i = 0; i < n; i += 1) {
    signedDiff.push(new Array(n));
    absDiff.push(new Array(n));
  }
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n; j += 1) {
      const sd = semanticMatrix[i][j] - lsaMatrix[i][j];
      signedDiff[i][j] = sd;
      absDiff[i][j] = Math.abs(sd);
    }
  }

  if (skipDiagonal) {
    for (let i = 0; i < n; i += 1) absDiff[i][i] = -Infinity;
  }

  // Upper-triangle only: mask the lower triangle so argmax never returns
  // it (the symmetric matrix means (i,j) == (j,i); the mirror is
  // suppressed after each pick below too, matching the Python's
  // precomputed-mask-plus-post-pick-cleanup approach).
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < i; j += 1) absDiff[i][j] = -Infinity;
  }
  ...
  for (let iter = 0; iter < k; iter += 1) {
    ...
    if (maxVal === -Infinity || maxVal <= 0.0) break;
    results.push({ i, j, signedDiff: signedDiff[i][j], absDiff: maxVal });
    // Symmetric cleanup: suppress both (i,j) and (j,i) so the mirror never
    // wins next iteration.
    absDiff[i][j] = -Infinity;
    absDiff[j][i] = -Infinity;
  }
 ...
 * (classify_direction), Convention A, exactly.
 *
 ...
 * deliberately bucketed with semantic_implementation.
 */
