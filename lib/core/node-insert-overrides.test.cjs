 ...
 *
 * R17-02 note: every insertNode call below supplies a valid `epistemic_type`
 * (required, no default, as of Task 5) even where epistemic_type itself is
 * not the behavior under test in THIS test, so the review_status/on_conflict
 * assertions exercise the code path they intend rather than short-circuiting
 * on the epistemic_type gate, which runs first. Dedicated epistemic_type
 * coverage lives in lib/core/node-insert-epistemic.test.cjs.
 *
...
test('overrides A: opts.confidence is a number -> bound confidence column', () => {
  ...
  try {
    ...
    insertNode(db, 'n1', 'Section', '{}', { confidence: 0.85, epistemic_type: 'observation' });
...
test('overrides B: opts.confidence absent -> confidence lands NULL', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n2', 'Section', '{}', { epistemic_type: 'observation' });
...
test('overrides C: opts.review_status in enum -> bound review_status column', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n3', 'HatState', '{}', { review_status: 'confirmed', epistemic_type: 'observation' });
...
test('overrides D: opts.review_status absent -> column DEFAULT proposed', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n4', 'HatState', '{}', { epistemic_type: 'observation' });
...
test('overrides E: opts.review_status out of enum THROWS before prepare()', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    assert.throws(() => {
      insertNode(db, 'n5', 'HatState', '{}', { review_status: 'made-up-status', epistemic_type: 'observation' });
...
test('overrides F: opts.on_conflict nothing -> existing row left byte-identical', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n6', 'Opportunity', '{"v":1}', { on_conflict: 'nothing', epistemic_type: 'observation' });
    insertNode(db, 'n6', 'Opportunity', '{"v":2}', { on_conflict: 'nothing', epistemic_type: 'observation' });
    const row = db.prepare('SELECT properties FROM nodes WHERE id = ?').get('n6');
    assert.strictEqual(row.properties, '{"v":1,"epistemic_type":"observation"}');
  } finally {
    if (db) closeRoomDb(db);
    rmTmpRoom(roomDir);
  }
...
test('overrides G: opts.on_conflict absent or update -> existing DO UPDATE unchanged', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n7', 'Opportunity', '{"v":1}', { epistemic_type: 'observation' });
    insertNode(db, 'n7', 'Opportunity', '{"v":2}', { on_conflict: 'update', epistemic_type: 'observation' });
    const row = db.prepare('SELECT properties FROM nodes WHERE id = ?').get('n7');
    assert.strictEqual(row.properties, '{"v":2,"epistemic_type":"observation"}');
  } finally {
    if (db) closeRoomDb(db);
    rmTmpRoom(roomDir);
  }
...
test('overrides H: invalid on_conflict value THROWS', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    assert.throws(() => {
      insertNode(db, 'n8', 'Opportunity', '{}', { on_conflict: 'delete-everything', epistemic_type: 'observation' });
    }, /invalid on_conflict/);
  } finally {
    if (db) closeRoomDb(db);
    rmTmpRoom(roomDir);
...
test('overrides I: re-inserting an existing id preserves review_status and type (no-downgrade contract)', () => {
  ...
  try {
    db = openRoomDb(roomDir);
    insertNode(db, 'n9', 'decision', '{"v":1}', { review_status: 'confirmed', confidence: 1.0, epistemic_type: 'decision' });
    // A later system re-projection with no review_status override must NOT
    // downgrade the already-confirmed row.
    insertNode(db, 'n9', 'decision', '{"v":2}', { epistemic_type: 'decision' });
    ...
    assert.strictEqual(row.properties, '{"v":2,"epistemic_type":"decision"}');
  } finally {
    if (db) closeRoomDb(db);
    rmTmpRoom(roomDir);
  }
...
test('overrides legacy B: invalid review_status still THROWS on legacy schema (fail closed everywhere)', () => {
  ...
  try {
    db = makeUnmigratedDb(roomDir);
    assert.throws(() => {
      insertNode(db, 'l2', 'Section', '{}', { review_status: 'nonsense', epistemic_type: 'observation' });
    }, /invalid review_status/);
  } finally {
    if (db) {
      try { db.close(); } catch (_e) { /* ignore */ }
...
test('overrides legacy C: on_conflict nothing honored on legacy schema', () => {
  ...
  try {
    db = makeUnmigratedDb(roomDir);
    insertNode(db, 'l3', 'Section', '{"v":1}', { on_conflict: 'nothing', epistemic_type: 'observation' });
    insertNode(db, 'l3', 'Section', '{"v":2}', { on_conflict: 'nothing', epistemic_type: 'observation' });
...
test('overrides legacy D: legacy branch stays exactly as before for the plain call (no overrides beyond epistemic_type)', () => {
  const roomDir = makeTmpRoom();
  let db;
  try {
    db = makeUnmigratedDb(roomDir);
    insertNode(db, 'l4', 'Section', JSON.stringify({ name: 'l4', label: 'OLD' }), { epistemic_type: 'observation' });
    insertNode(db, 'l4', 'Section', JSON.stringify({ name: 'l4', label: 'NEW' }), { epistemic_type: 'observation' });
