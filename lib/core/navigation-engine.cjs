...
// Phase 177-09 (BCH-07): the SEAM 3 behavioral-channel kill-switch. The modifier
// reads BEHAVIORAL_CHANNEL_ARMED (default LOCKED, owned by calibration-gate.cjs,
// shipped in 177-08) via the require idiom -- NEVER a re-typed `false` literal.
// When the flag is false (ALWAYS today, because no real calibration PASS has
// armed it) the modifier returns null UNCONDITIONALLY and the legacy path runs
// byte-identical. calibration-gate.cjs is a pure-local module (no Brain, no
// network), so this require adds ZERO Brain egress surface.
const { BEHAVIORAL_CHANNEL_ARMED } = require('./navigation/calibration-gate.cjs');
...
function resolveFireSkill(brain, weightApplied, tierMode, sensorReaches) {
  ...
  if (Array.isArray(sensorReaches) && sensorReaches.length > 0) {
    ...
    const cue = (top && typeof top === 'object')
      ...
      : null;
    const seam = behavioralChannelModifier(top, cue, BEHAVIORAL_CHANNEL_ARMED);
    if (seam !== null) {
      return seam.verb;
    }
  }
