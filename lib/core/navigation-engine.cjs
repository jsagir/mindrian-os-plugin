...
// Phase 343 Plan 06: the room-graph integrity ctx-assembly producer. Runs the
// SAME shared statement home the doctor organ uses (never a second copy) over
// a caller-threaded room.db handle at ctx-assembly time and threads the
// closed graph-integrity scalars onto sensorCtx (mirroring the SENS-11 and
// SENS-16 producers above), so the registered sensorGraphIntegrity can fire
// the contradiction reach. The db read is the producer's job; the sensor
// stays pure. The engine never OPENS room.db -- it uses the caller-threaded
// handle.
const { countGraphIntegrity } = require('./navigation/graph-integrity-counts.cjs');
// Phase 345-05: the strategy-node ctx-assembly producer (SENS-20). Reads the
// cadence/stall counters (lib/core/strategy/goal-cadence.cjs) and the
// three-mechanism cool-down (lib/core/strategy/strategy-throttle.cjs) over a
// caller-threaded room.db handle at ctx-assembly time and threads the closed
// scalars onto sensorCtx (mirroring the SENS-11/SENS-16/SENS-19 producers
// above), so the registered sensorStrategyReach can fire the contradiction
// reach. A plain top-level require here is SAFE (unlike sensor-strategy-
// reach.cjs's own require of goal-cadence.cjs, fixed this same plan by
// lazy-requiring navigation.cjs INSIDE goal-cadence.cjs): this module already
// requires navigation.cjs directly two lines above, so it is already the far
// end of that dependency graph, not a new entry point that could close a
// cycle back onto itself. The db read is the producer's job; the sensor
// stays pure. The engine never OPENS room.db -- it uses the caller-threaded
// handle.
const goalCadence = require('./strategy/goal-cadence.cjs');
const strategyThrottle = require('./strategy/strategy-throttle.cjs');
...
/**
 * Phase 346 (346-07, the decide() attachment): apply the arbiter's ranked
 * result onto an ALREADY-COMPUTED decision + trace, in place. Mirrors
 * applyProjectionLift's own discipline word for word in spirit: nothing is
 * added here, only the existing object's own fields change, and this
 * function writes exactly ONE pre-declared trace field (trace.arbitration)
 * plus one appended rationale clause.
 *
 * A null, non-object or array arbitrationResult (a caught resolver fault,
 * R7, or a malformed input) is a FULL no-op: the decision and trace stay
 * exactly what the pre-346 engine produced.
 *
 * This function must NEVER set decision.fire_skill, decision.offer_next_step
 * or decision.suppress_skills, and it re-ranks no sensor reach (it has no
 * reach-list parameter to re-rank in the first place): it reports a
 * posture, and the existing fire_skill ladder still decides what fires.
 * tests/test-346-no-second-brain.cjs and this file's own non-interference
 * sweep in tests/test-346-decide-attachment.cjs both pin this fence.
 */
function applyArbitration(decision, trace, arbitrationResult) {
  if (!arbitrationResult || typeof arbitrationResult !== 'object' || Array.isArray(arbitrationResult)) return;
  trace.arbitration = arbitrationResult;
  // Part 8: name the three axis values ONLY -- closed-enum tokens, never
  // prose, mirroring the sensor-reach rationale clause's own phrasing
  // discipline ('Sensor reach fired: ' + reach_id + '/' + posture + '.').
  // Guard each read so a malformed result yields the literal 'unknown'
  // rather than 'undefined' in the string, matching applyProjectionLift's
  // own 'unknown' fallbacks.
  const enforcementVal = (arbitrationResult.enforcement && typeof arbitrationResult.enforcement.value === 'string')
    ? arbitrationResult.enforcement.value : 'unknown';
  const deliveryVal = (arbitrationResult.delivery && typeof arbitrationResult.delivery.value === 'string')
    ? arbitrationResult.delivery.value : 'unknown';
  const autonomyVal = (arbitrationResult.autonomy && typeof arbitrationResult.autonomy.value === 'string')
    ? arbitrationResult.autonomy.value : 'unknown';
  const existing = trace.chosen_rationale || '';
  const sep = existing.length > 0 ? ' ' : '';
  trace.chosen_rationale = existing + sep +
    'Arbitration: ' + enforcementVal + '/' + deliveryVal + '/' + autonomyVal + '.';
  // This function must NEVER set decision.fire_skill, decision.offer_next_step
  // or decision.suppress_skills here, and it never re-ranks sensorReaches or
  // any other reach list: it reports a posture, the existing ladder still
  // decides what fires.
}
...
function decide(turn, context) {
  ...
  // Phase 353 Plan 02 Task 8 producer (D-353-3, R-353-H, R-353-N): fill
  // ctx.tierCandidates from the shipped section-command-ledger, feeding
  // f-selector-ranker.cjs::rankForSelector's EXISTING tierCandidates seam
  // (threaded through by lib/core/navigation-engine-offer.cjs::resolveOffer,
  // unchanged otherwise). Mirrors the SENS-16 producer's shape exactly: a
  // braced block, ctx-assembly-time read, caller-threaded ctx.tierCandidates
  // wins as the test seam, try/catch soft-fails to a no-op, and this
  // comment states the measured cost against the 1200ms NAV budget: the
  // producer's own wall time was measured at well under 1ms on the fixture
  // rooms (a single cached JSON read plus a small in-memory filter/sort over
  // at most a few dozen rows); see tests/test-353-decide-budget.cjs for the
  // exact numbers this comment cites at review time.
  //
  // Activation today: this block only fires when the caller supplies
  // ctx.sectionJobId directly, or ctx.section (a section slug resolved
  // through section-registry.cjs::getSectionJob). decide() itself carries
  // no session/focus handle today, so deriving a job id from the bound
  // room's OWN active focus (the richer signal lib/core/navigation/
  // section-gate.cjs::resolveFocusSection already reads for the filing
  // gate) is a natural next step for a future caller to wire, not
  ...
  if (!Array.isArray(ctx.tierCandidates)) {
    try {
      ...
      let jobId = (typeof ctx.sectionJobId === 'string' && ctx.sectionJobId) ? ctx.sectionJobId : null;
      if (!jobId && typeof ctx.section === 'string' && ctx.section) {
        // eslint-disable-next-line global-require
        const sectionRegistry = require('./section-registry.cjs');
        const canonRow = sectionRegistry.getSectionJob(ctx.section);
        jobId = (canonRow && canonRow.job_id) ? canonRow.job_id : null;
      }
      if (jobId) {
        const ledgerEntry = sectionRulingCandidates.buildLedgerCandidates({
          jobId: jobId,
          problemType: sensorTuple.problem_type,
          stage: sensorTuple.stage,
          decisionTraces: Array.isArray(ctx.recentCommands) ? ctx.recentCommands : [],
        });
        if (ledgerEntry) {
          ctx.tierCandidates = ledgerEntry;
        }
      }
    } catch (_e) {
      // soft-fail: decide() behaves byte-identically to today (sensor order)
    }
  }
  // Phase 343 Plan 06 producer (SENS-19, CENSUS-10/CENSUS-11): derive the
  // graph-integrity scalars sensorGraphIntegrity reads (ctx.graph_integrity)
  // from lib/core/navigation/graph-integrity-counts.cjs::countGraphIntegrity,
  // run on a caller-threaded room.db handle, mirroring the SENS-11 and
  // SENS-16 producers above.
  //
  // FOUR THINGS THIS COMMENT MUST STATE, because this is the place with no
  // gate (WD-6/WD-19, docs/343-ROOM-GRAPH-CENSUS-DECISIONS.md):
  //   1. The db read runs HERE, at ctx-assembly, NEVER inside the pure
  //      sensor (Canon Part 8 and Part 9).
  //   2. The engine does not OPEN room.db. It uses the caller-threaded
  //      handle (ctx.roomDb) when present, exactly as the SENS-11 and
  //      SENS-16 producers do.
  //   3. The measurement is the BOUND ROOM ONLY (WD-5): the engine holds one
  //      caller-threaded handle and has no registry reach at all, so the
  //      fleet census stays the doctor organ's job.
  //   4. NOTHING CHECKS THAT THIS BLOCK EXISTS. Removing it makes SENS-19
  //      register, rank, and stamp while it never fires, with no error
  //      anywhere -- the exact failure the end-to-end test in
  //      tests/test-343-sensor-registration.cjs exists to catch.
  //
  // Caller-threaded ctx.graph_integrity overrides win (the test seam that
  // lets a unit test fire the sensor with no db and no fixture at all).
  // Defensive; never throws.
  {
    ...
    if (roomDb) {
      try {
        const counts = countGraphIntegrity(roomDb);
        graphIntegrity = {
          edge_rows_missing_endpoint: counts.edge_rows_missing_endpoint,
          claim_nodes_no_anchor_new: counts.claim_nodes_no_anchor_new,
          claim_nodes_no_anchor_legacy: counts.claim_nodes_no_anchor_legacy,
          edge_rows_type_outside_allowlist: counts.edge_rows_type_outside_allowlist,
          proposed_nodes_past_window: counts.proposed_nodes_past_window,
          schema_variant: counts.schema_variant,
        };
      } catch (_e) {
        // soft-fail: degrade to no graph-integrity signal
        graphIntegrity = null;
      }
    }
    sensorCtx.graph_integrity =
      (ctx.graph_integrity && typeof ctx.graph_integrity === 'object')
        ? ctx.graph_integrity
        : graphIntegrity;
  }
  // Phase 345 producer (SENS-20): derive the cadence, stall and cool-down
  // scalars sensorStrategyReach reads. The db read runs HERE (ctx-assembly),
  // NEVER inside the pure sensor (Part 8/9). The engine does not OPEN
  // room.db; it uses the caller-threaded handle (ctx.roomDb) when present,
  // exactly as the SENS-11/SENS-16/SENS-19 producers above. The LOCAL
  // candidate node-id list (from goal-cadence.cjs::computeStrategyCounts) is
  // threaded onto sensorCtx for the gate handler ONLY -- it is never ridden
  // on the reach. Caller-threaded ctx.* scalar overrides win (the test seam
  // that lets a unit test fire the sensor with no db and no fixture at all).
  // Defensive; never throws.
  {
    let reachesSince = 0;
    let claimsSince = 0;
    let promotionsSince = 0;
    let artifactsSince = 0;
    // No shipped counter measures "unresolved contradictions" as a
    // since-last-proposal scalar (unlike reaches/claims/promotions/
    // artifacts, all backed by goal-cadence.cjs::computeStrategyCounts).
    // Honestly 0 by default, not a computed reading -- mirrors
    // readStallSignal's own null-default discipline for a quantity nothing
    // has measured, except this field's own numOr0 contract in the sensor
    // (sensor-strategy-reach.cjs) reads 0, never null, so the local default
    // here is 0, not null.
    const unresolvedContradictionsLocal = 0;
    let strategyCooldownActive = false;
    let strategyStallCountLocal = null;
    const roomDb = (ctx.roomDb && typeof ctx.roomDb.prepare === 'function') ? ctx.roomDb : null;
    if (roomDb) {
      try {
        const since = goalCadence.readLastProposalEpochMs(roomDb);
        const counts = goalCadence.computeStrategyCounts(roomDb, since);
        reachesSince = counts.reaches_since;
        claimsSince = counts.claims_since;
        promotionsSince = counts.promotions_since;
        artifactsSince = counts.artifacts_since;
        ...
        const throttle = strategyThrottle.isThrottled(roomDb, ctx.roomState, counts);
        strategyCooldownActive = !!(throttle && throttle.throttled === true);
        const stall = goalCadence.readStallSignal(roomDb, ctx.roomState);
        strategyStallCountLocal = (stall && typeof stall.stall_count === 'number') ? stall.stall_count : null;
      } catch (_e) {
    ...
    sensorCtx.promotionsSinceLastProposal =
      (typeof ctx.promotionsSinceLastProposal === 'number' && ctx.promotionsSinceLastProposal >= 0)
        ? ctx.promotionsSinceLastProposal : promotionsSince;
    sensorCtx.artifactsSinceLastProposal =
      (typeof ctx.artifactsSinceLastProposal === 'number' && ctx.artifactsSinceLastProposal >= 0)
        ? ctx.artifactsSinceLastProposal : artifactsSince;
    sensorCtx.unresolvedContradictions =
      (typeof ctx.unresolvedContradictions === 'number' && ctx.unresolvedContradictions >= 0)
        ? ctx.unresolvedContradictions : unresolvedContradictionsLocal;
    sensorCtx.strategyCooldownActive = (ctx.strategyCooldownActive === true) || strategyCooldownActive;
    // strategyStallCount must preserve null (Phase 346's named input,
    // STRAT-09): typeof-guarded to 'number', with the LOCAL fallback
    // defaulting to null, NEVER to 0 -- 0 would assert "measured, zero
    // stall" for a quantity nothing has computed.
    sensorCtx.strategyStallCount =
      typeof ctx.strategyStallCount === 'number' ? ctx.strategyStallCount : strategyStallCountLocal;
  ...
  // Phase 355 Plan 22 (HIPS-06, D-42): the surfacing seam records a fire-once
  // mark for the winning SENS-13 (eureka_bridge) reach, the moment it is
  // actually surfaced here -- the sensor itself stays pure (355-19 already
  // shipped the read-only dedup check that consumes this mark); this is the
  // ONE seam that decides surfacing, so it is the ONE seam that records it.
  // Fires only when ctx.roomDir resolves and the winning reach carries a
  // non-empty evidence.opportunity_handle. Lazy-required (never a load-time
  // dependency) and soft-fail (a ledger write fault must never break
  // decide()): no other side effect. Phase 355.1 extends this same mark to
  // the hookless surfaces later, calling this SAME named writer.
  if (typeof ctx.roomDir === 'string' && ctx.roomDir
      && sensorReaches[0] && typeof sensorReaches[0] === 'object'
      && sensorReaches[0].signal === 'eureka_bridge'
      && sensorReaches[0].evidence && typeof sensorReaches[0].evidence === 'object'
      && typeof sensorReaches[0].evidence.opportunity_handle === 'string'
      && sensorReaches[0].evidence.opportunity_handle) {
    try {
      const eurekaReachRunner = require('./eureka/eureka-reach-runner.cjs');
      eurekaReachRunner.markEurekaReachSurfaced(ctx.roomDir, sensorReaches[0].evidence.opportunity_handle);
    } catch (_e) {
      // soft-fail: a ledger write fault must never break decide()
    }
  }
  ...
  // Phase 346 (346-07, the decide() attachment): compute the arbitration
  // ONCE, before any return path, at the same nesting level as liftResult
  // above (mirrors its shape verbatim: computed once, try/catch degrading
  // to null, applied on both return paths via applyArbitration below).
  //
  // Ctx-assembly rule (346-RESEARCH.md, mirroring the MED-01 cortex producer
  // comment above at this function's sensorCtx block): the reads that
  // produce ctx.arbitration_inputs -- USER.md role_blend, jtbd-state, the
  // problem-type rung, the escape-hatch detection over the user's text, the
  // surface capabilities, the stall count once Phase 345 ships one -- run at
  // ctx-assembly, in the CALLER, NEVER here and NEVER inside the pure
  // resolver (lib/core/arbitration.cjs). The engine does not open room.db,
  // does not read USER.md and does not see the user's text for this
  // purpose; it consumes a caller-threaded plain object, exactly as it
  // consumes ctx.roomDb. stall_count is null until Phase 345 ships a
  // producer (WD-6); the resolver treats null as no signal, never a
  // fabricated zero.
  let arbitrationResult = null;
  try {
    const arbitrationModule = require('./arbitration.cjs');
    arbitrationResult = arbitrationModule.resolveArbitration(ctx.arbitration_inputs || {});
  } catch (_e) {
    // R7 fault-safety: a resolver fault degrades to null, which
    // applyArbitration() treats as a full no-op (pre-346 decision + trace
    // byte-identical).
  ...
  try {
    ...
    if (!quadruple) {
      ...
      // Phase 346 (346-07): the tier_0 early-return path rides the same
      // arbitration wire as the main path. A fault-safe no-op when
      // arbitrationResult is null (R7).
      applyArbitration(decision, trace, arbitrationResult);
    ...
    // Phase 346 (346-07): the main-path arbitration application. A fault-safe
    // no-op when arbitrationResult is null (R7). This function reports a
    // posture only -- it never touches fire_skill/offer_next_step/
    // suppress_skills, and it re-ranks no sensor reach.
    applyArbitration(decision, trace, arbitrationResult);
...
module.exports = {
  ...
  // Phase 346 (346-07): exposed for direct unit testing (the null/non-object/
  // array no-op proof and the 60-case non-interference sweep), following the
  // same exposed-for-testing convention as reachIdToSkillFamily and
  // resolveFireSkill above. Not called from outside decide() in production.
  applyArbitration: applyArbitration,
