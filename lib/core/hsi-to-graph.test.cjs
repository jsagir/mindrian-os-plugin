...
test('HARD-02 Test A: migrated schema accepts insertNode with no NOT NULL failure', () => {
  ...
  try {
    ...
    assert.doesNotThrow(() => {
      insertNode(db, sectionId, 'Section', props);
...
test('HARD-02 upsert: re-inserting the same id updates properties (ON CONFLICT)', () => {
  ...
  try {
    ...
    insertNode(db, sectionId, 'Section', JSON.stringify({ name: sectionId, label: 'OLD' }));
    insertNode(db, sectionId, 'Section', JSON.stringify({ name: sectionId, label: 'NEW' }));
