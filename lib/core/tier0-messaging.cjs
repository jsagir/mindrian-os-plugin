...
// Frozen four-member closed set. Order is the data4sci four-class error
// taxonomy mapping (research Pattern 1): no_key/tier_denied are
// validation-class, unreachable is transient-class (AVAIL-02's retry lives
// BEFORE this kind ever fires), not_ready is missing-information-class
// (never auto-retried; the right move is to queue enrichment).
...
const REASONS = Object.freeze({
  no_key: function (c) {
    return 'Methodology needs the Brain for ' + c.tool + ', and no key is set. Larry will not improvise it from memory.';
  },
...
const RENDER_COPY = Object.freeze({
  no_key: function () {
    return [
      'Methodology needs the Brain, and no key is set. I will not improvise it from memory.',
      'Drop a key in ~/.mindrian.env (chmod 600) or set MINDRIAN_BRAIN_KEY, then restart, or we keep working with your room context.',
    ];
  },
...
function larryRefusalLine(kind, detail) {
  ...
  switch (k) {
    case 'no_key':
      return 'No Brain key set. I will not improvise methodology.';
    case 'tier_denied':
