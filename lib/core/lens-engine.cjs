...
// weight, reusing the existing serial dispatch internally (weighted = ordered-
// serial by the supplied weight). The serial / parallel / single modes are
// unchanged.
...
async function rotate(opts) {
  ...
  if (mode === 'parallel') {
    ...
    // single has exactly one lens. weighted-by-context (Phase 131): the caller
    // (the source-lens driver) supplies the lens-name array ALREADY ordered by
    // descending Plan-02 weight, so ordered-serial dispatch IS weighted dispatch
    // -- the engine never recomputes or re-derives the weights (weighting is
    // OWNED by Plan 02's computeLensSet).
