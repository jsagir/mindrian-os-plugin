...
function listSubdirs(rootDir, opts) {
  ...
  function walk(dir, depth) {
    ...
    for (const entry of entries) {
      ...
      // Quick 260914-ntk (Findings 4.1, 4.2): categorical dot-directory skip.
      // scripts/generate-section-intelligence.cjs:258-259 already blanket-skips
      // every dot-directory with this identical rule, so before this fix the
      // checker was flagging .intelligence and .snapshots as rooms missing
      // ROOM.md/MINTO.md that the generator was never going to fill. SKIP_DIRS
      // stays in place -- it still covers the non-dot entries (node_modules,
      // dist, build).
