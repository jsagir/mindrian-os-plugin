...
//   override is a FAIL naming the resolved endpoint. This is the one
//   deliberate SECOND origin literal in the repo (tests/test-339-origin-
//   single-source.cjs allowlists exactly this file, alongside brain-
//   client.cjs itself, for exactly this reason): this constant answers
//   "is the resolved endpoint the canon one", so deriving it from
//   getBrainUrl() would make the comparison always true and destroy the
//   check outright. It moves in the SAME commit as brain-client.cjs line 24
//   (D-13 locked, phase 339, 2026-09-03) because it is a VALUE, not a
//   mechanism -- the flip only, no new machinery here.
// CANON_NODE_FLOOR / THEO_NODE_FLOOR: below the live node count for the
//   RESOLVED origin, the store is reported thin, naming the count and the
//   floor. PER ORIGIN, selected via THEO_ORIGINS (required from brain-
...
//   reason always wins. A not-ok from this layer is information-only for
//   the release train (scripts/doctor.cjs:1400-1401) EXCEPT for this named
//   reason, which is precisely why leaving it broken would be an honesty
//   failure: it would leave a real signal permanently red and destroy the
//   sense's ability to detect a genuinely wrong endpoint later.
...
async function _layer6(opts) {
  ...
  // Phase 339, 2026-09-03 (FLIP-11, D-12b, D-13 locked): the floor is PER
  // ORIGIN, selected through the SAME THEO_ORIGINS set the alias table
  // keys on (required above, never re-declared here). This is a VALUE
  // moving with the origin, not a mechanism: a MINDRIAN_BRAIN_URL rollback
  // to the incumbent resolves endpoint back to the incumbent, which
  // resolves nodeFloor back to CANON_NODE_FLOOR (29000) in the same motion,
  // so the assertion is never dropped and never silently retargeted.
  const nodeFloor = THEO_ORIGINS.indexOf(endpoint) !== -1 ? THEO_NODE_FLOOR : CANON_NODE_FLOOR;
  ...
  } else if (nodeCount < nodeFloor) {
    ok = false;
    reason = 'node_count=' + nodeCount + ' below floor=' + nodeFloor;
  } else {
    reason = 'node_count=' + nodeCount + ' at or above floor=' + nodeFloor + ', endpoint=' + endpoint;
  }
