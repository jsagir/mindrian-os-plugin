 ...
 *
 * Phase 343 Plan 05 (CENSUS-09) extension: each room also carries a
 * `claim_counter_metric` object (lib/core/navigation/claim-counter-metric.cjs,
 * the computation SENS-06's watched_by declaration in
 * lib/core/sensors/sensor-priority.cjs points at), summed into
 * `totals.claim_counter_metric`. The four integer fields are summed; the
 * divergence BOOLEAN is never summed or averaged -- it is reported as three
 * room-counts (`rooms_diverged`, `rooms_not_diverged`, `rooms_not_measurable`)
 * because an average of booleans is the ratio this pair's own contract
 * forbids. See docs/COUNTER-METRIC-DOCTRINE.md for the interpretation.
 *
...
// Phase 343 Plan 05 (CENSUS-09): the first counter-metric pair (WD-9), the
// computation SENS-06's own watched_by declaration in
// lib/core/sensors/sensor-priority.cjs points at. Called on the SAME open
// handle inside the same per-room try below, so no room is opened twice.
const { countClaimCounterMetric } = require('../navigation/claim-counter-metric.cjs');
...
function measureRoom(roomPath) {
  ...
  try {
    ...
    const claimCounterMetric = countClaimCounterMetric(db);
    return Object.assign(
      { has_db: true, schema_variant: variant },
      counts,
      { claim_counter_metric: claimCounterMetric }
    );
  } finally {
...
// The claim_counter_metric integer fields summed into totals. A fixed
// literal list, mirroring INTEGER_FIELDS above, so a stray key never rides
// the totals unreviewed.
const CLAIM_COUNTER_METRIC_INTEGER_FIELDS = [
  'claims_filed',
  'claims_filed_past_citation_lag',
  'claims_with_contradicts_edge',
  'claims_no_incoming_edge_past_citation_lag',
];

// sumClaimCounterMetricField(rooms, field) -> { sum, notMeasurableCount },
// reading each room's NESTED claim_counter_metric object rather than a
// top-level field. Same null-excluded-from-sum rule as sumField above.
function sumClaimCounterMetricField(rooms, field) {
  let sum = 0;
  let notMeasurableCount = 0;
  for (const room of rooms) {
    const ccm = room.claim_counter_metric;
    if (!ccm || typeof ccm !== 'object') continue;
    const v = ccm[field];
    if (v === null || v === undefined) {
      if (Object.prototype.hasOwnProperty.call(ccm, field)) notMeasurableCount += 1;
      continue;
    }
    if (typeof v === 'number') sum += v;
  }
...
// buildClaimCounterMetricTotals(rooms) -> the four summed counts plus THREE
// room-counts for the divergence boolean: rooms_diverged, rooms_not_diverged,
// rooms_not_measurable. The boolean itself is NEVER summed or averaged --
// an average of booleans is exactly the ratio this pair's own contract
// (343-05-PLAN.md counts_only_contract) forbids.
function buildClaimCounterMetricTotals(rooms) {
  const totals = {};
  for (const field of CLAIM_COUNTER_METRIC_INTEGER_FIELDS) {
    const { sum, notMeasurableCount } = sumClaimCounterMetricField(rooms, field);
    totals[field] = sum;
    if (notMeasurableCount > 0) totals[field + '_not_measurable_rooms'] = notMeasurableCount;
  }
  let roomsDiverged = 0;
  let roomsNotDiverged = 0;
  let roomsNotMeasurable = 0;
  for (const room of rooms) {
    const ccm = room.claim_counter_metric;
    if (!ccm || typeof ccm !== 'object') continue;
    if (ccm.divergence === true) roomsDiverged += 1;
    else if (ccm.divergence === false) roomsNotDiverged += 1;
    else roomsNotMeasurable += 1;
  }
  totals.rooms_diverged = roomsDiverged;
  totals.rooms_not_diverged = roomsNotDiverged;
  totals.rooms_not_measurable = roomsNotMeasurable;
  return totals;
}
...
function buildTotals(rooms) {
  ...
  totals.claim_counter_metric = buildClaimCounterMetricTotals(rooms);
  return totals;
}
...
function buildDetail(scope, rooms, totals) {
  ...
  const ccm = totals.claim_counter_metric || {};
  return measured.length + ' room(s) measured (' + scope + '): '
    ...
    + ccm.claims_filed + ' claim(s) filed, '
    + ccm.claims_with_contradicts_edge + ' claim(s) carrying a CONTRADICTS edge';
}
