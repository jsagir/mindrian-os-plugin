...
function checkAgainstServers(osServerPath, brainServerPath, ctx) {
  ...
  // Phase 265 code-review WR-02 fix: a fixed scratch path let two concurrent
  // `doctor --acceptance` runs on the same machine (a CI matrix job plus a
  // developer running doctor locally, or two CI jobs sharing a runner) point
  // the spawned mindrian-os server at the SAME room directory simultaneously,
  // risking SQLite lock contention. mkdtempSync per call mirrors the tmpHome
  // pattern in listToolsOverStdio above; best-effort cleanup, never throws.
  let tmpRoom;
  let osOutcome;
  try {
    tmpRoom = fs.mkdtempSync(path.join(os.tmpdir(), 'mindrian-mcp-surface-room-'));
    osOutcome = listToolsOverStdio(osServerPath, {
      timeoutMs: timeoutMs,
      envOverrides: {
        MINDRIAN_TRANSPORT: 'stdio',
        MINDRIAN_ROOM: tmpRoom,
      },
    });
  } catch (e) {
