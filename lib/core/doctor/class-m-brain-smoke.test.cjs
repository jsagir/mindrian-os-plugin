...
const L6_PASS_SEAMS = {
  mockBrainUrl: () => 'https://pws-brain-mcp.onrender.com',
...
(async function test11_store_identity_contract() {
  ...
  try {
    ...
    // failure reason, without the stale_replica_signature token.
    const belowFloorResult = await mod.checkBrainSmoke(Object.assign({}, passBase, {
      mockStats: async () => ({ totalRecordCount: 1000 }),
      mockQuery: async () => null,
    }));
    ...
    assert.equal(L6pass.payload.endpoint, 'https://pws-brain-mcp.onrender.com', 'payload.endpoint');
