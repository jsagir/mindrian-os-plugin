...
// resolveRoomPath(roomsHome, info) -> absolute room dir path.
// NOT copied verbatim from cascade-rooms-module.cjs:52-56 (232.1-REVIEW.md
// WR-01): that precedent only reads `info.path`, but `abs_path` is the
// canonical, actively-used registry precedence field across this codebase
// (lib/core/resolve-active-room.cjs's own doc comment: "prefer entry.abs_path
// if set, else entry.path"; lib/mcp/tool-router.cjs -- which THIS phase's own
// Plan 232.1-02 wires into -- honors it too). A room registered with only
// `abs_path` (no `path` key) is a real, common registry shape, not a
// hypothetical -- silently skipping it here would under-count the very
// census this phase exists to make trustworthy. Precedence: abs_path (used
// as-is) -> path (absolute as-is, else joined against roomsHome) -> null
// when neither is present, so callers skip the entry identically either way.
