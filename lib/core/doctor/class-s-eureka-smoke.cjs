...
// Quick 260917-o1e (R3a): the ONE owner of the two disk facts L1 and L5 each
// read. A PRESENCE probe: it reads two package.json paths off disk, never
// loads a model, never embeds, never reaches the network. Synchronous (the
// doctor `fix()` caller in graph-derive-health-module.cjs invokes it
// synchronously with no await -- scripts/doctor.cjs:2702-2707 calls fixFn
// with no await, so an async probe would return a Promise a doctor renders as
// a broken recovery row). Never throws: wrapped, returns an all-false shape
// on any error.
function encoderPresentSync(opts) {
  const o = (opts && typeof opts === 'object') ? opts : {};
  try {
    // deps: the L1 body minus the mockL1 seam -- fs.existsSync on sqlite-vec's
    // package.json, with the SAME reason strings L1 returns today.
    const root = o.pluginRoot || _pluginRoot();
    const depPkg = path.join(root, 'node_modules', 'sqlite-vec', 'package.json');
    let deps;
    if (!fs.existsSync(depPkg)) {
      deps = { ok: false, reason: 'sqlite-vec not installed under node_modules (npm install to fetch it)' };
    } else {
      let ver = 'unknown';
      try { ver = JSON.parse(fs.readFileSync(depPkg, 'utf8')).version || 'unknown'; } catch (_e) { /* keep unknown */ }
      deps = { ok: true, reason: 'sqlite-vec ' + ver };
    }

    // model: eurekaDepInstalled('@huggingface/transformers') plus the two
    // reason strings L5 returns today, verbatim.
    const { eurekaDepInstalled } = require('../eureka-deps-resolver.cjs');
    const probe = eurekaDepInstalled('@huggingface/transformers');
    const model = probe.installed
      ? {
        installed: true,
        where: probe.where,
        dir: probe.dir,
    ...
    return { present: deps.ok && model.installed, deps: deps, model: model };
  } catch (_e) {
    return {
      present: false,
      deps: { ok: false, reason: 'encoderPresentSync probe threw' },
      model: { installed: false, where: null, dir: null, reason: 'encoderPresentSync probe threw' },
    };
  }
}
...
// this layer must never probe it again. Quick 260917-o1e (R3a): delegates to
// encoderPresentSync so this file has ONE owner of the two disk facts; the
// externally observable return shape is byte-unchanged.
async function _layer1(opts) {
  if (opts.mockL1) return opts.mockL1();
  const probe = encoderPresentSync({ pluginRoot: opts.pluginRoot });
  return { ok: probe.deps.ok, reason: probe.deps.reason };
}
...
// Quick 260917-o1e (R3a): delegates to encoderPresentSync so this file has ONE
// owner of the two disk facts; the externally observable return shape
// (including `advisory: true`) is byte-unchanged.
async function _layer5(opts) {
  if (opts.mockL5) return opts.mockL5();
  const probe = encoderPresentSync({ pluginRoot: opts.pluginRoot });
  return { ok: probe.model.installed, advisory: true, reason: probe.model.reason };
}
...
module.exports = { checkEurekaSmoke, LAYERS, fixEurekaSmoke, PROBE_TIMEOUT_MS, encoderPresentSync };
