...
// Quick 260914-ntk (Finding 5.ii): data/deployment-surfaces.json:24 writes the
// manifest command as `bash "$HOME/.claude/statusline-mos"` on purpose --
// the $HOME form is the portable one. Class J's expandSurfacePath()
// (deployment-surfaces-module.cjs:66-81) already expands $HOME on both sides
// before comparing; class G (this module) simply never adopted the same
// normalization, so a $HOME-form command fell through to the stale-path /
// missing-target branches below instead of resolving. This adds the
// normalization to class G rather than asking the manifest to stop being
// portable. Not a regression of the intern-w1-statusline-room-mismatch KB fix
// -- that one fixed topology-blindness and which-file-to-check in this same
// module and never touched this regex.
function expandHomeTokens(cmd) {
...
function check(_ctx) {
  ...
  if (userSettings && userSettings.statusLine && typeof userSettings.statusLine.command === 'string') {
    ...
    // Finding 5.ii: expand $HOME / ${HOME} / a leading ~/ before both the
    // stale-path regex test and the extraction, so a portable $HOME-form
    // command resolves to the real path instead of tripping the stale check.
    const expandedCmd = expandHomeTokens(cmd);
    if (STALE_STATUSLINE_PATH_REGEX.test(expandedCmd)) {
      // Extract a path token from the command string. Try a unix or windows
      // absolute-path match first.
      const match = expandedCmd.match(/(\/[^\s"']+|[A-Za-z]:\\[^\s"']+)/);
      const candidate = match ? match[0] : expandedCmd;
  ...
  if (pluginSettings && pluginSettings.statusLine && typeof pluginSettings.statusLine.command === 'string') {
    ...
    // Finding 5.ii: expand $HOME tokens AFTER the CLAUDE_PLUGIN_ROOT
    // substitution, mirroring the order data/deployment-surfaces.json's
    // command strings are actually resolved in.
    const resolved = expandHomeTokens(pluginSettings.statusLine.command.replace(/\$\{CLAUDE_PLUGIN_ROOT\}/g, pluginRoot));
  ...
  if (userSettings && userSettings.statusLine && typeof userSettings.statusLine.command === 'string') {
    // Finding 5.ii: expand $HOME tokens before extracting the effective
    // override target.
    const overrideMatch = expandHomeTokens(userSettings.statusLine.command).match(/(\/[^\s"']+|[A-Za-z]:\\[^\s"']+)/);
    if (overrideMatch) {
