...
function check(ctx) {
  ...
  // WR-01: a room this pass could NOT reach is a warn, never a silent 'ok'.
  //
  // The earlier "the next run retries it" reasoning was wrong about its own
  // cadence. This module is cadence:'once'. The engine's anyHardError gate only
  // trips when a runner THROWS (scripts/doctor.cjs runAccumulativeEngine step 5),
  // and this module never throws: every per-room failure is caught and folded
  // into `errors`. So the watermark advances past introduced_version after this
  // ONE invocation regardless, the (applied_through, running] window is empty
  // forever after, and there is no "next run" for THIS module. A systemic failure
  // inside that single window (an unwritable queue path, a bad
  // MINDRIAN_ROOMS_HOME, a throwing sweep) would otherwise strand every damaged
  // room while the module reported a clean pass.
  //
  // What actually happens to a stranded room, stated instead of implied: the
  // sibling cadence:'always' graph-derive-health class keeps flagging it on every
  // doctor run and every SessionStart nudge, and the operator repairs it with
  // /mos:doctor --heal-room. The status now says so.
  return {
    ...
    detail: healed + ' of ' + scanned + ' registered room(s) '
      ...
      + ' for semantic derive'
      + (errors > 0
        ? '; ' + errors + ' room(s) could not be re-enqueued. This one-time pass does'
          + ' not run again: they stay flagged by graph-derive-health, run'
          + ' /mos:doctor --heal-room to repair them'
        : '')
      + (dryRun ? ' (dry-run)' : ''),
  };
}
