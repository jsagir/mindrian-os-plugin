...
function detectRoomHealth(roomDir) {
  ...
  // current encoder gap.
  base.encoderUnavailable = skipAt !== null && (attemptAt === null || skipAt >= attemptAt);
  ...
  // succeeded ones (Finding 5c).
  base.deriveAttemptCompleted = attemptAt !== null && base.queueCount === 0 && !base.encoderUnavailable;
...
function fix(ctx) {
  ...
  for (const entry of rooms) {
    ...
    // Quick 260914-ntk (Finding 1a/5c): a room that cannot derive because the
    // semantic encoder is missing must never be re-enqueued -- re-enqueueing a
    // room that cannot derive is the loop this task exists to break.
