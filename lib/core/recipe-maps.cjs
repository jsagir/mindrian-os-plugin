...
function _loadManifest() {
  ...
  try {
    ...
    _manifestCache = { methodology_tier: null, version: null, maps: [] };
  }
 ...
 *                     source_count }] }
 * The declared three-map binding read from data/harness-manifest.json: the
 * posture (command-registry), wiring (connector-registry), and ranked_next_reach
 * (brain-orchestration-projection) maps named by role + path + digest +
 * source_count. A missing / malformed manifest degrades to the documented empty
 * binding ({ methodology_tier: null, version: null, maps: [] }) and never throws,
 * mirroring the _loadProjection degrade idiom.
 *
