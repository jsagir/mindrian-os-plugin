...
const { roomsHomeEnv } = require('./rooms-home-env.cjs');

// ---------- Frozen constants (Canon Part 8 invariants) ----------

// ALLOWED_ROOT: canonical ~/MindrianRooms/ absolute path. Overridable via
// MINDRIAN_ROOMS_HOME (or MINDRIAN_ROOMS_ROOT) for tests; every code path
// still calls path.resolve + startsWith against this base. Evaluated once at
// module load, same as before this task (only the env READ changed).
