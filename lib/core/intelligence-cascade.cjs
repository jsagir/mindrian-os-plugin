...
<<<<<<< HEAD
 *   5. hsi-to-kuzu.cjs (write HSI results to SQLite)
=======
 *   5. hsi-to-lazygraph.cjs (write HSI results to LazyGraph SQLite)
  ...
  if (isMd && fs.existsSync(lazygraphDir)) {
    ...
    if (now - lastHsi < HSI_DEBOUNCE_MS) {
      ...
      if (fs.existsSync(hsiResultsPath)) {
        try {
          const hsiBridgeScript = path.join(SCRIPTS_DIR, 'hsi-to-lazygraph.cjs');
