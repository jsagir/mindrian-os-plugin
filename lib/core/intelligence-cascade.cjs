...
/** @type {Map<string, {files: string[], timeout: NodeJS.Timeout|null, waiters: Array<{resolve:Function,reject:Function}>, options: Object}>} */
...
async function _runCascadeSteps(roomDir, artifacts, options) {
  ...
  try {
    execSync(`bash "${path.join(SCRIPTS_DIR, 'compute-state')}" "${roomDir}"`, {
      timeout: HSI_TIMEOUT_MS,
      stdio: ['ignore', 'pipe', 'pipe']
    });
