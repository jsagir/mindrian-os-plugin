...
// the compute-hsi.py spawn site below. No other module in this file decides
// backend selection directly.
...
async function _runCascadeSteps(roomDir, artifacts, options) {
  ...
  if (anyMd && fs.existsSync(roomDbPath)) {
    if (debounced) {
      ...
      if (hsiBackend === 'python') {
        ...
        // 'cjs' backend (D-04 default): in-process hsi-engine.cjs, no Python
        // dependency to check -- the check-hsi-deps preflight is skipped
        // entirely on this branch since it only ever gated the Python leg.
        try {
          await hsiEngine.runTier1(roomDir);
          stepsResult.hsi = { status: 'ok' };
          hsiSuccess = true;
          stepsResult.hsiRanAt = Date.now();  // caller updates lastHsiByRoom
        } catch (e) {
      ...
      // Phase 355 D-52: the Python detect-reverse-salients.py detector is
      // retired from this live path -- it wrote the opposite (Convention B)
      // direction label. Reverse salients now come exclusively from
      // lib/core/rs-engine.cjs (the same engine /mos:find-bottlenecks uses,
      // scripts/scout-cadence-runner.cjs's rs-engine-cjs step, and
      // commands/scout.md / skills/scout/SKILL.md step 2). This cascade
      // never invoked rs-engine.cjs itself before this plan and does not
      // start now -- the key is kept only so the result shape stays stable
      // for any caller reading stepsResult.reverseSalients.
      stepsResult.reverseSalients = {
        status: 'retired',
        reason: 'Phase 355 D-52: Python detector off the live path; reverse salients come from lib/core/rs-engine.cjs'
      };
  ...
  //
  // Phase 240.1 Plan 03 (CTXL-01): the persist below used to be a blind
  // fs.writeFileSync, destroying the gsd_state_version/status stamp on every
  // automatic post-write cascade run (240.1-RESEARCH.md Finding 1C, write
  // site 2). Replaced with persistState(), the same preserve-or-notify
  // authority Plan 240.1-02 wired into lib/core/state-ops.cjs::computeState.
  // The execFileSync('bash', [...]) spawn shape is UNCHANGED (already the
  // Security-Domain-approved args-array form); only the write-and-report
  // pair changed. { status: 'version-mismatch' } is a new outcome on this
  // existing envelope, slotted in alongside 'ok' and 'error' without
  ...
  try {
    ...
    const verdict = persistState(roomDir, computedState);
    if (verdict && verdict.status === 'version-mismatch') {
      stepsResult.computeState = {
        status: 'version-mismatch',
        onDiskVersion: verdict.onDiskVersion,
        moduleVersion: verdict.moduleVersion,
        advice: verdict.advice,
      };
    } else if (verdict && verdict.status === 'error') {
      stepsResult.computeState = { status: 'error', message: verdict.message };
    } else {
  ...
  // Quick Task 260819-dmm: pass the output path explicitly. build-graph's own
  // default is CWD-relative (./dashboard/graph.json on both its python and
  // bash branches), so a repo-cwd caller used to rewrite the plugin's own
  // tracked copy on every cascade run. Anchoring on PLUGIN_ROOT (line 34)
  // makes the write land in the same place regardless of the caller's cwd.
  // DEFERRED (not taken here): room-scoping this output to
  // <roomDir>/.presentation/graph.json (the default build-graph-from-sqlite.cjs
  // already uses) is a product decision about where a room's own graph
  // belongs; out of scope for this drift fix.
  try {
    execFileSync('bash', [path.join(SCRIPTS_DIR, 'build-graph'), roomDir, path.join(PLUGIN_ROOT, 'dashboard', 'graph.json')], {
      timeout: 10000,
      stdio: ['ignore', 'pipe', 'pipe']
    });
