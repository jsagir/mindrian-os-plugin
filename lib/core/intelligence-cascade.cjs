 ...
 *   2. graph-index (SQLite artifact indexing via graph-ops)
 *   3. compute-hsi.py (HSI innovation scores)
 *   4. detect-reverse-salients.py (lagging component detection)
 *   5. hsi-to-graph.cjs (write HSI results to SQLite)
...
// Real rooms with 50+ artifacts exceed 5s on compute-hsi.py /
// detect-reverse-salients.py / hsi-to-graph.cjs and were aborting mid-run,
// producing partial .hsi-results.json and stale intelligence edges. The
// named constant makes future bumps a one-line change. The two intentional
// 15000ms sites (generate-presentation.cjs) stay untouched -- different
// script, different ceiling, tracked separately.
...
async function runCascade(roomDir, { trigger, filePath, section } = {}) {
  ...
  // Step 2: Graph index (SQLite artifact indexing -- direct call to graph-ops)
  if (isMd) {
    try {
  ...
  // Steps 3-5: HSI computation + reverse salients + SQLite bridge
  // Only runs when room.db exists (room has been graph-indexed before)
  // HSI Debounce (HOOK-01): skip if same room computed within 30s
  const roomDbPath = path.join(resolvedRoomDir, '.mindrian', 'room.db');
  if (isMd && fs.existsSync(roomDbPath)) {
    ...
    if (now - lastHsi < HSI_DEBOUNCE_MS) {
      ...
      // Step 5: HSI to SQLite bridge (only if .hsi-results.json exists)
  ...
  try {
    ...
    // Phase 84-05: also reads graph findings via readGraphFindings()
  ...
  return new Promise((resolve, reject) => {
    ...
    batch.timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (resolvedRoomDir && fs.existsSync(resolvedRoomDir)) {
          const batchRoomDb = path.join(resolvedRoomDir, '.mindrian', 'room.db');
          if (fs.existsSync(batchRoomDb)) {
            const lastHsi = lastHsiByRoom.get(resolvedRoomDir) || 0;
            const now = Date.now();
