...
async function _runCascadeSteps(roomDir, artifacts, options) {
  ...
  for (const art of artifacts) {
    ...
    if (isMd) {
      ...
      // collapse to one entry, and extra spawned drains no-op on the already-cleared
      // queue (the enqueue-then-drain idiom). The Phase-169 Stop sweep + SessionStart
      // drain stay registered as a harmless second net (RESEARCH OQ2/OQ3).
      try {
        ...
        if (process.env.MOS_NO_DETACHED_DERIVE !== '1') {
          try {
            const proc = spawn(
              process.execPath,
              [path.join(SCRIPTS_DIR, 'gsd-graph-derive-drain.cjs'), '--room', roomDir],
  ...
  if (anyMd && fs.existsSync(roomDbPath)) {
    if (debounced) {
      ...
      try {
        execSync(`bash "${path.join(SCRIPTS_DIR, 'check-hsi-deps')}"`, {
          timeout: HSI_TIMEOUT_MS,
          stdio: ['ignore', 'pipe', 'pipe']
        });
        try {
          execSync(`python3 "${path.join(SCRIPTS_DIR, 'compute-hsi.py')}" "${roomDir}"`, {
            timeout: HSI_TIMEOUT_MS,
            stdio: ['ignore', 'pipe', 'pipe']
          });
      ...
      if (hsiSuccess) {
        try {
          execSync(`python3 "${path.join(SCRIPTS_DIR, 'detect-reverse-salients.py')}" "${roomDir}"`, {
            timeout: HSI_TIMEOUT_MS,
            stdio: ['ignore', 'pipe', 'pipe']
          });
      ...
      if (fs.existsSync(path.join(roomDir, '.hsi-results.json'))) {
        try {
          execSync(`node "${path.join(SCRIPTS_DIR, 'hsi-to-graph.cjs')}" "${roomDir}"`, {
            timeout: HSI_TIMEOUT_MS,
            stdio: ['ignore', 'pipe', 'pipe']
          });
  ...
  if (fs.existsSync(presentationDir)) {
    try {
      execSync(`node "${path.join(SCRIPTS_DIR, 'generate-presentation.cjs')}" "${roomDir}" --output "${presentationDir}"`, {
        timeout: 15000,
        stdio: ['ignore', 'pipe', 'pipe']
      });
  ...
  try {
    const computedState = execSync(`bash "${path.join(SCRIPTS_DIR, 'compute-state')}" "${roomDir}"`, {
      timeout: HSI_TIMEOUT_MS,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe']
    });
  ...
  try {
    execSync(`bash "${path.join(SCRIPTS_DIR, 'build-graph')}" "${roomDir}"`, {
      timeout: 10000,
      stdio: ['ignore', 'pipe', 'pipe']
    });
  ...
  let analyzeOutput = '';
  try {
    analyzeOutput = execSync(`bash "${path.join(SCRIPTS_DIR, 'analyze-room')}" "${roomDir}"`, {
      timeout: 10000,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe']
    });
