...
async function _runCascadeSteps(roomDir, artifacts, options) {
  ...
  if (anyMd && fs.existsSync(roomDbPath)) {
    if (debounced) {
      ...
      // detect-reverse-salients.py stays COMPLETELY UNCHANGED, unconditional,
      // and un-gated -- explicitly OUT of this plan's D-04 dispatch-wiring
      // scope (Deferred Scope, 272-10-PLAN.md Objective). It has zero heavy
      // ML dependency and runs regardless of which HSI backend produced
      // .hsi-results.json, since both backends write the same on-disk schema.
      if (hsiSuccess) {
        try {
