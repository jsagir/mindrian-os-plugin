...
// classifies a stated research goal into one of six roadmap output-shapes via
// a signal/context/keyword cascade (Canon Part 11 R3, keyword demoted to
// fallback). posture 'hold' -- a standing suggestion, never an auto-open.
// Lives under lib/core/sensors/ so the routing fence + Part-8 sweep span it.
const { sensorRoadmapType } = require('./sensors/sensor-roadmap-type.cjs');

// Phase 343 Plan 06 detector (SENS-19 -- room-graph integrity -> the frozen
// contradiction reach at posture hold, CENSUS-10/CENSUS-11): fires when a
// bound room's per-room graph-integrity defect count crosses
// INTEGRITY_DEFECT_THRESHOLD (WD-19). Pure and synchronous; the room.db read
// runs in the ctx-assembly producer block in navigation-engine.cjs, never
// here. Lives under lib/core/sensors/ so the routing fence + Part-8 sweep
// span it.
const { sensorGraphIntegrity } = require('./sensors/sensor-graph-integrity.cjs');
...
const SENSOR_REGISTRY = [
  ...
  // Phase 264 detector (SENS-18 -- roadmap-type -> context_block roadmap-type-selector offer, R1):
  sensorRoadmapType,
  // Phase 343 Plan 06 detector (SENS-19 -- room-graph integrity -> the frozen contradiction reach at posture hold, CENSUS-10/CENSUS-11):
...
const SENSOR_REGISTRY_IDS = Object.freeze([
  ...
  'SENS-18',        // sensorRoadmapType
  'SENS-19',        // sensorGraphIntegrity
  'SENS-20',        // sensorStrategyReach
]);
