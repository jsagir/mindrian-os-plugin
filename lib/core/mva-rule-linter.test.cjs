 ...
 * Test 15: Phase 267.3-04's second amendment (live_deliverable, minted for
 *   commands/publish.md's live shareable URL). T12's sibling for the ADDED
 *   members, plus the fourth place a term must land that T13/T14 do not reach:
 *   the rule doc's "## Vocabulary amendments" section, where the written reason
 *   for the term lives.
 *
...
// ---------- T15: 267.3-04 -- the amendment members are intact AND each is documented ----------

run('T15 267.3 amendments: all three added members present, and each carries a rule-doc amendment entry', () => {
  const { REWARD_TYPES } = freshLinter();

  // The three terms the two Phase 267.3 amendments minted. T12 guards the
  // v1.13.0 original six the same way; this is its sibling for the additions,
  // so a later edit cannot silently drop or respell one of them either.
  //   methodology_reframe          -- 267.3-02, ruling D-B
  //   --none (diagnostic surface)  -- 267.3-02, ruling D-B
  //   live_deliverable             -- 267.3-04, navigator ruling Row 13
  //                                   (originating example commands/publish.md:149)
  const amended = [
    'methodology_reframe',
  ...
  for (const v of amended) {
    assert.ok(
      REWARD_TYPES.has(v),
      'a Phase 267.3 amendment member was dropped or respelled: ' + v
    );
  }
  ...
  assert.equal(
    REWARD_TYPES.size,
    9,
    'REWARD_TYPES should hold the v1.13.0 six plus the three 267.3 amendment members; got ' +
    REWARD_TYPES.size + ': ' + JSON.stringify([...REWARD_TYPES])
  );

  // The rule doc's own law: "A code change without an entry in this section is
  // a command-level invention wearing a library's clothes." T13 already checks
  // the allowed-values BULLET. This checks the AMENDMENT ENTRY, which is where
  // the reasoning lives, so a term cannot be added to the Set and the bullet
  // list while its written justification is skipped.
  const docPath = path.resolve(REPO_ROOT, 'docs', 'reward-before-investment-rule.md');
  const doc = fs.readFileSync(docPath, 'utf8');
  const amendIdx = doc.indexOf('## Vocabulary amendments');
  assert.ok(amendIdx >= 0, 'rule doc must carry a "## Vocabulary amendments" section');
  const amendSection = doc.slice(amendIdx);
  for (const v of amended) {
    assert.ok(
      amendSection.includes('### `' + v + '`'),
      'rule doc has no "### `' + v + '`" amendment entry; the term was added to the code ' +
      'without its written reason (Phase 267.3 GUARD-02)'
    );
  }
});
