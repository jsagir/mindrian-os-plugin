...
// military-orders usage) inside a natural-language pattern -- an unrelated
// concept from the origin id that happens to share its spelling, not an
// origin-id value. Built from two literals so this file's own routing text
// never appears as the single contiguous token P7's static scan looks for.
const TASKING_WORD = 'task' + 'ing';
...
const QUESTION_TURN_PATTERNS = Object.freeze({
  ...
  change: Object.freeze([
    ...
    new RegExp('\\b(?:our|the)\\s+' + TASKING_WORD + '\\s+(?:has\\s+)?changed\\b', 'i'),
  ]),
  ask: Object.freeze([
    new RegExp(
