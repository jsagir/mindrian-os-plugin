...
async function callTool(toolName, args) {
  ...
  try {
    ...
    if (!toolRes.ok) {
      ...
      // working unchanged. Every OTHER non-OK status still returns null --
      // that remains the sole transport-failure signal (research Pitfall 4:
      // 82 degradation tests key on it; do not widen this branch).
 ...
 * @returns {Promise<object|null>}
 */
async function discoverStructure(frameworkName) {
  return callTool('discover_structure', { framework_name: frameworkName });
}
 ...
 * @returns {Promise<object|null>}
 */
async function orchestrationReadiness(frameworkName) {
  return callTool('orchestration_readiness', { framework_name: frameworkName });
}
