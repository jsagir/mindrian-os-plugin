 ...
 * no_key failure-edge refusal copy (tier0-messaging.cjs) so the reason names
 ...
 *
 * Phase 250-01 (HONEST-01, doctrine kill list site #11) -- MARK, not flip.
 * These chains are hardcoded, served with source: 'tier0', and are NOT
 * graph-grounded. Per the Phase 250 provenance contract they must never be
 * presented as methodology without disclosure (no `BRAIN` provenance mark).
 * The guard-site behavior flip (refusing instead of serving this chain on a
 * methodology ask) is SWEEP-01 territory (Phase 252) -- the 247-02 do-not-
 * widen note on the null branch binds (82 degradation tests key on it), so
 * this comment marks the surface without changing behavior here.
 *
