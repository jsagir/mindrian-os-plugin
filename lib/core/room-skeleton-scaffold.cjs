...
// Phase 353 Plan 02 (Task 4): the repo's own YAML frontmatter parser
// (package.json dependency, ruling recorded at
// tests/test-275-section-schema.cjs:26-28). Used ONLY to read an existing
// CONTEXT.md's frontmatter (ruling_fingerprint, and to strip a legacy
// frontmatter-free body's fence if present); never to gray-matter.stringify
// a full re-serialization, which would reformat untouched keys.
...
/**
 * Phase 275-08 CR-01 fix: escape a free-text value for safe embedding inside
 * a double-quoted YAML scalar. Any SECTION_METADATA/IDENTITY_DIRECTORIES
 * `statement`/`purpose` string can contain YAML-significant characters
 * (a mid-sentence ": " is a mapping-value indicator, an embedded `"` or
 * `\` breaks a double-quoted scalar) and renderTemplate does a raw string
 * substitution with no YAML awareness. Templates that render free text into
 * frontmatter MUST wrap the token in double quotes (`key: "{{TOKEN}}"`) and
 * pass the value through this helper first, so this class of defect cannot
 * recur silently for any future text edit. Order matters: backslashes must
 * be escaped before quotes, or a value ending in `\"` would double-escape.
 *
 * @param {*} value  coerced via String()
 * @returns {string}
 */
 ...
 *                                mutated in place
 */
// Phase 353 Plan 02 (Task 4, D-353-7, R-353-M, R-353-L): the ruling document
// generator. A section's CONTEXT.md becomes six marked, fingerprinted parts
// (Job, Methodology sequence, Writing rules, Gates, Checks, Commands that
// write here) ABOVE the preserved authored Inputs/Process/Outputs/Human
// check prose (T-275-13 amendment: templates stay frontmatter-free on disk;
// the LANDED document's frontmatter is composed here, at write time).
const RULING_MARKER_BEGIN_TEXT =
  '<!-- mos:ruling:begin  DO NOT EDIT. Regenerated from data/section-command-ledger.json ' +
  'and lib/core/section-registry.cjs by the room-skeleton scaffold or doctor section-ruling ' +
  'fix. Edits inside this block are drift. -->';
const RULING_MARKER_END = '<!-- mos:ruling:end -->';
const RULING_WRITER_CONTRACT_VERSION = 'ruling-doc-writer-v1';

function _rulingLedgerRow(ledger, jobId) {
  if (!ledger || !ledger.rows || !jobId) return null;
  return ledger.rows[jobId + '|*|*'] || null;
}

function _rulingFingerprint(canonRow, primaryRow, secondaryRow) {
  const payload = JSON.stringify({
    job_id: canonRow.job_id,
    secondary_job_id: canonRow.secondary_job_id,
    primary: primaryRow,
    secondary: secondaryRow,
    writer_contract_version: RULING_WRITER_CONTRACT_VERSION,
  });
  return crypto.createHash('sha256').update(payload).digest('hex');
}

function _renderMethodologySequenceBlock(jobId, ledgerRow, label) {
  if (!ledgerRow || ledgerRow.length === 0) {
    return label + ' (`' + jobId + '`): no ledger row for this job; the sensor order applies unchanged.';
  }
  const top = ledgerRow.slice(0, 3);
  const items = top.map((c) => '  - `' + c.command + '`' + (c.framework ? ' (' + c.framework + ')' : ''));
  return label + ' (`' + jobId + '`):\n' + items.join('\n');
}

function _renderCommandsThatWriteHereBlock(ledgerRow) {
  if (!ledgerRow || ledgerRow.length === 0) return '- No ledger row for this job yet.';
  const ground = ledgerRow.filter((c) => c.source === 'ground-truth').map((c) => '`' + c.command + '`');
  const framework = ledgerRow.filter((c) => c.source === 'framework-matched').map((c) => '`' + c.command + '`');
  const lines = [];
  if (ground.length > 0) lines.push('- Ground truth (shipped ledger): ' + ground.join(', '));
  if (framework.length > 0) lines.push('- Framework-matched (shipped ledger): ' + framework.join(', '));
  if (lines.length === 0) lines.push('- No ledger row for this job yet.');
  return lines.join('\n');
}

/**
 * Compose the generated ruling block (a separate string, never spliced INTO
 * authored prose -- T-275-13). Returns { block, fingerprint }.
 */
function buildRulingBlock(canonRow, ledger) {
  const primaryRow = _rulingLedgerRow(ledger, canonRow.job_id);
  const secondaryRow = canonRow.secondary_job_id ? _rulingLedgerRow(ledger, canonRow.secondary_job_id) : null;
  const fingerprint = _rulingFingerprint(canonRow, primaryRow, secondaryRow);

  const lines = [
    RULING_MARKER_BEGIN_TEXT,
    '## 1. Job',
    'Primary job: `' + canonRow.job_id + '`' +
      (canonRow.secondary_job_id ? ', secondary: `' + canonRow.secondary_job_id + '`' : '') +
      '. Source: `data/section-job-canon.json` (Phase 353).',
    '',
    '## 2. Methodology sequence',
    _renderMethodologySequenceBlock(canonRow.job_id, primaryRow, 'First'),
  ];
  if (canonRow.secondary_job_id) {
    lines.push(_renderMethodologySequenceBlock(canonRow.secondary_job_id, secondaryRow, 'Then'));
  }
  lines.push(
    '',
    '## 3. Writing rules',
    '- No em-dashes; hyphens only.',
    '- Feynman-simplified, JTBD-oriented prose.',
    "- Never substitute another section's job content here; see this section's own `Do NOT load:` line below.",
    '',
    '## 4. Gates',
    '- Filing gate default `flag` (D-353-8): a job mismatch on `artifact_file` or `claim_write` lands the write with a disclosure; `strict` refuses. Every filed claim anchors to `jtbd:' + canonRow.job_id + '`.',
    '',
    '## 5. Checks',
    "- `doctor section-ruling` reports ruling_fingerprint drift, an undeclared job, and any claim filed after this module's introduced_version with no anchor edge.",
    '',
    '## 6. Commands that write here',
    _renderCommandsThatWriteHereBlock(primaryRow),
    '',
    RULING_MARKER_END
  );
...
/**
 * writeSectionContracts(roomDir, sectionList, result) -- the L2 ruling
 * document generator (Phase 353 Plan 02 supersedes the Phase 275 verbatim
 * copier this function used to be).
 *
 * A section with no CONTEXT.md gets one CREATED from the shipped template
 * plus the generated parts (R-353-M). A section with an existing CONTEXT.md
 * keeps every byte below `mos:ruling:end` byte-identical; only the
 * frontmatter and the marked region are replaced, and ONLY when the
 * recomputed `ruling_fingerprint` actually differs from what is already on
 * disk -- an unchanged-inputs regeneration is a true no-op (byte-identical),
 * never a needless rewrite. A section whose job is undeclared (e.g. a canon
 * load failure, or a slug with no canon row) degrades to the old
 * verbatim-copy-if-absent behavior rather than generating a ruling block
 * with nothing to rule on.
 *
 * No `{{TOKEN}}` substitution pass ever runs here (T-275-13): the generated
 * block is composed separately (buildRulingBlock) and spliced in as its own
 * string, never substituted into contract prose.
 *
 * @param {string} roomDir
...
function writeSectionContracts(roomDir, sectionList, result) {
  ...
  try {
    ...
    sectionRegistry = require('./section-registry.cjs');
  } catch (_e) {
  ...
  try {
    // eslint-disable-next-line global-require
    ledger = require(path.join(__dirname, '..', '..', 'data', 'section-command-ledger.json'));
  } catch (_e) {
    ledger = null;
  }
  ...
  for (const slug of sectionList) {
    ...
    const canonRow = sectionRegistry ? sectionRegistry.getSectionJob(slug) : null;
    if (!canonRow || !canonRow.job_id) {
      // Undeclared job: fall back to the verbatim-copy-if-absent behavior.
      // Canon Part 9: never overwrite an existing file.
      if (!fs.existsSync(targetPath)) {
        if (atomicWrite(targetPath, templateContent)) {
    ...
    const { block, fingerprint } = buildRulingBlock(canonRow, ledger);
    ...
    if (fs.existsSync(targetPath)) {
      try {
        ...
        existingFrontmatter = grayMatter(existingContent).data;
      } catch (_e) {
    ...
    if (
      existingContent !== null &&
      existingFrontmatter &&
      existingFrontmatter.ruling_fingerprint === fingerprint &&
      existingContent.indexOf(RULING_MARKER_END) !== -1
    ) {
      // No-op: unchanged inputs, already generated with this exact
      // fingerprint. Two consecutive generations leave the file
      // byte-identical because the second generation never writes.
      continue;
    }

    // The authored tail this generation MUST preserve byte-for-byte:
    //   - if a ruling block already exists, everything strictly after its
    //     end marker;
    //   - else if some other content exists (e.g. a Plan 01/275 verbatim
    //     copy with no marker yet), its body below any pre-existing
    //     frontmatter fence;
    //   - else (no file at all) the shipped template body (R-353-M).
    let authoredTail;
    if (existingContent !== null && existingContent.indexOf(RULING_MARKER_END) !== -1) {
      const idx = existingContent.indexOf(RULING_MARKER_END) + RULING_MARKER_END.length;
      authoredTail = existingContent.slice(idx).replace(/^\n+/, '\n');
    } else if (existingContent !== null) {
      authoredTail = '\n' + grayMatter(existingContent).content.replace(/^\n+/, '');
    } else {
    ...
    const frontmatterBlock = [
      '---',
      'icm_layer: 2',
      'job_id: "' + escapeYamlDoubleQuoted(canonRow.job_id) + '"',
    ...
    const newContent = frontmatterBlock + block + '\n' + authoredTail;

    if (atomicWrite(targetPath, newContent)) {
      if (existingContent === null) {
        result.contracts_created.push(slug);
      } else {
        result.ruling_regenerated.push(slug);
      }
    } else {
      result.errors.push('contract_write_failed:' + slug);
    }
...
function scaffoldRoomSkeleton(roomDir, opts) {
  ...
  const result = {
    ...
    contracts_created: [],
    // Phase 353 Plan 02 (Task 4): slugs whose L2 CONTEXT.md was
    // REGENERATED this run (frontmatter + the marked ruling block replaced,
    // authored prose below the end marker preserved byte-for-byte). Distinct
    // from contracts_created (a brand-new file with no prior content).
    ruling_regenerated: [],
    ...
    // Phase 353 Plan 01 (Task 3): whether lib/core/room-map.cjs successfully
    // built + wrote .mindrian/room-map.json this run, and how many icm_self
    // blocks it wrote. A map failure degrades to map_written:false plus a
    // pushed warning (see below); it never throws the scaffold.
    map_written: false,
  ...
  if (sectionTpl) {
    for (const section of sectionList) {
      ...
      if (!fs.existsSync(sectionRoomMd)) {
        ...
        // CR-01 fix: STATEMENT/SECTION_PURPOSE stay raw for the body blockquote
        // and body paragraph (plain markdown, no YAML significance there);
        // STATEMENT_YAML/SECTION_PURPOSE_YAML are the escaped values the
        // template substitutes into the double-quoted frontmatter keys.
        const sectionSubs = {
          STATEMENT: meta.statement || meta.purpose,
          STATEMENT_YAML: escapeYamlDoubleQuoted(meta.statement || meta.purpose),
          SECTION_NAME: section,
          SECTION_NAME_TITLE_CASE: titleCase,
          SECTION_PURPOSE: meta.purpose,
          SECTION_PURPOSE_YAML: escapeYamlDoubleQuoted(meta.purpose),
  ...
  if (identityTpl) {
    for (const dirName of Object.keys(IDENTITY_DIRECTORIES)) {
      ...
      if (!fs.existsSync(identityRoomMd)) {
        ...
        // CR-01 fix (same class of defect, independently reproduced against
        // references/ROOM.md during the CR-01 fix pass): DIRECTORY_PURPOSE
        // stays raw for the body paragraph; DIRECTORY_PURPOSE_YAML is the
        // escaped value the template substitutes into the double-quoted
        // frontmatter `purpose:` key.
        const identitySubs = {
          DIRECTORY_TYPE: meta.directory_type,
          DIRECTORY_PURPOSE: meta.purpose,
          DIRECTORY_PURPOSE_YAML: escapeYamlDoubleQuoted(meta.purpose),
        };
        ...
        if (atomicWrite(identityRoomMd, identityContent)) {
          ...
          // WR-01 fix: every other write site in this function pushes a
          // named error on atomicWrite failure; this loop was the one
          // asymmetric silent-swallow path.
          result.errors.push('identity_write_failed:' + dirName);
        }
      }
    }
  } else {
  ...
  // Phase 353 Plan 01 (Task 3): build the room map + write the icm_self
  // blocks LAST, after every ROOM.md this function creates already exists
  // on disk. Required lazily, inside the try, so a room-map.cjs load
  // failure can never break scaffolding for an unrelated room -- the same
  // degrade-to-warning discipline this function already applies to
  // thinness-acknowledgment below.
  try {
    // eslint-disable-next-line global-require
    const roomMap = require('./room-map.cjs');
    const builtMap = roomMap.buildRoomMap(roomDir);
    const writeResult = roomMap.writeRoomMap(roomDir, builtMap);
    if (writeResult && writeResult.ok) {
      const blockResult = roomMap.writeSelfBlocks(roomDir, builtMap);
      result.map_written = true;
      result.self_blocks_written = (blockResult && Array.isArray(blockResult.written)) ? blockResult.written.length : 0;
    } else {
      result.warnings.push('room_map_write_failed');
    }
  } catch (_e) {
    result.warnings.push('room_map_write_failed');
  }
...
module.exports = {
  ...
  // Phase 353 Plan 02 Task 9: exported so lib/core/doctor/section-ruling-
  // module.cjs can recompute a section's expected ruling_fingerprint
  // without re-implementing the sha256-over-canon-plus-ledger recipe.
  buildRulingBlock: buildRulingBlock,
