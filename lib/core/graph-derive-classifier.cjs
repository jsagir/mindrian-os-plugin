...
function resolveFloors() {
  return {
    converges: resolveFloor('DERIVE_CONVERGES_FLOOR', DERIVE_CONVERGES_FLOOR_DEFAULT),
    informs: resolveFloor('DERIVE_INFORMS_FLOOR', DERIVE_INFORMS_FLOOR_DEFAULT),
  };
}
 ...
 *
 * injectOpts is the test seam: { scoreFn?, scoreOpts? }. scoreFn defaults to the
 * rs-differential-scorer scoreMeasured; scoreOpts is merged into the scoreMeasured
 * opts so tests inject encodeFn / _forceUnavailable deterministically.
 *
 * Returns:
...
async function scoreBasedDeriveFn(step, injectOpts) {
  ...
  // D-04 upstream silence: encoder unavailable -> derive nothing here.
  if (scoreResult.semantic === null || scoreResult.semantic === undefined) return [];
  if (scoreResult.warning === 'encoder_unavailable') return [];
