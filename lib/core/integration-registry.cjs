...
const INTEGRATION_CATALOG = {
  ...
  velma: {
    env: ['VELMA_API_KEY', 'MODULATE_API_KEY'],
...
function detectSingle(name, opts) {
  ...
  // Environment variable check (supports string or array of env var names)
  if (catalog.env) {
    const envNames = Array.isArray(catalog.env) ? catalog.env : [catalog.env];
    result.env_set = envNames.some(name => !!process.env[name]);
  }
