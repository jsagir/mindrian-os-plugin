 ...
 * catch -- a bare-server-key matcher that never fired -- so it was proof of
 * nothing, and it laundered a live defect into this helper's own grounding
 * comment. Corrected in Phase 239; see scripts/check-brain-tool-liveness.cjs,
 * the gate that now PROVES the shape against a real MCP tools/list handshake
 * rather than a comment asserting it. The other half of the original
 * citation is genuinely correct and stands unchanged: the same
 * double-underscore shape is visible in live registered tool names
 * (mcp__langtalks-graph-expert__query_relationship). The reason this wrapper
 * has to exist at all: Claude Code evaluates a hook matcher per tool-call
 * event and never validates it against a live tool registry at startup, so a
 * matcher pointing at a removed tool does not warn or error, it silently
 * stops firing forever. That silent no-op is a dead seam the product itself
 * gives no signal for.
 *
