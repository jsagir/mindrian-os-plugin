...
function findIcmSelfBlockRange(body) {
  ...
  // Convert line indices back to character offsets. `body.split('\n')`
  // yields N pieces joined by N-1 real newlines, so summing `length + 1`
  // for every piece up to and including the LAST one over-counts by one
  // character (there is no newline after the final piece). When the block
  // runs to the end of the body (endLine === lines.length), the true end
  // offset is simply body.length; otherwise every line before endLine WAS
  // followed by a real newline in the original text, so the summation is
  // exact.
  let start = 0;
...
// line. Never adds a separator newline when there is nothing after the
// block (T-353 regression guard: an unconditional trailing '\n' here plus
// the caller's own '\n---\n' produced a growing blank line on every
// rewrite -- caught by tests/test-275-section-schema.cjs's pre-existing
// "a second run is byte-identical to the first" assertion).
function spliceIcmSelfIntoFrontmatter(frontmatterBody, blockText) {
  const range = findIcmSelfBlockRange(frontmatterBody);
  if (range) {
    const before = frontmatterBody.slice(0, range.start);
    const after = frontmatterBody.slice(range.end);
    if (after.length === 0) return before + blockText;
    return before + blockText + '\n' + after;
  }
