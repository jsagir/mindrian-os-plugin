 ...
 *
 *   1. Numeric position    'switch to 8' (verb required)      -> registry[N-1]
...
// Numeric: 'switch to 8' or 'go to 5' or 'jump to 3' or 'move to 2'. The
// imperative verb is REQUIRED (2026-06-28 fix): a bare integer like '1' is
// a reply to a numbered menu, NOT a room-switch command. The UserPromptSubmit
// payload carries only the user prompt (never the prior assistant turn), so
// the hook cannot see an outstanding menu to disambiguate -- requiring the
// verb is the disambiguation. A bare '1' now falls through to the similarity
// heuristic, where a lone integer tokenizes to nothing (length<2 dropped) and
// stays silent. Bounded to a small synonym set so we don't accidentally match
// prose like 'jump 3 hoops' (no 'to' keyword).
