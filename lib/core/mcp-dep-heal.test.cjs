...
const FALLBACK = ['@modelcontextprotocol/server', '@modelcontextprotocol/sdk', 'zod'];
...
test('bug_011: productionDepNames returns the full dependency set from package.json', () => {
  ...
  try {
    ...
    assert.ok(names.length > FALLBACK.length, 'full set must be broader than the fallback (' + FALLBACK.length + ' deps)');
  } finally {
...
test('bug_011: ensureDepsPresent detects a partial tree (sdk+zod present, ext-apps absent)', () => {
  ...
  try {
    // With the narrow 2-dep probe (the pre-fix default, and NOT today's
    // FALLBACK, which Phase 267 Plan 05 grew to 3 entries) this tree looks
    // healthy. Confirm that first, to prove the bug was real. This is a
    // literal historical probe, deliberately not FALLBACK -- FALLBACK is the
    // CURRENT default (today 3 deps), not the pre-fix bug's narrow set.
    const PRE_FIX_NARROW_PROBE = ['@modelcontextprotocol/sdk', 'zod'];
    let narrowSawMissing = false;
    for (const d of PRE_FIX_NARROW_PROBE) {
