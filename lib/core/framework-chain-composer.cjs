 ...
 *
 * Phase 205-06 (SCOPE-6 pipelining). The SENS-10 exits COMPOSE, they are NOT a
 * menu. Given a SENS-10 `cause` enum (answer_unheard / assertion_unvalidated /
 * stuck_unlocated / wrong_frame -- from lib/core/sensors/sensor-circularity.cjs),
 * this reads the ordered command chain from recipe-maps.recipeForCause and builds
 * an ordered step list: the autonomous_safe RECON PREFIX -> the MATERIAL GATE
 * (bono) -> the TELL SYNTHESIS terminal.
 *
 * Every step's autonomous_safe / posture is SOURCED from
 * recipe-maps.postureForCommand (the registry authority, the ONE posture door) --
 * it is NEVER fabricated here (T-166-02 / T-205-06-E). A command with no registry
 * autonomous_safe flag degrades to a withhold-default halt step (posture 'halt',
 * gate:true) -- never a fabricated run. This is the compose half of the wiring:
 * composeWorkflow -> validateChainAutonomy -> chain-executor.runChain, where
 * runChain auto-runs the leading autonomous_safe recon steps and HALTS at the
 * first material gate (GUIDED safe-halt, Reach rule 8 / Phase 166).
 *
 * An unknown / empty cause -> []. Never throws.
 *
 * @param {string} cause -- a SENS-10 cause enum
 * @returns {Array<{step:number, command:string, autonomous_safe:boolean, posture:string, gate:boolean}>}
 */
function composeWorkflow(cause) {
  let recipeMaps = null;
  try {
    recipeMaps = require('./recipe-maps.cjs');
  } catch (_e) {
    return []; // recipe-maps unavailable -> nothing to compose (degrade, not throw)
  }
  const recipe = recipeMaps.recipeForCause(cause);
  if (!Array.isArray(recipe) || recipe.length === 0) return [];

  return recipe.map(function (command, i) {
    // Source the posture from the registry authority -- NEVER a fabricated
    // autonomous_safe (T-166-02). A command unknown to the registry degrades to
    // the withhold-default halt ({ autonomous_safe:false, posture:'halt' }).
    let verdict = { command: command, autonomous_safe: false, posture: 'halt' };
    try {
      const p = recipeMaps.postureForCommand(command);
      if (p && typeof p === 'object') verdict = p;
    } catch (_e) {
      // withhold-default: a resolver fault halts, never runs
    }
    const autonomousSafe = verdict.autonomous_safe === true;
    return {
      step: i + 1,
      command: command,
      autonomous_safe: autonomousSafe,
      posture: verdict.posture,
      // a non-run step is the material gate where the GUIDED safe-halt fires.
      gate: verdict.posture !== 'run',
    };
  });
}
...
module.exports = {
  ...
  // Phase 205-06 (SCOPE-6 pipelining): the SENS-10 cause -> compose path.
