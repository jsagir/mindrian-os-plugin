 ...
 * the data there). mapFrameworkToCommandSlug() delegates to the resolver
 * too (falling back to the legacy table only for names the registry does
 * not know yet) so any remaining caller also gets the resolver answer.
 * FRAMEWORK_TO_COMMAND_SLUG and KNOWN_FRAMEWORKS stay exported (122-05
 * prunes FRAMEWORK_TO_COMMAND_SLUG to a pass-through).
 *
 ...
 *
 * Maps a framework name (in any case) to a /mos: command slug. Phase 122-04:
 * delegates to the resolver first (lib/workflow/command-resolver.cjs
 * commandsForFramework -- the only door, reads only data/command-registry.json,
 * never touches the Brain); falls back to the legacy FRAMEWORK_TO_COMMAND_SLUG
 * table only for names the generated registry does not know yet, and finally
 * to FALLBACK_COMMAND_SLUG ('beautiful-question'). 122-05 prunes the legacy
 * table; this delegation keeps every remaining caller on the resolver answer.
 *
...
function mapFrameworkToCommandSlug(name) {
  ...
  try {
    ...
    // resolver unavailable -> fall through to the legacy table
  }
  // Legacy table fallback for names the registry does not know yet.
...
module.exports = {
  ...
  // Exported for back-compat (Phase 122-04): delegates to the resolver
  // first, falls back to the legacy table; 122-05 prunes the legacy table.
