...
async function generateStage7Roadmap(roomDir, opportunities, opts) {
  ...
  const seedDir = path.join(resolvedRoom, 'opportunity-bank', seedSlug);
  const roadmapDir = path.join(seedDir, STAGE7_FOLDER);
  ...
  // ICM Layer 0 (CLAUDE.md decision 15: EVERY directory gets a ROOM.md, no
  // exceptions). The recursive mkdir above implicitly creates the intermediate
  // seed folder (opportunity-bank/trending-to-absurd-<seed>/); it is a directory
  // this generator OWNS, so it gets its own identity file. Without this, the
  // seed folder is a ROOM.md-less directory -- exactly the gap the Wave-6
  // adversarial verdict (CHECK 1, room_md_nesting) is designed to catch. The
  // write is idempotent, so if a sibling TTA surface already created the seed
  // folder + its ROOM.md, this overwrites with an equivalent identity body.
  writeRoomMd(seedDir, 'Trending-to-Absurd Seed: ' + seedLabel,
    'The trending-to-absurd opportunity subtree for the ' + seedLabel + ' seed. '
    + 'Holds the absurd-extrapolation artifacts and the Stage 7 roadmap.');
