 ...
 *   - Plan 120-01 selector-dispatcher.pickShape (F.1 next-move surfacing; Phase 188-01 SFS-06)
 *
 ...
 *   dispatching the surface. The other two enforcement points are schema.cjs::
...
// type='DERIVED_FROM') BEFORE dispatching the surface. If the count is < 1, emits a
...
//
// On success: dispatches the breakthrough as an F.1 next-move via
// selector-dispatcher.pickShape (Phase 188-01 SFS-06: Breakthrough is a MOVE now,
// not a shape), emits a breakthrough_surfaced memory_event, and flips the
// Breakthrough node's properties.surfaced = true.
function surfaceBreakthrough(breakthrough, options) {
  ...
  // the surface dispatch. Defense in depth on top of schema.cjs::validateProvenance
  // and the retained renderer's artifact_ids check.
  ...
  // Phase 188-01 (SFS-06): re-home the breakthrough surface. Bare 'F.7' is the
  // canonical dial now (D-10 canonical ten); Breakthrough is a MOVE, not a shape,
  // so it surfaces as an F.1 next-move through the same SEED-020 single door. The
  // provenance HARD FLOOR is already enforced above (the DERIVED_FROM SQL check
  // returns 'provenance_required' before this point), so a provenance-less
  // breakthrough never reaches this dispatch. If the dispatcher is unavailable
  // (e.g. isolated unit testing of the surface gate), degrade gracefully -- we
  // still emit the breakthrough_surfaced event and flip the node property because
  // those are the load-bearing side effects per the test contract.
  let rendered = null;
  ...
  try {
    ...
    if (row) {
      ...
      // Phase 194-06 Task 1 (PSB-08 pre-req): bump last_modified_at alongside
      // last_seen_at so this read-merge-write on the breakthrough node's
      // properties moves the reconcile-guard CAS token. Both stamps share one now.
      const nowMs = Date.now();
      props.surfaced = true;
      props.surfaced_at = nowMs;
      db.prepare(
        "UPDATE nodes SET properties = ?, last_seen_at = ?, last_modified_at = ? WHERE id = ?"
      ).run(JSON.stringify(props), nowMs, nowMs, breakthrough.id);
    }
  } catch (_e) { /* swallow */ }
