...
/**
 * BRAIN_SHAPED_TOOL_MATCHER -- the scrutiny-scope authority, alongside the
 * byte-unchanged BRAIN_TOOL_MATCHER trust-scope authority above. Quick task
 * 260906-gr1: closes the Part 8 egress-guard bypass for an MCP connector
 * registered under a non-canonical key (the live `theo` case, recorded as an
 * accepted residual risk in docs/339-NOTE-theo-desktop-connector-key.md
 * Section 2 and superseded in Section 8).
 *
 * (a) TWO PREDICATES, TWO DIFFERENT QUESTIONS. `isBrainTool` (above) answers
 * "is this THE trusted Brain connector?" (trust) and is byte-unchanged by
 * this addition. `isBrainShapedTool` (below) answers "might this be carrying
 * methodology traffic to a Brain backend, so Part 8 must inspect it?"
 * (scrutiny). Brain-shaped grants INSPECTION, never TRUST: the guard can
 * only block or gate a Brain-shaped call, never grant an allow the payload
 * had not already earned. So threat T3 (anti-impersonation) is unaffected --
 * `mcp__evil-brain__brain_ask` becomes Brain-shaped (inspected) while
 * `isBrainTool` on the same name stays false (still not trusted).
 *
 * (b) A RULE, NOT A ROSTER. Alternation 2 below is a connector-agnostic
 * suffix rule (`brain_<verb>`), not a maintained list of known operation
 * names. The docblock above this one already records the Phase 239 drift
 * incident: two copies of a tool-name pattern (hooks.json and this module's
 * own prefix test) went independently stale the moment the Brain server
 * shipped inside a plugin. A roster is exactly that same shape of liability
 * -- one more list that can silently drift out of sync with the live tool
 * surface. A structural rule has no roster to drift.
 *
 * (c) THE DISJOINTNESS PROOF, BY NAME. This plugin ships its own LOCAL
 * `mindrian-os` MCP server (`.mcp.json`), whose tools carry user room
 * content BY DESIGN -- that is the local side of Canon Part 8, not the
 * Brain side. `room_bind`, `graph_write`, `gate_render`, `gate_answer`,
 * `chain_run` and `room_search` are LOCAL `mindrian-os` tool names and are
 * therefore deliberately OUTSIDE this scope: none of them starts with
 * `brain_`. `tests/test-260906-gr1-brain-shaped-tool-gate.cjs` is the
 * standing guard that keeps this true -- it enumerates the local tool
 * surface from disk and asserts zero collisions, so a future local tool
 * named `brain_*` becomes a red test rather than a silently bricked local
 * write path.
 *
 * (d) KNOWN BOUND. The connector-key character class in alternation 2
 * (`[a-z0-9_-]+`) is lowercase-only, matching the existing plugin-prefix
 * class in BRAIN_TOOL_MATCHER above. An uppercase connector key falls
 * outside this scope.
 */
const BRAIN_SHAPED_TOOL_MATCHER = 'mcp__(?:(?:plugin_[a-z0-9_-]+_)?(?:mindrian-brain|pws-brain-mcp)__.*|[a-z0-9_-]+__brain_[a-z0-9_]+)';

/**
 * _BRAIN_SHAPED_TOOL_RE -- anchored RegExp compiled once at module scope,
 * mirroring the exact anchoring asymmetry of _BRAIN_TOOL_RE above:
 * hooks/hooks.json carries the UNANCHORED string (Claude Code evaluates
 * matchers unanchored), and this in-code re-check anchors with ^...$
 * because the tighter posture is the backstop.
 */
const _BRAIN_SHAPED_TOOL_RE = new RegExp('^' + BRAIN_SHAPED_TOOL_MATCHER + '$');
...
/**
 * SHIM_BACKED_BRAIN_TOOL_MATCHER -- the ROUTE-scope authority, the third of
 * three predicates now answering three different questions: trust
 * (isBrainTool), inspection scope (isBrainShapedTool), and route
 * (isShimBackedBrainTool: does this scope provably reach
 * bin/mindrian-brain-mcp-client.cjs, so brain-client.cjs::callTool's second
 * classification and its egress_disclosure actually run there).
 *
 * Quick task 260917-ild (Codex F1): quick task 260917-dgf keyed the Part 8
 * hook's ambiguous-verdict allow on isBrainTool, which ALSO trusts
 * mcp__pws-brain-mcp__* -- a direct HTTPS connector registered outside this
 * plugin (lib/mcp/brain-composition-census.cjs's ruling: pws-brain-mcp is
 * direct HTTPS with no local plugin code anywhere in its path). No local
 * code sits between that connector and the remote backend, so
 * brain-client.cjs::callTool's second classification -- the one the
 * ambiguous-verdict allow's own argument depends on -- never runs on that
 * route, and the allow was unearned there.
 *
 * .mcp.json registers exactly ONE Brain server key, "mindrian-brain", whose
 * command is bin/mindrian-brain-mcp-client.cjs; that shim requires
 * lib/core/brain-client.cjs and every handler delegates to callTool. That
 * single server key is the reason this pattern DROPS the pws-brain-mcp
 * alternation BRAIN_TOOL_MATCHER carries: only two tool-name shapes provably
 * reach the shim --
 *   mcp__mindrian-brain__*                  (project scope)
 *   mcp__plugin_mos_mindrian-brain__*        (plugin scope, Desktop and
 *                                             Cowork resolve here too --
 *                                             same shim, same client)
 * -- so the alternation below keeps only "mindrian-brain", never
 * "pws-brain-mcp".
 *
 * Same bounded [a-z0-9_-]+ plugin-prefix class and the same
 * anti-impersonation reasoning as _BRAIN_TOOL_RE above (threat T3): a
 * foreign server whose name merely contains "mindrian-brain" cannot gain
 * this route classification via an over-broad pattern.
 */
const SHIM_BACKED_BRAIN_TOOL_MATCHER = 'mcp__(?:plugin_[a-z0-9_-]+_)?mindrian-brain__.*';

/**
 * _SHIM_BACKED_BRAIN_TOOL_RE -- anchored RegExp compiled once at module
 * scope, mirroring the anchoring posture of _BRAIN_TOOL_RE and
 * _BRAIN_SHAPED_TOOL_RE above.
 */
const _SHIM_BACKED_BRAIN_TOOL_RE = new RegExp('^' + SHIM_BACKED_BRAIN_TOOL_MATCHER + '$');

/**
 * isShimBackedBrainTool(toolName) -- matcher for the ROUTE question. Quick
 * task 260917-ild (Codex F1). See the SHIM_BACKED_BRAIN_TOOL_MATCHER
 * docblock above for the full reasoning. Same defensive typeof guard the
 * sibling predicates use.
 *
 * @param {string} toolName    tool name string from PreToolUse stdin
 * @returns {boolean}          true iff this scope provably routes through
 *                             bin/mindrian-brain-mcp-client.cjs into
 *                             brain-client.cjs::callTool
 */
function isShimBackedBrainTool(toolName) {
  return typeof toolName === 'string' && _SHIM_BACKED_BRAIN_TOOL_RE.test(toolName);
}

/**
 * isBrainShapedTool(toolName) -- matcher for the widened Part 8 scrutiny
 * scope. Quick task 260906-gr1. See the BRAIN_SHAPED_TOOL_MATCHER docblock
 * above for the full reasoning (the two-predicates distinction, why this is
 * a rule and not a roster, the local-surface disjointness proof, and the
 * lowercase-only bound).
 *
 * @param {string} toolName    tool name string from PreToolUse/PostToolUse stdin
 * @returns {boolean}          true iff the tool is Brain-shaped (in scope for
 *                             Part 8 inspection, which is NOT the same as
 *                             being the trusted Brain connector)
 */
 ...
 *
 * If isBrainShapedTool returns false, returns a passthrough envelope (no
 * updatedToolOutput). Other tools' responses are not modified. Quick task
 * 260906-gr1: retargeted from isBrainTool to isBrainShapedTool so this
 * function has ONE gate rather than two that can diverge from the live
 * hook (scripts/brain-response-sanitize-hook.cjs), which builds its own
 * envelope inline and is not on this code path today -- precisely why a
 * divergence here would have gone unnoticed.
 *
...
module.exports = {
  ...
  isBrainTool,
  isBrainShapedTool,
  isShimBackedBrainTool,
  BRAIN_TOOL_MATCHER,
  BRAIN_SHAPED_TOOL_MATCHER,
  SHIM_BACKED_BRAIN_TOOL_MATCHER,
  PII_PATTERNS,
  ALLOWLIST,
};
