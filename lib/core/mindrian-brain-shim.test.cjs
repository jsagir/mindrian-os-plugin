...
(function test2_sixTools() {
  ...
  try {
    ...
    // Each of the 6 distinct names must appear exactly once.
    const names = matches.map((m) => m.replace(/.*brain_/, 'brain_').replace(/['"].*/, ''));
    const expected = ['brain_ask', 'brain_query', 'brain_schema', 'brain_search', 'brain_stats', 'brain_write'];
    for (const n of expected) {
      assert.equal(names.filter((x) => x === n).length, 1,
        'tool name ' + n + ' must appear exactly once; got ' + names.filter((x) => x === n).length);
    }
    ok(label);
  } catch (err) { fail(label, err); }
})();
...
// ---------------------------------------------------------------------------
// Test 7: zero fetch / http / brain.mindrian / https? in shim source.
// ---------------------------------------------------------------------------
(function test7_delegationProperty() {
  const label = 'shim source contains zero fetch( / http. / brain.mindrian / https? matches (Canon Part 8)';
  try {
    const src = fs.readFileSync(SHIM_PATH, 'utf8');
    // Strip leading-line comments for the audit (the doc header explicitly
    // references the forbidden tokens as PROHIBITED -- we're scanning code).
    // The plan's adversarial scan uses raw grep so we mirror that here.
    // However the doc header is allowed to reference the tokens in english
    // as long as they're not active code. To stay conservative we match
    // each forbidden token and report exactly where it appears.
    const forbidden = [
      { re: /fetch\(/g, name: 'fetch(' },
      { re: /\bhttp\./g, name: 'http.' },
      { re: /brain\.mindrian\.ai/g, name: 'brain.mindrian.ai' },
      { re: /mindrian-brain\.onrender\.com/g, name: 'mindrian-brain.onrender.com' },
      { re: /https?:\/\//g, name: 'https?://' },
    ];
    // Active-code scan: strip block comments and line comments before testing.
    let code = src.replace(/\/\*[\s\S]*?\*\//g, '');
    code = code.replace(/^[ \t]*\/\/.*$/gm, '');
    for (const f of forbidden) {
      const m = code.match(f.re) || [];
      assert.equal(m.length, 0,
        'forbidden network surface "' + f.name + '" found ' + m.length + ' time(s) in active code');
    }
    ok(label);
  } catch (err) { fail(label, err); }
})();
