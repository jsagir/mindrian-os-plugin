 ...
 *   reserved NO_ROOM_SLUG sentinel as the EXPLICIT primary (`primary ===
 *   NO_ROOM_SLUG`), or as the SOLE member of `bound` with no real room slug
 *   alongside it and a null primary (WR-01/WR-02, 360-REVIEW.md, fix for
 *   CR-equivalent P-2 gap: `consumeSessionBinding` writes `bound` from the
 *   F.8 card's confirmed multi-select toggle set independently of
 *   `primary`, so a navigator who checks both a real room's box AND "dev
 *   repo / no room" without also picking a primary produces exactly
 *   `{ bound: ['some-room', NO_ROOM_SLUG], primary: null }` -- that shape
 *   must NOT be read as "chose no room", because a real room is also
 *   sitting right there in `bound`; today's picker behavior applies).
...
function unboundPickerSuppression(args) {
  try {
    ...
    if (!realPrimary) {
      // WR-02 fix: the sentinel only means "chose no room" when it is the EXPLICIT
      // primary, or when it is the SOLE entry in `bound` (no real room slug riding
      // alongside it). A `bound` that also holds a real room slug, with `primary`
      // null, is an ambiguous multi-select toggle state, not a remembered
      // no-room choice -- fall through to the cwd-outside-unbound check (or null)
      // below, same as any other unbound-with-a-real-room-candidate session.
      const sentinelIsExplicitPrimary = rawPrimary === NO_ROOM_SLUG;
      const sentinelIsSoleBinding = realBound.length === 0 && rawBound.indexOf(NO_ROOM_SLUG) !== -1;
      if (sentinelIsExplicitPrimary || sentinelIsSoleBinding) return 'no-room-remembered';
