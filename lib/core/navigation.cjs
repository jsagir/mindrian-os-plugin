...
const reasoningWrite = require('./navigation/reasoning-write.cjs');
const goalAnchor = require('./navigation/goal-anchor.cjs');
const jtbdAnchor = require('./navigation/jtbd-anchor.cjs');
const sectionGate = require('./navigation/section-gate.cjs');
const chainState = require('./navigation/chain-state.cjs');
...
const verification = require('./navigation/verification.cjs');
...
module.exports = {
  ...
  // Phase 348 (SUPER-05): object-shorthand re-export forwards every argument
  // (including the third opts bag) unchanged; the chokepoint surface must
  // not swallow a parameter, or includeSuperseded/skipped is unreachable
  // from every MCP caller.
  ...
  // Reasoning-node writer (quick task 260903-i2x -- T2's node-writing half of
  //   the ratified MOS Reasoning Constitution v3.1.0; additive re-export so
  //   gate_answer's approve branch and artifact_file reach the shared
  //   typed-node + provenance-edge writer through the navigation chokepoint,
  //   never a direct room.db open (Canon Part 9 substrate guard).
  //   writeReasoningNode mints a decision/claim node at review_status=
  //   'proposed' (NEVER confirmed -- DC-4, Part 9 role 5) plus SOURCED_FROM /
  //   USES_FRAMEWORK provenance edges; degrades to writing nothing extra
  //   (never an error, never a fabricated id) when its inputs are absent.
  //   REASONING_NODE_ID / FRAMEWORK_NODE_ID are the shared id-minting
  //   helpers so callers and tests share one convention. The writer takes
  //   (db, params) over a caller-owned room.db handle, so this file stays
  //   the single door. Same thin additive-re-export pattern as
  //   writeClaimNode / writeEdge / writeDecisionNode above.)
  writeReasoningNode: reasoningWrite.writeReasoningNode,
  REASONING_NODE_ID: reasoningWrite.REASONING_NODE_ID,
  FRAMEWORK_NODE_ID: reasoningWrite.FRAMEWORK_NODE_ID,

  // Goal-anchor writer (Phase 345-06 -- Card (b), 345-ICM-CONSULT R2/R3; a
  //   thin additive re-export, same idiom as logMemoryEvent / writeEdge /
  //   writeReasoningNode above, so the strategy card builder reaches the
  //   idempotent `goal:<room-slug>` anchor mint through the navigation
  //   chokepoint, never a direct room.db open (Canon Part 9 substrate guard).
  //   mintGoalAnchor upserts a payload-free node at review_status='proposed'
  //   (promoted to 'confirmed' only by navigation.confirmNode on a gate
  //   approve, plan 07); GOAL_ANCHOR_ID is the shared id-minting helper so
  //   the card builder and the tests share one convention. Order is
  ...
  // jtbd-anchor writer (Phase 353 Plan 02 Task 6 -- D-353-8, R-353-I,
  //   R-353-J; the same thin additive re-export idiom as mintGoalAnchor
  //   above, so a filing tool reaches the idempotent `jtbd:<job_id>` anchor
  //   mint through the navigation chokepoint, never a direct room.db open.
  //   mintJtbdAnchor upserts a payload-free 'jtbd'-typed node (NEVER
  //   'claim') at review_status='proposed'; JTBD_ANCHOR_ID is the shared
  //   id-minting helper. Order is load-bearing, repeating the mint-before-
  //   edge rule once more here because this is the chokepoint every caller
  //   actually requires: the caller MUST mint before writeEdge ever names
  //   this id, because writeEdge probes no endpoint (D-169-11) and a
  //   SOURCED_FROM edge to an unminted anchor becomes a dangling row.)
  mintJtbdAnchor: jtbdAnchor.mintJtbdAnchor,
  JTBD_ANCHOR_ID: jtbdAnchor.JTBD_ANCHOR_ID,

  // Section filing gate (Phase 353 Plan 02 Task 7 -- D-353-8, R-353-D). A
  //   thin additive re-export so lib/mcp/tools/views.cjs::fileArtifact and
  //   lib/mcp/tools/claim.cjs's claim_write handler reach the gate through
  //   the navigation chokepoint. resolveFocusSection reads the active
  //   focus's own section property (claim_write's only route to a section,
  //   since its MCP schema is not widened); evaluateFilingGate is the pure
  //   verdict function; GATE_MODES is the closed two-member enum ('flag'
  //   default, 'strict').
  resolveFocusSection: sectionGate.resolveFocusSection,
  evaluateFilingGate: sectionGate.evaluateFilingGate,
  GATE_MODES: sectionGate.GATE_MODES,
  // Phase 353 Plan 02 Task 9: the section-ruling doctor module's
  // claims_without_anchor statement, read through this chokepoint so the
  // doctor module itself carries no raw SELECT.
  countClaimsWithoutJtbdAnchor: sectionGate.countClaimsWithoutJtbdAnchor,

  // Chain-state writer (Phase 347-03 -- SHARED-01; a thin additive
  //   re-export so chain callers reach the typed chain-state writer through
  //   the Part 9 chokepoint instead of requiring the submodule directly.
  //   writeChainStateRecord mints a chain_state node plus its mandatory
  //   SOURCED_FROM anchor edge and, when a predecessor record already
  //   exists for the run, a FEEDS_INTO successor edge; readChainState walks
  //   the ordered records back for a run, and reconstructStepInput rebuilds
  //   one step's input from the graph alone. The writer takes a
  //   caller-owned room.db handle and opens nothing itself, so this file
  //   stays the single door. The records are a PROJECTION of
  //   pipeline-state.json, which remains the sole chain-state truth for
  //   resume position under D-166-02 and B1 (docs/2026-09-14-CHAIN-SHARED-
  //   STATE-CONTRACT.md Section 6); on disagreement the file wins. Same
  //   thin additive-re-export pattern as writeClaimNode / writeEdge /
  //   writeReasoningNode above.)
  writeChainStateRecord: chainState.writeChainStateRecord,
  readChainState: chainState.readChainState,
  reconstructStepInput: chainState.reconstructStepInput,
  ...
  // RCA eureka-entity-extraction-boilerplate-candidates: the one sanctioned
  // deletion in this door, bound to the machine-authored legacy entity rows
  // typed-entity.cjs itself minted (see its docblock). Quick task 260917-ild
  // (Codex F2/F3) narrowed the second parameter: purgeLegacySelfReferentialEntities
  // (db, opts) now requires opts.scaffoldKinds (the scaffold-kind memory-kind
  // vocabulary) and deletes a candidate row only when every one of its
  // DESCRIBES edges resolves to a scaffold-kind memory_artifact anchor, never
  // by id/source_path/type shape alone; any review_status other than proposed
  // or NULL is not a candidate at all. This function re-export is a bare
  // reference, so the added second parameter passes through with no edit
  // here. Same thin additive-re-export pattern; no caller reaches the rows by
  // raw SQL.
  purgeLegacySelfReferentialEntities: typedEntity.purgeLegacySelfReferentialEntities,
  ...
  FRAME_ORIGINS: typedFrame.FRAME_ORIGINS,
  classifyFrameChange: typedFrame.classifyFrameChange,
  readFrameProvenance: typedFrame.readFrameProvenance,

  // Phase 358-07 (Rome B2): the governing question is a room-level record
  // written only by lib/core/frame-provenance.cjs; these are local reads and
  // constants, nothing crosses to the Brain (Canon Part 8).
  FRAME_ORIGINS_ORDERED: typedFrame.FRAME_ORIGINS_ORDERED,
  frameOriginInfo: typedFrame.frameOriginInfo,
  GOVERNING_QUESTION_ROLE: typedFrame.GOVERNING_QUESTION_ROLE,
  GOVERNING_QUESTION_NODE_ID: typedFrame.GOVERNING_QUESTION_NODE_ID,
  readGoverningQuestionVersions: typedFrame.readGoverningQuestionVersions,

  // B1 verification record and room portrait. These are descriptive local
  // writes/reads; recording a check never promotes a claim.
  recordClaimVerification: verification.recordClaimVerification,
  readVerificationPortrait: verification.readVerificationPortrait,
  VERIFICATION_CHECK_STATUSES: verification.CHECK_STATUSES,
  VERIFICATION_AGAINST_KINDS: verification.AGAINST_KINDS,
  VERIFICATION_METHODS: verification.METHODS,
  VERIFICATION_RESULTS: verification.RESULTS,

  // Phase 358-01 (Rome B1): additive re-exports only. The claim view and
  // portrait are local reads; nothing here reaches the Brain (Canon Part 8).
  VERIFICATION_RUNGS: verification.VERIFICATION_RUNGS,
  verificationRungInfo: verification.rungInfo,
  deriveVerificationStatus: verification.deriveCheckStatus,
  readClaimVerification: verification.readClaimVerification,
  listClaimsForChecking: verification.listClaimsForChecking,
  renderVerificationPortraitLines: verification.renderPortraitLines,
  renderClaimViewLines: verification.renderClaimViewLines,
  renderClaimListLines: verification.renderClaimListLines,
...
// Arbitration-decision logging (Phase 346-05 -- ARB-07/ARB-08; a thin
// additive re-export, same idiom as logMemoryEvent / writeEdge /
// getRoomContext above, so a future decide() caller (346-07) reaches the
// arbitration audit-row writer through the navigation chokepoint, never a
// direct require of the internal navigation/arbitration-log.cjs. Canon
// Part 8: the row is LOCAL room.db bookkeeping, enum-token payload only,
// never prose, never crosses to Brain. Canon Part 9: a system-bookkeeping
// audit node under the Part 9 v1.5 carve-out (created_by=system
// review_status=confirmed is canon-legal because the row records what the
// system DECIDED, not a venture truth-claim).
//
// Defined via Object.defineProperty with a LAZY getter, not a top-level
// require + static property, because a top-level require here closes a
// real cycle: navigation.cjs -> arbitration-log.cjs -> arbitration.cjs ->
// decision-axes.cjs -> navigation-engine.cjs -> require('./navigation.cjs')
// (this very file, still mid-load, module.exports still the default {}).
// That is the exact "module exports {} at first access inside a circular
// dependency" defect lib/core/navigation/room-birth.cjs:74-77 already
// documents and lib/core/brain-client.cjs:1650/2347 already lazy-requires
// around. The getter defers the require until the property is actually
// read, by which point every module in the cycle has finished loading, so
// it returns the SAME function reference arbitration-log.cjs exports
// (verified by tests/test-346-arbitration-event.cjs's identity assertion),
// never a wrapper.
Object.defineProperty(module.exports, 'logArbitrationDecision', {
  enumerable: true,
  configurable: true,
  get: function () {
    return require('./navigation/arbitration-log.cjs').logArbitrationDecision;
  },
});
