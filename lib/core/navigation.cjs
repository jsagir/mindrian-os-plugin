...
module.exports = {
  ...
  //   reach the dedicated calibration_observations row store through the
  //   navigation chokepoint, never a direct require of navigation/calibration-log.cjs.
  ...
  //   audit-marker memory_event (calibration_observation_logged). SHADOW-ONLY:
  //   it logs, it does not fire; no reach is minted, routing_source stays legacy.
  //   Canon Part 8: the row store is a DEDICATED LOCAL table, structurally invisible
  //   to buildBrainPacket (BCH-14 holds by construction); the audit marker carries
  //   scalars + enum handles only, never a row body, never crosses to Brain. Same
  //   thin additive-re-export idiom as logMemoryEvent / writeEdge / getRoomContext.)
