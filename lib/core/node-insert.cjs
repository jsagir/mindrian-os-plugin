 ...
 *
 * On the legacy un-migrated 3-column schema, confidence / review_status /
 * on_conflict-as-a-column are moot (there are no such columns to write); the
 * legacy INSERT branch itself is otherwise unchanged from its pre-R17 shape.
 *
 * R17-02 (260903-gdm Task 5): `opts.epistemic_type` is REQUIRED (no default)
 * and validated against the closed ALLOWED_EPISTEMIC_TYPES enum ('observation',
 * 'extracted_fact', 'derived_fact', 'model_derived_assertion', 'interpretation',
 * 'hypothesis', 'assumption', 'conclusion', 'recommendation', 'decision')
 * BEFORE any other work in insertNode, including before isMigratedSchema() and
 * before any prepare() -- mirrors writeEdge's edge_type gate structurally.
 * Absent, non-string, or out-of-enum THROWS. Storage (Task 4 Decision 3):
 * merged into the properties JSON blob, NOT a new column -- no schema
 * migration, applies uniformly on BOTH schemas (properties is a column on
 * both). A caller-supplied properties.epistemic_type key is always
 * overridden by the validated value (protected-key discipline, mirrors
 * typed-claim.cjs's PROTECTED_CLAIM_KEYS), never the other way around.
 *
...
// R17-02 (260903-gdm Task 5): the closed epistemic_type enum, mirroring the
// edges.cjs ALLOWED_EDGE_TYPES structural precedent. Navigator-approved
// (Task 4, Decision 4) per-writer mapping lives in each call site's
// insertNode(...) call, not here; this Set is only the fail-closed gate.
...
// Merge the validated epistemic_type into the caller's properties JSON blob
// (Task 4 Decision 3: properties JSON, no schema migration, no new column).
// Protected-key discipline (mirrors typed-claim.cjs's PROTECTED_CLAIM_KEYS):
// epistemic_type is set AFTER parsing, so a caller-supplied
// properties.epistemic_type can NEVER shadow the validated value.
function mergeEpistemicType(propertiesJson, epistemicType) {
  let obj;
  try {
    const parsed = JSON.parse(propertiesJson);
    obj = isPlainObject(parsed) ? parsed : {};
  } catch (_e) {
    throw new Error('insertNode: properties must be valid JSON to merge epistemic_type');
  }
  obj.epistemic_type = epistemicType;
 ...
 * @param {string} overrides.epistemic_type - REQUIRED (R17-02). One of
 *   ALLOWED_EPISTEMIC_TYPES ('observation', 'extracted_fact', 'derived_fact',
 *   'model_derived_assertion', 'interpretation', 'hypothesis', 'assumption',
 *   'conclusion', 'recommendation', 'decision'). Absent, non-string, or
 *   out-of-enum THROWS before prepare(), with no default. Stored inside the
 *   properties JSON blob (Task 4 Decision 3: no schema migration, no new
 *   column); a caller-supplied properties.epistemic_type key is always
 *   overridden by this validated value, never the other way around.
 * @returns {void}
 * @throws {Error} if epistemic_type, review_status, or on_conflict is
 *   missing (epistemic_type only) or present but invalid
 *
 * NOT closed by this chokepoint (named coverage gaps, per the ratified R17
 * exclusions): nodes written by lib/core/navigation/memory-events.cjs and
 * lib/core/rs-sqlite-mirror.cjs carry NO validated epistemic_type.
 */
function insertNode(conn, id, type, properties, overrides) {
  ...
  // R17-02: epistemic_type is REQUIRED and validated FIRST, before any other
  // work, before isMigratedSchema(), and before any prepare(). Fail closed;
  // mirrors the writeEdge edge_type gate in navigation/edges.cjs. Required
  // means required; there is no default.
  if (typeof opts.epistemic_type !== 'string' || !ALLOWED_EPISTEMIC_TYPES.has(opts.epistemic_type)) {
    throw new Error('insertNode: invalid epistemic_type ' + JSON.stringify(String(opts.epistemic_type).slice(0, 40)));
  }
  properties = mergeEpistemicType(properties, opts.epistemic_type);
