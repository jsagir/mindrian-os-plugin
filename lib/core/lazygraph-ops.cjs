...
const EDGE_TYPES = ['INFORMS', 'CONTRADICTS', 'CONVERGES', 'ENABLES', 'INVALIDATES', 'CONFIRMS', 'DEFERRED', 'BELONGS_TO', 'REASONING_INFORMS', 'HSI_CONNECTION', 'REVERSE_SALIENT', 'ANALOGOUS_TO', 'STRUCTURALLY_ISOMORPHIC', 'RESOLVES_VIA', 'CAUSES', 'ROOT_CAUSE_OF', 'CASCADES_TO', 'EXTRACTED_FROM', 'WHITESPACE_DETECTED', 'WHITESPACE_NEAR', 'DISCOVERY_CYCLE_SOURCE', 'ADDRESSES', 'IN_DOMAIN'];
...
async function initSchema(conn) {
  ...
  await conn.query(`CREATE REL TABLE IF NOT EXISTS CONFIRMS(FROM Artifact TO Artifact, reason STRING, decision_timestamp STRING)`);
  await conn.query(`CREATE REL TABLE IF NOT EXISTS DEFERRED(FROM Artifact TO Artifact, decision_timestamp STRING)`);
  ...
  // Discovery Cycle source tracking (Phase 64)
  await conn.query(`CREATE REL TABLE IF NOT EXISTS DISCOVERY_CYCLE_SOURCE(
    FROM WhitespaceZone TO Artifact,
    discovery_method STRING DEFAULT '',
    cycle_timestamp STRING DEFAULT ''
  )`);
...
async function graphStats(conn) {
  ...
  // Count Opportunity nodes (Phase 72)
  let opportunityCount = 0;
  try {
    const oppRows = await queryGraph(conn, 'MATCH (o:Opportunity) RETURN count(*) AS cnt');
    opportunityCount = oppRows[0]?.cnt || 0;
  } catch (e) { /* table may not exist yet in older .lazygraph databases */ }
  ...
  const WHITESPACE_EDGE_ROUTES = {
    ...
    'DISCOVERY_CYCLE_SOURCE': 'WhitespaceZone_Artifact',
  };
  const OPPORTUNITY_EDGE_ROUTES = {
    'ADDRESSES': 'Opportunity_Artifact',
    'IN_DOMAIN': 'Opportunity_Section',
  };
  const edges = {};
  for (const edgeType of EDGE_TYPES) {
    ...
    } else if (OPPORTUNITY_EDGE_ROUTES[edgeType] === 'Opportunity_Artifact') {
      query = `MATCH (o:Opportunity)-[:${edgeType}]->(a:Artifact) RETURN count(*) AS cnt`;
    } else if (OPPORTUNITY_EDGE_ROUTES[edgeType] === 'Opportunity_Section') {
      query = `MATCH (o:Opportunity)-[:${edgeType}]->(s:Section) RETURN count(*) AS cnt`;
    } else {
  ...
  return {
    nodes: { Artifact: artifactCount, Section: sectionCount, CausalClaim: causalCount, WhitespaceZone: whitespaceCount, Opportunity: opportunityCount },
    edges,
    total: { nodes: artifactCount + sectionCount + causalCount + whitespaceCount + opportunityCount, edges: totalEdges },
  };
}
...
async function linkDiscoveryCycleSource(conn, zoneId, artifactId, discoveryMethod, cycleTimestamp = '') {
  ...
  await conn.query(
    `MATCH (w:WhitespaceZone {id: '${esc(zoneId)}'}), (a:Artifact {id: '${esc(artifactId)}'})
     MERGE (w)-[r:DISCOVERY_CYCLE_SOURCE]->(a)
     ON CREATE SET r.discovery_method = '${esc(discoveryMethod)}', r.cycle_timestamp = '${esc(ts)}'
     ON MATCH SET r.discovery_method = '${esc(discoveryMethod)}', r.cycle_timestamp = '${esc(ts)}'`
  );
  return true;
}

// --- Decision Edge Types (Phase 69: APPROVE/REJECT/DEFER) ---

/**
 * Create a CONFIRMS edge between two artifacts (used when user rejects an insight).
 * Rejection confirms the existing state -- the insight was wrong.
 * @param {object} conn - KuzuDB Connection
 * @param {string} sourceId - Source artifact ID
 * @param {string} targetId - Target artifact ID
 * @param {string} reason - Reason for rejection/confirmation
 * @param {string} timestamp - ISO timestamp of the decision
 * @returns {Promise<boolean>}
 */
async function createConfirmsEdge(conn, sourceId, targetId, reason, timestamp) {
  await conn.query(
    `MATCH (a:Artifact {id: '${esc(sourceId)}'}), (b:Artifact {id: '${esc(targetId)}'})
     MERGE (a)-[r:CONFIRMS]->(b)
     ON CREATE SET r.reason = '${esc(reason)}',
                   r.decision_timestamp = '${esc(timestamp)}'
     ON MATCH SET r.reason = '${esc(reason)}',
                  r.decision_timestamp = '${esc(timestamp)}'`
  );
  return true;
}
...
/**
 * Create a DEFERRED edge between two artifacts (used when user defers a decision).
 * @param {object} conn - KuzuDB Connection
 * @param {string} sourceId - Source artifact ID
 * @param {string} targetId - Target artifact ID
 * @param {string} timestamp - ISO timestamp of the decision
 * @returns {Promise<boolean>}
 */
async function createDeferredEdge(conn, sourceId, targetId, timestamp) {
  await conn.query(
    `MATCH (a:Artifact {id: '${esc(sourceId)}'}), (b:Artifact {id: '${esc(targetId)}'})
     MERGE (a)-[r:DEFERRED]->(b)
     ON CREATE SET r.decision_timestamp = '${esc(timestamp)}'
     ON MATCH SET r.decision_timestamp = '${esc(timestamp)}'`
  );
  return true;
}

// --- Opportunity Bank nodes + edges (Phase 72: OPP-04) ---

/**
 * Create or update an Opportunity node in KuzuDB (MERGE = idempotent).
 * @param {object} conn - KuzuDB Connection
 * @param {object} opp - Opportunity object with problem_hash, problem, mirror_solution, domain, etc.
 * @returns {Promise<{problem_hash: string}>}
 */
...
/**
 * Create ADDRESSES edge from Opportunity to Artifact.
 * Only creates edge if both nodes exist.
 * @param {object} conn - KuzuDB Connection
 * @param {string} problemHash - Opportunity problem_hash
...
/**
 * Create IN_DOMAIN edge from Opportunity to Section.
 * Only creates edge if both nodes exist.
 * @param {object} conn - KuzuDB Connection
 * @param {string} problemHash - Opportunity problem_hash
...
module.exports = {
  ...
  // Decision Edges (Phase 69: APPROVE/REJECT/DEFER)
  createConfirmsEdge,
  createDeferredEdge,
  // Opportunity Bank (Phase 72: OPP-04)
