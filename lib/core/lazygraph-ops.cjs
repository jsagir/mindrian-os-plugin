...
async function indexArtifact(conn, roomDir, filePath) {
  ...
  // Phase 27-03: Meeting-aware indexing (file-meeting artifacts)
  if (methodology === 'file-meeting') {
    const meetingName = extractFrontmatter(content, 'meeting_name');
    const meetingDate = extractFrontmatter(content, 'meeting_date');

    // Extract meeting_id from attribution block
    const meetingIdMatch = content.match(/attribution:[\s\S]*?meeting_id:\s*["']?([^"'\n]+)/);
    const meetingId = meetingIdMatch
      ? meetingIdMatch[1].trim()
      : (meetingDate ? meetingDate + '-meeting' : id);

    // Create/merge Meeting node
    if (meetingId) {
      await conn.query(
        `MERGE (m:Meeting {id: '${esc(meetingId)}'})
         ON CREATE SET m.name = '${esc(meetingName)}', m.date = '${esc(meetingDate)}'
         ON MATCH SET m.name = '${esc(meetingName)}', m.date = '${esc(meetingDate)}'`
      );

      // Create SEGMENT_OF edge
      const segmentType = extractFrontmatter(content, 'segment_type');
      await conn.query(
        `MATCH (a:Artifact {id: '${esc(id)}'}), (m:Meeting {id: '${esc(meetingId)}'})
         MERGE (a)-[:SEGMENT_OF {segment_type: '${esc(segmentType)}', confidence: 0.8}]->(m)`
      );
    }

    // Speaker indexing from attribution block
    const speakerMatch = content.match(/attribution:[\s\S]*?speaker:\s*["']?([^"'\n]+)/);
    const roleMatch = content.match(/attribution:[\s\S]*?role:\s*["']?([^"'\n]+)/);
    const profilePathMatch = content.match(/attribution:[\s\S]*?profile_path:\s*["']?([^"'\n]+)/);

    if (speakerMatch) {
      const speakerName = speakerMatch[1].trim().replace(/["']$/g, '');
      const speakerRole = roleMatch ? roleMatch[1].trim().replace(/["']$/g, '') : '';
      const profilePath = profilePathMatch ? profilePathMatch[1].trim().replace(/["']$/g, '') : '';
      const speakerId = 'speaker/' + speakerName.toLowerCase().trim().replace(/\s+/g, '-');

      // MERGE Speaker node
      await conn.query(
        `MERGE (sp:Speaker {id: '${esc(speakerId)}'})
         ON CREATE SET sp.name = '${esc(speakerName)}', sp.role = '${esc(speakerRole)}',
                       sp.profile_path = '${esc(profilePath)}'
         ON MATCH SET sp.name = '${esc(speakerName)}', sp.role = '${esc(speakerRole)}',
                      sp.profile_path = '${esc(profilePath)}'`
      );

      // MERGE SPOKE_IN edge (Speaker -> Meeting)
      if (meetingId) {
        await conn.query(
          `MATCH (sp:Speaker {id: '${esc(speakerId)}'}), (m:Meeting {id: '${esc(meetingId)}'})
           MERGE (sp)-[:SPOKE_IN]->(m)`
        );
      }

      // CONSULTED_ON edge (Speaker -> Section) based on artifact's section
      await conn.query(
        `MATCH (sp:Speaker {id: '${esc(speakerId)}'}), (sec:Section {name: '${esc(section)}'})
         MERGE (sp)-[:CONSULTED_ON]->(sec)`
      );
    }
  }
