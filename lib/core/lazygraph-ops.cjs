 ...
 * SQLite-backed per-project queryable knowledge graph via better-sqlite3.
...
async function openGraph(roomDir) {
  ...
  // conn === db in SQLite world (KuzuDB had separate Database + Connection)
 ...
 * @param {import('better-sqlite3').Database} conn - better-sqlite3 Database instance
 * @param {string} roomDir - Absolute path to room directory
 * @param {string} filePath - Absolute path to .md file
...
async function indexArtifact(conn, roomDir, filePath) {
  ...
  for (const link of wikilinks) {
    ...
    // Find target artifacts in the target section
  ...
  return Promise.resolve({ id, section, title, contentHash });
}
...
async function rebuildGraph(conn, roomDir) {
  ...
  // If process crashes mid-rebuild, the DB rolls back to pre-rebuild state
  const discovery = discoverSections(resolved);
  const sectionNames = discovery.all;
  let artifactCount = 0;

  const rebuildTxn = conn.transaction(() => {
    ...
    for (const sectionName of sectionNames) {
      ...
      for (const file of files) {
        // indexArtifact is async-wrapped but better-sqlite3 is sync
        // Safe to call within transaction since no real async ops inside
        const filePath = path.join(sectionDir, file);
        indexArtifact(conn, resolved, filePath); // returns Promise but sync internally
        artifactCount++;
      }
    }
  });
