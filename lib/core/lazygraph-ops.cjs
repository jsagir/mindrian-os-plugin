...
async function openGraph(roomDir) {
  ...
  // into the DatabaseSync options object. This is the most-used room.db
  // opener in the repo (roughly 38 call sites). Without it, node:sqlite
  // fails a contended write in 0ms with SQLITE_BUSY; the option turns that
  // into a roughly 5s busy-wait window. Strictly more forgiving - a longer
  // wait, never a new failure mode - because WAL readers never block writers.
  // Adapted from the one correct site, lib/core/room-db.cjs:242-251.
  const db = new DatabaseSync(dbPath, { timeout: 5000 });
  db.exec('PRAGMA journal_mode = WAL');
  db.exec('PRAGMA foreign_keys = ON');

  // D-276-4 follow-up (registered, not built here): routing this opener
  // through lib/core/room-db.cjs's openRoomDb would also close Phase 273's
  // M12 and C2's residue, but was NOT done in Phase 276 because openRoomDb
  // is synchronous and returns a bare handle while openGraph is async and
  // returns { db, conn } across roughly 38-50 call sites (re-measured at
  // 276-09 execution time: 50 non-test occurrences of "openGraph(" in the
  // tree, up from the plan's own "roughly 38" estimate), and because
  // openRoomDb runs a 7-step migration chain on every open. Named as a
  // registered follow-up of Phase 276, tracked in the ROADMAP by plan
  // 276-16, not silently dropped.
  initSchema(db);
...
async function addWhitespaceZone(conn, zone) {
  ...
  // Phase 355-16 (D-16): an optional lib/core/verification-stamp.cjs
  // toNodeProps(stamp) shape rides this SAME write when the caller attaches
  // it to the zone object -- no new writer, no raw SQL. Every caller that
  // does not attach a stamp (all pre-355 call sites) is byte-identical: none
  // of these keys is ever present on their zone object, so this loop is a
  // no-op for them.
  const STAMP_PROP_KEYS = ['verification', 'backend', 'direction', 'judge', 'path', 'path_labels', 'path_edges', 'path_len', 'reason'];
  for (const key of STAMP_PROP_KEYS) {
    if (Object.prototype.hasOwnProperty.call(zone, key)) baseProps[key] = zone[key];
  }

  const props = JSON.stringify(baseProps);
