...
async function rebuildGraph(conn, roomDir, _visited) {
  ...
  try {
    ...
    // index against the nodes table the wipe above just scoped-deleted from.
    //
    // WHY THIS IS A SEPARATE STATEMENT BESIDE clearIndexerOwnedRows, NOT
    // FOLDED INTO IT. eureka_fts (the fts5 virtual table owned by
    ...
    //
    // The identical guarded block is duplicated (not extracted to a shared
    // helper) in scripts/build-ecosystem-graph.cjs's own BEGIN/COMMIT wrap --
    // the same choice that file already makes for the BEGIN/COMMIT/ROLLBACK
    // shape itself. Both copies must be kept in sync if either changes.
    (function reconcileFtsIndexInline() {
      try {
        ...
        if (!tri.ensureFtsAvailable().ok) return; // FTS5 not compiled in on this build
        if (!tri.tableExists(conn, 'eureka_fts')) return; // index never built on this db (Pitfall 2)
