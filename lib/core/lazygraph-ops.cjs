...
const EDGE_TYPES = ['INFORMS', 'CONTRADICTS', 'CONVERGES', 'ENABLES', 'INVALIDATES', 'BELONGS_TO', 'REASONING_INFORMS', 'HSI_CONNECTION', 'REVERSE_SALIENT', 'ANALOGOUS_TO', 'STRUCTURALLY_ISOMORPHIC', 'RESOLVES_VIA', 'CAUSES', 'CASCADES_TO', 'EXTRACTED_FROM'];
...
async function initSchema(conn) {
  ...
  // CAUSES edges link claims into causal chains. CASCADES_TO tracks assumption failure propagation.
  await conn.query(`
  ...
  await conn.query(`CREATE REL TABLE IF NOT EXISTS CAUSES(
    ...
    discovery_method STRING DEFAULT 'heuristic'
  )`);
...
async function graphStats(conn) {
  ...
  const edges = {};
  // Edge type → source/target node types
  const CAUSAL_EDGE_ROUTES = {
    'CAUSES': 'CausalClaim_CausalClaim',
  ...
  return {
    nodes: { Artifact: artifactCount, Section: sectionCount, CausalClaim: causalCount },
    edges,
    total: { nodes: artifactCount + sectionCount + causalCount, edges: totalEdges },
  };
}
...
/**
 * Create a CausalClaim node in the graph.
 * A CausalClaim is a cause-effect assertion extracted from a room artifact.
 * @param {object} conn - KuzuDB Connection
 * @param {object} claim - Causal claim data
 * @param {string} claim.id - Unique claim ID (e.g., 'causal-001')
 * @param {string} claim.cause - What causes the effect
 * @param {string} claim.effect - What happens as a result
 * @param {string} claim.mechanism - HOW the cause produces the effect
 * @param {number} [claim.confidence=0.5] - 0-1 confidence
 * @param {string[]} [claim.evidence=[]] - Supporting artifact IDs
 * @param {string} [claim.source_artifact=''] - Artifact this was extracted from
 * @param {string} [claim.domain=''] - Domain (materials, business, competitive, etc.)
 * @param {string} [claim.falsifiable_prediction=''] - Testable prediction
 * @param {number} [claim.novelty_score=0.0] - 0-1 how surprising vs consensus
 * @returns {Promise<boolean>}
 */
async function createCausalClaim(conn, claim) {
  ...
  // Link back to source artifact if provided
  if (claim.source_artifact) {
    try {
      await conn.query(
        `MATCH (c:CausalClaim {id: '${id}'}), (a:Artifact {id: '${esc(claim.source_artifact)}'})
         MERGE (c)-[:EXTRACTED_FROM]->(a)`
      );
    } catch (e) {
      // Source artifact may not exist in graph yet -- skip silently
    }
  }
...
/**
 * Create a CAUSES edge between two CausalClaim nodes.
 * Links claims into causal chains (A causes B causes C).
 * @param {object} conn - KuzuDB Connection
 * @param {string} sourceClaimId - Source CausalClaim ID
 * @param {string} targetClaimId - Target CausalClaim ID
 * @param {object} [props={}] - Edge properties
 * @param {number} [props.strength=0.5] - 0-1 causal strength
 * @param {string} [props.mechanism=''] - How source causes target
 * @param {string} [props.direction='forward'] - forward|feedback|bidirectional
 * @param {string} [props.discovery_method='heuristic'] - heuristic|llm|user|brain
 * @returns {Promise<boolean>}
 */
async function createCausesEdge(conn, sourceClaimId, targetClaimId, props = {}) {
  const strength = props.strength || 0.5;
  const mechanism = esc(props.mechanism || '');
  const direction = esc(props.direction || 'forward');
  const method = esc(props.discovery_method || 'heuristic');

  await conn.query(
    `MATCH (a:CausalClaim {id: '${esc(sourceClaimId)}'}), (b:CausalClaim {id: '${esc(targetClaimId)}'})
     MERGE (a)-[r:CAUSES]->(b)
 ...
 * Tracks assumption failure propagation: if A breaks, B breaks too.
 * @param {object} conn - KuzuDB Connection
 * @param {string} sourceClaimId - CausalClaim that, if invalidated...
 * @param {string} targetClaimId - ...causes this claim to also be at risk
 * @param {object} [props={}] - Edge properties
 * @param {string} [props.cascade_type='invalidation'] - invalidation|weakening|reversal
 * @param {string} [props.severity='medium'] - low|medium|high|critical
 * @param {number} [props.path_length=1] - Hops from original failure
 * @returns {Promise<boolean>}
 */
async function createCascadeEdge(conn, sourceClaimId, targetClaimId, props = {}) {
  const cascadeType = esc(props.cascade_type || 'invalidation');
  const severity = esc(props.severity || 'medium');
  const pathLength = props.path_length || 1;
