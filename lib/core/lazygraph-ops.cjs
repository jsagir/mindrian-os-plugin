...
const EDGE_TYPES = ['INFORMS', 'CONTRADICTS', 'CONVERGES', 'ENABLES', 'INVALIDATES', 'BELONGS_TO', 'REASONING_INFORMS', 'HSI_CONNECTION', 'REVERSE_SALIENT', 'ANALOGOUS_TO', 'STRUCTURALLY_ISOMORPHIC', 'RESOLVES_VIA', 'EXTRACTED_FROM'];
...
async function initSchema(conn) {
  ...
  // Causal Extraction node table (Phase 53)
  await conn.query(`
  ...
  // Provenance edge: CausalClaim extracted from Artifact
  await conn.query(`CREATE REL TABLE IF NOT EXISTS EXTRACTED_FROM(FROM CausalClaim TO Artifact)`);
}
