 ...
 * the number exists.
 *
 * Phase 266 Plan 05 (MCPFIX-03 gap closure, dated 2026-08-27): CONNECT_PATH_BUDGET_MS
 * is now a PER-PROCESS ceiling spanning every connect-path heal call an MCP
 * entry point makes at module scope, not a per-call number (266-VERIFICATION.md
 * Truth #5 found the per-call enforcement compounding to a measured 60296ms
 * worst case across four calls, roughly double the host's ~30000ms connect
 * timeout). The four invariant tests below still hold unchanged: only the
 * SCOPE of CONNECT_PATH_BUDGET_MS changed, not its VALUE (15000ms) or its
 * position in the chain (STALE > DEFAULT, CONNECT < DEFAULT, CONNECT < STALE,
 * WAIT > STALE, CONNECT < 30000). No assertion in this file changed.
 *
