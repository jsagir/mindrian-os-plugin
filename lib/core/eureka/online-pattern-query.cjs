 ...
 *
 * DG-2 (fence mechanism): the Phase 196 runtime egress hook matches
 * mcp__brain_.* only (hooks.json), so Tavily/web egress is NOT covered by it.
 * This module wires the fence INLINE rather than relying on that hook. A
 * hook-level PreToolUse matcher for the web surface is an additive follow-up,
 * never a replacement for this inline audit.
 *
