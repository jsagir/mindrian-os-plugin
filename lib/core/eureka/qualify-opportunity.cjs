...
// measured-zero (D-18). Phase 355-18 (D-29, Canon Part 12): a finite number
// no longer prints the raw digits -- it renders the word 'measured' (the
// value is withheld, not zeroed; the F.1 card is pedagogy over verdict, and
// a bare decimal reads as a score the navigator is meant to chase). Strings
// and booleans are untouched; the typed 'unknown' stays verbatim.
function componentValue(v) {
  if (typeof v === 'number' && Number.isFinite(v)) return 'measured';
...
/**
 * formatComponentLines(components, harvestIndex, stamp?) -- the D-18
 * machine-readiness WHY lines (components = existing measured signals; the
 * Q1..Q8 rubric = the human card checks). The index line names its version +
 * EXPERIMENTAL status on every emission (advisory, never a surfacing filter
 * beyond the Q2 hard gate). Missing inputs render the typed 'unknown'
 * verbatim. Phase 355-18 (HIPS-05, D-27): when `stamp` is a real
 * verification-stamp.cjs Stamp, its formatStampLines('cli') lines are
 * PREPENDED as the very first WHY lines (the path replaces the number) --
 * absent, the return is byte-identical to the pre-355 shape. Never throws.
 */
function formatComponentLines(components, harvestIndex, stamp) {
  const lines = [];
  if (isPlainObject(stamp)) {
    try {
      ...
      // Defensive: a malformed stamp must never block the rest of the card.
    }
  }
  if (!isPlainObject(components) && !isPlainObject(harvestIndex)) return lines;
...
function renderQualificationCard(candidate, deref, opts) {
  try {
    ...
    // Phase 355-18 (HIPS-05, D-27): the candidate's own stored stamp props
    // (verification-stamp.cjs's toNodeProps shape -- the same flat, closed
    // key set a stamped opportunity node carries), re-parsed through
    // fromNodeProps when present. A missing or malformed candidate.stamp
    // degrades to no stamp block (the render is pure presentation and must
    // never throw on a stamp-less candidate).
    let stamp;
    if (isPlainObject(candidate.stamp)) {
      try {
        const verificationStamp = require('../verification-stamp.cjs');
        stamp = verificationStamp.fromNodeProps(candidate.stamp);
      } catch (_e) {
        stamp = undefined;
      }
    }
    const whyLines = formatRubricLines(candidate.rubric)
      .concat(formatComponentLines(candidate.components, candidate.harvest_index, stamp));
