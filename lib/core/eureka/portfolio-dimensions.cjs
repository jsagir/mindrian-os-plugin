...
//
// Phase 355 D-47: scoreMeasured's direction label flipped to follow
// lib/core/direction-convention.cjs (355-10 Task 2, same commit as this
// swap) -- the label's own MEANING is now correct, but the pair-to-
// feasibility MAP below is swapped so the meaning-to-feasibility
// relationship, and therefore eureka's ranking, is byte-for-byte unchanged.
// Before this flip 'structural_transfer' carried today's bridge/transfer
// data (the shared-words-divergent-meaning case, the mirror of the label's
// own name); that SAME underlying signed_diff sign now arrives labeled
// 'semantic_implementation', so the bridge/transfer branch moves to that
// label here. The tension this preserves -- today's ranking rewards
// shared-words pairs over genuine meaning bridges -- is recorded for the
// next engine phase (355-RESEARCH.md Open Question 1); a ranking CHANGE is
// out of this honesty pass's scope. Proof this swap holds the ranking still:
// tests/fixtures/355/eureka-ranking-pin.json (pinned before the flip).
function feasibilityFromRs(rs) {
  const r = rs || {};
  if (!r.passes) return FEASIBILITY.fail;
  const ad = Number(r.abs_diff);
  if (r.direction === 'semantic_implementation') {
    if (Number.isFinite(ad) && ad >= BRIDGE_BAND_LO && ad <= BRIDGE_BAND_HI) {
      return FEASIBILITY.bridge;
    }
    return FEASIBILITY.transfer;
  }
  if (r.direction === 'structural_transfer') return FEASIBILITY.semantic;
  // Unknown / 'none' direction but passes true: the paraphrase-risk floor, never the bridge.
