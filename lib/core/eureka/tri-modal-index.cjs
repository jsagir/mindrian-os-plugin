 ...
 *     // fts_pruned counts the orphan eureka_fts rows this run reconciled away.
 *   reconcileFtsOrphans(db)       -> { pruned }   (Phase 244: the canonical
 *                                    set-based prune of eureka_fts rows whose
 *                                    node_id is no longer in `nodes`;
 *                                    self-guarding, and the ONE copy of that SQL)
...
// ---------- reconcileFtsOrphans ----------
//
// Phase 244 (RCA eureka-fts-orphan-rows-block-release-gate): the CANONICAL
// set-based orphan prune for eureka_fts. An "orphan" is an eureka_fts row whose
// node_id is no longer in `nodes` -- the row a node's deletion leaves behind.
//
// WHY THIS EXISTS AS ITS OWN EXPORT. `indexNodes` reconciled eureka_fts ONLY
// per LIVE node (its single lexical DELETE was `WHERE node_id = ?`, bound to
// ids read straight out of `nodes`), so every id it deleted was by construction
// an id that still existed and an orphan row was immortal. Meanwhile
// `fts-index-lifecycle.cjs::ftsIndexState` DEFINES index_stale as
// orphan_rows >= FTS_STALE_ORPHAN_FLOOR, and the doctor's eureka-fts-health
// module ships fix_supported:false and points every repair at the lazy
// build-on-first-miss path (requestFtsBuild -> scripts/fts-index-drain.cjs ->
// indexNodes). The sanctioned repair therefore structurally could not clear the
// one condition the release gate flags. Observed live on
// motj-ecosystem/sub-rooms/jonathan-contractor-motj: a drain that provably did
// real work left orphan_rows at exactly 451, byte-identical before and after.
//
// SELF-GUARDING, so every call site is a one-liner and cannot forget a guard:
//   (a) ensureFtsAvailable().ok -- some node:sqlite builds lack the FTS5
//       extension entirely (the 2026-07-13 Windows field finding).
//   (b) tableExists(db, 'eureka_fts') -- this db may simply never have built
//       the index, which is the DEFAULT state of every live room.
// Either guard short-circuits to { pruned: 0 }: an honest "nothing reconciled",
// never a false success.
//
// THIS FUNCTION DELIBERATELY DOES NOT SWALLOW. The two pre-existing callers on
// the full-graph-rebuild path (lazygraph-ops.cjs::rebuildGraph and
// scripts/build-ecosystem-graph.cjs) keep their OWN try/catch swallow, because
// there a reconcile fault must not abort an otherwise-succeeding rebuild. On the
// BUILD path the opposite is true: a swallowed fault would recreate this exact
// bug -- a "successful" build that silently did not reconcile. Letting it
// propagate hands it to fts-index-drain.cjs's existing keep-on-failure + retry +
// permanent-failure-record discipline, which is the NEVER SILENT ROT contract
// that file already states.
//
// Canon Part 9: eureka_fts is a DERIVED, freely rebuildable projection of the
// nodes table. This writes zero typed edges, zero memory_event rows, zero node
// mutations, and takes a caller-owned handle (it never opens room.db).

function reconcileFtsOrphans(db) {
  if (!ensureFtsAvailable().ok) return { pruned: 0 };
  if (!tableExists(db, 'eureka_fts')) return { pruned: 0 };
  const info = db.prepare('DELETE FROM eureka_fts WHERE node_id NOT IN (SELECT id FROM nodes)').run();
...
// embedTexts batch (opts.encodeFn injectable for offline tests). Nodes with
// empty text are skipped. Degrades to a lexical-only index (no vectors) when the
// encoder is unavailable, never throws.
//
// The lexical leg reconciles in TWO moves, and it needs both:
//   (1) a set-based reconcileFtsOrphans pass that drops every row whose node is
//       gone from `nodes` (rows no per-node statement can ever name), and
//   (2) the per-node DELETE-then-INSERT upsert that refreshes each LIVE node's
//       row, which is what keeps repeated runs from duplicating rows.
// Move (2) alone -- the pre-Phase-244 behavior -- made this an upsert over live
// nodes, NOT a reconcile, despite the "full-corpus reindex" label. See
// reconcileFtsOrphans above for the full RCA.
//
// Order is irrelevant here (unlike rebuildGraph, which must reconcile AFTER its
// reindex because it wipes and regenerates `nodes` inside the same transaction):
// indexNodes only ever READS `nodes`, so the orphan set is stable throughout.
//
// Deliberately NOT wrapped in BEGIN/COMMIT: this function awaits embedTexts
// mid-body, and holding a write transaction across that await would lock room.db
// for the whole embed batch.
...
async function indexNodes(db, opts) {
  ...
  // Lexical leg: set-based orphan prune, then per-node delete-then-insert
  // (idempotent). GATED on the FTS5 capability verdict (Phase 219-02): an absent
  // build skips the WHOLE leg, prune included (bi-modal degrade), instead of
  // throwing on a table that was never created. The prune therefore sits INSIDE
  // the capability gate, never outside it.
  const ftsLive = openInfo.fts_backend === 'fts5' && tableExists(db, 'eureka_fts');
  let ftsPruned = 0;
  if (ftsLive) {
    // Phase 244: NOT wrapped in a try/catch. A prune fault must surface to the
    // drain's retry discipline rather than becoming another silent "successful"
    // build that did not reconcile. See reconcileFtsOrphans above.
    ftsPruned = reconcileFtsOrphans(db).pruned;
  ...
  return {
    ...
    // Additive provenance (Phase 244): how many orphan rows this run reconciled
    // away. 0 when there was nothing to prune OR when the lexical leg was
    // skipped entirely. Additive-safe: every call site reads named fields
    // (idx.embedded / idx.vec_backend); none deep-equals this shape.
    fts_pruned: ftsPruned,
  };
}
...
module.exports = {
  ...
  // Phase 244: promoted from _test to a public export. The eureka_fts index
  // has no production lifecycle (it is absent by default in every live
  // room), so a caller outside this module must be able to distinguish
  // index_absent from zero_hits; reaching into _test from lib/ would import
  // a test seam into the runtime path. The _test.tableExists alias below is
  // kept pointing at the same function so no existing test breaks.
  tableExists: tableExists,
  // Phase 244 (RCA eureka-fts-orphan-rows-block-release-gate): the canonical
  // orphan prune. Public because the two full-graph-rebuild callers
  // (lib/core/lazygraph-ops.cjs::rebuildGraph and
  // scripts/build-ecosystem-graph.cjs) delegate to it instead of each carrying
  // their own copy of the DELETE ... NOT IN SQL, which their own comments
  // already flagged as a sync hazard. There is now exactly ONE copy.
  reconcileFtsOrphans: reconcileFtsOrphans,
  _test: {
    STOPWORDS: STOPWORDS,
