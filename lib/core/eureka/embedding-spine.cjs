...
// Phase 341 Plan 02 (D-09): the single side-directory resolution authority
// for the heavy @huggingface/transformers dependency, shared by this
// module's lazy requires, the /mos:eureka enable installer, and (plan
// 341-03) Class S's model_installed layer. A tiny local module with no
// heavy transitive cost -- safe to require at module top; only the
// transformers require itself stays lazy (Canon Decision #8).
const { requireEurekaDep } = require('../eureka-deps-resolver.cjs');
...
// Phase 341 Plan 03 (D-10/D-11): the ONE canonical remedy sentence for "the
// embedding model package is not installed." Exported so report-html.cjs (and
// any other consumer) reference this constant rather than re-typing the
// sentence -- one string, never three drifting copies. Appended ONLY to the
// encoder_unavailable detail whose cause is a missing package (the
// requireEurekaDep null arm in getEncoder, below); never appended to the
// _forceUnavailable test hook or to a load_failed detail, since a remedy
// naming the wrong cause is worse than no remedy at all.
const ENCODER_UNAVAILABLE_HINT = 'model not installed - run /mos:eureka enable (one-time, about 380 MB)';
...
//
// Phase 272-08 fix (Rule 1 -- real bug found live, not introduced by this
// plan): env.allowRemoteModels is a mutable property on transformers.js's
// module-level singleton env object, so setting it false here LEAKED past
// this function's own return -- the very next real pipeline(...) load in
// getEncoder (below) inherited allowRemoteModels=false and could never
// reach the network on an actual cache MISS, degrading a genuine first-run
// download into a hard load_failed instead. Save/restore the prior value in
// a finally so the offline-only probe never poisons a subsequent real load.
async function isModelCached(model, cacheDir, dtype) {
  // Phase 341 Plan 02: resolved through the single eureka-deps-resolver
  // authority (side directory first, plugin node_modules fallback) instead
  // of a bare require. requireEurekaDep never throws; a null return is the
  // same "dependency absent" case the prior try/catch handled.
  const transformers = requireEurekaDep('@huggingface/transformers');
  if (!transformers) {
...
async function getEncoder(opts) {
  ...
  // Phase 341 Plan 02: resolved through the single eureka-deps-resolver
  // authority instead of a bare require. requireEurekaDep never throws; a
  // null return preserves the exact encoder_unavailable envelope shape the
  // prior try/catch produced on a require failure.
  const transformers = requireEurekaDep('@huggingface/transformers');
  if (!transformers) {
    return {
      success: false,
      error: 'encoder_unavailable',
      detail: 'require_failed: dependency not installed - ' + ENCODER_UNAVAILABLE_HINT,
    };
  }
...
module.exports = {
  ...
  ENCODER_UNAVAILABLE_HINT: ENCODER_UNAVAILABLE_HINT,
  _test: {
