...
// resolveCacheDir: MINDRIAN_MODEL_CACHE override else transformers.js env.cacheDir.
// `env` is the @huggingface/transformers env object (may be undefined if the dep
// failed to load). Returns null when it cannot be determined (treated as a hit,
// so we never over-notify).
...
// isModelCached: does the model's weight dir already exist under cacheDir? The
// transformers.js cache layout is <cacheDir>/<owner>/<model>. Defensive: any
// error (or unknown cacheDir) returns TRUE so an uncertain state never triggers
// a spurious "downloading" notice. Exported via _test for the doctor L3 probe.
function isModelCached(model, cacheDir) {
  try {
    if (!cacheDir) return true; // unknown -> treat as hit (never over-notify)
    const modelPath = path.join(cacheDir, ...String(model).split('/'));
...
function maybeEmitDownloadNotice(model, cacheDir) {
  const cached = isModelCached(model, cacheDir);
  ...
  if (!_downloadNoticeShown) {
    ...
    try {
      process.stderr.write(
        ...
        + ') - a one-time download; later runs use the local cache.\n'
      );
...
async function getEncoder(opts) {
  ...
  try {
    ...
    const firstDownload = maybeEmitDownloadNotice(model, resolveCacheDir(env));
