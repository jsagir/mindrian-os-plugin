...
// response) is coerced to 'what' -- fail-open to today's behavior (threat T-T2-01).
...
// Coerce a raw model labels object into the closed set, defaulting every unknown
// or missing candidate to 'what' (fail-open, threat T-T2-01).
function _coerceLabels(names, raw) {
  ...
  for (const n of names) {
    ...
    labels[n] = (v && VALID_LABELS.has(v)) ? v : 'what';
  }
  return labels;
}
