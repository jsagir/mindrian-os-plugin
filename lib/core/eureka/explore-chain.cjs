...
// ---------------------------------------------------------------------------
// Phase 265-18 (RADAR-26): the probe-first concurrent runner + the dispatch
// decision (parallel pre-pass vs the original sequential runChain path).
// Both take everything they need as parameters (no closure over roomDir /
// options) so they are directly unit-testable with a stubbed onStep and no
// real room (tests/test-265-explore-parallel.cjs).
// ---------------------------------------------------------------------------

// Normalize an onStep-shaped result into a trace entry the SAME shape
// runChain's own trace entries carry ({ step, chain_output, quality }).
function _toTraceEntry(step, result) {
  const r = isPlainObject(result) ? result : {};
  return {
    step: step,
    chain_output: r.chain_output !== undefined ? r.chain_output : null,
    quality: (typeof r.quality === 'string') ? r.quality : null,
  };
}

// Fan a set of steps out CONCURRENTLY via Promise.all (mirrors the shipped
// lib/core/bono/cell-fanout.cjs Promise.all idiom, the navigator-confirmed
// final design; see the require comment at the top of this file). Promise.all,
// NOT allSettled: a rejection propagates exactly as the sequential path's
// await did, so a swallowed rejection never hides a failed leg (the
// silent-success class this repo's threat register exists to catch).
// resolveFanoutCap clamps N even though N is at most 3 here -- the SAME
// clamp cell-fanout reuses from lib/core/futures/orchestrator.cjs, not a new
// cap.
//
// Promise.all resolves the returned array in the SAME order as `fanSteps`
// (input order), NOT completion order, so building the trace directly from
// this call preserves FRAMEWORK_LEGS order without any post-hoc sort.
function runFanConcurrently(fanSteps, wrappedOnStep, previousOutput) {
  const cap = resolveFanoutCap({ fanout: fanSteps.length });
  const bounded = fanSteps.slice(0, cap);
  return Promise.all(bounded.map(function (step) {
    return Promise.resolve(wrappedOnStep(step, previousOutput)).then(function (result) {
      return _toTraceEntry(step, result);
    });
  }));
}

/**
 * runAnalysisLegsParallel(analysisSteps, ctx) -> Promise<{ trace, short_circuited }>
 *
 * ctx: { wrappedOnStep, policy } (policy from resolveExploreCostPolicy).
 *
 * cost-conscious mode: probes `policy.probeLeg` (deep_research) ALONE via
 * wrappedOnStep. If probeClears(result) is false, returns immediately with a
 * trace containing ONLY the probe entry and `short_circuited` set -- the
 * other three legs are NEVER dispatched (the load-bearing cost-guard
 * property: nothing is spent beyond the probe). When the probe clears, the
 * remaining legs fan out concurrently.
 *
 * all-legs mode: no probe -- every leg in `analysisSteps` fans out
 * concurrently, unconditionally.
 */
async function runAnalysisLegsParallel(analysisSteps, ctx) {
  const wrappedOnStep = ctx.wrappedOnStep;
  const policy = ctx.policy;
  const steps = Array.isArray(analysisSteps) ? analysisSteps : [];

  if (policy.mode === 'cost-conscious') {
    const probeStep = steps.find((s) => s.leg === policy.probeLeg) || steps[0];
    const remaining = steps.filter((s) => s !== probeStep);

    const probeResult = await wrappedOnStep(probeStep, null);
    const probeEntry = _toTraceEntry(probeStep, probeResult);

    if (!probeClears(probeResult)) {
      return {
        trace: [probeEntry],
        short_circuited: { leg: probeStep.leg, reason: 'probe_quality_low' },
      };
    }

    const fanTrace = await runFanConcurrently(remaining, wrappedOnStep, probeEntry.chain_output);
    return { trace: [probeEntry].concat(fanTrace), short_circuited: null };
  }

  // all-legs mode: no probe; fan every leg concurrently, unconditionally.
  const fanTrace = await runFanConcurrently(steps, wrappedOnStep, null);
  return { trace: fanTrace, short_circuited: null };
}

// Decorate a halt with the D-20 OFFER, mirroring the pre-265-18 decoration
// exactly (never on the file_explored step -- that gate always halts on
// posture, not on engine absence).
function _decorateOffer(run, offer) {
...
/**
 * runExploreDispatch(steps, analysisSteps, filingSteps, opts) ->
 *   Promise<{ run, dispatch }>
 *
 * The dispatch decision this plan adds. opts: { wrappedOnStep, gateFn,
 * onHalt, roomDir, journal, retries, sleep, maxSteps, policy, absent,
 * manual, offer }.
 *
 * - absent && !manual: the D-20 engine-absent OFFER must fire on the FIRST
 *   engine-backed step before anything executes. Takes the ORIGINAL
 *   sequential runChain(steps, ...) path unconditionally, so the OFFER
 *   semantics stay byte-identical. dispatch: {mode:'sequential',
 *   reason:'engine_absent'}.
 * - !policy.parallel (EXPLORE_PARALLEL=0): the operator escape hatch, same
 *   original sequential path. dispatch: {mode:'sequential',
 *   reason:'parallel_disabled'}.
 * - otherwise: runs the probe-first parallel pre-pass, then the filing step
 *   ALONE through runChain with the SAME gateFn/onHalt/roomDir/journal/
 *   retries/sleep/maxSteps it always received, merging the pre-pass trace
 *   ahead of the filing trace. On a probe short-circuit, the run halts with
 *   reason 'quality_early_stop' (the exact reason the sequential chain used)
 *   and filing NEVER runs. ANY throw from the pre-pass falls back to the
 *   ORIGINAL sequential runChain(steps, ...) path, dispatch:
 *   {mode:'sequential', reason:'parallel_failed:<message>'} -- never silent.
 */
async function runExploreDispatch(steps, analysisSteps, filingSteps, opts) {
  const sequentialOpts = {
    onStep: opts.wrappedOnStep,
    onHalt: opts.onHalt,
    gateFn: opts.gateFn,
    roomDir: opts.roomDir,
    journal: opts.journal === true,
    retries: typeof opts.retries === 'number' ? opts.retries : 0,
    sleep: typeof opts.sleep === 'function' ? opts.sleep : undefined,
    maxSteps: typeof opts.maxSteps === 'number' ? opts.maxSteps : undefined,
  };

  if (opts.absent && !opts.manual) {
    const run = await runChain(steps, sequentialOpts);
    return { run: _decorateOffer(run, opts.offer), dispatch: { mode: 'sequential', reason: 'engine_absent' } };
  }

  if (!opts.policy.parallel) {
    const run = await runChain(steps, sequentialOpts);
    return { run: _decorateOffer(run, opts.offer), dispatch: { mode: 'sequential', reason: 'parallel_disabled' } };
  }

  try {
    const prePass = await runAnalysisLegsParallel(analysisSteps, {
      wrappedOnStep: opts.wrappedOnStep,
      policy: opts.policy,
    });

    if (prePass.short_circuited) {
      const probeEntry = prePass.trace[0];
      const run = {
        trace: prePass.trace,
        completed: false,
        haltedAt: { step: probeEntry.step, reason: 'quality_early_stop', quality: probeEntry.quality },
        partial: false,
      };
      return {
        run: _decorateOffer(run, opts.offer),
        dispatch: { mode: 'parallel', probe: opts.policy.probeLeg, short_circuited: true },
      };
    }

    const filingRun = await runChain(filingSteps, sequentialOpts);
    const run = {
      trace: prePass.trace.concat(filingRun.trace),
      completed: filingRun.completed,
      haltedAt: filingRun.haltedAt,
      partial: filingRun.partial === true,
    };
    return {
      run: _decorateOffer(run, opts.offer),
      dispatch: { mode: 'parallel', probe: opts.policy.probeLeg, short_circuited: false },
    };
  } catch (e) {
    const run = await runChain(steps, sequentialOpts);
    const message = String((e && e.message) || e || 'unknown').slice(0, 200);
    return {
      run: _decorateOffer(run, opts.offer),
      dispatch: { mode: 'sequential', reason: 'parallel_failed:' + message },
    };
  }
}
 ...
 *   costMode, parallel   Phase 265-18 (RADAR-26) overrides for
 *                        resolveExploreCostPolicy (opts.costMode beats
 *                        EXPLORE_COST_MODE; opts.parallel beats
 *                        EXPLORE_PARALLEL). See runExploreDispatch below.
 *
 * Returns { ok, trace, completed, haltedAt, research_mode, providers,
 *           engine_mode, filed, dispatch } -- never throws. `dispatch` is
 *   Phase 265-18 (RADAR-26): {mode:'parallel'|'sequential', reason, ...} --
 *   which path actually ran and why. Never silent.
 */
async function exploreOpportunity(roomDir, opportunityNodeId, opts) {
  ...
  try {
    ...
    // Phase 265-18 (RADAR-26): split once so the probe-first pre-pass can run
    // the four analysis legs and the material filing step through separate
    // runChain-shaped dispatches (see runExploreDispatch below).
    const analysisSteps = steps.filter((s) => s.leg !== 'file_explored');
    const filingSteps = steps.filter((s) => s.leg === 'file_explored');
    const costPolicy = resolveExploreCostPolicy(process.env, {
      parallel: typeof options.parallel === 'boolean' ? options.parallel : undefined,
      costMode: typeof options.costMode === 'string' ? options.costMode : undefined,
    });
    ...
    // ---- Phase 265-18 (RADAR-26): the dispatch decision. Probe-first
    // parallel pre-pass when the policy and the engine allow it; the
    // ORIGINAL sequential runChain(steps, ...) path (byte-identical to the
    // pre-265-18 behavior) on the engine-absent OFFER, the operator escape
    // hatch, or ANY throw from the parallel path. dispatch always names
    // which path ran and why -- see runExploreDispatch's own doc comment. ----
    const { run, dispatch } = await runExploreDispatch(steps, analysisSteps, filingSteps, {
...
module.exports = {
  ...
  MANUAL_ENGINE_MODE,
  APPROVE_VERBS,
  // Phase 265-18 (RADAR-26): the probe-first cost guard + the concurrent
  // runner (exported primarily for direct testability with a stubbed onStep
  // and no real room; exploreOpportunity is the intended production caller).
  resolveExploreCostPolicy,
  probeClears,
  PROBE_LEG,
  COST_MODES,
  runAnalysisLegsParallel,
  runExploreDispatch,
};
