...
// Phase 341 Plan 03 (D-10): the canonical "model not installed" remedy
// sentence, sourced from the ONE constant embedding-spine.cjs exports rather
// than re-typed here -- one string, never a second drifting copy.
const { ENCODER_UNAVAILABLE_HINT } = require('./embedding-spine.cjs');

// Phase 355-18 (HIPS-04, HIPS-05, D-08, D-27, D-29, D-30): the ranked table's
// score/similarity cells are replaced by the pair's own verification stamp --
// the tier word plus its methodology-graph path, or the honest unverified
// reason. Pure formatter, zero fs, zero network (matches this module's own
// posture).
const verificationStampFormat = require('../verification-stamp-format.cjs');
...
// Phase 355-18 (D-27, D-29): render a ranked row's verification stamp as
// PLAIN TEXT (escaped by the caller before it reaches a cell) -- the tier
// word plus the undirected path text on a verified stamp, or the honest
// unverified reason word on an unverified one. A missing/absent stamp (a
// report rendered without --stamp) reads honestly as 'not checked'. Never a
// decimal: formatPathText and REASON_WORDS are both closed vocabularies with
// no numeric leg.
function stampCellText(stamp) {
  if (!stamp || typeof stamp !== 'object') return 'not checked';
  const isVerified = stamp.verification === 'strong' || stamp.verification === 'indirect';
  if (isVerified) {
    const nodes = stamp.path ? stamp.path.nodes : [];
    const edges = stamp.path ? stamp.path.edges : [];
    return stamp.verification + ': ' + verificationStampFormat.formatPathText(nodes, edges);
  }
  const reasonWord = verificationStampFormat.REASON_WORDS[stamp.reason] || stamp.reason || 'not checked';
  return 'unverified: ' + reasonWord;
}
...
function buildModeBanner(provenance) {
  ...
  if (isReasoningMode(p)) {
    const rawCause = p.degrade_cause || 'encoder_unavailable';
    const cause = escapeHtml(rawCause);
    // Phase 341 Plan 03 (D-10): when the cause is encoder_unavailable (the
    // model package not being installed, including the default when no cause
    // was stated at all), the rendered caveat carries the exact remedy
    // sentence -- read from the ONE exported constant, escaped the same way
    // every other interpolated value in this file is escaped.
    const remedy = (rawCause === 'encoder_unavailable')
      ? (' ' + escapeHtml(ENCODER_UNAVAILABLE_HINT) + '.')
      : '';
    const L = [];
    ...
    L.push('    <p><strong>2. Why the basis is weak.</strong> The embedding encoder was unavailable'
      ...
      + ' (differential_score and semantic_similarity) are structurally null. Only a lexical-overlap'
      + ' (Jaccard) measure was computed here, and Phase 355-18 (D-29) withholds it from every render'
      + ' too -- the rubric verdict below is the honest evidence, never a fabricated number.' + remedy + '</p>');
...
function rankedTable(ranked, provenance) {
  ...
  L.push('<table>');
  if (reasoning) {
    // Phase 355-18 (D-29): NEVER an lsa_similarity column -- reasoning-mode
    // pairs have no rs pair to verify against Theo, so this render shows the
    // rubric verdict + mode only, no number of any kind.
    L.push('  <tr><th>Rank</th><th>Pair (A x B)</th><th>Verdict</th><th>Mode</th></tr>');
    for (let i = 0; i < rows.length; i += 1) {
      ...
      L.push('  <tr><td>' + escapeHtml(r.rank) + '</td><td>' + pair + '</td><td>'
        + escapeHtml(r.verdict) + '</td><td>' + rowMode + '</td></tr>');
    }
  } else {
    // Phase 355-18 (D-08, D-27, D-29): the Score column is gone -- the
    // composite stays data in portfolio-report.json, never rendered here
    // again. Its cell is the pair's own verification stamp: a tier word plus
    // the undirected methodology-graph path (or the honest unverified
    // reason). A D-30 backstop sweep runs over the plain-text stamp string
    // BEFORE it is escaped into the cell, so a stray decimal can never
    // survive into the HTML.
    L.push('  <tr><th>Rank</th><th>Pair (A x B)</th><th>Verification</th><th>Mode</th></tr>');
    for (let i = 0; i < rows.length; i += 1) {
      const r = rows[i] || {};
      // The D-30 sweep runs over the WHOLE row's plain text (pair titles
      // included, not only the stamp cell) BEFORE escaping -- a title that
      // happens to carry a decimal-shaped substring is withheld too, the
      // same backstop the md render applies over its own per-row text.
      const swept = verificationStampFormat.assertNoScalar([
        String(r.a_title || r.a || ''),
        String(r.b_title || r.b || ''),
        stampCellText(r.stamp),
      ]).lines;
      const pair = escapeHtml(swept[0]) + ' x ' + escapeHtml(swept[1]);
      const verifText = swept[2];
      const rowMode = escapeHtml(r.mode || 'embedded');
      L.push('  <tr><td>' + escapeHtml(r.rank) + '</td><td>' + pair + '</td><td>'
        + escapeHtml(verifText) + '</td><td>' + rowMode + '</td></tr>');
    }
  }
  L.push('</table>');
  return L.join('\n');
...
function statementCards(statements) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (mode === 'reasoning') {
      // Phase 355-18 (D-29): the lsa_similarity tag is gone too -- reasoning
      // mode renders no number of any kind, encoder-derived or otherwise.
