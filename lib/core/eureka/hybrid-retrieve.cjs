...
async function loadReranker(model) {
  // Offline/test seam mirroring MINDRIAN_FORCE_FTS_ABSENT (tri-modal-index.cjs)
  // and MINDRIAN_FORCE_ENGINE_ABSENT (219 D-20): a non-empty value deterministically
  // forces the unavailable-rerank degrade path regardless of whether
  // @huggingface/transformers is actually installed on this machine, so the
  // 'rerank_unavailable' warning path is testable hermetically and does not
  // depend on host install state or live model-weight downloads.
  if (typeof process.env.MINDRIAN_FORCE_RERANK_ABSENT === 'string'
    && process.env.MINDRIAN_FORCE_RERANK_ABSENT !== '') {
    throw new Error('rerank forced absent (MINDRIAN_FORCE_RERANK_ABSENT)');
  }
