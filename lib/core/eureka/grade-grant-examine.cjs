 ...
 * function.
 *
 * THE DEBATE HAS TEETH (2026-08-06 design-pass reconciliation, independent Fable review):
 * the original pass computed the findings AND the scored verdict entirely BEFORE runDebate
 * ran, so the sequential debate could never change a single finding: seven independent
 * verdicts wearing a debate costume (exactly the degeneration risk the reviewer-panel
 * design flagged as its open risk). consolidatePanel + applyPanelChallenges below close
 * that gap: a later reviewer's argument step, reading the chain's previousOutput (the
 * PRIOR reviewers' arguments -- runDebate already threads it, zero substrate changes),
 * may CHALLENGE an earlier reviewer's finding DOWNWARD ONLY (evidenced -> asserted ->
 * absent: the panel is at least as strict as its strictest reviewer, never less),
 * sustained challenges mutate the merged findings BEFORE scoreApplication runs, and
 * EVERY challenge (sustained / rebutted-stands / ignored-upward / unmatched) plus every
 * merge collision lands in a disputes[] record so disagreement SURFACES at the ruling
 * gate instead of being silently averaged away (the false-success failure class this
 * repo watches for). The same pass flipped the merge-collision rule from first-write-wins
 * (fail OPEN: a broken upstream could keep 'evidenced' over a later 'absent') to
 * stricter-status-wins (fail CLOSED).
 *
...
// The strictness order of the finding statuses (higher rank = more credit). A
// challenge may only move a finding to a LOWER rank; a merge collision keeps the
// LOWER rank (fail closed). Frozen data, shared by applyPanelChallenges and
// reviewerCellsToFindings.
const FINDING_STATUS_RANK = Object.freeze({ evidenced: 2, asserted: 1, absent: 0 });
...
// reviewerCellsToFindings(cells, rubric, collisions) -- the (d) reshape (design section
// (d)): flatten N reviewer cells' evidence arrays back into the EXACT
// {criterion_id, status, note}[] shape scoreApplication already accepts. Because
// `category` partitions the criteria with no overlap (a single-value enum field, never
// multi-tag), no two reviewers can ever produce a finding for the SAME criterion in a
// healthy run -- a collision means something upstream is broken, so the dedupe FAILS
// CLOSED: the STRICTER (lower-ranked) status wins, never the first writer (first-write-
// wins would let a broken upstream keep 'evidenced' over a later 'absent', the
// false-success failure class -- flipped in the 2026-08-06 design-pass reconciliation,
// module header). An optional `collisions` array collects a record per differing-status
// collision so consolidatePanel can surface it as a dispute. Prefers a cell's richer
...
function reviewerCellsToFindings(cells, rubric, collisions) {
  const list = Array.isArray(cells) ? cells : [];
  const sink = Array.isArray(collisions) ? collisions : null;
  ...
  for (const cell of list) {
    ...
    for (const item of source) {
      ...
      if (indexById.has(criterionId)) {
        const existing = findings[indexById.get(criterionId)];
        if (existing.status !== status) {
          const kept = (FINDING_STATUS_RANK[status] < FINDING_STATUS_RANK[existing.status])
            ? status
            : existing.status;
          if (sink) {
            sink.push({
              ...
              kept: kept,
              resolution: 'collision_stricter_kept',
            });
          }
          if (kept !== existing.status) {
            existing.status = kept;
            if (typeof item.note === 'string' && item.note.length > 0) existing.note = item.note;
          }
        }
        continue;
      }
      ...
      indexById.set(criterionId, findings.length - 1);
    }
  }
  return findings;
}

// applyPanelChallenges(findings, challenges) -- the downgrade-only cross-examination
// resolver (the piece that makes the debate a DEBATE, module header). A challenge is
// { criterion_id, to_status, challenged_by, reason, rebutted? } -- emitted by a LATER
// reviewer's argument step after reading the chain's previousOutput (the prior
// reviewers' arguments; runDebate already threads it, zero substrate changes).
// Resolution rules, deliberate and asymmetric:
//   - DOWNWARD ONLY: to_status must rank strictly below the finding's current status
//     (evidenced -> asserted -> absent). An upward or equal challenge is IGNORED but
//     still recorded (resolution 'ignored_upward') -- the panel is at least as strict
//     as its strictest reviewer, never less.
//   - A challenge the owning reviewer rebutted in a later step (rebutted: true, the
//     judgment layer's call) STANDS at the original status but still lands in disputes
//     (resolution 'stands'). Nothing is silently averaged; every disagreement renders
//     at the ruling gate.
//   - A challenge naming a criterion with no finding is recorded 'ignored_unmatched'
//     (never invents a finding to downgrade).
//   - A sustained challenge mutates a COPY of the finding (resolution 'sustained') and
//     appends the reason to its note so the scoring table shows why.
// Returns { findings, disputes }. Pure (input findings never mutated). Never throws.
function applyPanelChallenges(findings, challenges) {
  const base = Array.isArray(findings)
    ? findings.map(function (f) { return isPlainObject(f) ? Object.assign({}, f) : f; })
    : [];
  const list = Array.isArray(challenges) ? challenges : [];
  const disputes = [];
  const indexById = new Map();
  for (let i = 0; i < base.length; i += 1) {
    if (isPlainObject(base[i]) && isNonEmptyString(base[i].criterion_id)) {
      indexById.set(base[i].criterion_id, i);
    }
  }
  for (const ch of list) {
    if (!isPlainObject(ch) || !isNonEmptyString(ch.criterion_id)) continue;
    const toStatus = grantGrant.FINDING_STATUS_VALUES.has(ch.to_status) ? ch.to_status : null;
    const record = {
      criterion_id: ch.criterion_id,
      from: null,
      to: toStatus,
      challenged_by: isNonEmptyString(ch.challenged_by) ? ch.challenged_by : 'unknown',
      reason: isNonEmptyString(ch.reason) ? ch.reason : '',
      resolution: null,
    };
    if (!indexById.has(ch.criterion_id)) {
      record.resolution = 'ignored_unmatched';
      disputes.push(record);
      continue;
    }
    const finding = base[indexById.get(ch.criterion_id)];
    record.from = finding.status;
    if (toStatus === null
      || FINDING_STATUS_RANK[toStatus] >= FINDING_STATUS_RANK[finding.status]) {
      record.resolution = 'ignored_upward';
      disputes.push(record);
      continue;
    }
    if (ch.rebutted === true) {
      record.resolution = 'stands';
      disputes.push(record);
      continue;
    }
    finding.status = toStatus;
    if (record.reason.length > 0) {
      finding.note = (isNonEmptyString(finding.note) ? finding.note + '; ' : '')
        + 'challenged by ' + record.challenged_by + ': ' + record.reason;
    }
    record.resolution = 'sustained';
    disputes.push(record);
  }
  return { findings: base, disputes: disputes };
}

// consolidatePanel(cells, rubric, opts) -- the panel's single consolidation door: merge
// the collected cell readings (collisions fail closed, recorded), apply the debate's
// downgrade-only challenges (opts.challenges), and return
// { findings, disputes, dropped } where `findings` is EXACTLY the array
// scoreApplication already consumes (run it unchanged from here) and `disputes` is the
// union of merge collisions + challenge records the ruling gate renders. opts.dropped
// passes the fan-out's dropped[] list through for disclosure. Pure. Never throws.
function consolidatePanel(cells, rubric, opts) {
  const options = isPlainObject(opts) ? opts : {};
  const collisions = [];
  const merged = reviewerCellsToFindings(cells, rubric, collisions);
  const applied = applyPanelChallenges(merged, options.challenges);
  return {
    findings: applied.findings,
    disputes: collisions.concat(applied.disputes),
    dropped: Array.isArray(options.dropped) ? options.dropped : [],
  };
}
...
function buildReviewerDebateOptions(cells, hypothesis, rubric, opts) {
  ...
  // Disputes are SCALAR-summarized into the ruling record (counts only, never content
  // bytes): a panel verdict carrying 2 sustained downgrades must be distinguishable
  // from an undisputed one at the ruling gate and in the graph.
  const disputes = Array.isArray(options.disputes) ? options.disputes : [];
  const sustainedCount = disputes.filter(function (d) {
    return isPlainObject(d) && d.resolution === 'sustained';
  }).length;
  const disputeSuffix = disputes.length > 0
  ...
  const rulingSummary = isNonEmptyString(options.rulingFindingText)
    ...
    : ('Reviewer-panel ruling for ' + programId + ': ' + ruling.verb + disputeSuffix);
  const rulingFinding = {
    finding: {
