...
test('R8 round-trip: an as-of query with T_tx before a supersession still returns the superseded fact A', () => {
  ...
  try {
    ...
    // fact-in-force; B will supersede A.
    const aCreated = 100;
    const bCreated = 1_000;
    insertVersion(db, 'rt:A', { created_at: aCreated, valid_from: aCreated, valid_to: null, invalidated_at: null });
    insertVersion(db, 'rt:B', { created_at: bCreated, valid_from: bCreated, valid_to: null, invalidated_at: null });

    // Supersede A with B at a fixed reference now (the Plan 04 helper closes A:
    // invalidated_at = supersedeNow, valid_to = B.valid_from).
    ...
    // As-of BEFORE the supersession (T_tx between A.created_at and supersedeNow):
    // A was still the known, in-force fact. queryAsOf must return A.
    const before = queryAsOf(db, LOGICAL_KEY, 4_000, 4_000);
    ok(before, 'pre-supersession as-of returns a row');
    equal(before.id, 'rt:A', 'pre-supersession (T_tx=4000 < invalidated 5000) still returns A');

    // As-of AFTER the supersession: A is closed (invalidated_at=5000), B is the
    // open fact. queryAsOf must return B, not A.
    const after = queryAsOf(db, LOGICAL_KEY, 6_000, 6_000);
