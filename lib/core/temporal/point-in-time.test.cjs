...
test('R8 round-trip: an as-of query with T_tx before a supersession still returns the superseded fact A', () => {
  ...
  try {
    ...
    // Phase 348-03 (SUPER-02): rt:A is type 'claim', a truth-claim type, and
    // the human-attribution guard now also gates the path into 'superseded'
    // for such nodes (transitions.cjs setsSuperseded). This is a mechanism
    // test (the bitemporal-close round trip), not an attribution test, so an
    // explicit non-agent byUser ('navigator') replaces the pre-Phase-348
    // implicit 'system' default, which would now be correctly refused.
    const supersedeNow = 5_000;
    const sres = supersede(db, 'rt:A', 'rt:B', { now: () => supersedeNow, byUser: 'navigator' });
