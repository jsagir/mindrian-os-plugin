...
//
// Phase 348-03 (SUPER-02): the human-attribution guard now also gates the
// path into 'superseded' for TRUTH_CLAIM_TYPES nodes (transitions.cjs
// setsSuperseded). claim:A here is type 'claim', a truth-claim type, so every
// supersede() call below carries an explicit non-agent byUser ('navigator')
// -- this suite proves the bitemporal-close / edge / audit MECHANISM, not
// attribution, and the pre-Phase-348 default byUser of 'system' would now be
// correctly refused. Byte-identical mechanism assertions, human-attributed.
//
...
test('R8: supersede closes A non-lossily (row present, invalidated_at set, valid_to == B.valid_from)', () => {
  ...
  try {
    ...
    const res = supersede(db, 'claim:A', 'claim:B', { now: () => fixedNow, byUser: 'navigator' });
...
test('R8: a SUPERSEDES edge B->A exists', () => {
  ...
  try {
    supersede(db, 'claim:A', 'claim:B', { now: () => 1750000000000, byUser: 'navigator' });
