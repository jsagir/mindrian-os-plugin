...
// unchanged.
//
// Frozen six-member closed set. Order is the data4sci four-class error
// taxonomy mapping (research Pattern 1) plus rate_limited and egress_blocked:
// no_key/tier_denied are validation-class, unreachable is transient-class
...
// failure, and never coerced from or into any of the other five.
const REFUSAL_KINDS = Object.freeze(['no_key', 'unreachable', 'tier_denied', 'not_ready', 'rate_limited', 'egress_blocked']);
...
// downstream compat (shim, statusline, doctor smoke). The other four kinds
// get honest sibling statuses -- never reuse DIRECTOR_NOT_AVAILABLE (or any
// other kind's status) for a transport, tier, readiness, or constitutional
// failure (that reuse IS the conflation bug).
const KIND_STATUS = Object.freeze({
  ...
  // DIRECTOR_NOT_AVAILABLE or BRAIN_UNREACHABLE -- a constitutional block is
  // neither "no key" nor "an outage", and conflating it with either IS the
  // G1 defect this phase exists to close.
...
const RENDER_COPY = Object.freeze({
  // Phase 339 (FLIP-04, D-08, 2026-09-03): both no_key and unreachable name
  // the two-command update path (UPDATE_PATH_SENTENCE, required from
  // update-path.cjs, never retyped here). The observed situation this keys
  // on: a suspended incumbent origin makes brain-client.cjs's
  // _tryAutoRegister (lib/core/brain-client.cjs:337-346) fail, so a FRESH
  // stale install lands in no_key with an HTTP failure detail, while a
  // session that already registered before the suspension lands in
  // unreachable instead -- both populations need the same instruction, so
  // both copy blocks gain the same sentence. Deliberately NOT done: this
