...
// ---------------------------------------------------------------------------
(function pb8_01() {
  const blocked = classify(CONTENT_SET_OBJECT, { toolName: 'mcp__brain_query' });
  assert.strictEqual(blocked.verdict, 'block', 'PB8-01: CONTENT-SET object must BLOCK');

  const allowed = classify(CLEAN_MOVE_SET, { toolName: 'mcp__brain_query' });
  assert.strictEqual(allowed.verdict, 'allow', 'PB8-01: clean MOVE-SET packet must ALLOW');

  const amb = classify(NEITHER_SHAPE, { toolName: 'mcp__brain_query' });
...
(function pb8_05() {
  ...
  for (let i = 0; i < 1000; i++) {
    classify(CLEAN_MOVE_SET, { toolName: 'mcp__brain_query' });
  }
...
(function pb8_09() {
  ...
  rows.forEach(function (row, idx) {
    ...
    const res = classify(payload, { toolName: 'mcp__brain_query' });
