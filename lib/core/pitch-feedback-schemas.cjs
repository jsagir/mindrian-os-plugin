  ...
  branches: z
    .array(
      z.object({
        ...
        // Free text that mixes prose with the cited transcript quote. Byte-verbatim
        // quoting (DI-6) is NOT enforced here: a schema has no transcript to compare a
        // span against, and support items legitimately interleave prose and quote, so a
        // regex would be either vacuous or a mask over real misses - and would perturb
        // the frozen JSON-schema contract. Verbatim quoting is enforced where it can be
        // checked against the source: the Stage B prompt (rubric-huji.md Section 3b) and
        // the deterministic D1 quote-verifier (huji-eval.cjs, single-quote-aware per DI-7).
...
if (require.main === module) {
  ...
  // Phase 341-04 (D-02): the slim npm artifact does not ship the dev-only
  // GSD phase tree this path used to point at, so this self-test now writes
  // to a fresh temp directory instead. toJsonSchemas(outDir) itself is
  // unchanged; production callers (scripts/huji-run-one.cjs,
  // scripts/huji-eval.cjs) use inlineSchemaJson and never call
  // toJsonSchemas, so this is a self-test-only path change.
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mos-pitch-schemas-'));
