...
// Phase 354-05 (SYS-01, CTX-SYMLINK): the same realpath-containment helper
// every other room read/write site uses. reasoning-ops has its own
// caller-supplied `section` parameter (never passed through
// tool-router.cjs's SECTION_RE/safeResolveSection guard), so it needs its
// own containment gate -- see _containedReasoningPath below.
const { isRealpathContained, assertRealpathContained } = require('./room-path-containment.cjs');
...
// ---------------------------------------------------------------------------
// Path containment (Phase 354-05, SYS-01, CTX-SYMLINK)
// ---------------------------------------------------------------------------

/**
 * _containedReasoningPath(roomDir, section, fileName) -> the absolute path
 * to <roomDir>/.reasoning/<section>/<fileName>, or null when it escapes the
 * room -- lexically (a caller-supplied section carrying '..' segments) or
 * via realpath (an EXISTING .reasoning/<section> symlink pointing outside
 * the room, the same CTX-SYMLINK shape as the write-tool and SDK-read
 * seams this plan closes elsewhere). Module-private: every read and write
 * site below routes through this one function rather than joining the path
 * inline, so there is exactly one place this room's own reasoning-file
 * layout convention (`.reasoning/<section>/<fileName>`) is spelled out.
 *
 ...
 * @returns {string|null}
 */
function _containedReasoningPath(roomDir, section, fileName) {
  const resolved = path.resolve(roomDir);
  const target = path.resolve(resolved, '.reasoning', String(section), String(fileName));
  const lexicallyContained = target === resolved || target.startsWith(resolved + path.sep);
  if (!lexicallyContained) return null;
  if (!isRealpathContained(roomDir, target)) return null;
  return target;
}
...
function generateReasoning(roomDir, sectionName) {
  ...
  for (const section of targetSections) {
    // Phase 354-05 (SYS-01): a section whose .reasoning/<section> is (or
    // would resolve through) a symlink escaping the room is refused before
    // any directory is created or byte is written; sibling legitimate
    // sections in the same generate-all call keep working.
    const target = _containedReasoningPath(resolved, section, 'REASONING.md');
    if (!target) continue;
    const sectionReasoningDir = path.dirname(target);
    fs.mkdirSync(sectionReasoningDir, { recursive: true });
    // TOCTOU re-assert (T-354-10): mkdirSync can itself traverse a symlink
    // created between the check above and this line.
    if (!isRealpathContained(resolved, sectionReasoningDir)) continue;
...
function getReasoning(roomDir, section) {
  const reasoningPath = _containedReasoningPath(roomDir, section, 'REASONING.md');
  if (!reasoningPath) return { error: `Section outside room: ${section}` };
...
function listReasoning(roomDir) {
  ...
  return sections.all.map(section => {
    const reasoningPath = _containedReasoningPath(resolved, section, 'REASONING.md');
    if (!reasoningPath) return { section, has_reasoning: false };
    const content = safeReadFile(reasoningPath);
...
function verifyReasoning(roomDir, sectionName) {
  // Phase 354-05 (SYS-01): the objective's truth line covers "every
  // reasoning-ops read and write path", so this read site is routed through
  // the same containment gate even though it was not individually named in
  // the plan's read/write site list (getReasoning, listReasoning,
  // getReasoningFrontmatter) -- it has the identical shape and risk.
  const reasoningPath = _containedReasoningPath(roomDir, sectionName, 'REASONING.md');
  if (!reasoningPath) return { error: `Section outside room: ${sectionName}` };
  const content = safeReadFile(reasoningPath);
...
function createRun(roomDir, section) {
  ...
  // Phase 354-05 (SYS-01): runPath is built from a generated run id, not a
  // caller-supplied section -- assert the parent directory's containment
  // rather than routing through _containedReasoningPath (which assumes a
  // <section> path segment this call has none of).
  assertRealpathContained(resolved, runsDir, 'reasoning runs directory');
...
function getReasoningFrontmatter(roomDir, section, field) {
  const filePath = _containedReasoningPath(roomDir, section, 'REASONING.md');
  if (!filePath) return { error: `Section outside room: ${section}` };
