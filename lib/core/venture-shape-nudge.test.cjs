...
test('Test 10 (FIX-05 Part 8 fence): the f_selector_decision payload contains NO substring of the input sentence -- only scalar boolean + source enum', () => {
  const { dir, db } = freshRoom();
  const SENTENCE = 'I want to build a startup that validates a founder venture thesis with paying customers';
  ...
  // Part 8 fence: NO substring of the user sentence anywhere in the payload.
  const words = SENTENCE.split(/\s+/).filter((w) => w.length >= 5);
  for (const w of words) {
    assert.equal(propsRaw.includes(w), false, 'Part 8 breach: sentence substring "' + w + '" leaked into payload');
  }
