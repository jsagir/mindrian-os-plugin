...
async function fetchCorpus(args) {
  ...
  // parse as an absolute http(s) URL. Non-http(s) schemes (file:, ftp:, ...)
  // reject with a TypeError BEFORE any dispatch -- no local/off-scheme fetch
  // ever. Runs AFTER the audit above for chokepoint uniformity.
  ...
  switch (source) {
    ...
    case 'tavily-extract':
      return adapterTavilyExtract(query, opts);
    case 'brain-cypher':
