 ...
 * edge in the LOCAL graph. Phase 103-02 ships the side-log stub; Phase
 * 103-05 wires the actual room-db.cjs HAS_JTBD edge write.
 *
...
function GRAPH_EDGE_LOG(){ return path.join(MEMORY_DIR(), 'graph-edge-pending.log'); }
...
function ensureDir() {
  ...
  if (!fs.existsSync(ROOM_MD_FILE())) {
    const body = [
      ...
      '- graph-edge-pending.log: side-log of intended Canon Part 4 graph edges. Drained by Phase 103-05 PostToolUse.',
      '',
      '## Privacy contract',
      '',
...
// ---------------------------------------------------------------------
// Canon Part 4 graph-edge stub.
// Phase 103-05 wires this to lib/core/room-db.cjs HAS_JTBD edges. Until
// then, append intent to a side-log so the Canon Part 4 connection is
// auditable post-facto.
// ---------------------------------------------------------------------

function writeGraphEdge(action, roomSlug, jtbd, props) {
  try {
    ensureDir();
    ...
    fs.appendFileSync(GRAPH_EDGE_LOG(), line + '\n');
  } catch (err) {
    logStderr('writeGraphEdge', err);
  }
}
...
function promoteIfEligible(roomSlug, withinSessionState) {
  try {
    ...
    writeGraphEdge('promote', roomSlug, cur.jtbd, { state: 'in_flight', ts: now });
...
function parkJtbd(roomSlug, jtbd, reason) {
  try {
    ...
    writeGraphEdge('park', roomSlug, jtbd, { state: 'parked', reason: reasonStr, ts: now });
...
function completeJtbd(roomSlug, jtbd, completionShape, completionEvidence) {
  try {
    ...
    writeGraphEdge('complete', roomSlug, jtbd, {
      state: 'completed',
...
module.exports = {
  ...
  // Internal helpers exposed for hermetic tests + future Phase 103-05 wiring.
  _internal: {
