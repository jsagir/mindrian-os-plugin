    ...
    if (result && result.shape !== 'error' && !result.passthrough) {
      ...
      if (isFShape) {
        ...
        if (payloadObj.emitTelemetry === true) {
          ...
          // Phase 209-06 (H3): the PRIMARY side-channel producer.
          //
          // stop-hook-fires-card-on-option-shaped-prose-sentence (2026-09-17,
          // SECOND pass -- the first pass's SESSIONLESS_UNION_WINDOW_MS window
          // narrowing was live-disconfirmed; it shrank the cross-session
          // collision's probability, it did not close it). This call site
          // previously supplied NO sessionId at all ("this call site never had
          // one" -- the original comment here, now corrected). Traced the
          // actual callers/environment instead of assuming: `payloadObj` never
          // carries a real session id today (grep-confirmed, no pickShape call
          // site anywhere in this repo passes one), and the MCP SDK's
          // `extra.sessionId` never reaches this seam either (it is not a tool
          // handler). BUT `process.env.CLAUDE_CODE_SESSION_ID` -- the SAME env
          // var `lib/core/session-binding.cjs`'s `resolveEffectiveSessionId`
          // already uses as its own stdio-transport fallback, and which
          // `.planning/debug/resolved/registry-active-session-unbound-
          // inheritance.md` LIVE-confirmed on this exact machine (`ps -p <pid>`
          // matching a running `claude` process, its env carrying
          // CLAUDE_CODE_SESSION_ID, explicitly noted there as "the session
          // UUID the hook reader also uses") -- IS reachable here: Claude Code
          // sets this env var on every subprocess it spawns for the live
          // session (the Bash-tool / MCP-server process this dispatcher runs
          // inside included), and it is the SAME UUID the Stop hook itself
          // reads from its own stdin `session_id` field. Threading it through
          // means the mint lands in THAT session's own real bucket, matched by
          // the Stop hook via an EXACT key, not a time-windowed guess -- closing
          // the false-positive (a different session's mint can never match this
          // session's real id) while genuinely preserving the true-positive
          // same-turn detection the NO_SESSION_KEY union existed for (no window
          // needed at all when the key is exact).
          //
          // `payloadObj.sessionId` is checked FIRST (explicit beats env,
          // mirroring resolveEffectiveSessionId's own precedence) in case any
          // future or as-yet-unaudited caller ever does have a real MCP
          // `extra.sessionId` to hand in. When NEITHER resolves (the var is
          // genuinely absent -- outside a live Claude Code process, e.g. a bare
          // unit test), sessionId stays undefined and recordReachedGate's own
          // existing NO_SESSION_KEY fallback applies; card-fire-
          // sidechannel.cjs's scopedRecords no longer unions that bucket into
          // ANY other session's read at all (see that module's own doc
          // comment) -- an unresolved id now degrades to "invisible to every
          // other session," never "leaks into every other session."
          //
          // Gated on the SAME emitTelemetry flag as the two calls above (Canon
          // Part 8 fs_scope: lib/render/render-v2.cjs's Zone-4 dispatcher call
          // must stay zero-FS). Lazy-required inside try/catch; a writer fault
          // never affects the presentation result.
          ...
          try {
            ...
            const resolvedSessionId =
              (typeof payloadObj.sessionId === 'string' && payloadObj.sessionId.length > 0)
                ...
                : (typeof envSessionId === 'string' && envSessionId.length > 0)
                  ? envSessionId
                  : undefined;
