 ...
 * DIAL_REACH_K=5. "Rank 5, preview 5, choose 3."
 *
...
// Rank 5, preview 5, choose 3. (UI-SPEC Section 1.3 / V9.)
const DIAL_REACH_K = 5;
...
// The 5 canonical reaches (frozen, UI-SPEC Section 0), in doctrine order. Each
// carries its canonical_verb (persisted to the graph edge; NEVER rendered as
// the row label -- the Feynman-JTBD label is a render-time alias, Plan 04) and
// the per-reach Brain-reachability class. Registry-only reaches (no Brain
// signal) default brain_confidence=0.5 so they can never solo-cross 0.70 --
// a cold room yields zero markers by design (UI-SPEC Section 6.4).
const REACH_DEFS = [
...
// renders (the dial renders the 5 canonical reaches), but the call exercises
 ...
 *
 * ReachList {
 *   reaches: Reach[<=DIAL_REACH_K(5)]  // ranked desc by score
...
function buildReachList(roomState) {
  ...
  reaches.sort((a, b) => b.score - a.score);

  // Apply the frozen 0.70 / 0.15 gate per-reach.
  _applyFrozenGate(reaches, tierMode);

  const total_count = reaches.length;
