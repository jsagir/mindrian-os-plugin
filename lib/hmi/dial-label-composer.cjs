...
// ---------- Phase 355 Plan 22 (HIPS-06, HIPS-05, D-41): the stamped-finding
// card variant on the deep_research family ----------
//
// A SENS-13 (eureka_bridge) reach that fired the deep_research reach carries
// its own verification stamp (lib/core/sensors/sensor-eureka.cjs's evidence
// bag), and the card it surfaces must show that stamp instead of the
// ordinary {topic}/{framework} JTBD sentence. reach_id and signal stay
// unchanged (no seventh reach, no lockstep). This variant bypasses the
// normal {slot} resolution ladder entirely -- it is selected the moment the
// caller's slotContext carries signal:'eureka_bridge' plus a non-empty
// stamp_verification, and is identical to
// verification-stamp-format.cjs's formatStampLines(stamp, 'card')[0] for the
// same stamp (that module is never required here -- render-only, zero fs,
// same literal text, kept in lockstep by tests/test-dial-label-bank-drift.cjs).
const STAMPED_UNVERIFIED_CARD_LINE = 'unverified - novel or hallucinated, verify with an expert';

function isStampedFindingSlotContext(slotContext) {
  const sc = (slotContext && typeof slotContext === 'object') ? slotContext : null;
  return !!sc
    && sc.signal === 'eureka_bridge'
    && typeof sc.stamp_verification === 'string'
    && sc.stamp_verification.length > 0;
}

function composeStampedFindingLabel(slotContext) {
  const verification = slotContext.stamp_verification;
  const isVerified = verification === 'strong' || verification === 'indirect';
  if (isVerified) {
    const stampPath = (typeof slotContext.stamp_path === 'string') ? slotContext.stamp_path : '';
    return 'verified through ' + stampPath;
  }
  return STAMPED_UNVERIFIED_CARD_LINE;
}
...
function composeLabel(reachId, slotContext) {
  ...
  // Phase 355 Plan 22 (D-41): the stamped-finding variant, deep_research
  // only. Bypasses {slot} resolution entirely -- a stamped card is never
  // degraded to the generic JTBD line, since it already carries its own
  // real content.
  if (reachId === 'deep_research' && isStampedFindingSlotContext(slotContext)) {
    return Object.freeze({
      label: composeStampedFindingLabel(slotContext),
