 ...
 *   The other three families (context_block / contradiction / cross_room) are
 *   render-only and NEVER invoke the audit.
 *
 * License: BSL 1.1.
 */
'use strict';
...
// ---------- The 5 frozen template families (UI-SPEC Section 9) ----------
//
...
// ---------- The generic JTBD fallback (degradation ladder, deepest rung) ----------
//
// When a slot cannot resolve, the family drops to the generic JTBD one_line
// from jtbd-taxonomy.json (the `explore` entry: "No specific job - general
// thinking, talking, ranging."). NEVER render a raw {topic} slot. The file
// read is cached after first load; a read failure degrades to a hardcoded
// safe line (Tier 0 contract).
