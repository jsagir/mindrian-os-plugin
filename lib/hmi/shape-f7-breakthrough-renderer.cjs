 ...
 *
 * Phase 120-01 -- Breakthrough Surface renderer.
 *
 * Phase 188-01 (SFS-06) REPOINT: "Breakthrough" is no longer one of the canonical
 *   ten shapes (D-10). Bare 'F.7' is the CANONICAL DIAL now; this module is NO
 *   LONGER reachable through the bare-F.7 single door in selector-dispatcher.cjs.
 *   It is RETAINED, unchanged, as the definitional HOME of the frozen 5-verb set
 *   (F7_VERBS) and the D-20 provenance HARD FLOOR. The breakthrough re-homes to an
 *   F.1 next-move (scanner.cjs::surfaceBreakthrough dispatches requestedShape 'F.1');
 *   this renderer stays the source of truth for the verb vocabulary + the render's
 *   provenance refusal so the floor is preserved at the new home.
 *
 * Historical routing note (kept for grep continuity): this module was formerly
 *   wired as shape-f7-breakthrough-renderer.cjs behind requestedShape:'F.7'.
 *
