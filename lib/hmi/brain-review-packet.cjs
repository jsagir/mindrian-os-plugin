 ...
 * neighborhood off the focus node, filters to CONTRADICTS edges, and returns the
 * COUNT plus the deduped set of GENERIC framework handles involved (read off the
 * neighbor node `framework` enum field when present). NEVER reads or returns a
 * finding body / summary / artifact text. Wrapped: any read failure returns the
 * empty shape (degrade, never crash).
 */
function _localContradictions(db, focusNodeId) {
  ...
  try {
    ...
    const contradicts = neighbors.filter(function (n) {
      return n && (n.edgeTypeIn === 'CONTRADICTS' || n.edgeTypeOut === 'CONTRADICTS');
    });
    const handles = [];
    for (const n of contradicts) {
      // Only a GENERIC framework handle is eligible. Audit it before it can be
      // surfaced (defense-in-depth: even a local handle passes the egress gate).
      const raw = (n && typeof n.framework === 'string') ? n.framework : null;
      const audited = _safeAudit(raw);
      if (audited && handles.indexOf(audited) === -1) handles.push(audited);
    }
    return { count: contradicts.length, framework_handles: handles };
  } catch (_e) {
    return empty;
  }
}
...
function buildBrainReviewPacket(input) {
  ...
  // 1. LOCAL contradictions (never sent to Brain).
