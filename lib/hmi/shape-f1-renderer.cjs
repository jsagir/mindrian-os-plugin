...
// Phase 205 (item 8 + item 11): the SHIPPED Phase 188.1 elevation-framed
// what-you-get lines. When a selector row carries a reach family, the row STATES
// the elevation the navigator receives (vertical -> "Go a level deeper...",
// horizontal -> "Connect two threads...", lateral -> "Bring in a reference from
// outside...") instead of a mechanism-blank generic JTBD line. The canonical_verb
// still persists to the graph edge upstream and NEVER renders here -- the
// dial-label-composer engine/screen separation (Canon Part 8). Required at top;
// dial-label-composer never requires this renderer, so there is no cycle.
const dialLabelComposer = require('./dial-label-composer.cjs');
...
/**
 * elevationRowLabel(family) -- frame a selector row as the ELEVATION the
 * navigator receives, reusing the shipped Phase 188.1 what-you-get lines
 * (dial-label-composer.elevationDefault). `family` is a reach family object (it
 * carries `.elevation`, one of vertical | horizontal | lateral). The returned
 * line is SLOT-FREE (a degraded-safe what-you-get); it never contains the
 * mechanism-blank "No specific job - general thinking" string and never leaks the
 * canonical_verb (which persists to the graph edge, not the screen). A missing /
 * malformed family degrades to the composer's own generic fallback.
 */
function elevationRowLabel(family) {
  return dialLabelComposer.elevationDefault(family && typeof family === 'object' ? family : {});
}
...
function renderShapeF1(input) {
  ...
  // Phase 205 (item 8 + item 11): an OPTIONAL per-row reach-family array parallel
  // to `verbs`. When elevationFamilies[i] is a reach family object, row i's
  // DISPLAYED label becomes the elevation-framed what-you-get (elevationRowLabel)
  // instead of the mechanism verb -- the row STATES the elevation the navigator
  // receives. The verb primitive still rides in contract.verbs (the selectable
  // handle / graph edge); only the screen text changes. Absent / null -> the row
  // renders the verb exactly as before (byte-stable, additive).
  const elevationFamilies = Array.isArray(opts.elevationFamilies) ? opts.elevationFamilies : null;
  ...
  const lines = verbs.map((verb, i) => {
    ...
    let display = verb;
    if (elevationFamilies && elevationFamilies[i] && typeof elevationFamilies[i] === 'object') {
      display = elevationRowLabel(elevationFamilies[i]);
    }
    return prefix + ' ' + String(i + 1) + '. ' + display;
  });
...
module.exports = {
  ...
  // Phase 205 (item 8 + item 11): the elevation-framed row-label helper (reuses
  // the shipped Phase 188.1 dial-label-composer.elevationDefault).
  elevationRowLabel: elevationRowLabel,
