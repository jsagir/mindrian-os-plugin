...
async function ensureDaemon(opts) {
  ...
  // MINDRIAN_MCP_DAEMON=1 marks a process spawned BY this function (the
  // shared daemon) as distinct from a real Cowork client that happens to set
  // MINDRIAN_TRANSPORT=http itself -- surface-detect.cjs resolves both to the
  // SAME surface:'cowork' at detectSurface() time (198-08, D-06/D-07 fix):
  // the daemon serves ALL flagged surfaces (D-01 topology), not only
  // Cowork's own isMcpFirst('cowork') check, so bin/mindrian-mcp-server.cjs
  // uses this marker to activate its own pidfile/port-discovery/SSE wiring
  // whenever ANY surface is flagged on, not only when 'cowork' itself is.
  const env = Object.assign({}, process.env, { MINDRIAN_TRANSPORT: 'http', MINDRIAN_MCP_DAEMON: '1' }, (opts.env || {}));
