 ...
 *
 * PHASE 347 PROJECTION (WD-347-2, docs/2026-09-14-CHAIN-SHARED-STATE-
 * CONTRACT.md Section 6): Phase 347 adds chain_state nodes to room.db as a
 * PROJECTION of this store, not as a competing one. The graph records exist
 * so a step's input is reconstructible from the room graph alone; they are
 * derived, and THIS FILE remains the sole truth for chain position and
 * resume state. On disagreement THIS STORE WINS and the graph row is
 * reported stale -- the same rule commands/pipeline.md:113 already states
 * for the artifact-frontmatter secondary index. No code path reads the
 * graph as the resume cursor. Promotion of the graph to primary is a named
 * follow-on (WD-347-2), and it is deliberately not taken here, because it
 * would require amending this declaration, rewriting chain_run's resume,
 * and adding T-198-12 anti-spoofing for a graph-reconstructed gate.
 *
