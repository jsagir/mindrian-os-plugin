...
// Phase 354-05 (SYS-01, CTX-SYMLINK): the one realpath-containment helper
// every room read/write site now inherits through safeResolveSection below.
const { assertRealpathContained } = require('../core/room-path-containment.cjs');
...
// Phase 355 D-07: the two direction wire ids live in exactly ONE module now.
const { DIRECTIONS } = require('../core/direction-convention.cjs');
 ...
 *
 * Phase 354-05 (SYS-01, CTX-SYMLINK): the lexical check alone is not
 * sufficient -- an EXISTING symlink inside roomDir (e.g. room/research ->
 * an outside directory) passes it (the symlink's own path is lexically
 * inside roomDir), while its realpath is not. A realpath containment check
 * now runs after the lexical one, so every existing caller of this function
 * (views.cjs fileArtifact, room.cjs searchRoom, this file's own chain-state
 * fallback below) inherits the fix with no per-caller edit.
 *
 ...
 * @throws {Error} If resolved path escapes roomDir lexically (plain Error)
 *   or via realpath (Error with code 'ROOM_PATH_ESCAPE').
 */
function safeResolveSection(roomDir, section) {
  ...
  assertRealpathContained(base, resolved, 'section ' + section);
  return resolved;
}

/**
 * Phase 347-09 (SHARED-09): raw node properties for a chain_state record.
 *
 * navigation.readChainState's own toRecord() returns a FIXED field set
 * (node_id, run_id, step_index, kind, command, body, quality, tier,
 * produced_by, schema_variant) -- it never surfaces an arbitrary caller-
 * supplied extraProps key (writeChainStateRecord's own escape valve). The
 * optional routing declarations this render reads (on_pass, on_fail,
 * fan_out, reviewer, halted, halt_reason) travel exactly that way, so this
 * reads the node's own properties JSON directly rather than widening
 * chain-state.cjs's fixed return shape (out of scope for this plan; that
 * module is read-only per 347-09's own files_modified). A column name is
 * never caller-supplied: one prepared SELECT bound with a placeholder.
 *
 * @param {import('node:sqlite').DatabaseSync} db
 * @param {string} nodeId
 * @returns {object|null}
 */
function _readRawChainStateProperties(db, nodeId) {
  try {
    const row = db.prepare('SELECT properties FROM nodes WHERE id = ?').get(nodeId);
    if (!row || typeof row.properties !== 'string') return null;
...
/**
 * Phase 347-09: map a chain_state record's `quality` (high|medium|low|null)
 * onto generateMermaidChain's existing four-value status vocabulary
 * (complete|running|pending|failed). No fifth value is introduced.
 *
 * @param {string|null} quality
 * @returns {'complete'|'running'|'pending'|'failed'}
 */
function _chainStateQualityToStatus(quality) {
  if (quality === 'high') return 'complete';
  if (quality === 'medium') return 'running';
  if (quality === 'low') return 'failed';
  return 'pending';
}

/**
 * Phase 347-09: find the newest chain_state node's run_id, so visualize-
 * chain renders the most recently recorded run without a caller-supplied
 * run_id parameter (the room_graph schema carries none). `nodes` keeps its
 * default SQLite rowid regardless of schema variant (the id column's own
 * `PRIMARY KEY` never declares `WITHOUT ROWID`), so ORDER BY rowid DESC
 * reliably names the last-inserted row across the wide/mid/legacy variants
 * (lib/core/navigation/CONTEXT.md), without depending on a created_at
 * column that only the wide variant carries.
 *
 * @param {import('node:sqlite').DatabaseSync} db
 * @returns {string|null}
 */
function _newestChainRunId(db) {
  try {
    const row = db.prepare('SELECT properties FROM nodes WHERE type = ? ORDER BY rowid DESC LIMIT 1')
      .get('chain_state');
    if (!row || typeof row.properties !== 'string') return null;
    const parsed = JSON.parse(row.properties);
    return (parsed && typeof parsed.run_id === 'string' && parsed.run_id.length > 0) ? parsed.run_id : null;
  } catch (_e) {
    return null;
  }
}
...
/**
 * Phase 347-09: map the ordered readChainState() records for one run onto
 * generateMermaidChain's step shape, folding in whatever optional routing
 * declarations (on_pass, on_fail, fan_out, reviewer) travel on each
 * record's own raw properties, and attaching a recorded halt (halted +
 * halt_reason) to the LAST record only -- the step the run actually halted
 * on, never fabricated onto an earlier one.
 *
 * @param {import('node:sqlite').DatabaseSync} db
 * @param {Array<object>} records - navigation.readChainState's return value
 * @returns {Array<object>} steps, generateMermaidChain's own input shape
 */
function _chainStateRecordsToSteps(db, records) {
  return records.map((record, idx) => {
    const raw = _readRawChainStateProperties(db, record.node_id) || {};
    const step = {
      name: (typeof record.command === 'string' && record.command)
        ? record.command
        : `step_${record.step_index != null ? record.step_index : idx}`,
      status: _chainStateQualityToStatus(record.quality),
    };
    if (typeof raw.framework === 'string' && raw.framework) step.framework = raw.framework;
    if (raw.on_pass !== undefined && raw.on_pass !== null) step.on_pass = raw.on_pass;
    if (raw.on_fail !== undefined && raw.on_fail !== null) step.on_fail = raw.on_fail;
    if (Array.isArray(raw.fan_out) && raw.fan_out.length > 0) step.fan_out = raw.fan_out;
    if (raw.reviewer && typeof raw.reviewer === 'object' && typeof raw.reviewer.kind === 'string') {
      step.reviewer = { kind: raw.reviewer.kind };
    }
    if (idx === records.length - 1 && raw.halted === true) {
      step.halted = true;
      if (typeof raw.halt_reason === 'string' && raw.halt_reason) step.halt_reason = raw.halt_reason;
    }
    return step;
  });
}
...
/**
 * 354-09 (THEO-01 executable-chain validation, T-354-19): the response for
 * act / act-chain / act-swarm when brainRouter.validateChain rejects the
 * recommendation's chain. No pipelineState.initChain and no loadReference
 * call happens for a rejected chain -- the caller returns this response
 * BEFORE either -- so a chain the methodology registry cannot run never
 * becomes the room's active pipeline state.
 * @param {object} rec - the brainRouter.recommend() result
 * @param {{valid: boolean, reason?: string}} validation
 * @returns {string}
 */
function renderNotExecutable(rec, validation) {
  const parts = [
    '## Framework Recommendation (not executable)',
    `\n**Chain:** ${Array.isArray(rec.chain) ? rec.chain.join(' -> ') : ''}`,
    `**Source:** ${rec.source}`,
    `**Reason:** ${validation && validation.reason}`,
  ];
  if (Array.isArray(rec.frameworks) && rec.frameworks.length > 0) {
    parts.push(`**Frameworks considered:** ${rec.frameworks.join(', ')}`);
  }
  return parts.join('\n') + formatSuggestedNext(
    'orchestration',
    { command: 'act-dry-run' },
    'Chain failed validation - run act-dry-run to inspect the rejected chain before retrying'
  );
}

/**
 * 354-09 (THEO-01 provenance): renders the '**Provenance:**' line a valid,
 * Brain-sourced recommendation carries -- rec.chain_type
 * ('ranked_candidates', never 'feeds_into') and, when present,
 * rec.provenance.method. Returns '' for a local recommendation (no
 * chain_type at all), so the existing act/act-chain/act-swarm layout is
 * unchanged for the local-fallback path.
 * @param {object} rec - the brainRouter.recommend() result
 * @returns {string}
 */
function renderProvenanceLine(rec) {
  if (!rec.chain_type) return '';
  const method = rec.provenance && rec.provenance.method;
  return `\n**Provenance:** ${rec.chain_type}${method ? ' (' + method + ')' : ''}`;
}
...
function registerRouterTools(server, roomDir, pluginRoot, larryContext, surface) {
    ...
    async ({ command, section, query }) => {
      switch (command) {
        ...
        case 'visualize-chain': {
          ...
          // First: the real recorded run. openRoomDbForCaller returns null on
          // a cold room (no room.db yet) instead of creating one as a side
          // effect of merely rendering (same door discipline as
          // runResearchPipeline above). Closed in a finally, caller-owned.
          let db = null;
          try {
            db = navigation.openRoomDbForCaller(roomDir);
            if (db) {
              const runId = _newestChainRunId(db);
              if (runId) {
                const records = navigation.readChainState(db, runId);
                if (records.length > 0) {
                  chainSteps = _chainStateRecordsToSteps(db, records);
                }
              }
            }
          } catch (_e) {
          ...
          // Second: no chain_state record exists AND a section was supplied
          // -- the documented fallback, unchanged, still behind the same
          // traversal guard as before.
          if (chainSteps.length === 0 && section) {
          ...
          // Third: neither source has anything. Honest single statement --
          // NOT a fabricated pending list (the render-side form of the
          // chain-step-dispatcher.cjs:43-46 honesty rule).
          if (chainSteps.length === 0) {
            return textResponse('No chain run has been recorded in this room yet.'
              + formatSuggestedNext('room_state', { command: 'suggest-next' }, 'Run a chain first, then visualize-chain will draw its real path'));
          }
  ...
  server.registerTool(
    ...
    {
      title: 'Methodology',
      description: 'Run a PWS innovation methodology framework. Larry guides you through the selected framework and files results to your Data Room.',
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      title: 'Intelligence',
      description: 'Run intelligence operations: find connections, grade ventures, build thesis, research. Larry evaluates with calibrated assessment.',
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      title: 'Orchestration',
      // Phase 355 D-23 (HIPS-03): scout-hsi's own sentence is honest on
      // purpose -- it is reference text under a compute-shaped name, still a
      // member of UNIMPLEMENTED_MUTATING_ORCHESTRATION below, never wired to
      // hsiEngine.runTier1 (that would cover one of three pipeline steps and
      // add a write path plus a model download on Desktop/Cowork).
      description: 'Manage rooms (rooms-list, rooms-new, rooms-open, rooms-close, rooms-archive, rooms-where), run scout intelligence (scout, scout-health, scout-deadlines, scout-competitors, scout-hsi, scout-snapshot), and handle admin, onboarding, models and scheduled tasks. rooms-open genuinely switches the active room. scout and its variants only return the scout reference plus current room context, not gathered intelligence; run /mos:scout on the CLI for the real scan (a .snapshots/ state capture, a competitor report, the HSI pipeline). scout-hsi: reference only, no compute; run `/mos:scout hsi` in Claude Code. Treat act, act-chain and act-swarm differently: they execute multi-step work with little supervision, so prefer act-dry-run first and do not invoke them for a decision the user has not already framed.',
      inputSchema: z.object({
    ...
    async ({ command, context, room, confirmArchived }, extra) => {
      ...
      if (command === 'act' || command === 'act-chain' || command === 'act-dry-run' || command === 'act-swarm') {
        ...
        // 354-09 (THEO-01 executable-chain validation, T-354-19): computed
        // ONCE, right after recommend(), BEFORE any pipelineState.initChain
        // or loadReference(rec.chain[0]) call below -- act-dry-run reuses
        // this SAME validation object rather than recomputing it.
        const validation = brainRouter.validateChain(roomDir, rec.chain);

        if (command === 'act') {
          if (!validation.valid) {
            return textResponse(renderNotExecutable(rec, validation));
          }
          ...
          const provenanceLine = renderProvenanceLine(rec);
          if (provenanceLine) parts.push(provenanceLine);
        ...
        if (command === 'act-chain') {
          ...
          const provenanceLine = renderProvenanceLine(rec);
          return textResponse(`## Framework Chain\n\n**Full Sequence:**\n${chainSteps}\n\n**Confidence:** ${(rec.confidence * 100).toFixed(0)}%\n**Source:** ${rec.source}\n**Reasoning:** ${rec.reasoning}${provenanceLine}` + formatSuggestedNext('methodology', { command: rec.chain[0] }, 'Start chain with first framework'));
        }
      ...
      if (unimplementedMutation) {
        parts.push(`\n> **NOT EXECUTED.** This returns the \`${command}\` instructions, not a completed operation. Nothing has changed yet. Follow the Reference steps below and verify the result before reporting success.`);
        // Phase 355 D-23 (HIPS-03): scout-hsi is the one command in this
        // set that reads as compute-shaped (a scout intelligence command),
        // not merely a declared-but-unimplemented mutation, so it gets its
        // own explicit banner line naming the real CLI path.
  ...
  server.registerTool(
    ...
    {
      title: 'Room Bind',
      description: 'Bind this MCP session to a room (D-03: explicit room wins; cwd auto-bind when launched inside a room; a structured binding-card signal on genuine ambiguity). The room this session binds to becomes the target for every subsequent write this session makes.',
      inputSchema: z.object({
        room: z.string().optional()
  ...
  server.registerTool(
    ...
    {
      title: 'Eureka Critic',
      description: 'Grounding Guard calibration ruling: quantized scalars + closed enums in, a coarse verdict/confidence/reasoning_tag out; no room content ever crosses this boundary.',
      inputSchema: z.object({
        ...
        // Phase 355 D-07: sourced from direction-convention.cjs's DIRECTIONS,
        // not a local copy of the two literal ids.
        surprise_type: z.enum(DIRECTIONS)
