...
const ALL_TOOL_COMMANDS = [
  ...
  'room', 'status', 'new-project', 'setup', 'update', 'help', 'suggest-next', 'persona', 'opportunities', 'funding', 'graph',
  // methodology group
...
function registerRouterTools(server, roomDir, pluginRoot, larryContext) {
  ...
  server.tool(
    'data_room',
    `Manage your MindrianOS Data Room: state, sections, opportunities, funding, personas, knowledge graph. Query and manage the per-project knowledge graph (KuzuDB). Use graph-query with Cypher to answer questions about relationships between room artifacts. ${compact.slice(0, 80)}`,
    {
