...
function registerRouterTools(server, roomDir, pluginRoot, larryContext, surface) {
  ...
  server.tool(
    'room_content',
    'Create and update the entities that live inside a room: projects, opportunities, funding stages, personas, and detected integrations. This is the WRITE surface (new-project, setup, file-opportunity, create-funding, generate-personas, invoke-persona, organize), so reach for it when room content needs to be added or changed. Use room_state instead when you only need to READ the room: its health, computed stage, or a suggested next move. room_state never mutates anything, this tool usually does.',
    {
