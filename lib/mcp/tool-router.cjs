 ...
 *
 * Registers 6 high-level MCP tools that dispatch to all 45 CLI commands.
 * Hierarchical design keeps total tool definition under 5000 tokens
 * (vs 30-60K for flat 41-tool registration).
 *
 * Router tools:
 *   1. data_room    — room state, sections, project lifecycle, personas (14 commands)
 ...
 *
 * Total: 45 commands across 6 router tools (verified via ALL_TOOL_COMMANDS).
 */
...
/**
 * Flat array of all 45 CLI command names for parity checking.
...
const ALL_TOOL_COMMANDS = [
  ...
  'room', 'status', 'new-project', 'setup', 'update', 'help', 'suggest-next', 'persona', 'opportunities', 'funding',
  // methodology group
...
function registerRouterTools(server, roomDir, pluginRoot, larryContext) {
  ...
  server.tool(
    'data_room',
    `Manage your MindrianOS Data Room: state, sections, opportunities, funding, personas. ${compact.slice(0, 80)}`,
    {
