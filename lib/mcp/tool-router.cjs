...
// the previous room.
//
// rooms-open is now implemented and executes (see its branch below). These
// three remain unimplemented BY DESIGN in this RCA's scope -- rooms-new has its
// own separate open RCA with a different root cause
...
//
// Membership rule: a command belongs here if it MUTATES persistent state AND
// has no executing branch. Add an executing handler, remove it from this set.
// Read-only fallback commands (rooms-list, rooms-where, scout*, models, ...)
// are deliberately absent: returning documentation instead of data is a
// capability gap, not a false claim about a state change.
...
function registerRouterTools(server, roomDir, pluginRoot, larryContext, surface) {
  ...
  server.tool(
    'room_content',
    'Create and update the entities that live inside a room: projects, opportunities, funding stages, personas, and detected integrations. This is the WRITE surface (new-project, setup, file-opportunity, create-funding, invoke-persona); generate-personas now ROUTES to /mos:persona --parallel by default and only writes when explicitly asked for a preview, so reach for this tool when room content needs to be added or changed. Use room_state instead when you only need to READ the room: its health, computed stage, or a suggested next move. room_state never mutates anything, this tool usually does.',
    {
  ...
  server.tool(
    'meeting',
    'Turn a raw meeting transcript into structured Data Room content. file-meeting parses a transcript and files it as a room entry, pipeline runs the full meeting-intelligence pass over what was filed (decisions, commitments, open questions), and speakers resolves who said what. Reach for this whenever the user pastes or points at a conversation: institutional knowledge lives in meetings rather than documents, and until a transcript is filed here it stays invisible to every other tool in this server.',
    {
  ...
  server.tool(
    'orchestration',
    'Manage rooms (rooms-list, rooms-new, rooms-open, rooms-close, rooms-archive, rooms-where), run scout intelligence (scout, scout-health, scout-deadlines, scout-competitors, scout-hsi, scout-snapshot), and handle admin, onboarding, models and scheduled tasks. The room and scout operations are ordinary reads and writes, so use them freely. Treat act, act-chain and act-swarm differently: they execute multi-step work with little supervision, so prefer act-dry-run first and do not invoke them for a decision the user has not already framed.',
    {
