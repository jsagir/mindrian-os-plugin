...
function registerRouterTools(server, roomDir, pluginRoot, larryContext) {
  ...
  server.tool(
    ...
    async ({ command, context, section }) => {
      ...
      return textResponse(response + formatSuggestedNext('room_state', { command: 'analyze' }, 'Methodology complete - analyze room to detect new patterns from this framework'));
    }
  );
  ...
  server.tool(
    ...
    async ({ command, context, section }) => {
      // Causal commands have specific chaining patterns
      let suggestedNext;
