...
// Born-wired SOURCE of truth (Part 11 R1/R16). The four sensors are pure
// reads (hitl_shape 'none'); framework_run reaches a genuine material-step
// fork (hitl_shape 'F.1', the same shape gate_render/gate_answer carry).
...
const connectors = [
  {
    ...
    connector: 'mcp-tool',
    hitl_shape: 'none',
    hitl_why: 'Pure read: pulls the top candidate reach through the shipped dispatchSensors path, no fork.',
  },
  {
    ...
    connector: 'mcp-tool',
    hitl_shape: 'none',
    hitl_why: 'Pure read: pulls the full candidate-reach set through the shipped dispatchSensors path, no fork.',
  },
  {
