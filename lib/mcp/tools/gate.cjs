...
// -----------------------------------------------------------------------
// T-198-10 spoofing mitigation: a single-use, in-memory ledger of gate_ids
// THIS process minted via gate_render. gate_answer rejects any gate_id not
// present here (or expired) -- a forged or replayed answer never reaches
// navigation.cjs.
// -----------------------------------------------------------------------
const _liveGates = new Map();
const LIVE_GATE_TTL_MS = 30 * 60 * 1000; // 30 minutes

function _mintLiveGate(gateId, card) {
  _liveGates.set(gateId, { card: card, mintedAt: Date.now() });
}

function _consumeLiveGate(gateId) {
  const entry = _liveGates.get(gateId);
  if (!entry) return null;
  _liveGates.delete(gateId); // single-use: consumed whether or not the TTL held
  if (Date.now() - entry.mintedAt > LIVE_GATE_TTL_MS) return null;
  return entry;
}
...
function register(server, ctx) {
  server.tool(
    ...
    async ({ gate_id, header, kind, ambiguous, select_mode, options }, extra) => {
      ...
      _mintLiveGate(result.card.gate_id, result.card);
      const response = {
        ok: true,
      ...
      if (result.answer) {
        _consumeLiveGate(result.card.gate_id);
  ...
  server.tool(
    ...
    async ({ gate_id, chosen, verdict }, extra) => {
      const live = _consumeLiveGate(gate_id);
      if (!live) {
