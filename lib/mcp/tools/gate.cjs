...
// rung; everything else gets the headless structured-text rung -- Claude
// Code declares `elicitation: {}` as of build 2.1.280 (live wire tee,
// 2026-09-23; re-confirmed at 2.1.281, 2026-09-24, 267-TRIPOLAR-PROBES.md),
// so rung (a) is the live CLI gate path by navigator ruling (CTX 2026-09-23,
// "let elicitation take over on CLI"); Desktop and Cowork are unprobed as of
// 2026-09-24 (267-TRIPOLAR-PROBES.md, Task 3 pending); a 2026-07-28-pinned
// connection still returns undefined from getClientCapabilities(), falling
// to rung (b)/(c); the ladder keeps all three rungs regardless).
//
 ...
 * getClientCapabilities()). Claude Code declares it as of build 2.1.280
 * (live wire tee, 2026-09-23; re-confirmed at 2.1.281, 2026-09-24,
 * 267-TRIPOLAR-PROBES.md) -- rung (a), the inline elicitInput round trip, is
 * the live CLI gate path by navigator ruling (CTX 2026-09-23, "let
 * elicitation take over on CLI"). Desktop and Cowork are unprobed as of
 * 2026-09-24 (267-TRIPOLAR-PROBES.md, Task 3 pending); a 2026-07-28-pinned
 * connection still returns undefined from getClientCapabilities(), falling
 * to rung (b)/(c). CLAUDE_HOST_SURFACES still identifies Claude hosts
 * (cli/desktop/cowork) for rung (b) via the D-07 MINDRIAN_MCP_FIRST surface
 * list on ctx.surface. Any other caller (VS Code, MCP Inspector, an
 * unrecognized surface) falls to the headless text rung.
 */
    ...
    'gate_render',
    {
      ...
      inputSchema: z.object({
        ...
        subject_node_id: z.string().min(1).optional()
          .describe("Opaque LOCAL room-graph node id, the card's subject. On an approve verdict for a kind 'general' card whose subject is a proposed claim node, gate_answer promotes that claim to confirmed through navigation.confirmNode; evidence nodes are never promoted. Canon Part 8: must never be a Brain identifier or room content."),
