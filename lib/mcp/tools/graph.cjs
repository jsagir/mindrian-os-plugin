...
//
// D-07: graph_write / memory_event CHANGE behavior (they write), so their
// registration itself is gated behind isMcpFirst(ctx.surface) -- when the flag
// is off for this surface, those two tools are never even offered to the
// client (Task 1's "guard the new-tool registration behind isMcpFirst"
// instruction, literally). graph_query is read-only and always registered.
//
...
module.exports = {
  ...
  graphWrite,
  memoryEvent,
  _internal: { resolveSessionRoomDir },
};
