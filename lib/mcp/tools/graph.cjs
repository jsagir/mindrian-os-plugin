...
// longer imported here; the read path never consulted it directly, and the
// former resolveSessionRoomDir independent copy (which did) is retired.
//
  ...
  server.tool(
    ...
    'Write a typed edge between two graph nodes. Routes EXCLUSIVELY through navigation.cjs (the single Part 9 chokepoint) -- never opens the graph store directly.',
    {
      ...
      read_version: z.number().nullable().optional()
        .describe('Optional CAS token (last_modified_at) from a prior read of source_id; a lost update is rejected as a conflict instead of silently clobbering. This check fails open (proceeds as a normal write, no conflict reported) when source_id\'s node is missing, or when the guard read itself errors; a pass is not proof the node existed or was unchanged.'),
    },
