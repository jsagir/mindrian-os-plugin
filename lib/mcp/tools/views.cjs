...
// Phase 354-05 (SYS-01, CTX-SYMLINK): the write boundary is enforced by
// realpath at the moment of the write, through the same helper
// safeResolveSection now inherits (assertRealpathContained) plus the
// atomic contained write that never follows a symlink leaf.
const { assertRealpathContained, writeFileContained } = require('../../core/room-path-containment.cjs');
...
// Phase 353 Plan 02 Task 7 (D-353-8): best-effort read of the room's
// current stated job (Phase 345's persisted goal/jtbd, NOT a Brain call --
// pure local JSON at .mindrian/jtbd-state.json). Used ONLY as the filing
// gate's `servesJtbd` signal; a missing or unreadable file degrades to
// `null` (no known job to compare, so the gate defaults to 'match' rather
// than manufacturing a disclosure out of missing data).
...
function fileArtifact(db, roomDir, params) {
  ...
  // before any byte, memory_event or node is written -- never falls through
  // to the fs.mkdirSync/write below.
  let scopeDir;
  try {
    scopeDir = safeResolveSection(roomDir, p.section || null);
  } catch (e) {
    if (e && e.code === 'ROOM_PATH_ESCAPE') {
      return { ok: false, reason: 'path_escape', section: p.section || null };
    }
    return { ok: false, reason: 'section_traversal', section: p.section || null };
  }
  ...
  // Phase 353 Plan 02 Task 7 (D-353-8): the filing gate. Runs AFTER the
  // section is known (p.section, already validated at the MCP schema
  // boundary by SECTION_RE) and BEFORE any byte is written. Mode resolution
  // order (section-gate.cjs header): an explicit mode on the call if one
  // existed (none does yet), else the room's own setting (none exists yet),
  // else 'flag' -- the shipped default.
  let servesJtbd = null;
  try {
  ...
  const gateMode = 'flag';
  const gateResult = navigation.evaluateFilingGate({ section: p.section || null, servesJtbd: servesJtbd, mode: gateMode });
  ...
  // TOCTOU re-assert (T-354-10): mkdirSync can itself traverse a symlink
  // created between the check above and this line. Re-assert before the
  // write ever touches disk.
  assertRealpathContained(roomDir, scopeDir, 'section ' + (p.section || null));
  const filePath = path.join(scopeDir, filename);
  writeFileContained(roomDir, filePath, typeof p.content === 'string' ? p.content : '');
  ...
  // Phase 353 Plan 02 Task 7 (D-353-8, R-353-K): a job mismatch under
  // 'flag' rides this SAME EXISTING mcp_client_event_logged call (never a
  // second event) with four added properties. No new EVENT_TYPES member.
  const eventProps = {
    label: 'artifact_file',
  ...
  if (gateResult.verdict === 'mismatch') {
    eventProps.job_mismatch = true;
    eventProps.declared_job = gateResult.declared_job;
    eventProps.section_job = gateResult.section_job;
    eventProps.reason = gateResult.reason;
  }
  const logResult = navigation.logMemoryEvent(db, 'mcp_client_event_logged', eventProps);
  ...
  // Phase 353 Plan 02 Task 7 (D-353-8, R-353-I/J): mint-then-edge anchor,
  // added alongside the existing reasoning-node write, never replacing it.
  let anchorEdge = null;
  if (artifactId) {
    try {
      ...
      if (nodeId) {
        ...
        // Canon Part 9 floor: bookkeeping never blocks a write. A failed
        // mint or edge never touches reasoningNode.ok; the write already
        // succeeded. gateResult.section_job is the anchor target (the
        // SECTION's own declared job, independent of match/mismatch): every
        // successful write into a resolved section anchors, regardless of
        // whether the write's own declared_job happened to match it.
...
function register(server, ctx) {
  ...
  server.tool(
    'artifact_file',
    "File a markdown artifact into this session's bound room. Resolves the section via the shipped SECTION_RE/safeResolveSection path-traversal guard, now also enforcing realpath containment so an existing symlinked section, destination or parent whose realpath leaves the room is refused and the final write never follows a symlink leaf (atomic replace-in-place instead); injects a stable artifact_id (lib/core/artifact-id.cjs), and logs the filing through navigation.cjs's logMemoryEvent (the ONLY door) -- never opens the graph store directly. Also writes a typed claim node (epistemic_type defaults to 'conclusion') plus SOURCED_FROM provenance edges to evidence_node_ids, through the SAME shared writer gate_answer's approve branch uses.",
    {
