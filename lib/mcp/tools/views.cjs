...
//
// D-07: artifact_file WRITES (it changes room state), so its registration is
// flag-gated behind isMcpFirst(ctx.surface) -- same discipline as graph_write/
// memory_event (198-04 Task 3). view_compile is read-only and always
// registered.
//
...
function register(server, ctx) {
  ...
  // artifact_file -- WRITE. D-07: registration itself is flag-gated (never
  // offered to the client when the flag is off for this surface), mirroring
  // graph_write/memory_event (198-04 Task 3).
