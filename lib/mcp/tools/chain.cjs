...
/**
 * _buildMaterialStepCard(step, nodeCtx) -> the gate superset card composed
 * for a halted material step. Options' ids ARE the gate_answer verdict
 * vocabulary (approve/reject/defer) so renderGate's verdictFor identity-maps
 * the chosen option straight to the verdict -- no extra translation layer.
 *
 * quick task 260903-h27: `nodeCtx` is an optional second parameter carrying
 * caller-supplied provenance ({ subjectNodeId, evidenceNodeIds }) threaded
 * from the halt site below. Absent or malformed nodeCtx defaults the card's
 * subject_node_id to null and evidence_node_ids to [] -- normalizeCard
 * (gate-render.cjs) is what actually enforces those defaults/degradation;
 * this function only forwards what it was given.
 */
function _buildMaterialStepCard(step, nodeCtx) {
  const label = (step && (step.command || step.framework)) || ('step ' + (step && step.step));
  const ctx = (nodeCtx && typeof nodeCtx === 'object') ? nodeCtx : {};
  return {
    ...
    subject_node_id: (typeof ctx.subjectNodeId === 'string' && ctx.subjectNodeId.length > 0) ? ctx.subjectNodeId : null,
    evidence_node_ids: Array.isArray(ctx.evidenceNodeIds) ? ctx.evidenceNodeIds : [],
  };
}

// ---------------------------------------------------------------------------
// SHARED-07 (Phase 347-08, 347-RESEARCH.md Pitfall 7): the resume remainder,
// resolved by step id through the SAME chainExecutor.resolveSuccessor the
// loop uses -- not a second successor authority, just the one the loop
// already has, called from the one file away it used to be forgotten in.
//
// The old `list.slice()` arithmetic on a reference-found index (chain.cjs
// :321-322, pre-347-08: `idx + 1` fed straight into `.slice`) was the second
// linear assumption in this system: it assumed a halted chain's remainder is
// "everything after this array position," which stopped being true the
// moment 347-07 taught composeWorkflow/chain-executor.cjs to branch
// (on_pass/on_fail/fan_out). It was also reference-equality based
// (`findIndex((s) => s === haltedStep)`), which silently resolves to the
// FIRST occurrence when the same step object appears more than once in the
// list (the fan-out engine's own synthetic cell trace entries,
// `{step: fan_out_step_id, fan_out_of: coordinator_id}`, are never `===` any
// `list` member -- they carry the SAME declared id, never the SAME
// reference). 347-07 and 347-08 landed as a pair for exactly this reason:
// teaching the loop to branch without teaching the resume path the same
// routing would make a halted branch resume the wrong remainder, silently
// (T-347-08-01).
// ---------------------------------------------------------------------------

// Local id/index lookup mirroring chain-executor.cjs's own _stepIdOf /
// _stepIndexById (347-07) exactly. NOT a second successor authority -- these
// two helpers only materialize the ids chainExecutor.resolveSuccessor names;
// they never decide WHAT the successor is.
...
/**
 * _findHaltedIndex(list, haltedStep) -> the halted step's position in
 * `list`, or -1 when it cannot be found by either identity.
 *
 * Reference identity is tried FIRST (the common case: `haltedStep` IS the
 * exact `list` element the loop halted on -- the precise, unambiguous
 * signal when it resolves). Declared-id identity is the FALLBACK, reached
 * only when the reference is absent from `list` -- e.g. a fan-out cell's
 * synthetic step object, which names an existing declared step id but is
 * never the same object instance as that step's own `list` entry.
 */
function _findHaltedIndex(list, haltedStep) {
  const byRef = list.findIndex(function (s) { return s === haltedStep; });
  if (byRef >= 0) return byRef;
  return _stepIndexById(list, _stepIdOf(haltedStep));
}

/**
 * _buildResumeRemainder(list, haltedStep) -> { restSteps, reason, haltedIndex }.
 *
 * `reason` is non-null ONLY when the halted step could not be located in
 * `list` at all (neither by reference nor by declared id) -- preserves
 * today's `idx >= 0` guard exactly (T-347-08-04): an empty remainder, a
 * named reason, never a throw.
 *
 * `haltedIndex` (CR-02, 347 code review) is the halted step's own position
 * in `list` (the SAME value `_findHaltedIndex` resolves internally, exposed
 * rather than recomputed by the caller), or -1 when `reason` names
 * 'halted_step_not_found'. `chainRun` uses it to anchor the halted step's
 * own chain_state record and to compute the continuation's absolute
 * step_index offset -- both relative to `list`'s own numbering, which the
 * caller must add its own already-in-effect offset to for a nested resume.
 *
 * Two remainder shapes, chosen by the halted step's own declaration:
 *
 *   1. `fan_out` declared: resolveSuccessor has no fan-out opinion (it only
 *      knows on_pass / on_fail / index+1 -- chain-executor.cjs dispatches
 *      fan-out as its own routing shape immediately after the coordinator's
 *      trace entry, never through resolveSuccessor). The remainder IS the
 *      declared fan-out targets, in declared order, looked up by id.
 *
 *   2. Otherwise: a forward walk through chainExecutor.resolveSuccessor,
 *      one hop at a time, collecting the ordered step objects until it
 *      returns null or a step id repeats (T-347-08-03: the walk is bounded
 *      by a visited-id set, so a cyclic route can never build an unbounded
 *      remainder at resume time -- the loop's own EXEC-06 maxSteps brake is
 *      what actually stops a live back-edge; this guard only stops the
 *      RESUME-TIME PREVIEW walk from looping forever).
 *
 * `verdict` is passed as `null` on every hop: the halted step has not (yet)
 * executed at halt time (gate_halt / forced_material / budget_brake never
 * call onStep before halting -- see chain-executor.cjs's own halt sites),
 * so there is no captured quality to feed resolveSuccessor. `null` takes
 * the SAME non-failed branch a real 'high'/'medium' quality would
 * (_verdictFailed only treats the literal 'low' string, or a verdict object
 * naming it, as failed) -- a declared on_pass wins, otherwise the plain
 * `index + 1` floor. For the one halt reason where the step DID already
 * execute (quality_early_stop), 347-07 already guarantees `on_fail` is
 * absent whenever that halt fires (the loop defers to the back-edge instead
 * of halting when on_fail IS declared), so resolveSuccessor's failed branch
 * would fall through to the exact same `index + 1` default `null` produces
 * here -- passing `null` uniformly is provably equivalent, not a shortcut.
 */
function _buildResumeRemainder(list, haltedStep) {
  const haltedIndex = _findHaltedIndex(list, haltedStep);
  if (haltedIndex < 0) {
    return { restSteps: [], reason: 'halted_step_not_found', haltedIndex: -1 };
  }

  if (Array.isArray(haltedStep && haltedStep.fan_out) && haltedStep.fan_out.length > 0) {
    const fanOutRemainder = [];
    haltedStep.fan_out.forEach(function (targetId) {
      const idx = _stepIndexById(list, targetId);
      if (idx >= 0) fanOutRemainder.push(list[idx]);
    });
    return { restSteps: fanOutRemainder, reason: null, haltedIndex: haltedIndex };
  }

  const restSteps = [];
  const visited = new Set([_stepIdOf(list[haltedIndex])]);
  let currentIndex = haltedIndex;
  for (;;) {
    const nextId = chainExecutor.resolveSuccessor(list[currentIndex], null, list, currentIndex);
    if (nextId === null || nextId === undefined) break;
    if (visited.has(nextId)) break; // T-347-08-03: bounded walk, never unbounded on a cycle.
    const nextIndex = _stepIndexById(list, nextId);
    if (nextIndex < 0) break;
    visited.add(nextId);
    restSteps.push(list[nextIndex]);
    currentIndex = nextIndex;
  }
  return { restSteps: restSteps, reason: null, haltedIndex: haltedIndex };
}
 ...
 *   runId?: string, stepIndexOffset?: number,
 * }} opts
 */
async function chainRun(steps, opts) {
  ...
  // CR-01 (347 code review): mint (or accept, on a resumed continuation) the
  // ONE run_id for this invocation BEFORE the dispatcher closure is built,
  // and hand the SAME value to both the dispatcher (the read side of
  // SHARED-04's predecessor lookup) and chain-executor.cjs's own runChain
  // call (the write side, chain-executor.cjs:955/:1289 CR-01 fix) below --
  // this is the wiring chain-step-dispatcher.cjs's own doc comment names as
  // "the phase 347-05 seam ... a later plan's scope" that no later plan ever
  // closed. o.runId lets a resumed continuation (_executeResumedEntry below)
  // reuse the ORIGINAL halted run's id instead of starting a new one
  // (CR-02): every step of one logical chain -- pre-halt, the approved gate
  // step, and the post-halt remainder -- lands under one shared run_id.
  const runId = (typeof o.runId === 'string' && o.runId.length > 0)
    ? o.runId
    : ('run:' + Date.now() + ':' + crypto.randomBytes(4).toString('hex'));
  // CR-02: the absolute step_index a resumed continuation's OWN i=0 maps to,
  // so chain-executor.cjs's write loop continues the ORIGINAL chain's
  // numbering instead of restarting at 0. 0 on a fresh (non-resumed) START
  // call -- byte-identical write behavior to before this fix.
  const stepIndexOffset = (typeof o.stepIndexOffset === 'number' && o.stepIndexOffset >= 0)
    ? Math.floor(o.stepIndexOffset) : 0;
  const onStepFn = (typeof o.onStep === 'function') ? o.onStep : makeChainStepDispatcher(roomDir, { sessionId: o.sessionId, targetSection: (typeof o.targetSection === 'string' && o.targetSection.length > 0) ? o.targetSection : null, runId: runId });
  ...
  const result = await chainExecutor.runChain(list, {
    ...
    roomDir: roomDir,
    // CR-01 / CR-02: the SAME run_id the dispatcher closure above reads from,
    // and the absolute step_index offset for a resumed continuation.
    runId: runId,
  ...
  const remainder = _buildResumeRemainder(list, haltedStep);
  const restSteps = remainder.restSteps;
  // CR-02: `remainder.haltedIndex` is relative to THIS call's own `list`; add
  // this call's own `stepIndexOffset` (0 on a fresh START, the prior halt's
  // continuation offset on a resumed one) to get the ABSOLUTE step_index the
  // halted step occupies in the original chain's numbering. null when the
  // halted step could not be located at all (remainder.reason names why) --
  // _executeResumedEntry below skips the approved-step record write rather
  // than anchoring it at a fabricated index.
  const absoluteHaltedIndex = (typeof remainder.haltedIndex === 'number' && remainder.haltedIndex >= 0)
    ? (stepIndexOffset + remainder.haltedIndex)
    : null;
  const previousOutput = result.trace.length > 0 ? result.trace[result.trace.length - 1].chain_output : null;

  const card = _buildMaterialStepCard(haltedStep, { subjectNodeId: o.subjectNodeId, evidenceNodeIds: o.evidenceNodeIds });
  ...
  const resumeEntry = {
    ...
    // CR-01 / CR-02 (347 code review): the ONE run_id this whole logical
    // chain shares, and the halted step's own ABSOLUTE step_index (null when
    // unresolvable). _executeResumedEntry uses `runId` to write the approved
    // step's own chain_state record and to re-thread the continuation's
    // opts.runId (so the remainder's records land under the SAME run rather
    // than an unrelated newly-minted one), and `haltedIndex` both to anchor
    // that record and to compute the continuation's opts.stepIndexOffset.
    runId: runId,
    haltedIndex: absoluteHaltedIndex,
    // quick task 260903-h27: carried alongside gateRenderCtx and re-passed
    // into the recursive chainRun opts at the tail-continuation call below
    // (_executeResumedEntry). Without this, a SECOND material halt later in
    // the same chain would silently render a card with empty provenance
    // even though the first halt had it -- the caller-supplied node ids
    // would only ever reach the FIRST gate, not any that follow.
    subjectNodeId: o.subjectNodeId,
    evidenceNodeIds: o.evidenceNodeIds,
...
async function _executeResumedEntry(entry, gateAnswer) {
  ...
  // CR-02 (347 code review): the one chain_state record neither runChain
  // loop's own write call site ever reaches -- this direct onStepFn call sits
  // OUTSIDE both loops, so the approved material step (the one step every
  // halted chain stops on) would otherwise get NO anchored record at all.
  // Written under the SAME run_id the pre-halt prefix used, at the halted
  // step's own ABSOLUTE step_index, so readChainState's FEEDS_INTO walk finds
  // it in between the prefix and the remainder rather than a hole. Best-
  // effort (writeApprovedStepRecord degrades to null on any refusal/fault,
  // exactly like the loop's own writer) -- a failed or unanchorable write
  // never blocks the approved step from executing or the chain from
  // continuing (T-347-04-01's own discipline, unchanged).
  if (typeof entry.runId === 'string' && entry.runId.length > 0
    && typeof entry.haltedIndex === 'number' && entry.haltedIndex >= 0) {
    chainExecutor.writeApprovedStepRecord({
      roomDir: entry.roomDir,
      runId: entry.runId,
      stepIndex: entry.haltedIndex,
      step: entry.haltedStep,
      chainOutput: chainOutput,
      quality: (typeof stepResult.quality === 'string') ? stepResult.quality : null,
      subjectNodeId: entry.subjectNodeId,
    });
  }
  ...
  const rest = await chainRun(entry.restSteps, {
    ...
    // quick task 260903-h27: re-thread the caller-supplied node ids onto the
    // tail continuation, or a second material halt later in this same chain
    // would silently lose them (see the doctrine comment on resumeEntry
    // above, in chainRun).
    subjectNodeId: entry.subjectNodeId,
    evidenceNodeIds: entry.evidenceNodeIds,
    // CR-02: reuse the SAME run_id (never re-mint one for the remainder), and
    // continue the ABSOLUTE step numbering one past the approved gate step --
    // so the remainder's own chain_state records land contiguously after it
    // under the one shared run, instead of fragmenting across an unrelated
    // new run restarting at step_index 0.
    runId: entry.runId,
    stepIndexOffset: (typeof entry.haltedIndex === 'number' && entry.haltedIndex >= 0) ? (entry.haltedIndex + 1) : undefined,
  });
...
function register(server, ctx) {
  server.registerTool(
    ...
    {
      title: 'Chain Resolve',
      description: "Resolve an ordered framework-name chain to its command plan via lib/workflow/command-resolver.cjs::composeWorkflow -- the ONE governed reach path (R4: mints no second resolver). Read-only: executes nothing.",
    ...
    'chain_run',
    {
      ...
      inputSchema: z.object({
        ...
        subject_node_id: z.string().min(1).optional()
          .describe('Opaque LOCAL room-graph node id, provenance only. Carried onto the halt card if this chain halts at a material step; nothing consumes it yet. Not used on the gate_answer resume branch (that path consumes a ledger entry that already carries it). Canon Part 8: must never be a Brain identifier or room content.'),
        evidence_node_ids: z.array(z.string().min(1)).optional()
          .describe('Opaque LOCAL room-graph node ids, provenance only. Carried onto the halt card if this chain halts at a material step; nothing consumes it yet. Not used on the gate_answer resume branch (that path consumes a ledger entry that already carries it). Canon Part 8: must never be Brain identifiers or room content.'),
      }),
    },
    async ({ chain, gate_answer, subject_node_id, evidence_node_ids }, extra) => {
      ...
      const workflow = chainResolve(chain);
      const result = await chainRun(workflow, { roomDir: roomDir, sessionId: sessionId, gateRenderCtx: renderCtx, subjectNodeId: subject_node_id, evidenceNodeIds: evidence_node_ids });
      return textResponse(result, result.ok === false);
    }
  );
}
...
module.exports = {
  ...
  _internal: {
    ...
    // quick task 260903-h27: exported so the T2 gate-card node-fields test
    // can drive the card builder directly (no-nodeCtx defaults vs. a
    // populated nodeCtx) without standing up a full chain halt.
    _buildMaterialStepCard: _buildMaterialStepCard,
    ...
    // Phase 347-08 (SHARED-07): exported so
    // tests/test-347-resume-nonlinear.cjs can drive the id-based remainder
    // computation directly (linear floor, on_pass, fan_out, not-found,
    // duplicate-object fallback) without standing up a full chain halt for
    // every case.
    _buildResumeRemainder: _buildResumeRemainder,
  },
};
