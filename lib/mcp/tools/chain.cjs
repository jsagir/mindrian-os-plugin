...
/**
 * _buildResumeRemainder(list, haltedStep) -> { restSteps, reason }.
 *
...
function _buildResumeRemainder(list, haltedStep) {
  ...
  if (haltedIndex < 0) {
    return { restSteps: [], reason: 'halted_step_not_found' };
  }
  ...
  if (Array.isArray(haltedStep && haltedStep.fan_out) && haltedStep.fan_out.length > 0) {
    ...
    return { restSteps: fanOutRemainder, reason: null };
  }
  ...
  return { restSteps: restSteps, reason: null };
}
