...
function buildStopGateCard(turn) {
  // Phase 359-03 (D-10, N-3, Finding 5): a DECLARED turn's card is built from
  // Larry's own declared labels (moonshot last, per N-3), not re-extracted
  // from bracket-shaped text -- a pure-prose declaration carries no `[n]`
  // markers for rawExtractOptionLabels to find.
  const labels = (turn && turn.fork_declared === true && Array.isArray(turn.declared_labels) && turn.declared_labels.length > 0)
    ? turn.declared_labels.slice()
    : rawExtractOptionLabels(turn && turn.output_text);
  const options = labels.length > 0
    ? labels.map((l) => ({ label: l }))
    : [{ label: 'Continue' }, { label: 'Stop and clarify' }];
  // quick task 260903-h27: NO functional change here, by confirmed design.
  // A Stop event carries no node context at all -- this card is composed
  // purely from rawExtractOptionLabels(turn.output_text) above, the
  // recovered option labels of a turn that ALREADY HAPPENED, and no caller
  // of handleStopEvent supplies a node id anywhere in this file. So
  // subjectNodeId / evidenceNodeIds deliberately stay at normalizeCard's own
  // defaults (null / []) on this card. This is a confirmed absence,
  // established by reading this function, not an oversight -- a future
  // reader should not go inventing a source for them here.
  return {
    kind: 'stop',
...
// _safeDeclaredIdentity -- Phase 359-03 (D-10, Finding 5): the same guarded-
// call pattern as _safeCtxHash above, for the SAME reason (a version skew in
// which check-card-fire.cjs does not export declaredIdentity -- for example
// GATE357-08's own mutation leg, which reverts scripts/check-card-fire.cjs
// to its pre-357 bytes while this file stays at HEAD). Unwrapped, that would
// throw inside handleStopEvent's dedup step and mask the real verdict behind
// 'handler-error', which is exactly the parity break the mutation gate
// exists to catch. Degrades to '' (declaredIdentity's own not-declared
// value), never throws.
function _safeDeclaredIdentity(turn) {
  try {
    return typeof checkCardFire.declaredIdentity === 'function' ? checkCardFire.declaredIdentity(turn) : '';
  } catch (_e) {
    return '';
  }
}
...
async function handleStopEvent(sessionId, stopContext) {
  try {
    ...
    // Phase 359-03 (D-10, N-3, Finding 5): a pure-prose declared fork carries
    // no glyph and no bracket labels, so gate_signature is '' -- without this
    // fallback every declared fork in a session would share the SAME dedup
    // key and only the first would ever fire. declaredIdentity(turn) is ''
    // for a non-declared turn, so this is additive only.
    const gateContext = {
      gate: 'stop',
      sid: sessionId,
      subject: turn.gate_signature || _safeDeclaredIdentity(turn) || '',
      material: material,
    };
