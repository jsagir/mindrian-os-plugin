 ...
 * @param {string|undefined} sessionId
 * @returns {{room_dir: string|null, sections: number, stale: number, minto_pending: number}}
 */
function closeOutRoom(roomDir, sessionId) {
  if (!roomDir || typeof roomDir !== 'string' || !fs.existsSync(roomDir)) {
    return { room_dir: null, sections: 0, stale: 0, minto_pending: 0 };
  }
  ...
  return { room_dir: roomDir, sections: snap.sections, stale: snap.stale, minto_pending: mintoPending };
}
