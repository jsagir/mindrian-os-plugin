...
// ---------------------------------------------------------------------------
// 354-17 (THEO-01 recall extension): the pre-vetted, offline-checkable,
// Jev-scored framework->command ledger (scripts/build-framework-command-
// ledger.cjs -> data/framework-command-ledger.json), consulted ONLY after
// the exact KNOWN_METHODOLOGIES slug check below (354-09) finds nothing --
// never a live runtime guess, never a widening of that exact-match safety
// property. Read with zero vendor calls.
//
// HONESTY: the committed default ledger is offline-seed (confidence_floor:
// null). _lookupLedgerCommand's own rule below (source !== 'offline-seed'
// required whenever confidence_floor is null) means that shipped ledger
// promotes ZERO candidates -- by design, until a navigator runs a
// jev-scored build pre-release.
// ---------------------------------------------------------------------------

/**
 * Resolve the ledger path. `MINDRIAN_FRAMEWORK_LEDGER_PATH` is a test-only
 * override (never read by production code paths otherwise) so regression
 * tests can point at a fixture-built ledger without ever touching the
 * shared, committed data/framework-command-ledger.json on disk -- this
 * matters because multiple concurrent sessions can be reading/writing this
 * repo at once.
 * @returns {string}
 */
function _ledgerPath() {
  return process.env.MINDRIAN_FRAMEWORK_LEDGER_PATH
    ? path.resolve(process.env.MINDRIAN_FRAMEWORK_LEDGER_PATH)
    : path.resolve(__dirname, '../../data/framework-command-ledger.json');
}

let _ledgerCacheValue = null;
let _ledgerCachePath = null;

/**
 * Lazy, fail-closed ledger read. Loaded once via fs.readFileSync + JSON.parse;
 * any failure (missing file, malformed JSON) returns `{ rows: {} }` rather
 * than throwing into brainRoute -- an absent or broken ledger degrades to
 * "recovers nothing", never a crash.
 * @returns {{ rows: object, confidence_floor?: (number|null) }}
 */
function _ledgerCache() {
  const p = _ledgerPath();
  if (_ledgerCacheValue && _ledgerCachePath === p) return _ledgerCacheValue;
  try {
    const raw = fs.readFileSync(p, 'utf8');
    _ledgerCacheValue = JSON.parse(raw);
  } catch (_e) {
    _ledgerCacheValue = { rows: {} };
  }
  _ledgerCachePath = p;
  return _ledgerCacheValue;
}

/**
 * Look up a Theo framework label in the pre-scored ledger.
 *
 * Returns the highest-score row whose `score` is at or above the ledger's
 * own shipped `confidence_floor`. When `confidence_floor` is null (e.g. an
 * offline-seed ledger, uncalibrated), a row may only ever match when its
 * `source !== 'offline-seed'` -- an uncalibrated fallback ledger must never
 * silently promote a candidate.
 *
 * Defense in depth (T-354-22): a returned row's `command_id` is checked
 * AGAIN against the live `KNOWN_METHODOLOGIES` export here, even though
 * the builder already scoped it at build time -- a stale or hand-edited
 * ledger can never hand back a command id this module would not itself
 * certify.
 *
 * @param {string} frameworkLabel
 * @returns {{ command_id: string, score?: number, confidence?: (number|null), source?: string }|null}
 */
function _lookupLedgerCommand(frameworkLabel) {
  if (typeof frameworkLabel !== 'string' || frameworkLabel.length === 0) return null;
  const ledger = _ledgerCache();
  const rows = (ledger && typeof ledger.rows === 'object' && ledger.rows) || {};

  let candidateRows = rows[frameworkLabel];
  if (!Array.isArray(candidateRows)) {
    // Theo aliasing can vary casing -- try a case-insensitive exact match
    // over the ledger's own keys before giving up.
    const lower = frameworkLabel.toLowerCase();
    const foundKey = Object.keys(rows).find((k) => k.toLowerCase() === lower);
    candidateRows = foundKey ? rows[foundKey] : null;
  }
  if (!Array.isArray(candidateRows) || candidateRows.length === 0) return null;

  const floor = (typeof ledger.confidence_floor === 'number') ? ledger.confidence_floor : null;
  let best = null;
  for (const row of candidateRows) {
    if (!row || typeof row.command_id !== 'string') continue;
    if (!KNOWN_METHODOLOGIES.includes(row.command_id)) continue; // defense in depth
    const qualifies = (floor === null)
      ? row.source !== 'offline-seed'
      : (typeof row.score === 'number' && row.score >= floor);
    if (!qualifies) continue;
    if (!best || (typeof row.score === 'number' && row.score > (typeof best.score === 'number' ? best.score : -Infinity))) {
      best = row;
    }
  }
  return best;
...
async function brainRoute(roomDir, stateContent, intent) {
  ...
  // 354-17 (THEO-01 recall extension): additive tracking alongside the
  // 354-09 gather above -- `candidateFrameworks` remembers which framework
  // FIRST produced a given raw slug (first-seen wins, matching
  // `commandCandidates`' own dedup), and `emptyCommandFrameworks` names any
  // framework whose option carried NO command-slug candidate at all (Theo
  // has no USES_FRAMEWORK edge for it -- the realistic "nothing to fall
  // back on" shape this plan exists to recover). Neither array changes what
  // `commandCandidates` or `frameworks` themselves contain.
  const commandCandidates = [];
  const candidateFrameworks = new Map();
  const emptyCommandFrameworks = [];
  const frameworks = [];
  if (anchorFramework && typeof anchorFramework === 'string' && !frameworks.includes(anchorFramework)) {
    frameworks.push(anchorFramework);
  }
  for (const opt of options) {
    const frameworkLabel = (opt && typeof opt.framework === 'string' && opt.framework.length > 0) ? opt.framework : null;
    let hadSlug = false;
    if (opt && Array.isArray(opt.commands)) {
      for (const slug of opt.commands) {
        if (typeof slug === 'string' && slug.length > 0) {
          hadSlug = true;
          if (!commandCandidates.includes(slug)) {
            commandCandidates.push(slug);
            if (frameworkLabel && !candidateFrameworks.has(slug)) candidateFrameworks.set(slug, frameworkLabel);
          }
        }
      }
    }
    if (!hadSlug && frameworkLabel && !emptyCommandFrameworks.includes(frameworkLabel)) {
      emptyCommandFrameworks.push(frameworkLabel);
    }
    if (frameworkLabel && !frameworks.includes(frameworkLabel)) {
      frameworks.push(frameworkLabel);
    }
  }

  const rejectedCandidates = [];
  const chain = [];
  let ledgerAssisted = false;
  for (const raw of commandCandidates) {
    ...
    // 354-17: the exact-slug check just failed for this candidate --
    // consult the pre-vetted ledger for the framework that produced it
    // BEFORE rejecting (never instead of the exact check above, never
    // widening it).
    const framework = candidateFrameworks.get(raw) || null;
    const ledgerRow = framework ? _lookupLedgerCommand(framework) : null;
    if (ledgerRow) {
      if (!chain.includes(ledgerRow.command_id)) chain.push(ledgerRow.command_id);
      ledgerAssisted = true;
    } else if (rejectedCandidates.length < 10) {
      rejectedCandidates.push(raw);
    }
  }
  // 354-17: a framework whose option produced NO command-slug candidate at
  // all never entered commandCandidates above -- there was no exact-slug
  // check to fail. Treat it the same way: consult the ledger by framework
  // label; on a miss it still lands in rejected_candidates exactly as any
  // other unrecovered candidate would.
  for (const framework of emptyCommandFrameworks) {
    const ledgerRow = _lookupLedgerCommand(framework);
    if (ledgerRow) {
      if (!chain.includes(ledgerRow.command_id)) chain.push(ledgerRow.command_id);
      ledgerAssisted = true;
    } else if (rejectedCandidates.length < 10) {
      rejectedCandidates.push(framework);
    }
  }
  ...
  // 354-17 (THEO-01 recall extension): additive provenance fields only --
  // no existing field renamed or removed. `ledger_source` is present only
  // when `ledger_assisted` is true.
  const provenance = {
    provider: 'brain',
    ...
    ledger_assisted: ledgerAssisted,
  };
  if (ledgerAssisted) provenance.ledger_source = 'framework-command-ledger';
