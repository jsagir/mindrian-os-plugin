...
async function brainRoute(roomDir, stateContent, intent) {
  ...
  // A real chain came back despite the missing next_gate shape (e.g. built
  // from anchorFramework alone) -- this was not actually a miss, clear the
  // note so recommend() never discloses a non-event.
  _lastBrainRouteMissNote = null;
...
async function recommend(roomDir, options = {}) {
  ...
  // does not touch or widen). Covers the cause that block cannot see: the
  // Brain ANSWERED (isAvailable() stays true post-flip) but its response
  // carried no `next_gate` shape, so brainRoute() above returned null with
  // no readable reason. `_lastBrainRouteMissNote` was set (or left null) by
  // THIS call's own already-awaited brainRoute() invocation; reading it
  // here is synchronous, not a fresh async probe, so no extra await or
  // network reach happens. ADDITIVE field only: never replaces localRec's
  // shape, never a field any existing consumer reads today, so this cannot
  // regress anyone destructuring {chain, confidence, source, reasoning,
  // target_sections}. Best-effort; never blocks the return.
  try {
