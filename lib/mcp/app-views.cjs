...
function scanRoomData(roomDir) {
  ...
  const threadsFile = path.join(roomDir, '.lazygraph', 'threads.json');
  ...
  // Also check KuzuDB export
  const kuzuExport = path.join(roomDir, '.lazygraph', 'graph-export.json');
  if (fs.existsSync(kuzuExport)) {
    try {
      const data = JSON.parse(fs.readFileSync(kuzuExport, 'utf-8'));
      if (data.nodes) {
