...
function registerPrompts(server, roomDir, pluginRoot) {
  ...
  // -------------------------------------------------------------------------
  // 7-9. Runtime-loop prompts (2026-08-19) -- the hookless-surface protocol as
  // pickable entries in Desktop's prompt menu, so the CMD reflexes (session-
  // start binding, statusline, act) are one click instead of remembered prose.
  // -------------------------------------------------------------------------
  server.prompt(
    'bind-room',
    {
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text:
              'Run the session-start binding: call room_list, present the rooms as a selection (gate_render when available), and room_bind my choice. Confirm the binding and read back the room state (room_state_bound) in one short line. Do not write any room artifact before the binding is confirmed.',
          },
        },
      ],
    })
  ...
  server.prompt(
    'status',
    {
      description: 'One-line status: room, stage, health, and suggested next move (the statusline, in prose).',
      arguments: [],
    },
    async () => ({
      messages: [
        {
          ...
          content: {
            type: 'text',
            text:
              'Call status_read and room_state (suggest-next) and render ONE line: room name, venture stage, health glyph, and the single best next move. No preamble.',
          },
        },
      ],
    })
  ...
  server.prompt(
    'act',
    {
      description: "Run Larry's best-pick methodology for the current room state (resolve, gate, then chain_run the approved sequence).",
      arguments: [
        { name: 'goal', description: 'Optional goal or focus to steer the pick', required: false },
      ],
    },
    async (args) => ({
      messages: [
        {
          ...
          content: {
            type: 'text',
            text:
              `${args && args.goal ? `Goal: ${args.goal}\n\n` : ''}Act: call suggest_next for the current room, pick the best next methodology, present it as a decision gate (gate_render), and on approval resolve and execute it via chain_resolve -> chain_run -- letting chain_run auto-run the autonomous-safe prefix and halt at the first material gate. Never hand-execute chain steps chain_run can govern.`,
          },
        },
      ],
    })
