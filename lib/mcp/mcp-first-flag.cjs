...
// .planning/debug/resolved/mcp-write-path-disabled-on-cli-host.md): the
// sentence above claiming "On Claude Code that is harmless: slash commands
// and hooks do the writing" was the load-bearing false premise, and it was
// wrong. Hooks LOG events; they do not file artifacts or write graph edges on
// the model's behalf. A tool visible in tools/list that ALWAYS refuses is a
// worse failure mode than the tool never existing: it invites the model to
// try, get refused, and then take the ungoverned fallback of writing straight
// to disk itself, with no artifact_id injection and no memory_event journal
// row. That fallback was observed live in this repo's own history before the
// fix. Claude Code is now on the permitted side of this gate (see case 2 in
// isWritePathEnabled's JSDoc below): these three tools ARE the governed Part 9
// door, so refusing them on the home surface protected nothing.
//
 ...
 *      on every host. The hand-set flag ALWAYS wins over auto-detection. No
 *      regression for anyone already setting it.
 *   2. Claude Code -> true. These three tools ARE the governed Part 9 door:
 *      everything they do routes through navigation.cjs, so refusing them
 *      protected nothing and provoked an ungoverned bypass (2026-08-19
 *      amendment, see the header block above this function).
 *   3. Any other confidently-recognized Tier-0 host -> true (the original
 *      Gap D flip: Cursor, VS Code/Copilot, Goose, Zed, Cline, Continue,
 *      Windsurf, Devin).
 *   4. Everything else -> false. That is now exactly two populations: an
 *      UNKNOWN or pre-initialize client (an unidentified caller does not
 *      silently gain write access) and the other Tier-1 hosts (Grok Build,
 *      OpenCode: they have their own hook channel).
 *
 * Case 4 is NOT a silent skip. The tools it gates are now ALWAYS present in
...
function isWritePathEnabled(opts) {
  try {
    ...
    if (tier.host === 'claude-code') return true;
