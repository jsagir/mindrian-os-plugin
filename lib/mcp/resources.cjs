...
// every write-side site now uses. A discovered section name legitimately
// falls outside SECTION_RE's [a-z0-9-]+ charset (this handler does not
// apply that regex -- see the room-section handler below), so containment
// is enforced by realpath, not by a character-class check.
...
function registerResources(server, ctx) {
  ...
  server.resource(
    ...
    async (uri, params, extra) => {
      ...
      const sectionDir = path.resolve(roomDir, sectionName);

      // Phase 354-05 (SYS-01, CTX-SYMLINK): lexical containment first (a
      // discovered sectionName can legitimately fall outside SECTION_RE, so
      // this is the same startsWith idiom tool-router.cjs's lexical leg
      // uses, not that shared function itself), then realpath containment
      // -- an EXISTING section symlink (e.g. room/research -> an outside
      // directory) passes the lexical check but not this one.
      const lexicallyContained = sectionDir === path.resolve(roomDir) || sectionDir.startsWith(path.resolve(roomDir) + path.sep);
      if (!lexicallyContained || !isRealpathContained(roomDir, sectionDir)) {
        return {
          contents: [{
            uri: uri.href,
            mimeType: 'text/markdown',
            text: `Section "${sectionName}" is outside this Data Room and was not read.`,
          }],
        };
      }
      ...
      // A file symlink pointing outside the room is omitted, never read
      // (Rule 2 -- a listed name alone is not proof it is safe to open).
      files = files.filter((f) => isRealpathContained(roomDir, path.join(sectionDir, f)));
