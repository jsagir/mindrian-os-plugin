 ...
 * Single implementation of the `startsWith('---')` / `indexOf('\n---', 3)` /
 * `slice(end + 4)` idiom (previously inline in extractDisplayName). A document
 * with no frontmatter is returned unchanged; a malformed / unterminated block
 * degrades to returning the content as-is (never throws). Non-string input
 * yields ''.
 */
...
function extractDisplayName(profilePath, folderName) {
  ...
  // Scan post-frontmatter body for first H1 (single strip idiom, see helper)
