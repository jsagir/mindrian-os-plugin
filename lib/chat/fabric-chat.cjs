...
var GRAPH_SCHEMA = {
  ...
  edgeTypes: [
    ...
    { name: 'HSI_CONNECTION', from: 'Artifact', to: 'Artifact', properties: ['hsi_score REAL', 'lsa_sim REAL', 'semantic_sim REAL', 'surprise_type TEXT', 'breakthrough_potential REAL', 'tier TEXT'], description: 'Hidden Structural Insight -- a Surprise connection (Phase 355: structural_transfer = same meaning in different words, semantic_implementation = same words with different meaning; re-derive from lsa_sim / semantic_sim; strings stored before the room\'s next HSI run may use the retired convention)' },
    { name: 'REVERSE_SALIENT', from: 'Section', to: 'Section', properties: ['differential_score REAL', 'innovation_type TEXT', 'source_artifact TEXT', 'target_artifact TEXT', 'innovation_thesis TEXT'], description: 'Bottleneck -- where understanding lags behind ambition (Hughes 1983) (Phase 355: structural_transfer = same meaning in different words, semantic_implementation = same words with different meaning; innovation_type is honest only when the edge\'s own source is rs-engine, NULL otherwise; a non-rs-engine-sourced edge may carry the retired convention)' },
 ...
 * @param {object} conn - node:sqlite DatabaseSync object (from lazygraph-ops openGraph)
