...
var GRAPH_SCHEMA = {
  ...
  edgeTypes: [
    ...
    { name: 'CAUSES', from: 'Artifact', to: 'Artifact', properties: ['mechanism (STRING)', 'confidence (DOUBLE)', 'framework (STRING)', 'direction (STRING)'], description: 'One Entry directly causes or triggers effects in another' },
    { name: 'ROOT_CAUSE_OF', from: 'Artifact', to: 'Artifact', properties: ['chain_length (INT64)', 'intermediate_causes (STRING)', 'confidence (DOUBLE)', 'discovery_source (STRING)'], description: 'Traces backward to the root cause of a problem chain' }
  ],
  sampleQueries: [
    ...
    { question: 'What causes what in this room?', cypher: "MATCH (a:Artifact)-[r:CAUSES]->(b:Artifact) RETURN a.title AS cause, b.title AS effect, r.mechanism, r.confidence" },
    { question: 'What are the root causes of problems?', cypher: "MATCH (a:Artifact)-[r:ROOT_CAUSE_OF]->(b:Artifact) RETURN a.title AS root_cause, b.title AS symptom, r.chain_length, r.confidence" }
  ]
};
...
function buildSchemaDescription(schema) {
  ...
  lines.push('');
  lines.push('### Relationship Tables (14 Thread Types)');
