...
var GenerativeTools = (function() {
  ...
  // T-354-15: a color value that is not a strict hex color is dropped entirely
  // rather than passed through to a style attribute, so no CSS (e.g.
  // background-image:url(...)) can be injected through an insight_card item.
  ...
  function renderInsightCard(comp) {
    ...
    if (comp.items && comp.items.length) {
      for (var i = 0; i < comp.items.length; i++) {
        ...
        var itemColor = safeColor(item.color);
        var colorDot = itemColor
          ? '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + escapeHtml(itemColor) + ';margin-right:6px;vertical-align:middle;"></span>'
          : '';
