...
// detectNoActiveRoom's `reg.active` check was a Bucket B-reader of the racing
// global string; it now routes through resolveActiveRoom (D-03). This module
// is purely LOCAL (no fetch, no Brain, no network -- Canon Part 8); requiring
// it does not breach the HARD RULES above.
const sessionBinding = require('../core/session-binding.cjs');
...
/**
 * Phase 119-00 helper: return true iff there is no active room for this
 * session (no session-scoped binding AND no last_active/active fallback).
 *
 ...
 *
 * Phase 128.1-03b: previously read `reg.active` from registry.json directly --
 * a Bucket B-reader of the racing global string. It now routes through
 * session-binding.resolveActiveRoom so two concurrent sessions each see their
 * OWN active-room state (D-03). The resolver does the session-keyed lookup
 * plus the last_active/active fallback internally and never throws; an
 * unreadable / absent registry resolves to an empty room (fail-open to
 * auto-create, preserving the prior behavior). The `roomsHome` argument is
 * retained for signature stability but no longer drives resolution -- the
 * resolver reads the canonical registry from MINDRIAN_ROOMS_HOME itself.
 *
 * Pure file-read; no network, no Brain MCP. Canon Part 8 boundary preserved.
 *
 * @param {string} roomsHome  retained for signature stability (unused)
 * @returns {boolean}
 */
function detectNoActiveRoom(roomsHome) {  // eslint-disable-line no-unused-vars
  try {
    ...
    return true; // resolver failure -> treat as no-active-room (fail-open to auto-create)
  }
}
