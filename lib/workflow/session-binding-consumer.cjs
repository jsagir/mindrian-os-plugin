 ...
 *     (a slug in the new set that was not in the prior binding) it attempts the doctor
 *     bind-time check + presence register, GUARDED so the Wave-5 job's absence in this
 *     wave is a no-op (never hard-depends on it, never blocks the bind). The reserved
...
// Per newly-bound room, attempt the doctor bind-time check + presence register. Both
// are GUARDED: the doctor --bind-check job lands in Wave 5 (194-07), so its absence
// here is a no-op; presence register is fire-and-forget by construction. Never throws
// (the caller already runs inside a try/catch, this is defense-in-depth).
function _bindTimeJob(slug, sessionId, home) {
  const roomDir = path.join(_resolveHome(home), slug);

  // Doctor bind-time structural check (D-10). Wave-5 job: guard on its presence so a
  // missing exported bindTimeCheck is a clean no-op and NEVER blocks the bind.
  try {
    const doctor = _safeRequire(path.join(REPO, 'scripts', 'doctor.cjs'));
    if (doctor && typeof doctor.bindTimeCheck === 'function') {
      doctor.bindTimeCheck(roomDir);
    }
  } catch (_e) { /* Wave-5 job absent or faulted -> no-op, never block the bind */ }
