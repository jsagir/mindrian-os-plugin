...
// The governance-candidate contract (189-01). LAYER_VALUES is the closed enum the
// WHO write validates `layer` against before writing ATTRIBUTED_TO. Pure, zero-I/O.
const governanceCandidate = require(path.join(REPO, 'lib', 'core', 'memory', 'governance-candidate.cjs'));
const LAYER_VALUES = governanceCandidate.LAYER_VALUES;
...
// The WHO edge type (189-01 frozen-set member). A confirmed memory ALWAYS carries
// an explicit memory_layer (Part 8 CONTENT-SET LOCAL; a closed-enum property only).
const ATTRIBUTED_EDGE_TYPE = 'ATTRIBUTED_TO';
  ...
  return {
    closeOffer: function (closeArgs) {
      ...
      if (a.pick === 'accept') {
        ...
        // WHO (T-189-04-02): AFTER the REMEMBERED_AS write, attribute the memory
        // with an EXPLICIT memory_layer. WHO is a choice, never a silent default:
        // a candidate whose layer is not a member of the closed LAYER_VALUES enum
        // fails CLOSED (reason:'layer_required') rather than writing an
        // unattributed memory. The REMEMBERED_AS edge is already on disk; we write
        // ZERO further edges and never promote / never push to the remembered
        // ledger on a fail-closed.
        if (!(typeof cand.layer === 'string' && LAYER_VALUES.has(cand.layer))) {
          return { ok: false, reason: 'layer_required' };
        }
        const attributedTo = (typeof gateCtx.attributedTo === 'string' && gateCtx.attributedTo.length > 0)
          ? gateCtx.attributedTo
          : ('session:' + (gateCtx.session_id || 'unknown'));
        const aw = nav.writeEdge(gateCtx.db, {
          source_id: source_id,
          target_id: attributedTo,
          edge_type: ATTRIBUTED_EDGE_TYPE,
          properties: {
            memory_layer: cand.layer,
            attributed_at: Date.now(),
          },
        });
        if (!aw || aw.ok !== true) {
          return { ok: false, reason: (aw && aw.reason) ? aw.reason : 'attribution_write_failed' };
        }
        ...
        const result = { ok: true, edge_type: REMEMBERED_EDGE_TYPE };

        // T-189-04-01 (never automatic cross-room promotion): when the chosen layer
        // is cross-room, this closer performs NO cross-room linking here. It only
        // FLAGS the item as pending a SEPARATE, distinct second confirm. The CALLER
        // (runGovernanceGate's consumer) surfaces cross_room_pending to the
        // already-shipped Phase-195 umbilical F.8 gate (runUmbilicalGate) as a
        // second confirm -- never folded into this WHAT/HOW confirm. This module
        // NEVER performs the cross-room link itself and writes no cross-room edge.
        if (cand.layer === 'cross-room') {
          const pendingItem = {
            target_id: source_id,
            target_room: (typeof gateCtx.source_room === 'string' && gateCtx.source_room.length > 0)
              ? gateCtx.source_room
              : null,
            relevance: confidence,
            signal: 'governance_promotion',
          };
          result.cross_room_pending = true;
          result.cross_room_candidate = pendingItem;
          gateCtx.cross_room_pending.push(pendingItem);
        }
...
function runGovernanceGate(args) {
  ...
  const gateCtx = {
    ...
    nav: a.nav,
    byLabel: byLabel,
    // WHO attribution seams: the attributed-to target + the source room used for a
    // cross-room pending item. attributedTo defaults to session:<session_id> in the
    // closer when the caller supplies neither.
    session_id: (typeof a.session_id === 'string' && a.session_id.length > 0) ? a.session_id : null,
    attributedTo: (typeof a.attributedTo === 'string' && a.attributedTo.length > 0) ? a.attributedTo : null,
    source_room: (typeof a.source_room === 'string' && a.source_room.length > 0) ? a.source_room : null,
    remembered: [],
    not_remembered: [],
    // Every cross-room-layer confirm lands here as an item needing a SEPARATE,
    // second confirm (the caller drives runUmbilicalGate); it is never auto-linked.
    cross_room_pending: [],
  };
  ...
  return {
    ...
    // Every cross-room-layer confirm collected here needs a SEPARATE second confirm
    // via cross-room-umbilical-closer.runUmbilicalGate (the caller's job) -- this
    // gate NEVER auto-links across rooms (T-189-04-01).
    cross_room_pending: gateCtx.cross_room_pending,
    // D-09 precedent (188-06 / 195-05): decide() re-entry is the CALLER's job; this
    // module never re-invokes decide() itself. should_reoffer signals the caller to
    // re-offer (e.g. to surface the cross-room second confirm) on the next turn.
    should_reoffer: true,
...
module.exports = {
  ...
  ATTRIBUTED_EDGE_TYPE: ATTRIBUTED_EDGE_TYPE,
