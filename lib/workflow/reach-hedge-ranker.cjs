...
//   (i) fire-and-forget maybeUpdateHedgeWeights only when db is present AND the
//       caller did not opt into read-only mode.
...
// same act as writing. Under readOnly the ranker skips step (i) entirely and routes
// any degrade token to roomState.degradeSink instead of a memory_event. Steps (b)
// through (h) are IDENTICAL in both modes, so the pick never changes: the mode
// removes writes only. The caller is expected to supply a ?mode=ro handle as well,
// which makes the guarantee mechanical rather than a promise; this flag is what
// stops the useless write attempt before sqlite has to reject it.
// ---------------------------------------------------------------------------
