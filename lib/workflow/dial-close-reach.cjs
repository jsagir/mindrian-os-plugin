...
// IRW-06 resolve+fire: the ONLY framework->command door (degrade-don't-fabricate
// built in). command-resolver is a workflow sibling, NOT a navigation submodule,
// so it does not violate the Part 9 single-chokepoint import audit.
const commandResolver = require('./command-resolver.cjs');
// IRW-06 filing fallback: wireAccept is the readback-error fallback per the
// intelligence-orchestrator ORCH-03 doctrine. fileEvidenceWithReadback +
// surfaceFileEvidenceResult are re-exported from navigation.cjs (used via the
// chokepoint); wireAccept is required from findings-wirer directly (it is NOT a
// navigation submodule, so the Part 9 import audit still passes).
const findingsWirer = require('../core/findings-wirer.cjs');
...
// IRW-06 resolve + fire + file. On a committed reach (sync/pivot) AFTER the
// SELECTED_REACH bookkeeping edge has landed, resolve reach.framework through the
// ONLY sanctioned door (commandResolver.commandsForFramework). This mirrors the
// intelligence-orchestrator STEP 5 resolve-and-fire loop verbatim:
//   - empty resolution -> DEGRADE: return a structured "run <framework> manually"
//     instruction. NEVER fabricate a slug.
//   - non-empty -> fire cmds[0] (the real /mos:<engine> command) through the
//     injected fireCommand seam (the in-conversation stand-in for Larry running
//     the command), then land the engine artifact via fileEvidenceWithReadback
//     (fallback wireAccept on a readback error), surfacing the FILEVAL honesty
//     result. The artifact is a TRUTH-CLAIM node and lands review_status
//     'proposed' (Part 9 role 5) -- this resolve+fire NEVER promotes a truth-claim
//     and NEVER folds a confirmNode promotion into the SELECTED_REACH bookkeeping.
//
// Returns the resolve/fire/file annotations to merge onto the closeReach result:
//   { degraded:false, fired_command:'/mos:...', filing:{ok,...}, surfaced:{...} }
//   { degraded:true,  fired_command:null, manual_run:'run <framework> manually' }
function _resolveFireFile(db, reach, o) {
  const framework = (reach && typeof reach.framework === 'string') ? reach.framework : '';
  // The resolver is the ONLY door; degrade-don't-fabricate is built in.
  const cmds = commandResolver.commandsForFramework(framework);

  if (!Array.isArray(cmds) || cmds.length === 0) {
    // DEGRADE -- never fabricate. Tell the navigator to run the framework manually.
    return {
      degraded: true,
      fired_command: null,
      manual_run: 'run ' + (framework || 'the framework') + ' manually',
    };
  }

  const firedCommand = cmds[0];

  // Fire the real /mos: command. In-conversation this is Larry running the
  // command; the injected fireCommand seam is the surface-agnostic stand-in so
  // the transaction test can assert the real command fired (not a stub).
  if (typeof o.fireCommand === 'function') {
    try { o.fireCommand(firedCommand); } catch (_e) { /* a fire-surface failure never rolls back the edge */ }
  }

  const out = { degraded: false, fired_command: firedCommand };

  // Land the engine artifact (ORCH-03 filing field). Only when the caller
  // supplied the evidence params the fired command produced; absent params means
  // a surface-only family (e.g. six-hats) that produces no EvidenceClaim.
  const fe = (o.fileEvidence && typeof o.fileEvidence === 'object') ? o.fileEvidence : null;
  if (fe) {
    let filing = navigation.fileEvidenceWithReadback(db, fe);
    // Readback error -> FALL BACK to wireAccept (ORCH-03 LOCKED decision 2).
    if (!filing || filing.ok !== true) {
      const fallback = findingsWirer.wireAccept(db, fe);
      if (fallback && fallback.ok === true) filing = fallback;
      else if (filing && typeof filing === 'object') { /* keep the structured failure to surface honestly */ }
    }
    out.filing = filing;
    // FILEVAL honesty: surface exactly what landed (never claim a filing that
    // did not happen). surfaceFileEvidenceResult is a pure function, no egress.
    out.surfaced = navigation.surfaceFileEvidenceResult(filing);
  }
...
function closeReach(args) {
  ...
  if (outcome === 'defer' || outcome === 'reject') {
    ...
    const res = selectorDecisions.recordSelectorDecision({
      ...
      framework: reach.framework,
      reason: o.reason,
      // Phase 158-01 (RJP-06 / SC-02): forward the rejected reach's reach_id so
      // the f_selector_decision row is keyed per reach_id (the count-in-window in
      // Plan 02/03 reads rejects per reach_id). The rendered dial reach object
      // provably carries reach_id (dial-reach-orchestrator.cjs:219-237); a
      // non-dial caller leaves reach.reach_id undefined, which the
      // recordSelectorDecision enum-gate ignores (byte-stable for non-dial
      // callers). Additive: one key, no signature change.
  ...
  // IRW-06: resolve reach.framework -> real command, FIRE it, and land the engine
  // artifact. This runs AFTER the SELECTED_REACH bookkeeping edge (above) on every
  // committed reach (sync + pivot). An unresolvable framework DEGRADES to a
  // manual-run instruction (never a fabricated slug); the bookkeeping edge has
  // already landed either way. Kept DISTINCT from any confirmNode truth-claim
  // promotion (Part 9 carve-out): the filed EvidenceClaim lands 'proposed'.
  const fired = _resolveFireFile(db, reach, o);
  ...
  const out = {
    ...
    decision_id: reach.decision_id,
    degraded: fired.degraded === true,
    fired_command: fired.degraded ? null : (fired.fired_command || null),
  };
  if (fired.degraded) out.manual_run = fired.manual_run;
  if (fired.filing) out.filing = fired.filing;
  if (fired.surfaced) out.surfaced = fired.surfaced;
