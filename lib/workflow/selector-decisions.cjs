...
// FIX-05 (Phase 150.6-03): classify the user sentence at the f_selector_decision
// WRITE seam so the Phase 119 receipt nudge (lib/core/venture-shape-nudge.cjs)
// can read a scalar boolean venture_classified property on existing events. The
// classifier is shipped, synchronous, cached, length-guarded, Hebrew-guarded,
// and zero-network on the sync path -- safe to call inline at emission time.
// We consume classify(); we NEVER modify it.
const { classify } = require('../core/mva-classifier.cjs');
...
// Phase 158-01 (RJP-06 / SC-02 / D-03): the frozen 6-set of machine reach_ids.
// Mirrored here as a flat frozen const (NOT imported from
// lib/hmi/dial-reach-orchestrator.cjs) to avoid a require cycle and to keep this
// module decoupled -- exactly the cortex-reach-adapter.cjs:51-58 idiom. When the
// optional reach_id arg is a member of this set it rides the f_selector_decision
// payload as a scalar enum; an off-set value is IGNORED (the enum-gate, mirroring
// cortex-reach-adapter's KNOWLEDGE_TYPES membership check). With NO reach_id arg
// the payload is byte-identical to the pre-158 shape (RJP-02 foundation). The
// reach_id is a generic machine token (Part 8 safe); the BODY of a rejection
// reason NEVER enters via this path.
...
// through. When present, it is classified at the write seam and ONLY the scalar
// boolean venture_classified + classification_source enum are added to the
// f_selector_decision payload (Canon Part 8: the sentence itself never enters
// the payload). When absent, neither key is added (no-sentence path unchanged).
//
...
function recordSelectorDecision(args) {
  ...
  // FIX-05 (Phase 150.6-03): when a user sentence is in hand, classify it at the
  // write seam and stage ONLY the scalar boolean venture_classified + the
  // classification_source enum for the f_selector_decision payload. Canon Part 8
  // fence (HARD): the sentence is the classify() INPUT only; it is NEVER stored
  // in the payload (the memory_event log is queried by cross-room aggregators).
  // When no sentence is present, add NEITHER key, preserving the no-sentence path
  // and the nudge's existing safe-default.
  let ventureFields = null;
  const sentence = (typeof o.sentence === 'string' && o.sentence.length > 0) ? o.sentence : null;
  if (sentence) {
    const cls = classify(sentence);
    ventureFields = {
      venture_classified: cls.venture,
      classification_source: cls.source,
    };
  }

  // Phase 158-01 (RJP-06 / SC-02 / D-03): the OPTIONAL reach_id enum, additive on
  // the FIX-05 optional-merge idiom above. When o.reach_id is a string AND a
  // member of the frozen REACH_IDS 6-set, stage {reach_id} for the payload merge
  // (the count-in-window in Plan 02/03 keys rejects per reach_id). When it is
  // absent OR off-set, reachField stays null and the merge adds NOTHING, so the
  // payload is byte-identical to the pre-158 shape (every non-dial caller
  // unaffected; RJP-02 byte-stability). This is the cortex-reach-adapter enum-gate
  // idiom: an off-enum value is dropped before storage (T-158-01-02 tamper
  // mitigation -- no prose can be smuggled into a frozen-enum field).
  let reachField = null;
  if (typeof o.reach_id === 'string' && REACH_IDS.indexOf(o.reach_id) !== -1) {
    reachField = { reach_id: o.reach_id };
  }
  ...
  //   }
  // ---------------------------------------------------------------------
  const memEvent = navigation.logMemoryEvent(db, 'f_selector_decision', Object.assign({
    decision,
    command,
    framework,
    reason,
...
// Consumer (F-selector renderer) is responsible for the follow-up do-command call;
...
module.exports = {
  ...
  // Phase 158-01: the frozen 6-set the optional reach_id enum-gate checks against
  // (exposed symbolically so Plan 02/03 + the keying test reference it, not a
  // hard-coded literal). Mirrors dial-reach-orchestrator REACH_IDS by value.
  REACH_IDS,
