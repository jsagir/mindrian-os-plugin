...
// and NO SKILL.md (BCH-05 adversarial grep). The engine owns the composed
// decision; the model emits observations only (the bright line).
//
...
// ---------------------------------------------------------------------------
// Phase 244 (TRIG-03) -- the MMR (Maximal Marginal Relevance) diversity pass.
//
// Closes the crowding-out gap left open by fusion (_applyTierFusion above):
// fusion can promote a buried cross-family candidate into contention, but it
// cannot stop three near-duplicate same-family candidates from occupying all
// three top slots once they ARE all in contention. This pass is the greedy
// diversity cut that fusion is not -- Carbonell and Goldstein SIGIR 1998,
// "The Use of MMR, Diversity-Based Reranking for Reordering Documents and
// Producing Summaries."
//
// CANONICAL FORM (this is what is implemented):
//   MMR = argmax over d in pool of [ lambda * Rel(d, q) - (1 - lambda) * max Sim(d, d_selected) ]
// where lambda = 1 means pure relevance, lambda = 0 means pure diversity.
//
// LAMBDA-ORIENTATION GOTCHA: ROADMAP SC3 states the formula inverted, as
// `(1-lambda)*relevance - lambda*max_similarity_to_selected`. The two are
// algebraically equivalent under `lambda' = 1 - lambda`, but the SEMANTICS OF
// THE KNOB ARE FLIPPED: under the ROADMAP's form, someone writing lambda=0.7
// intending "mostly relevance" would actually get "mostly diversity". This
// implementation follows the CANONICAL Carbonell orientation, and the
// constant is named MMR_LAMBDA_RELEVANCE so the name itself carries the
// semantics: 0.7 means mostly RELEVANCE weight, not mostly diversity. ROADMAP
// SC3's one-line formula needs a navigator amendment; this is not silently
// edited here (244-07-SUMMARY.md records the exact amendment needed).
//
// The similarity term reuses the ALREADY-SHIPPED Jaccard primitive
// (lib/core/eureka/lexical-overlap.cjs::lexicalOverlap, versioned
// 'jaccard-v1') -- Canon Part 7 (reuse before build). This is the THIRD
// production instance of that primitive, not a new similarity measure. An
// embedding-based similarity is deliberately excluded: the exclusion reason
// is LATENCY BUDGET, not missing capability -- a per-turn transformer model
// load is orders of magnitude beyond the 1200ms NAV budget
// (navigation-engine.cjs:820), even though sqlite-vec and
// @huggingface/transformers are already pinned dependencies and
// vector-store.cjs already runs a local embedding store (244-RESEARCH.md
// Finding F-10). The lexical leg already separates cleanly at negligible
// cost: at most 6x6 Jaccard comparisons per turn given MAX_K=3 and
// DIAL_REACH_K=6.
//
// The env-tunable lambda, resolved through the SAME idiom
// _resolveTrigRrfK uses above (validate finite, clamp to the closed interval
// 0..1, fall back to the default). Resolved ONCE at module load, mirroring
// TRIG_RRF_K -- a test that needs a different lambda must delete the module
// from require.cache and re-require (see tests/test-244-mmr-diversity.cjs).
function _resolveMmrLambda() {
  const raw = process.env.TRIG_MMR_LAMBDA;
  if (typeof raw === 'string' && raw.trim() !== '') {
    const v = Number(raw);
...
const MMR_LAMBDA_RELEVANCE = _resolveMmrLambda();

// The similarity projection: a LOCAL, non-prose handle built from `command`,
// `jtbd_label` and `framework` ONLY. Canon Part 8 keeps this similarity
// computation over closed LOCAL command handles rather than user or Brain
// content -- `jtbd_summary`, `teaching` and `why` (or any field that can
// carry prose) are deliberately excluded from the projection. Every field is
// guarded with a string check so a missing field contributes an empty
// string rather than 'undefined' leaking into the token stream.
...
// _applyMmrDiversity(list, k) -- the greedy MMR cut. Pure, sync, no I/O.
//
// No-op guard conditions (returns the INPUT REFERENCE untouched, byte-
// identical, mirroring the _applyRoleLevelBias envelope immediately above):
//   - list is null or not an array
//   - list has 0 or 1 entries (nothing to diversify against)
//   - only one distinct `tier_family` value is present across every row
//     (including the case where no row carries the field at all, which
//     counts as a single implicit "undefined" family). A single-family list
//     has no cross-family hit to protect, so running the greedy loop would
//     reorder for no benefit and would risk perturbing the byte-identical
//     pre-244 path -- this is exactly the pre-244 shape when no tier
//     candidates were ever supplied upstream (_applyTierFusion is itself a
//     no-op then, so `list` here carries no `tier_family` field on any row).
function _applyMmrDiversity(list, k) {
  if (!Array.isArray(list) || list.length <= 1) return list;

  const families = new Set(list.map(function (it) { return it && it.tier_family; }));
  if (families.size <= 1) return list;

  // Relevance normalization: use `rrf_score` when present (Plan 04 stamps
  // it on fused rows), otherwise fall back to `score`. Normalized to 0..1
  // across the CURRENT list by dividing by the maximum, guarding a zero (or
  // negative) maximum. This normalization is needed HERE but was not needed
  // for fusion: RRF fused by array POSITION and never touched magnitudes, so
  // sign/scale never mattered there; MMR SUBTRACTS a 0..1 similarity from
  // the relevance term, so the two must share a scale or the subtraction is
  // meaningless.
  const rawRel = list.map(function (it) {
    if (it && typeof it.rrf_score === 'number' && Number.isFinite(it.rrf_score)) return it.rrf_score;
    if (it && typeof it.score === 'number' && Number.isFinite(it.score)) return it.score;
    return 0;
  });
  let maxRaw = -Infinity;
  for (const r of rawRel) { if (r > maxRaw) maxRaw = r; }
  const rel = rawRel.map(function (r) { return (maxRaw > 0) ? (r / maxRaw) : 0; });

  // Function-local require (the navigation-engine.cjs:510 idiom, already
  // used by _applyTierFusion above) so a module-load cycle is never risked.
  const { lexicalOverlap } = require('../core/eureka/lexical-overlap.cjs');

  const kk = (typeof k === 'number' && Number.isFinite(k) && k > 0) ? Math.floor(k) : list.length;

  const pool = list.map(function (it, idx) {
    return { row: it, rel: rel[idx], text: _mmrTextOf(it) };
  });
  const selected = [];
  const selectedTexts = [];

  while (selected.length < kk && pool.length > 0) {
    let bestIdx = 0;
    let bestScore = -Infinity;
    for (let i = 0; i < pool.length; i += 1) {
      const cand = pool[i];
      let maxSim = 0;
      for (let j = 0; j < selectedTexts.length; j += 1) {
        const sim = lexicalOverlap(cand.text, selectedTexts[j]);
        if (sim > maxSim) maxSim = sim;
      }
      const mmr = MMR_LAMBDA_RELEVANCE * cand.rel - (1 - MMR_LAMBDA_RELEVANCE) * maxSim;
      if (mmr > bestScore) {
        bestScore = mmr;
        bestIdx = i;
      }
    ...
    selectedTexts.push(chosen.text);
  }

  // Copy-on-write, mirroring _applyRoleLevelBias: base rows are never
  // mutated, and every returned entry is a fresh copy even when no field is
  // added, so an object-identity check on the input array's entries always
  // shows the returned entries are copies.
  return selected.map(function (s) { return Object.assign({}, s.row); });
}
...
function rankForSelector(args) {
  ...
  // Phase 311 (SEED-052, D-02/D-03): the caller-declared admin-visibility
  // flag. STRICT `=== true` on purpose -- a stray string, number, or object
  // must never grant admin. Absent or falsy reads as "not admin", which is
  // the fail-closed default: a caller that never wires this flag gets the
  // SAFE (filtered) behavior automatically, never the leaky one. The ranker
  // only ever reads this already-resolved boolean; it never resolves admin
  // identity itself (mirrors the opts._applyDecayWeight injection above).
  const isAdmin = (o.isAdmin === true);
  ...
  for (const cmd of commands) {
    ...
    // Phase 311 (SEED-052, D-03) fail-closed: admin-visibility filter. Keys
    // on the literal value 'admin', not on presence of a visibility field, so
    // this stays a targeted filter rather than a general visibility gate.
    // The candidate never reaches scored.push(...) below, so it can never
    // appear in the ranked output at any score when the caller has not
    // declared admin.
    if (cmd.visibility === 'admin' && !isAdmin) continue;
  ...
  // Phase 244 (TRIG-03): the MMR diversity pass, inserted AFTER fusion and
  // BEFORE the sens10-or-slice branch -- MMR IS the cut, so it must see the
  // full fused list before anything removes a candidate from consideration.
  // Absent multi-family signal (the pre-244 shape, or a fusion pass that
  // never ran because no tierCandidates were supplied) this is a
  // byte-identical no-op: `diversified` is `fused` BY REFERENCE, so the
  // sens10-or-slice branch below lands on exactly the list it did pre-244.
  const diversified = _applyMmrDiversity(fused, k);
  ...
  const finalList = (sens10 && sens10.fired === true)
    ? _applySens10Flip(diversified, sens10, k)
...
module.exports = {
  ...
  _applyTierFusion,
  TRIG_RRF_K,
  // Phase 244 (TRIG-03): the MMR diversity pass + its lambda constant.
  // Exported (not _test-only, mirroring _applyTierFusion above) so
  // consumers and tests read the engine-owned values directly.
  _applyMmrDiversity,
