...
function enrichRoom(manifest, opts) {
  ...
  // ----- 7. IMPORT-10 branding pass (Phase 80-06) -----
  // applyBranding is the LAST write per artifact so footer + canonical
  // frontmatter + callouts see the final post-wikilink content.
  stats.branded = 0;
  const brandingCtx = {
  ...
  for (const f of manifest.files) {
    if (!f.destination_abs_path || !f.classification) continue;
    if (!fs.existsSync(f.destination_abs_path)) continue;
    try {
      branding.applyBranding(
        f.destination_abs_path,
        f.classification,
        Object.assign({}, brandingCtx, {
          source_path: f.source_path,
          source_wikilinks: f.source_wikilinks || []
        })
      );
      stats.branded++;
    } catch (e) {
      manifest.warnings = manifest.warnings || [];
      manifest.warnings.push({ type: 'branding_failed', context: (f.id || f.source_path) + ': ' + e.message });
    }
  }
  ...
  manifest.stage_states.enrich.branded = stats.branded;
  return stats;
}
