 ...
 *   3.  (removed Phase 150-04 MEM-07) SECTION_WEIGHTS deleted -- composition stays
 *       rule-based per 150-CONTEXT D-03; the weighted-scorer table was dead code.
...
run('Test 3: SECTION_WEIGHTS is removed (composition stays rule-based per 150-CONTEXT D-03)', () => {
  const shared = requireShared();
  // Phase 150-04 MEM-07: the dead weighted-scorer table was deleted. The shared
  // module must NOT export SECTION_WEIGHTS; the section KEY contracts remain.
  assert.equal(shared.SECTION_WEIGHTS, undefined, 'SECTION_WEIGHTS must no longer be exported');
  assert.ok(Array.isArray(shared.REQUIRED_SECTION_KEYS), 'REQUIRED_SECTION_KEYS retained');
  assert.ok(Array.isArray(shared.OPTIONAL_SECTION_KEYS), 'OPTIONAL_SECTION_KEYS retained');
});
