...
/**
 * Seed a room fixture at tmp:
 *   <tmp>/.room-root
 *   <tmp>/.mindrian/
 *   <tmp>/<section>/ROOM.md  (one per listed section)
 *   <tmp>/<section>/<artifact>.md   (optional artifact per section)
 * Returns the roomDir.
 */
 ...
 */
function invokeOnStop(roomDir, extraEnv) {
...
test('3-section room -> session-snapshot.json has 3 section entries', () => {
  ...
  const room = seedRoom(tmp, ['market-analysis', 'problem-definition', 'solution-design']);
  // Give each section a healthy MINTO so readTriple returns exists:true
  writeMinto(path.join(room, 'market-analysis'), { governing_thought: 'TAM bottom-up.' });
  writeMinto(path.join(room, 'problem-definition'), { governing_thought: 'Problem bounded.' });
  writeMinto(path.join(room, 'solution-design'), { governing_thought: 'Solution viable.' });

  const res = invokeOnStop(room);
  assert.equal(res.status, 0, 'on-stop should exit 0: stderr=' + (res.stderr || ''));

  const snapPath = path.join(room, '.mindrian', 'session-snapshot.json');
...
test('snapshot section shape matches readTriple() (room, state, reasoning)', () => {
  ...
  const room = seedRoom(tmp, ['market-analysis']);
  writeMinto(path.join(room, 'market-analysis'), { governing_thought: 'Market exists.' });

  const res = invokeOnStop(room);
  assert.equal(res.status, 0, 'on-stop exits 0: ' + (res.stderr || ''));

  const snap = JSON.parse(
    fs.readFileSync(path.join(room, '.mindrian', 'session-snapshot.json'), 'utf8')
  );
...
test('pending queue drains during on-stop; remainder surfaces in stale if any', () => {
  ...
  const room = seedRoom(tmp, ['market-analysis', 'problem-definition']);
  writeMinto(path.join(room, 'market-analysis'), { governing_thought: 'M.' });
  writeMinto(path.join(room, 'problem-definition'), { governing_thought: 'P.' });
  ...
  dbnc.enqueue(room, 'market-analysis', 'pre-existing');
  dbnc.enqueue(room, 'problem-definition', 'pre-existing');
  const preDrain = dbnc.peek(room);
  ...
  const mindrianEntries = fs.readdirSync(path.join(room, '.mindrian'));
...
test('sentinel last_generated_at (1970) -> minto-stale.json lists that section', () => {
  ...
  const room = seedRoom(tmp, ['competitive-analysis', 'market-analysis']);
  // competitive-analysis is stale (sentinel); market-analysis is fresh.
  writeMinto(path.join(room, 'competitive-analysis'), { stale: true, governing_thought: 'C.' });
  writeMinto(path.join(room, 'market-analysis'), { governing_thought: 'M.' });

  const res = invokeOnStop(room);
  assert.equal(res.status, 0, 'on-stop exits 0: ' + (res.stderr || ''));

  const stalePath = path.join(room, '.mindrian', 'minto-stale.json');
...
test('STATE.md write path still fires (Phase 84 on-stop contract preserved)', () => {
  ...
  const statePath = path.join(room, 'STATE.md');
...
test('session-snapshot.json and minto-stale.json written atomically (no tmp leakage)', () => {
  ...
  const room = seedRoom(tmp, ['a', 'b', 'c']);
  writeMinto(path.join(room, 'a'), { governing_thought: 'A.' });
  writeMinto(path.join(room, 'b'), { stale: true, governing_thought: 'B.' });
  writeMinto(path.join(room, 'c'), { governing_thought: 'C.' });
