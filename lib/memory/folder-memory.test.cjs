...
function test_7_roomReferencesExtracted(sync) {
  ...
  const teamRef = r.room.references.find((x) => x.type === 'team');
  assert.ok(teamRef, 't7: team wikilink present');
  assert.ok(
    teamRef.target.indexOf('team/') === 0 || teamRef.target.indexOf('/team/') !== -1,
    't7: team wikilink target shape'
  );
}
...
function test_12_computeHealthPerfect(sync) {
  // evidence_density=1.0 reaches the formula's upper bound. The plan's
  // intent is "perfect input -> max score"; the exact 0.9 vs 1.0 value
  // in the PLAN <behavior> block reflects a rounding drift. With
  // evidence_density 1.0: 0.3 + 0.2 + 0.2 + 0.1 + 0.2 = 1.0.
