 ...
 *
 * Amended Phase 355 D-04 (HIPS-01): the single-axis branches now come from
 * lib/core/direction-convention.cjs's classify(lsa, bert) (Test 1 and
 * Test 2's expected labels flip accordingly -- classify(0.5, 0.1) is
 * semantic_implementation, classify(0.1, 0.5) is structural_transfer);
 * below-floor pairs (neither axis clears 0.3, including the exact-0.3
 * boundary) now emit 'none' instead of a silent structural_transfer
 * default (Test 4, Test 5). Test 3 (hybrid) is unchanged.
 *
 * 8 scenarios for the Phase 3 deterministic 4-enum classifier on the
 * {diff, lsa, bert, passes} output of rs-differential-scorer.cjs:
 *
 *   classification IS one of:
 *     <single-axis>               -- exactly one of lsa/bert > 0.3; label
 *                                  from direction-convention.cjs classify(lsa, bert)
 *     'hybrid'                    -- both lsa > 0.3 AND bert > 0.3
 *     'none'                      -- neither axis clears 0.3 (Phase 355 D-04)
 *
 *   Test 1  single-axis, semantic_implementation label (lsa=0.5 bert=0.1)
 *   Test 2  single-axis, structural_transfer label (lsa=0.1 bert=0.5)
 *   Test 3  hybrid enum branch (lsa=0.5 bert=0.5; classification independent of passes)
 *   Test 4  neither axis clears floor (lsa=0.1 bert=0.1) -- 'none'
 *   Test 5  boundary lsa exactly 0.3 -- strict > so lsa does not count as high -- 'none'
...
function scenario1StructuralTransfer() {
  ...
  // Phase 355 D-04: single-axis label comes from direction-convention.cjs
  // classify(lsa, bert) = classify(0.5, 0.1) -> semantic - lsa = -0.4 <= 0
  // -> semantic_implementation (was structural_transfer pre-355).
  assert.equal(out.classification, 'semantic_implementation',
    'lsa=0.5 bert=0.1 should be semantic_implementation (Phase 355 D-04); got ' + out.classification);
...
function scenario2SemanticImplementation() {
  ...
  // Phase 355 D-04: classify(0.1, 0.5) -> semantic - lsa = 0.4 > 0 ->
  // structural_transfer (was semantic_implementation pre-355).
  assert.equal(out.classification, 'structural_transfer',
    'lsa=0.1 bert=0.5 should be structural_transfer (Phase 355 D-04); got ' + out.classification);
  assert.equal(out.bridge_concept, 'cross-domain isomorphism', 'bridge_concept matches mapping');
}
...
function scenario4DefaultFallback() {
  ...
  // Phase 355 D-04: neither axis clears the 0.3 floor -- no directional
  // signal, so 'none' (was a silent structural_transfer default pre-355).
  assert.equal(out.classification, 'none',
    'lsa=0.1 bert=0.1 (both below threshold) should be none (Phase 355 D-04); got ' +
    out.classification);
  assert.equal(out.bridge_concept, null, 'none classification has null bridge_concept');
}
...
function scenario5BoundaryLsaExact() {
  ...
  // 0.3 is NOT > 0.3 (strict comparison), so lsa does not count as high;
  // bert (0.1) also does not clear 0.3 -- neither axis high -> 'none'
  // (Phase 355 D-04; was a silent structural_transfer default pre-355).
  assert.equal(out.classification, 'none',
    'lsa=0.3 exactly (not > 0.3) with bert below floor should be none (Phase 355 D-04); got ' +
    out.classification);
  assert.equal(out.bridge_concept, null, 'none classification has null bridge_concept');
}
