...
function makeAuraMock(perBranchRecords) {
  ...
  function recordFromNode(nodeObj) {
    return {
      get: function (key) {
        // Default key 'd' for RSDiscovery, 'rs' for ReverseSalient,
        // 'i' for Innovation, etc. We return the nodeObj on any key
        // request because the test mock is permissive.
        return {
