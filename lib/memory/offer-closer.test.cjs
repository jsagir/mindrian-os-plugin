 ...
 *
 * RED until 135-03 (the closer module does not exist yet):
...
// runRed: tracked RED target that depends on the unbuilt closer (135-03).
// Expected to fail today; reported as RED, not a hard failure, so this Wave 0
// suite lands with Task 1. 135-03 promotes these to run() once offer-closer.cjs
// exists.
function runRed(name, fn) {
  try {
    ...
    process.stdout.write('RED ' + name + ' (expected RED until 135-03)\n');
    red += 1;
  }
}
...
// ---------- RED until 135-03: the closer module orchestration ----------

runRed('SC7: closer builds an F.1 payload whose verbs end with Free-Text', () => {
...
runRed('SC7: an accept/defer/reject pick routes to recordSelectorDecision; Free-Text to recordSelectorMiss', () => {
  const closer = tryRequireCloser();
  assert.notEqual(closer, null, 'lib/workflow/offer-closer.cjs must exist (135-03)');
