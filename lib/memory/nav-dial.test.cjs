...
function makeStatuslineFixture(label, opts) {
  ...
  TMPDIRS.push(tmp);
  // The context-monitor expects .rooms/registry.json inside the workspace
  // dir (data.workspace.current_dir). Build the fixture so the workspace
  // dir IS the parent that holds .rooms/ and the room subdir, matching
  // the production layout where users `cd` into their MindrianRooms parent.
  const workspace = path.join(tmp, 'workspace');
  fs.mkdirSync(workspace, { recursive: true });
  const roomDir = path.join(workspace, 'fixture-room');
  fs.mkdirSync(roomDir, { recursive: true });
  // Minimal STATE.md.
  fs.writeFileSync(path.join(roomDir, 'STATE.md'),
    '---\nproject_name: fixture-room\n---\n');
  // Registry at workspace/.rooms/registry.json (where context-monitor reads it).
  const regDir = path.join(workspace, '.rooms');
  ...
  return { tmp, workspace, roomDir };
}

function runContextMonitor(workspace) {
...
run('Test 13: context-monitor with trace -> output contains Larry:', () => {
  ...
  const out = runContextMonitor(fx.workspace);
  const stripped = out.stdout.replace(/\x1b\[[0-9;]*m/g, '');
  assert.equal(out.stderr, '', 'context-monitor must not error: ' + out.stderr);
  // Room resolution + dial wiring both required.
  assert.ok(stripped.includes('fixture-room'),
    'context-monitor must resolve fixture-room from registry. Got: ' +
    JSON.stringify(stripped));
  assert.ok(stripped.includes('Larry:'),
    'with trace + resolved room, dial MUST render. Got: ' +
    JSON.stringify(stripped));
  // The active label is Blend at weight 0.8 -- assert the Blend label
  // appears between the other two so the user reads the engine state.
  assert.ok(stripped.includes('Blend'),
    'dial must show Blend label for weight 0.8. Got: ' +
    JSON.stringify(stripped));
});
...
run('Test 14: context-monitor without trace -> no Larry: segment', () => {
  ...
  // Pre-91 statusline byte-stable: room name must still resolve.
  assert.ok(stripped.includes('fixture-room'),
    'pre-91 statusline must still render room name. Got: ' +
    JSON.stringify(stripped));
});

run('Test 15: context-monitor warm latency < 1500ms wall-clock (spawn budget)', () => {
  const fx = makeStatuslineFixture('warm-latency', {
    trace: modeATrace(0.8, 'blend')
  });
  // Warm: run once to populate any caches.
  runContextMonitor(fx.workspace);
  const N = 5;
  const samples = [];
  for (let i = 0; i < N; i += 1) {
    const start = process.hrtime.bigint();
  ...
  // path. We instead assert wall-clock is under a generous bound for the
  // spawned process (Node.js cold-start + script run). The MINTO segment in
