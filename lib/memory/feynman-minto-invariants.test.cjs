 ...
 *
 * 22 discrete test cases across 5 violation categories:
 *   existence, schema, freshness, coherence, atomicity.
 * Test 22 (Phase 241 F-2) also asserts the schema_version scope-creep
 * guard: raising governing_thought to critical must not also raise
 * schema_version.
 *
...
// 22. Phase 241 F-2: governing_thought severity is critical, and the
// schema_version scope-creep guard: a MINTO.md with governing_thought
// present but schema_version missing must STILL return error, not
// critical. This is the standing assertion that fails if someone later
// raises schema_version too (RESEARCH.md's Anti-Patterns / R-05).
run(
  'Test 22: governing_thought severity is critical; schema_version guard stays error',
  function () {
    const missingGT = withFrontmatterDelete(
      'governing_thought: "The venture reduces time between insight and validated decision across every dimension simultaneously."'
    );
    const pGT = tmpFile('f2-missing-gt.md', missingGT);
    const rGT = validate(pGT);
    assert.equal(rGT.severity, SEVERITY.CRITICAL, 'aggregate severity is critical');
    assert.ok(
      rGT.violations.some(function (v) {
        return v.field === 'governing_thought' && v.severity === SEVERITY.CRITICAL;
      }),
      'a violation with field governing_thought at CRITICAL exists'
    );

    const missingSV = withFrontmatterDelete('schema_version: "1.0"');
    const pSV = tmpFile('f2-missing-sv.md', missingSV);
    const rSV = validate(pSV);
    assert.equal(
      rSV.severity,
      SEVERITY.ERROR,
      'schema_version alone missing still aggregates to error, not critical'
    );
    assert.ok(
      rSV.violations.some(function (v) {
        return v.field === 'schema_version' && v.severity === SEVERITY.ERROR;
      }),
      'a violation with field schema_version at ERROR exists'
    );
  }
);
