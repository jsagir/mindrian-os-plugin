...
function testChainLayerMaterializedNote() {
  // Phase 172-10 populated curated_chains (the R6 cure) and Phase 172-08 added the
  // cross-class command -> pipeline -> framework edges; the chain layer is now
  // MATERIALIZED, so the top-level chain_layer_note reflects the emitted count and
  // is no longer SOURCE-EMPTY. The note stays LEGIBLE (it names curated_chains).
  const p = build();
  const chaining = p.edges.filter(
    (e) => e.type === 'CHAINS' || e.type === 'FEEDS_INTO' || e.type === 'PREREQUISITE'
  );
  assert.ok(chaining.length > 0, 'chain layer is materialized (curated_chains populated)');
  assert.strictEqual(typeof p.chain_layer_note, 'string', 'chain_layer_note is a string');
  assert.ok(!/SOURCE-EMPTY/.test(p.chain_layer_note), 'chain_layer_note is no longer SOURCE-EMPTY');
  assert.ok(/curated_chains/.test(p.chain_layer_note), 'chain_layer_note names curated_chains');
  assert.ok(/edge\(s\)/.test(p.chain_layer_note), 'chain_layer_note reflects the emitted edge count');
}
...
// ---- Task 2 (Plan 04): the deliberately un-wired fixture (BOG-06 acceptance) ----

const FIXTURE_PATH = path.join(
  __dirname, '..', '..', 'tests', 'fixtures', 'orchestration-unwired', 'UNWIRED-FIXTURE.md'
);

// Parse the `frameworks: [...]` list out of the fixture frontmatter so the test
// stays coupled to the fixture file (the framework name lives in ONE place).
function fixtureFrameworkName() {
  const md = fs.readFileSync(FIXTURE_PATH, 'utf8');
  const m = /frameworks:\s*\[\s*"([^"]+)"\s*\]/.exec(md);
  assert.ok(m, 'fixture declares a frameworks: ["..."] block');
  return m[1];
}

// Synthesize the MINIMAL projection that contains the fixture's framework as an
// UN-WIRED framework node (present in nodes[], no OPERATES->reach chain). The
// STALE leg is irrelevant here (this synthesized object intentionally does not
// match the on-disk file); the test reads ONLY res.unwired.
function synthUnwiredProjection(fwName) {
  return {
    ontology_ref: 'synthetic',
    generated_note: 'synthetic',
    chain_layer_note: 'synthetic',
    nodes: [
      { id: 'framework:' + fwName, kind: 'framework', methodology_tier: 'pws', name: fwName },
    ],
    edges: [],
  };
}
...
function testFixtureExists() {
  assert.ok(fs.existsSync(FIXTURE_PATH), 'the un-wired fixture file exists');
  const md = fs.readFileSync(FIXTURE_PATH, 'utf8');
  assert.ok(/frameworks:/.test(md), 'fixture declares a frameworks: block');
  assert.ok(!/^connector:/m.test(md), 'fixture ships NO connector: block (un-wired on purpose)');
}

function testFixtureFlagsUnwiredRed() {
  // The deliberately un-wired fixture framework makes validateProjection flag
  // UN-WIRED (fails RED).
  const fwName = fixtureFrameworkName();
  const proj = synthUnwiredProjection(fwName);
  const res = gen.validateProjection(proj);
  assert.ok(res.unwired.length > 0, 'the un-wired fixture framework fires UN-WIRED');
  const joined = res.unwired.join('\n');
  assert.ok(/UN-WIRED/.test(joined), 'message contains the UN-WIRED token');
  assert.ok(joined.includes(fwName), 'UN-WIRED names the fixture framework surface');
}

function testFixtureNotInLiveWalk() {
  // The fixture is NOT picked up by the live generator's source walk, so the live
  // repo --check stays green. listSourceFiles() walks commands/ + skills/ +
  // agents/ ONLY; the fixture lives under tests/fixtures/.
  const fwName = fixtureFrameworkName();
  const live = build();
  const liveNames = new Set(live.nodes.map((n) => n.name));
  assert.ok(!liveNames.has(fwName), 'the fixture framework is absent from the live projection');
  // And the live projection has zero UN-WIRED errors (the gate is not a
  // false-positive generator on the real repo).
  const res = gen.validateProjection(live);
  assert.strictEqual(res.unwired.length, 0, 'the live (fully-wired) projection passes UN-WIRED');
}

function testFullyWiredProjectionPasses() {
  // A fully-wired synthesized projection (a framework reached by a command that
  // declares a frozen reach_id) yields ZERO UN-WIRED errors -- proving the gate
  // is not a false-positive generator.
  const fwName = 'Fully Wired Probe Framework';
  const proj = {
    ontology_ref: 'synthetic',
    generated_note: 'synthetic',
    chain_layer_note: 'synthetic',
    nodes: [
      { id: 'framework:' + fwName, kind: 'framework', methodology_tier: 'pws', name: fwName },
      {
        id: 'command:/mos:probe', kind: 'command', methodology_tier: 'mindrian-operation',
        name: '/mos:probe', reach_id: 'context_block', hierarchy_rank: 99, posture: 'hold',
      },
    ],
    edges: [
      { type: 'OPERATES', from: 'command:/mos:probe', to: 'framework:' + fwName },
    ],
  };
  const res = gen.validateProjection(proj);
  assert.strictEqual(res.unwired.length, 0, 'a fully-wired framework yields zero UN-WIRED errors');
}
...
// ---------------------------------------------------------------------------
// Phase 157-05 (BOG-09 / BOG-10) -- the Part 8 boundary-scan assertions, added
// as a group to the bash suite. The full adversarial scan (with the
// planted-secret RED/GREEN proof) lives in
// tests/test-orchestration-projection-part8-boundary.cjs; these are the same
// invariants asserted from the bash suite so a regression is caught here too.
// ---------------------------------------------------------------------------

// The forbidden-value heuristic, mirrored from the standalone scan: a node/edge
// VALUE that looks like user content (a room/ path, an email, or over-cap free
// text). Returns a reason string or '' when clean.
const BOUNDARY_FREE_TEXT_CAP = 120;
const BOUNDARY_ROOM_RX = /(^|[\/\s])room\/[A-Za-z0-9._-]/;
const BOUNDARY_EMAIL_RX = /[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;
function boundaryForbiddenReason(v) {
  if (typeof v !== 'string') return '';
  if (BOUNDARY_ROOM_RX.test(v)) return 'room/ path';
  if (BOUNDARY_EMAIL_RX.test(v)) return 'email';
  if (v.length > BOUNDARY_FREE_TEXT_CAP) return 'over-cap free text';
  return '';
}
function boundaryCollectKeys(value, acc) {
  const set = acc || new Set();
  if (Array.isArray(value)) {
    for (const v of value) boundaryCollectKeys(v, set);
  } else if (value && typeof value === 'object') {
    for (const k of Object.keys(value)) {
      set.add(k);
      boundaryCollectKeys(value[k], set);
    }
  }
  return set;
}
function boundaryWalkStrings(value, visit) {
  if (typeof value === 'string') { visit(value); return; }
  if (Array.isArray(value)) { for (const v of value) boundaryWalkStrings(v, visit); return; }
  if (value && typeof value === 'object') {
    for (const k of Object.keys(value)) boundaryWalkStrings(value[k], visit);
  }
}

function testBoundaryFieldAllowlist() {
  // Every node key (incl. sub-object keys) is in NODE_FIELD_ALLOWLIST; every
  // edge key is in EDGE_FIELD_ALLOWLIST (BOG-10 zero user-content fields).
  assert.ok(Array.isArray(gen.NODE_FIELD_ALLOWLIST), 'NODE_FIELD_ALLOWLIST exported');
  assert.ok(Array.isArray(gen.EDGE_FIELD_ALLOWLIST), 'EDGE_FIELD_ALLOWLIST exported');
  const nodeAllow = new Set(gen.NODE_FIELD_ALLOWLIST);
  const edgeAllow = new Set(gen.EDGE_FIELD_ALLOWLIST);
  const p = build();
  for (const n of p.nodes) {
    for (const k of boundaryCollectKeys(n)) {
      assert.ok(nodeAllow.has(k), 'node ' + n.id + ' off-allowlist field "' + k + '"');
    }
  }
  for (const e of p.edges) {
    for (const k of Object.keys(e)) {
      assert.ok(edgeAllow.has(k), 'edge off-allowlist field "' + k + '"');
    }
  }
}

function testBoundaryTierOnEveryNode() {
  // methodology_tier present + legal on EVERY node (the boundary-keeper).
  const p = build();
  for (const n of p.nodes) {
    assert.ok(TIERS.has(n.methodology_tier),
      'node ' + n.id + ' lacks a legal methodology_tier');
  }
}

function testBoundaryPlantedSecretCaught() {
  // The heuristic catches a planted room/ path, email, and over-cap body (RED),
  // and the real per-surface (nodes+edges) data is clean (GREEN).
  assert.ok(boundaryForbiddenReason('room/problem/foo.md') !== '', 'planted room/ path caught');
  assert.ok(boundaryForbiddenReason('a@b.com') !== '', 'planted email caught');
  assert.ok(boundaryForbiddenReason('x'.repeat(BOUNDARY_FREE_TEXT_CAP + 1)) !== '', 'planted over-cap caught');
  const p = build();
  const hits = [];
  boundaryWalkStrings(p.nodes, (s) => { if (boundaryForbiddenReason(s)) hits.push(s.slice(0, 40)); });
  boundaryWalkStrings(p.edges, (s) => { if (boundaryForbiddenReason(s)) hits.push(s.slice(0, 40)); });
  assert.equal(hits.length, 0, 'real nodes/edges carry forbidden value(s): ' + hits.join(', '));
}

function testBoundaryZeroLiveBrain() {
  // The generator code (comments stripped) carries no brain-client require and
  // no fetch/http/curl/brain.query/brain.write call (BOG-09).
  const raw = fs.readFileSync(GEN_PATH, 'utf8');
  const code = raw.split('\n').filter((ln) => {
    const t = ln.trim();
    return !(t.startsWith('//') || t.startsWith('*') || t.startsWith('/*'));
  }).join('\n');
  assert.equal(/require\s*\(\s*[^)]*brain-client[^)]*\)/.test(code), false, 'no brain-client require');
  for (const rx of [/\bfetch\s*\(/, /\bhttps?\.(get|request)\s*\(/, /\bcurl\b/, /brain\s*\.\s*query\s*\(/, /brain\s*\.\s*write\s*\(/]) {
    assert.equal(rx.test(code), false, 'forbidden live-Brain/network call ' + rx);
  }
}
...
function run() {
  const tests = [
    ...
    ['chain-layer-materialized-note', testChainLayerMaterializedNote],
    ...
    ['fixture-exists', testFixtureExists],
    ['fixture-flags-unwired-red', testFixtureFlagsUnwiredRed],
    ['fixture-not-in-live-walk', testFixtureNotInLiveWalk],
    ['fully-wired-projection-passes', testFullyWiredProjectionPasses],
    ['check-tokens-in-source', testCheckTokensInSource],
    ['no-brain-client-require', testNoBrainClientRequire],
    // Phase 157-05 Part 8 boundary-scan group (BOG-09 / BOG-10).
    ['boundary-field-allowlist', testBoundaryFieldAllowlist],
    ['boundary-tier-on-every-node', testBoundaryTierOnEveryNode],
    ['boundary-planted-secret-caught', testBoundaryPlantedSecretCaught],
    ['boundary-zero-live-brain', testBoundaryZeroLiveBrain],
