...
// ---------------------------------------------------------------------------
// 4. Part 8: correlation_id is a LOCKED bare-16-hex hash scalar, not user content
// ---------------------------------------------------------------------------
test('Part 8: correlation_id is the locked bare-16-hex hash scalar, not raw user content', function () {
  const roomDir = freshRoomDir();
  spineEvents.logSuggestionSurfaced(roomDir, {
    surface: 'suggest-next',
    commands: [{ command: 'mos:diagnose', score: 0.9 }],
  ...
  // LOCKED contract of record: bare 16-char lowercase hex, NO 'c1:' version
  // prefix. sha256(utf8(name + '|' + label)).slice(0,16). Byte-identical to the
  // ids already on the live teaching graph.
  assert.match(props.correlation_id, /^[0-9a-f]{16}$/, 'correlation_id is bare 16-char lowercase hex (no c1: prefix)');
  // Value-anchor: it equals the shared chokepoint over the same (name,label).
  const expected = correlation.computeCorrelationId('Beautiful Question Framework', 'Framework');
  assert.strictEqual(props.correlation_id, expected, 'correlation_id agrees with the shared chokepoint by construction');
  assert.strictEqual(expected, '489f01a5417dec3c', 'the locked value for Beautiful Question Framework|Framework');
