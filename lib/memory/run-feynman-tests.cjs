...
const TEST_FILES = [
  ...
  // Phase 241-02: minto-debounce-consumer-census. Closes finding F-0
  // (.planning/debug/resolved/minto-debounce-consumer-dead-end.md), which
  // corrects the RCA's own methodology error: an extension-scoped grep
  // (scripts/intent-classifier.cjs) missed the real, extensionless
  // UserPromptSubmit consumer (scripts/intent-classifier). 5 tests: a
  // production census that walks directories (not extensions) proving a
  // real enqueue producer and a real drain-and-act consumer exist, a
  // vacuum-ban gate (zero production sites drain with a zero age floor),
  // a reachability check (the consumer is genuinely dispatched from
  // hooks/hooks.json, not merely wired), the full enqueue-to-drain-to-act
  // cycle (RCA Test 2), and that cycle's mutation proof (restoring the
  // retired vacuum breaks it; the real fix completes it).
  path.join(REPO_ROOT, 'lib', 'memory', 'minto-debounce-consumer-census.test.cjs'),
  ...
  // Closes finding F-1: the on-stop invariant report was discarded twice
  // over (a >/dev/null redirect, and a 1-second hard kill). 4 tests: LEG A
  // (systemMessage reaches scripts/on-stop's FINAL stdout JSON line) + its
  // mutation proof (restoring the discard turns it red), LEG B (an
  // injected slow validator still lands invariant-report.json and prunes
  // a ghost stale.json entry) + its mutation proof (restoring timeout 1
  // turns it red).
  path.join(REPO_ROOT, 'lib', 'memory', 'guardian-onstop-reaches-user.test.cjs'),
  // Phase 241-04: real-commit proof for the pre-commit guardian demotion
  // (SC3, F-3). Drives an actual `git commit` in a scratch repo against the
  // canonical installed hook (scripts/hooks/pre-commit-room-minto-guard.sh),
  // never runPreCommit() as a function. 4 tests: the guard actually fires
  // (anti-vacuity), advisory default lands a commit with enumerated
  // violations + a counted WARN, MINTO_PRECOMMIT_STRICT=1 rejects the
  // commit and nothing lands, a clean room still commits with zero
  // violation output (false-positive guard). GUARDIAN_PRECOMMIT_STAGED is
  // never assigned a path; the staged set always comes from a real
  // `git diff --cached`.
  path.join(REPO_ROOT, 'lib', 'memory', 'precommit-real-commit.test.cjs'),
  ...
  // Phase 232.1 Plan-01 gates (the room-graph density read):
  //   scenarios 1-3 -> the D-04 read-only door. Scenario 2 is the load-bearing
  //     one: a byte-identical sqlite_master + mtimeMs regression pin proving
  //     openRoomDbReadOnlyForCaller never mutates the room.db it measures. The
  //     older openRoomDbForCaller runs 13 CREATE TABLE IF NOT EXISTS
  //     statements plus 5 migrations on EVERY open, so a census through it
  //     would silently migrate every registered room on first run
  //     (232.1-RESEARCH.md Pitfall 1, reproduced empirically).
  //   scenarios 4-8 -> the room-graph-density registry module: D-01/D-02 (one
  //     entry, zero engine change), D-03 (registry walk), D-05 (a room with no
  //     room.db, and a schema-less room.db, both report 0/0 and never throw),
  //     T-232.1-02 (a corrupt registry entry soft-fails that ONE room), and
  //     D-06 (a machine-checkable forbidden-word guard over the rendered
  //     detail: a count is a measurement, never a health claim).
  path.join(REPO_ROOT, 'tests', 'test-232.1-room-graph-density.cjs'),
  //   Plan-02 (D-07, the room_state fold): 6 scenarios over roomDensityLine
  //     and its wiring into the status / get-state read branches. Scenario 3
  //     pins that the line reads the per-call activeRoomDir, never the frozen
  //     boot-time roomDir closure (the intern-w1-room-state-false-empty bug
  //     class); scenario 6 re-pins non-mutation through the ACTUAL MCP-facing
  //     path, against a room.db deliberately behind the migration chain so a
  //     wrong-door open is observable. Both were mutation-tested to confirm
  //     they fail when the wiring is wrong.
  path.join(REPO_ROOT, 'tests', 'test-232.1-room-state-density.cjs'),
  // Phase 233 Plan-01 gates (the graph-derive drain residual, RCA 4c + 4d):
  //   test-233-graph-derive-health -> 12 scenarios over the ONE shared
  //     detection signal and the class it feeds. Scenarios 1-2 are the
  //     load-bearing pair: a room with BELONGS_TO edges and ZERO cascade edges
  //     (the ~16-damaged-room shape) must report fail, and the same room with
  //     one cascade edge must report ok. Both are asserted against a REAL
  //     room.db seeded through the repo's own writers, never a mocked object,
  //     because this phase exists to close a false-success class (T-233-04).
  //     Scenarios 10-11 spawn doctor.cjs for real and then read the room's
  //     actual .mindrian/graph-derive-queue.json off disk, byte-comparing it
  //     across two --heal-room runs to pin idempotence.
  //   test-233-graph-derive-heal-retrofit -> 8 scenarios over the cadence:once
  //     auto-repair and the Tri-Polar nudge. Scenario 1 proves the retrofit
  //     touches ONLY what is broken (the healthy room's queue file is compared
  //     byte-for-byte before and after); scenario 5 pins the fix_supported:false
  //     export parity; scenarios 6-8 pin the Desktop/Cowork nudge as plain
  //     language with zero JSON or table characters.
  path.join(REPO_ROOT, 'tests', 'test-233-graph-derive-health.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-233-graph-derive-heal-retrofit.cjs'),
  // Phase 233 Plan-02 gates (the same RCA, items 4b + 4e):
  //   test-233-derivation-default-gate -> 10 scenarios over runDerivation's
  //     default deriveFn. The load-bearing one is scenario 2: it spies the REAL
  //     cached graph-candidate-producer exports object, so "the dead hosted
  //     transport was never reached" is a MEASURED call count of 0, not an
  //     inference from reading the control flow. Scenarios 7-8 pin that every
  //     injected-deriveFn caller (the drain, the backfill, every existing test)
  //     is untouched by either env var.
  //   test-233-drain-backfill-producer-parity -> 14 scenarios proving the
  //     headless drain and the in-session /mos:graph --derive backfill share ONE
  //     default producer. Proof A is source-level with the comment-stripping
  //     grep-gate-hygiene idiom (and A4 proves the stripper is honest rather
  //     than vacuous); proof B is LIVE: the shared classifier export is swapped
  //     for a recorder, both modules run their real default path over two
  //     byte-identical fixture rooms, and the resulting cascade edges are read
  //     back off both room.db files and compared as sets. A future edit that
  //     points one trigger at a different producer fails B1 or B2.
  path.join(REPO_ROOT, 'tests', 'test-233-derivation-default-gate.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-233-drain-backfill-producer-parity.cjs'),
  ...
  // Quick 260723-ad9: the WRITE half of the Phase 94-01 contract. room-registry
  // create/set-active + compute-state populate and preserve current_room.
  path.join(REPO_ROOT, 'lib', 'memory', 'statusline-active-room-write.test.cjs'),
  ...
  // Phase 223 (jtbd intelligence pipeline + governed double-fan bono): two
  // born-wired surfaces sharing one close-the-loop graph-write spine. The five
  // per-plan proof legs the tests/run-all-223.sh aggregator also gates
  // (223-VALIDATION test-infrastructure contract):
  //   hat-governance   -> Req 1 governed seams + heterogeneity + CR-01 no-thenable
  //   part8-egress     -> persona research SIGNAL->LOCAL, egress-guard gated
  //   close-loop       -> Req 4 D-01 dual store (.md first) + D-02 proposed edges
  //   supersedes-chain -> Req 2 walker + D-04 NULL review_status
  //   intel-pipeline   -> Req 3 dry-run + 3 gate halts + quality-low HALT
  path.join(REPO_ROOT, 'tests', 'test-223-hat-governance.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-223-part8-egress.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-223-close-loop.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-223-supersedes-chain.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-223-intel-pipeline.cjs'),
  ...
  // Phase 225 (zero-score no-match gate + WAL advisory): gap-closure on Phase
  // 194's shipped substrate. The three proof legs the tests/run-all-225.sh
  // aggregator also gates (SKIP-safe via run_if):
  //   zero-score-gate -> REQ-1/REQ-2 proving_case_2 FIRE + 4 legitimate-silence
  //     legs + consumer-compatibility trace proof (never reuses corpus[0] best.name)
  //   gate-degrade    -> REQ-3 poisoned binding/trace -> exit-0 silence (83-07 never-block)
  //   wal-advisory    -> REQ-4 doctor never-block WARN on SQLite<3.51.3 + live co-session
  path.join(REPO_ROOT, 'tests', 'test-225-zero-score-gate.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-225-gate-degrade.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-225-wal-advisory.cjs'),
  // CR-01 fix regression (code review, post-225-03): the zero-score gate's default
  // answer must union with, never replace, a session's prior multi-room bound set.
  path.join(REPO_ROOT, 'tests', 'test-225-answer-narrowing.cjs'),
  // Phase 226 (eureka reasoning-mode fallback, SEED-058): the encoder-free
  // /mos:eureka fallback. The eight D1-D8 legs the tests/run-all-226.sh aggregator
  // also gates (SKIP-safe via run_if), registered here per the 224-VALIDATION
  // test-infra contract so the D1 fabricated-number tripwire and the Part 9
  // banked-false posture check are permanent, not one-time review:
  //   null-legs        -> D1/G-1 REQ-1 fabricated-number prohibition (the hardest gate)
  //   rubric-parity    -> D3/G-2 REQ-2 analogy bar not relaxed encoder-free
  //   field-contract   -> D4/G-5 REQ-3 four-key byte-parity output shape
  //   degrade-cause    -> D7 REQ-6/8 honest cause named, no speculative trigger
  //   mode-disclosure  -> D6/G-4 REQ-5 mode label across md + json + html
  //   posture          -> D5/G-3 REQ-4 banked-false + zero opportunity nodes + upgrade delta
  //   pair-cap         -> D8/G-6 REQ-7 bounded fan-out end to end
  //   rejection-replay -> D3 REQ-2 the 212 junk classes stay rejected encoder-free
  path.join(REPO_ROOT, 'tests', 'test-226-null-legs.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-rubric-parity.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-field-contract.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-degrade-cause.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-mode-disclosure.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-posture.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-pair-cap.cjs'),
  path.join(REPO_ROOT, 'tests', 'test-226-rejection-replay.cjs'),
  // Phase 227 Plan-01, Requirement 1 (session-start mode-selection Decision
  // Gate firing checkpoint): a scripted doctor check now detects a silent
  // lane-pick skip. Covers the warn path (silent skip), the ok path (normal
  // recording), the D-05 turn-1 false-positive guard, never-throw on a
  // corrupt store, and the doctor-modules.json registry contract.
  path.join(REPO_ROOT, 'tests', 'test-227-mode-select-checkpoint.cjs'),
  // Phase 227 Plan-05, Requirement 3 (frontdoor-restraint regression floor):
  // four structural/static assertions replaying ignite-frontdoor-bypassed-
  // methodology-overfire.md's Test 4 scenario (an explore-invitation in a
  // fresh context), proving the 2026-06-24 fix (commit 7868dfbb) still
  // holds without a live human tester session.
  path.join(REPO_ROOT, 'tests', 'test-227-frontdoor-restraint.cjs'),
];
