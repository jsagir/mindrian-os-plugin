...
const TEST_FILES = [
  ...
  // Phase 102-00 Wave-0: 5 render-v2 contract stubs reserved for sibling
  // Plan 102-01 (lib/render/render-v2.cjs) to promote into real regression
  // fences. Each stub exits 0 today so the feynman ledger advances on this
  // commit; Plan 102-01 swaps in the assertion bodies WITHOUT changing the
  // registered paths below, so this list stays byte-stable across the
  // 102-00 -> 102-01 wave transition.
  //
  // Mapping to RENDER-102-* requirement IDs (.planning/REQUIREMENTS.md):
  //   test-render-v2-signature.cjs     -> RENDER-102-01 import surface
  //   test-render-v2-compaction.cjs    -> RENDER-102-02 operator-aware
  //                                        compaction (5 operators ->
  //                                        5 shape regimes per Canon Part 3)
  //   test-render-v2-jtbd-zone4.cjs    -> RENDER-102-03 JTBD-aware Zone 4
  //                                        (closed 10-verb vocabulary)
  //   test-render-v2-provenance.cjs    -> RENDER-102-04 _provenance envelope
  //                                        (LOCAL-only per Canon Part 8)
  //   test-render-v2-color-overlay.cjs -> RENDER-102-05 De Stijl 5-color
  //                                        overlay (cli-only; strip-ANSI
