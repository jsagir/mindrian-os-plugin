...
async function scenario5IndustrySignal() {
  const m = require('../core/rs-preprocessor.cjs');
  // Signal field carries rich domain-overlapping content so the boundary_flag
  // resolves to false; the test verifies that the signal field is mapped onto
  // the same preprocessing path as abstract (papers/patents both use abstract
  // while industry signals use signal -- per CONTEXT.md fetcher output shapes).
  const docs = [{
    id: 'sig-only-1',
    signal: 'CRISPR startup raised funding for cancer immunotherapy biomarker checkpoint inhibitor response platform development',
    source: 'tavily',
    fetched_at: '2026-04-26T00:00:00Z',
  }];
  const out = m.preprocess(docs, { domain_analysis: makeDomainAnalysis() });
  ...
  // All 6 da concepts (cancer, immunotherapy, biomarker, checkpoint, inhibitor,
  // response) are overlapping with the signal text -> overlap == 6/6 == 1.0.
  assert.ok(rec.concepts.length >= 5,
    'concept overlap >= 5 for full-domain signal; got ' + rec.concepts.length);
  assert.equal(rec.boundary_flag, false,
    'boundary_flag false (industry signal mapped onto same preprocessing path)');
}
