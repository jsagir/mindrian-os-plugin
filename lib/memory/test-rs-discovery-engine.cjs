...
function makeInvocationCounters() {
  return {
    ...
    queryMatrix: 0,
    fetcherAcademic: 0,
    // Phase 130.5-03: the academic corpus fetch is migrated onto the shared
    // fetchCorpus + research-cache substrate. fetcherAcademic.fetchAcademic is no
    // longer the engine's call site; fetchCorpus is. These counters track the
    // migrated path (kept fetcherAcademic for the suite-wide shape, now 0).
    fetchCorpus: 0,
    researchCacheGet: 0,
    researchCachePut: 0,
    fetcherPatents: 0,
...
function buildMockSet(opts) {
  ...
  const mocks = {
    ...
    // Phase 130.5-03: the migrated academic corpus path. fetchCorpus returns the
    // normalized results array directly (NOT an envelope) per the Plan 01
    // contract; the engine's fetchAcademicViaCorpus wraps it back into the
    // { papers, telemetry } envelope downstream consumers expect. researchCache
    // is a deterministic no-op stub (getCached -> null = miss, putCached -> path)
    // so the existing suite stays network-free.
    fetchCorpus: async function (args) {
      counts.fetchCorpus += 1;
      return [
        {
          id: 'oa:123',
          title: 'NV-center diamond magnetometry',
    ...
    researchCache: {
      getCached: function (roomDir, source, query) {
        counts.researchCacheGet += 1;
        return null; // miss -> engine falls through to fetchCorpus
      },
      putCached: function (roomDir, source, query, results) {
        counts.researchCachePut += 1;
        return '/tmp/test-room/.mindrian/research-cache/' + source + '.json';
      },
    },
    fetcherPatents: {
...
record('T1 happy path: full pipeline with mocked 89.1-89.5 modules; bundle shape verified', async function () {
  ...
  // Phase 130.5-03: the academic corpus fetch is migrated onto fetchCorpus +
  // the shared research-cache. The private fetcherAcademic.fetchAcademic path is
  // gone, so we assert the MIGRATED dependency was invoked (fetchCorpus on a
  // cache miss) and that the cache wrap engaged (getCached + putCached), not the
  // deprecated fetcher.
  assert.ok(counts.fetchCorpus >= 1, 'fetchCorpus must be invoked (migrated academic corpus path)');
  assert.ok(counts.researchCacheGet >= 1, 'researchCache.getCached must be invoked (cache wrap engaged)');
  assert.ok(counts.researchCachePut >= 1, 'researchCache.putCached must be invoked on a cache miss');
  assert.equal(counts.fetcherAcademic, 0, 'deprecated fetcherAcademic.fetchAcademic path must NOT be invoked');
...
record('T5 Canon Part 8 input audit: forbidden bytes in opts -> ExternalEgressViolation BEFORE any module invoked', async function () {
  ...
  assert.equal(counts.fetchCorpus, 0, 'fetchCorpus must NOT be invoked after audit throw (pre-egress fail-closed)');
...
record('T7 pause-state bubble: lookupUpstream pause -> pause envelope returned; NO further phase modules invoked', async function () {
  ...
  assert.equal(counts.fetchCorpus, 0, 'fetchCorpus must NOT run after pause');
