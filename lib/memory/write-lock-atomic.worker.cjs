...
try {
  ...
  //
  // Winner sleep duration tuned for the release-gate stability floor
  // (Phase 87-10). Under heavy WSL2 load, the original 500ms was too
  // short -- a slow-starting loser could complete its liveness check
  // AFTER the winner had already exited, see the dead-PID cleanup
  // path unlink the lock, and re-acquire as a second "winner".
  // Observed flake rate under 22-test suite parallelism: ~40%.
  // 3000ms is 6x headroom: empirically stable across 10+ consecutive
  // runs under the same load profile that produced the flake.
