...
async function main() {
  ...
  // Test 15: atomic write tmpfile naming.
  await runAsync('Test 15: tmpfile follows 88-04-B pattern (BRAIN.md.tmp.<rand>.brain)', async function () {
    installMockBrainClient({});
    const { sectionPath } = buildFixtureSection({});
    // Watch the section directory for tmp files by slowing the Brain client.
    const modPath = require.resolve('../core/brain-client.cjs');
    const cached = require.cache[modPath].exports;
    const origQuery = cached.query;
    const tmpSightings = [];
    cached.query = async function (cy) {
      // Snapshot section dir every call.
      const names = fs.readdirSync(sectionPath);
      for (const n of names) {
        if (n.startsWith('BRAIN.md.tmp') && n.endsWith('.brain')) {
          tmpSightings.push(n);
        }
      }
      return origQuery.call(this, cy);
    };
    const { deriveSection } = require('../core/brain-derivation.cjs');
    const r = await deriveSection(path.dirname(sectionPath), path.basename(sectionPath), {});
    assert.equal(r.success, true);
    // Tmpfile must have been observable during the flow (after first Brain query).
    const pattern = /^BRAIN\.md\.tmp\.[a-z0-9]+\.brain$/;
    const sighted = tmpSightings.find(n => pattern.test(n));
    assert.ok(sighted, 'expected tmpfile matching /BRAIN.md.tmp.<rand>.brain/; saw: ' + JSON.stringify(tmpSightings));
  });
