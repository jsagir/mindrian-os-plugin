...
function fmtStateLine(state, reasoning) {
  ...
  // Derive MINTO health glyph from reasoning_health_score
  let health = '--';
  if (reasoning && typeof reasoning.reasoning_health_score === 'number') {
    if (reasoning.reasoning_health_score >= 0.7) health = 'check';
    else if (reasoning.reasoning_health_score >= 0.4) health = 'warn';
    else health = 'low';
  }
