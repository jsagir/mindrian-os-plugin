...
function loadDispatcherWithMocks(opts) {
  ...
  require.cache[bcPath] = {
    ...
    exports: {
      ...
      // brain-derivation.cjs reads _test.sanitizeCypherInput at module load; the
      // mock must expose it (the curation modes never call deriveSection, but the
      // dispatcher requires brain-derivation at the top level). Identity is safe
      // for the load-time read.
      _test: { sanitizeCypherInput: function (s) { return String(s == null ? '' : s); } },
    },
  };
