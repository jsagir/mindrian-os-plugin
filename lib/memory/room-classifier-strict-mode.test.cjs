...
// The strict-mode detector is exported from scripts/intent-classifier.cjs
// when the module is required (i.e. require.main !== module). This lets
// the unit tests T1, T2, T3, T5 call detectStrictMode directly without
// the cost of a child-process spawn.
function loadClassifierModule() {
  // Clear require cache so a previous load doesn't leak STDIN_RAW from
  // the readStdinSync call. The classifier is designed to be required
  // safely by guarding the top-level execution behind require.main check.
  delete require.cache[CLASSIFIER];
