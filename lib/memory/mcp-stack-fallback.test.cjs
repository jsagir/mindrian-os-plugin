...
async function test_5_envelope_shape_uniformity() {
  ...
  // return tier='derived' source='derived' with experts[] preserved.
  {
    ...
    // mapExperts may return either an envelope or a bare array depending
    // on whether the wrap is shipped. T5 asserts the wrap IS shipped.
    if (Array.isArray(env)) {
      throw new Error('T5/experts: mapExperts returned bare array; expected envelope');
    }
