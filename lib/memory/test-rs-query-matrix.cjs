...
async function main() {
  ...
  await runScenario("T1: Happy path -- 60 queries from 7-field analysis", async () => {
    ...
    // Every query mentions either A or B (most mention both).
    let aMentions = 0;
    let bMentions = 0;
    for (const cat of CATEGORY_NAMES) {
      for (const q of out.categories[cat]) {
        if (q.indexOf('reverse-salient-framework') !== -1) aMentions += 1;
        if (q.indexOf('systems-thinking') !== -1) bMentions += 1;
      }
    }
    assert.ok(aMentions >= 40,
      'T1: A (reverse-salient-framework) appears in too few queries: ' + aMentions);
    assert.ok(bMentions >= 40,
      'T1: B (systems-thinking) appears in too few queries: ' + bMentions);
