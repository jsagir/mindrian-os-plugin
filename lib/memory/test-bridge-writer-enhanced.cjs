...
async function main() {
  ...
  await runScenario('8-adv-CANON-PART-8-breakthrough-leak', async () => {
    ...
    // not on a scrubbed subset. We use a different forbidden pattern
    // than Test 7 (Test 7 uses 'meeting with'+'Lawrence said'; this
    // test uses an SSN-like sequence) to prove multiple FORBIDDEN_
    // PATTERNS regexes all trip the chokepoint.
    pair.thesis = 'By applying X to Y, achieve Z (priority code 123-45-6789)';
