...
// ---------- Test 8b/8c: strict-mode false room-switch / binding-gate fixes ----------
//
// 2026-06-28 regression fences for the two confirmed false-positive classes:
//   Case 1 -- a bare numeric menu reply ("2") must NOT trip the strict-mode
//             room-switch override (only an explicit "switch to 2" may).
//   Case 2 -- a product-branded paste block (mindrianOS / Larry / room / mindrian)
//             must NOT trip the F.8 session-binding gate; a genuine room-name
//             mention still must.
// Both build the SAME object-form 2-room fixture as Test 8 (rooms keyed by slug
// so the similarity fingerprint loads each room's STATE.md by slug dir).

function buildStrictModeFixture(prefix, betaStateBody) {
  const home = mkTmp(prefix);
  const roomsHome = path.join(home, 'MindrianRooms');
  fs.mkdirSync(path.join(roomsHome, '.rooms'), { recursive: true });
  const bodies = {
    'alpha-venture': '',
    'beta-venture': betaStateBody || '',
  };
  for (const name of ['alpha-venture', 'beta-venture']) {
    const rd = path.join(roomsHome, name);
    fs.mkdirSync(path.join(rd, '.mindrian'), { recursive: true });
    fs.writeFileSync(path.join(rd, '.room-root'), '');
    fs.writeFileSync(
  ...
  // Object-form registry: Object.keys order = 1-indexed numeric position
  // (alpha-venture = 1, beta-venture = 2). active = alpha-venture.
  fs.writeFileSync(
    path.join(roomsHome, '.rooms', 'registry.json'),
    JSON.stringify({
      version: 2,
      active: 'alpha-venture',
      rooms: {
        'alpha-venture': { path: 'alpha-venture', status: 'active' },
        'beta-venture': { path: 'beta-venture', status: 'active' },
      },
    })
  );
  return { home, roomsHome, roomDir: path.join(roomsHome, 'alpha-venture'), firstSection: null };
}

gated('Test 8b: bare numeric menu reply does NOT trip strict-mode room-switch (Case 1)', () => {
  const fix = buildStrictModeFixture('t8b-numeric');
  // A bare "2" resolves to beta-venture (position 2) != active alpha-venture.
  // Pre-fix this fired a strict-mode override; post-fix it must be silent.
  const bare = runHook({ user_message: '2' }, fix);
  assert.equal(bare.status, 0);
  assert.ok(
    bare.stdout.indexOf('Strict-mode override') === -1,
    'bare "2" must NOT emit a strict-mode override; stdout=' + bare.stdout.slice(0, 400)
  );
  // An explicit "switch to 2" MUST still resolve via strict-mode.
  const explicit = runHook({ user_message: 'switch to 2' }, fix);
  assert.equal(explicit.status, 0);
  assert.ok(
    explicit.stdout.indexOf('Strict-mode override') !== -1,
    'explicit "switch to 2" must still emit a strict-mode override; stdout=' + explicit.stdout.slice(0, 400)
  );
});

gated('Test 8c: product-branded paste block does NOT trip the binding gate (Case 2)', () => {
  // beta-venture STATE.md body carries only product/brand/boilerplate tokens.
  // Capitalized 2+ word phrases become fingerprint entity tokens.
  const fix = buildStrictModeFixture(
    't8c-brand',
    'The Mindrian Larry teaching room. Onboarding Room Block here.'
  );
  // A paste block that only mentions the product brand must be silent: no
  // binding gate, no intent-mismatch warning.
  const brand = runHook(
    { user_message: 'Here is a paste block from mindrianOS onboarding. Larry room mindrian block.' },
    fix
  );
  assert.equal(brand.status, 0);
  for (const marker of ['bind session', 'Intent mismatch', 'intent mismatch', 'session unbound']) {
    assert.ok(
      brand.stdout.indexOf(marker) === -1,
      'brand paste must NOT emit "' + marker + '"; stdout=' + brand.stdout.slice(0, 500)
    );
  }
  // A genuine room-name mention still fires the binding gate.
  const genuine = runHook(
    { user_message: "let's work on beta venture strategy today" },
    fix
  );
  assert.equal(genuine.status, 0);
  assert.ok(
    genuine.stdout.indexOf('bind session') !== -1
      || genuine.stdout.indexOf('Intent mismatch') !== -1,
    'genuine room-name mention must still surface a room-scope prompt; stdout='
      + genuine.stdout.slice(0, 500)
  );
});
