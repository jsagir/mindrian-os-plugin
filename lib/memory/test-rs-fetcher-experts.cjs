...
async function scenario12AdvForbiddenInPaperIdOrSource() {
  const m = require('../core/rs-fetcher-experts.cjs');
  // CANON PART 8 adversarial: FORBIDDEN_PATTERNS[0] matches email pattern.
  // Inject forbidden patterns into BOTH paper.id and paper.source. Even
  // though neither is part of the 6-field expert contract, defense-in-depth
  // pre-return audit must catch any path that smuggles these strings into
  // output (via accidental promotion to a helper field, etc.).
