...
if (classifierIntegrated()) {
  ...
  run('Test 18: integration -- cold room (resolvable, engine silent) -> routing_source=legacy (NAV-01 honest negative)', () => {
    // NAV-01 COLD-ROOM HONEST NEGATIVE. This is the CANONICAL negative case for
    // the legacy->engine flip: the room RESOLVES (the fixed makeRoomsFixture
    // gives a real {slug, abs_path} the resolver accepts, so the engine block
    // at intent-classifier.cjs:1369 actually RUNS), but no reach fires and no
    // canonical-verb fire_skill is produced, so routing_source STAYS legacy.
    // A legacy trace here is CORRECT, not a failure -- it proves the flip is a
    // pure consequence of a fired reach, never an unconditional engine label.
    // Distinct from Test 17: Test 17 forces a null fire_skill via the
    // __NULL__ stub on a populated room; Test 18 asserts that a resolvable
    // room with the engine genuinely SILENT (no sensor signal) also degrades
    // honestly to legacy.
    const fix = makeRoomsFixture('t18');
    try {
      const res = runHook(fix.root, {
        // Engine silent: no fire_skill, no sensor signal -> honest negative.
        MOS_NAV_TEST_FIRE_SKILL: '__NULL__',
      });
      assert.equal(res.status === 0, true, 'classifier exit ' + res.status);
      const out = res.stdout || '';
      assert.equal(
        out.indexOf('routing_source: legacy') !== -1,
        true,
        'expected routing_source: legacy (cold-room honest negative); got: ' + out
      );
    } finally {
      fs.rmSync(fix.tmp, { recursive: true, force: true });
    }
  ...
  process.stdout.write(
    'skip Tests 16-18 (intent-classifier.cjs not yet integrated with router; gate clears in Task 2)\n'
  );
}
