 ...
 *
 *   Test 1  PINECONE_API_KEY absent -> graceful degradation
 *           returns {success: false, error: 'pinecone_api_key_missing'}
 *           does NOT throw.
...
async function scenario1ApiKeyAbsent() {
  ...
  try {
    ...
    assert.equal(threw, null, 'queryPineconeWithVectors must NEVER throw on missing key; got ' + (threw && threw.message));
    assert.ok(result, 'result returned');
    assert.equal(result.success, false, 'success false on missing key');
    assert.equal(result.error, 'pinecone_api_key_missing', 'error tag matches; got ' + result.error);
    captureResult('1-api-key-absent', result);
  } finally {
    if (prevKey !== undefined) process.env.PINECONE_API_KEY = prevKey;
  }
}

async function scenario2PythonAbsent() {
  // Set key so we get past the env-var gate.
  const prevKey = process.env.PINECONE_API_KEY;
  process.env.PINECONE_API_KEY = 'test-key-not-real';
...
async function scenario3HappyPath() {
  ...
  try {
    const m = require('../core/rs-pinecone-bridge.cjs');
    const result = await m.queryPineconeWithVectors('external:test-ns', 'cancer biology', 10);
    assert.ok(result, 'result returned');
...
(async function main() {
  ...
  await runScenario('1-api-key-absent', scenario1ApiKeyAbsent);
