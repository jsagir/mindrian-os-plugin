...
test('Test 14: hooks.json SessionStart array has length === 1 (the coordinator)', () => {
  const hooks = require(path.join(REPO_ROOT, 'hooks', 'hooks.json'));
  ok(hooks && hooks.hooks && Array.isArray(hooks.hooks.SessionStart));
  // After Task 2: length === 1. Before Task 2: skipped (Task 1 stage).
  if (!fs.existsSync(COORDINATOR_PATH)) return;
  equal(hooks.hooks.SessionStart.length, 1, 'exactly one SessionStart entry');
  const entry = hooks.hooks.SessionStart[0];
  ok(entry.hooks && entry.hooks.length > 0);
  const cmd = entry.hooks[0].command || '';
  ok(cmd.indexOf('sessionstart-coordinator.cjs') >= 0, 'command routes to coordinator');
});
