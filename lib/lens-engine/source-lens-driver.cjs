...
// fetch path. Returns the D-19 typed envelope { items, status, reason, freshness }
// -- a failure (or unknown source) degrades to items:[] WITH a typed status,
// never a silent empty (T-219-29). Never throws. roomDir empty -> no cache layer
...
async function fetchSourceCached(source, query, roomDir, fetchCorpusFn) {
  if (!source || typeof query !== 'string' || query.length === 0) {
    return { items: [], status: 'skipped', reason: 'no_source_mapped', freshness: 'unknown' };
  }
  ...
  try {
    ...
    // Per-source fetch failure degrades that lens to empty (Canon DoS
    // mitigation) -- but the outage stays VISIBLE as a typed error (D-19).
    return { items: [], status: 'error', reason: 'fetch_failed', freshness: 'unknown' };
  }
  if (typeof roomDir === 'string' && roomDir.length > 0) {
    try {
...
async function runSourceLens(opts) {
  ...
  async function perLensFn(lens, _ctx) {
    ...
    const fetched = source
      ? await fetchSourceCached(source, topic, roomDir, fetchCorpusFn)
      : { items: [], status: 'skipped', reason: 'no_source_mapped', freshness: 'unknown' };
